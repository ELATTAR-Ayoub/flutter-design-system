...
/// correctly, which is what a side-by-side specimen is; [ThemeTokens.light] is
/// its equivalent here and can be handed to any subtree.
///
/// Every field of [ThemeTokens] is one CSS custom property. Where the stylesheet
...
class ThemeTokens {
  const ThemeTokens._({
    ...
    required this.messageAccent,
    required this.messageAccentHover,
    ...
    required this.agentAccentMuted,
    required this.cube,
    required this.scrim,
  ...
  /// `--background-effect` — light `hsl(240 30% 98%)` L579 / dark `hsl(240 8% 10%)`
  /// L769.
  ///
  /// The page's light source, painted by the `body` rule as
  /// `radial-gradient(120% 90% at 62% 34%, var(--background-effect) 0%, transparent
  ...
  ///
  /// Computed by [OklabColor.fromOklchRelative], never hardcoded: it is derived
  ...
  final Color messageAccentHover;
  ...
  final Color agentAccentMuted;

  /// The twelve `--agent-cube-*` tokens — see [AgentCubeTokens].
  ///
  ...
  // relative-OKLCH syntax. They lived in `effects/feedback_surface.dart` under a
  ...
  /// only on the host element. `AmbientPattern.glowFor` is that second half.
  ...
  static final ThemeTokens light = _build(
    ...
    radius: 10,
    ink1: hslColor(240, 20, 20, 0.04),
    ink2: hslColor(240, 20, 20, 0.07),
    ink3: hslColor(240, 20, 20, 0.11),
    ink4: hslColor(240, 20, 20, 0.16),
    rim: hslColor(0, 0, 100, 0.85),
    rimStrong: hslColor(0, 0, 100, 0.4),
    wall: hslColor(240, 6, 82),
    chart1: Palette.action,
    chart2: Palette.valueDark,
    chart3: Palette.infoDeep,
    chart4: Palette.actionDark,
    chart5: Palette.successDeep,
    agentAccent: Palette.action,
    agentAccentMuted: hslColor(213, 100, 97),
    scrim: hslColor(240, 10, 3.9, 0.28),
  ...
  static final ThemeTokens dark = _build(
    ...
    foreground: hslColor(0, 0, 98),
    card: hslColor(240, 5.9, 10),
    cardForeground: hslColor(0, 0, 98),
    popover: hslColor(240, 5.9, 10),
    popoverForeground: hslColor(0, 0, 98),
    secondary: hslColor(240, 3.7, 15.9),
    secondaryForeground: hslColor(0, 0, 98),
    muted: hslColor(240, 3.7, 15.9),
    mutedForeground: hslColor(240, 4.9, 83.9),
    accent: hslColor(240, 5.3, 26.1),
    accentForeground: hslColor(0, 0, 98),
    border: hslColor(240, 3.7, 15.9),
    input: hslColor(240, 5.3, 26.1),
    pageGlow: hslColor(240, 8, 10),
    primary: Palette.action,
    primaryForeground: hslColor(0, 0, 100),
    ring: Palette.actionBright,
    actionText: Palette.actionBright,
    premiumText: Palette.valueBright,
    successText: Palette.success,
    warningText: Palette.warning,
    infoText: Palette.info,
    destructiveText: Palette.destructiveLifted,
    destructive: hslColor(0, 72.2, 50.6),
    destructiveForeground: hslColor(0, 0, 98),
    ...
    ink1: hslColor(0, 0, 0, 0.35),
    ink2: hslColor(0, 0, 0, 0.5),
    ink3: hslColor(0, 0, 0, 0.6),
    ink4: hslColor(0, 0, 0, 0.75),
    rim: hslColor(0, 0, 100, 0.14),
    rimStrong: hslColor(0, 0, 100, 0.28),
    wall: hslColor(240, 6, 8),
    chart1: Palette.actionBright,
    chart2: Palette.value,
    chart3: Palette.info,
    chart4: Palette.valueBright,
    chart5: Palette.success,
    agentAccent: Palette.actionBright,
    agentAccentMuted: hslColor(217, 40, 13),
    scrim: hslColor(240, 10, 3.9, 0.66),
    // `black` — the bottom of the same axis, for the same reason.
    bloomVoid: hslColor(0, 0, 0),
    bloomL: 1,
    bloomC: 1,
    bloomLift: 0.85,
    bloomHotC: 0.5,
  ...
  static ThemeTokens _build({
    required ResolvedColorMode kind,
    ...
    required Color premiumText,
    required Color successText,
    required Color warningText,
    required Color infoText,
    required Color destructiveText,
    ...
    required Color agentAccent,
    required Color agentAccentMuted,
    ...
    return ThemeTokens._(
      ...
      messageAccent: OklabColor.fromOklchRelative(
        primary,
        lightness: bubbleLightness,
        chromaScale: bubbleChromaScale,
      ),
      messageAccentHover: OklabColor.fromOklchRelative(
        primary,
      ...
      agentAccentMuted: agentAccentMuted,
      ...
      cube: kind == ResolvedColorMode.dark
          ? AgentCubeTokens.dark
...
/// This pass is the opening, and it is `feedback_surface.dart`'s move one family
/// over: the flat per-block values come here, the derivations stay with the
/// effect that performs them (`AgentCubeFaces` mixes the accent's own top and
/// right faces out of [accent] and [accentShade], so it cannot be flattened
/// into a block and does not move).
///
/// The note also asked that `AgentCubeTokens.of(BuildContext)` come with
/// them. It does not: it was a second resolver for what `ThemeScope.of` already
/// does, and the tokens now ride [ThemeTokens.cube] like every other one. That
/// is the whole of the API change — the two statics below are untouched, which
/// is what the avatar tests spend.
class AgentCubeTokens {
  const AgentCubeTokens({
    required this.top,
    required this.left,
    required this.right,
  ...
  /// `var(--color-action-bright)`. The single knob; `AgentAvatar.accent`
  /// overrides it.
  final Color accent;
  ...
  static final AgentCubeTokens light = AgentCubeTokens(
    ...
    right: hslColor(240, 14, 90),
    stroke: hslColor(240, 10, 79),
    accent: Palette.action,
    accentShade: hslColor(217, 45, 72),
    back: hslColor(240, 14, 86),
    ghostInk: hslColor(240, 5, 54),
    errorTop: hslColor(0, 70, 88),
    errorLeft: hslColor(0, 65, 82),
    errorRight: hslColor(0, 60, 75),
  ...
  static final AgentCubeTokens dark = AgentCubeTokens(
    ...
    right: hslColor(240, 5, 14),
    stroke: hslColor(240, 5, 31),
    accent: Palette.actionBright,
    accentShade: hslColor(217, 40, 32),
    back: hslColor(240, 6, 10),
    ghostInk: hslColor(240, 5, 45),
    errorTop: hslColor(0, 60, 46),
    errorLeft: hslColor(0, 62, 39),
    errorRight: hslColor(0, 63, 33),
    errorStroke: hslColor(0, 68, 58),
  );
}
