...
///
///  * [DsHistoryCard] decides what a conversation *can do* and what each of
///    those actions looks like. Built on [DsItem], not beside it — a history
///    row is a list row, and `Item` already owns the border, the radius, the
///    padding, the gap and the colour transition.
///  * [DsChatHistory] *arranges*. It is a drawer inside the console, not a
///    sheet over the page, and it reads every capability off the store.
///  * [DsHistorySearch] is a palette rather than a filter box, because the list
///    lives inside a console inside a dialog and the thing being searched for
///    is a sentence.
///  * [DsFlipController] is `core/use-flip.ts` — and the reason it paints
...
/// the running maximum of the old indices already placed. [DsFlipController]
/// ports that rule and bumps [DsFlipController.generationOf] for that row, so
/// the port paints what the reference paints — a teleport, plus one neighbour
/// replaying its entrance — and [DsFlipController.travel] keeps the inversion
...
///   [DsDurations.base] on [DsCurves.out], after an
...
///   [DsCurves.inOut]. One movement, so the rows below rise into the gap
...
class DsRowMotion extends StatefulWidget {
  const DsRowMotion({
    super.key,
    required this.child,
    this.generation = 0,
  ...
  /// [DsDurations.tick]. It rides the curve rather than a timer because
  ...
  static Duration get enterSpan => DsDurations.tick + DsDurations.base;

  /// Where the delay ends inside [enterSpan].
  static double get enterDelayFraction =>
      DsDurations.tick.inMicroseconds / enterSpan.inMicroseconds;

  /// The whole of `anim-row-in`: hold, then `pulls-row-in` on `--ease-out`.
  static Curve get enterCurve =>
      Interval(enterDelayFraction, 1, curve: DsCurves.out);

  @override
  State<DsRowMotion> createState() => _DsRowMotionState();
}

class _DsRowMotionState extends State<DsRowMotion>
    with TickerProviderStateMixin {
  late final AnimationController _enter = AnimationController(
    vsync: this,
    duration: DsRowMotion.enterSpan,
  );
  ...
  /// `dsAnimationDuration` reads the ambient `disableAnimations`, which is an
  ...
  void _play() {
    final Duration span = dsAnimationDuration(context, DsRowMotion.enterSpan);
  ...
  @override
  void didUpdateWidget(DsRowMotion old) {
    ...
    return AnimatedBuilder(
      ...
      builder: (BuildContext context, Widget? child) {
        if (_exit.value > 0 || widget.leaving) {
          ...
          final double slide =
              DsCurves.inOut.transform(t.clamp(0, 1)) / DsRowMotion.exitBreak;
          final double a = slide.clamp(0, 1);
          final double collapse = t <= DsRowMotion.exitBreak
              ? 1
              : 1 -
                  ((DsCurves.inOut.transform(t) - DsRowMotion.exitBreak) /
                          (1 - DsRowMotion.exitBreak))
                      .clamp(0, 1);
          return Align(
            ...
            child: Opacity(
              ...
              child: Transform.translate(
                offset: Offset(DsRowMotion.exitShift * a, 0),
                child: child,
              ),
            ),
          );
        ...
        final double t = DsRowMotion.enterCurve.transform(_enter.value);
        return Opacity(
          opacity: t,
          child: Transform.translate(
            offset: Offset(DsRowMotion.enterShift * (1 - t), 0),
            child: child,
          ),
        );
      },
...
///   [DsDurations.fast] on [DsCurves.curveIn].
/// * `anim-blur-in` — `opacity 0 → 1`, `blur(8px) → blur(0)`, over
///   [DsDurations.base] on [DsCurves.settle].
///
...
class DsBlurSwitch extends StatefulWidget {
  const DsBlurSwitch({super.key, required this.phase, required this.child});

  final DsSwitchPhase phase;
  final Widget child;
  ...
  @override
  State<DsBlurSwitch> createState() => _DsBlurSwitchState();
}

class _DsBlurSwitchState extends State<DsBlurSwitch>
  ...
  @override
  void didUpdateWidget(DsBlurSwitch old) {
    super.didUpdateWidget(old);
  ...
  void _restart() {
    final Duration d = switch (widget.phase) {
      DsSwitchPhase.idle => Duration.zero,
      DsSwitchPhase.out => DsDurations.fast,
      DsSwitchPhase.blurIn => DsDurations.base,
    };
    _c.duration = dsAnimationDuration(context, d);
  ...
  @override
  Widget build(BuildContext context) {
    if (widget.phase == DsSwitchPhase.idle) return widget.child;
    return AnimatedBuilder(
      animation: _c,
      builder: (BuildContext context, Widget? child) {
        final bool out = widget.phase == DsSwitchPhase.out;
        final double t = (out ? DsCurves.curveIn : DsCurves.settle)
            .transform(_c.value.clamp(0, 1));
        final double opacity = out ? 1 - t : t;
        final double radius = out
            ? DsBlurSwitch.outRadius * t
            : DsBlurSwitch.inRadius * (1 - t);
...
class DsFlipController extends ChangeNotifier {
  DsFlipController({this.duration = DsDurations.base});
  ...
  void _record() {
    ...
    _before.forEach((String id, Offset first) {
      final RenderObject? object = _keys[id]?.currentContext?.findRenderObject();
  ...
  static List<String> _movedByReconciliation(
    ...
    for (final String id in after) {
      ...
      // entrance once through [DsRowMotion.initState] rather than through a
      // generation bump.
...
/// takes.
enum DsHistoryConfirm { inline, dialog }

/// `HistoryCardProps['rename']`.
enum DsHistoryRename { inline, dialog }
...
class DsHistoryCard extends StatefulWidget {
  const DsHistoryCard({
    super.key,
  ...
  final DsConversationSummary conversation;
  ...
  @override
  State<DsHistoryCard> createState() => _DsHistoryCardState();
}

class _DsHistoryCardState extends State<DsHistoryCard> {
  final TextEditingController _draft = TextEditingController();
  final FocusNode _renameFocus = FocusNode(debugLabel: 'Conversation title');
  final GlobalKey<DsModalPortalState> _alert = GlobalKey<DsModalPortalState>();
  final GlobalKey<DsModalPortalState> _renameDialog =
      GlobalKey<DsModalPortalState>();
  ...
  void _startRename() {
    _draft.text = widget.conversation.title;
    if (widget.rename == DsHistoryRename.dialog) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _draft.selection =
          TextSelection(baseOffset: 0, extentOffset: _draft.text.length);
    });
  }
  ...
  void _dismissConfirm() {
    ...
    Future<void>.delayed(
      dsAnimationDuration(context, DsHistoryCard.confirmExit),
      () {
        if (!mounted) return;
        setState(() {
          _confirming = false;
  ...
  Widget build(BuildContext context) {
    ...
    final DsConversationSummary c = widget.conversation;

    final Widget row = DsItem(
      variant: DsItemVariant.outline,
      alignStart: true,
      media: DsItemMedia(
        child: DsIcon.lucide(
          widget.active
              ? DsLucide.messageSquareDot
              : c.pinned
                  ? DsLucide.pin
      ...
      content: DsItemContent(
        children: <Widget>[
          SizedBox(
            height: DsHistoryCard.titleHeight,
            child: Align(
          ...
          DsItemDescription(
            c.preview == null
                ? dsRelativeTimeOf(context, c.updatedAt)
                : '${dsRelativeTimeOf(context, c.updatedAt)} · ${c.preview}',
          ),
        ],
      ),
          ...
          if (widget.onPin != null)
            _fade(
              ...
              child: DsButton(
                ...
                child: DsIcon.lucide(
                  c.pinned ? DsLucide.pin : DsLucide.pinOff,
                  size: DsIconSize.sm,
                  tone: c.pinned ? DsIconTone.action : DsIconTone.muted,
                ),
              ),
            ),
          ...
          DsDropdownMenu(
            ...
            trigger: Builder(
              builder: (BuildContext context) => _fade(
                visible: _hovered || DsMenuTriggerScope.openOf(context),
                child: DsButton(
                  variant: DsButtonVariant.ghost,
                  size: DsButtonSize.iconSm,
                  ...
                  // [DsMenuPointerDown]; the handler is here only so the
            ...
            children: <DsMenuChild>[
              if (widget.onShare != null)
                DsMenuItem(
                  label: 'Share',
                  lucideIcon: DsLucide.share2,
                  onSelect: () => widget.onShare!(c.id),
                ),
              DsMenuItem(
                label: 'Rename',
                lucideIcon: DsLucide.pencil,
                onSelect: _startRename,
              ),
              if (widget.onPin != null)
                DsMenuItem(
                  label: c.pinned ? 'Unpin' : 'Pin',
                  lucideIcon: c.pinned ? DsLucide.pinOff : DsLucide.pin,
                  onSelect: () => widget.onPin!(c.id, !c.pinned),
                ),
              const DsMenuSeparator(),
              DsMenuItem(
                label: 'Delete',
                lucideIcon: DsLucide.trash2,
                variant: DsMenuItemVariant.destructive,
                onSelect: () {
                  if (widget.confirm == DsHistoryConfirm.dialog) {
    ...
    Widget card = ClipRRect(
      borderRadius: BorderRadius.circular(DsItem.radius),
      child: Stack(
        children: <Widget>[
          row,
          if (widget.confirm == DsHistoryConfirm.inline && _confirming)
    ...
    return Stack(
      children: <Widget>[
        card,
        if (widget.confirm == DsHistoryConfirm.dialog) _alertDialog(c),
  ...
  Widget _titleButton(DsThemeData theme) => _TapRegion(
  ...
  Widget _renameField(DsThemeData theme) => Shortcuts(
        ...
        child: Actions(
          ...
          child: DsInput(
            ...
            boxHeight: DsHistoryCard.titleHeight,
            radius: BorderRadius.circular(DsRadii.sm),
            padding: EdgeInsets.symmetric(horizontal: ds(1.5)),
            flat: true,
            onSubmitted: _commit,
          ),
        ),
  ...
  Widget _inlineConfirm(DsThemeData theme) => _ConfirmSlide(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: theme.destructive
                    .withValues(alpha: DsHistoryCard.confirmBorderAlpha),
              ...
              children: <Widget>[
                ...
                DsButton(
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      const DsIcon.lucide(DsLucide.x, size: DsIconSize.sm),
                ...
                DsButton(
                  ...
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: <Widget>[
                      const DsIcon.lucide(DsLucide.check, size: DsIconSize.sm),
                      SizedBox(width: DsButton.gapFor(DsButtonSize.sm)),
                      const Text('Delete'),
                    ],
                  ),
  ...
  /// [DsModalPortal] rather than [DsAlertDialog], and the difference is one
  /// line: this overlay is opened from a **menu row**, not from its own
  /// trigger, so the caller needs the portal's [DsModalPortalState] — and a
  /// key on the stateless `DsAlertDialog` wrapper resolves to the wrapper, not
  /// to the portal inside it. Everything else is what that widget passes:
  /// [DsJellyTransition], and *"not dismissible by overlay click"*.
  Widget _alertDialog(DsConversationSummary c) => DsModalPortal(
        key: _alert,
  ...
  /// doing it quickly.
  Widget _renameDialogPortal(DsConversationSummary c) => DsModalPortal(
        key: _renameDialog,
        transition: (
          BuildContext context,
          Animation<double> animation,
        ...
        content: (BuildContext context, VoidCallback close) => DsDialogContent(
          ...
          children: <Widget>[
            const DsDialogHeader(
              children: <Widget>[
                DsDialogTitle('Rename conversation'),
                DsDialogDescription(
            ...
            DsField(
              label: 'Title',
              child: DsInput(controller: _draft, onSubmitted: _commit),
            ),
            DsDialogFooter(
              children: <Widget>[
                DsButton(
...
///
/// In: `opacity 0 → 1` with `translateX(12%) → none` over [DsDurations.fast] on
/// [DsCurves.out]. Out: opacity only, over [DsDurations.tick] on
/// [DsCurves.curveIn] — **no retrace of the slide**, because the row underneath
...
class _ConfirmSlideState extends State<_ConfirmSlide>
  ...
  void didChangeDependencies() {
    ...
    if (_started) return;
    _started = true;
    _c.duration = dsAnimationDuration(context, DsDurations.fast);
    if (_c.duration == Duration.zero) {
      _c.value = 1;
    } else {
      _c.forward(from: 0);
  ...
  void didUpdateWidget(_ConfirmSlide old) {
    ...
    _c.duration = dsAnimationDuration(context, DsDurations.tick);
    if (_c.duration == Duration.zero) {
      _c.value = 0;
    } else {
      _c.reverse(from: 1);
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) {
          if (widget.closing) {
            return Opacity(
              opacity: DsCurves.curveIn.transform(1 - _c.value) > 0
                  ? 1 - DsCurves.curveIn.transform(1 - _c.value)
                  : 0,
              child: child,
            );
          }
          final double t = DsCurves.out.transform(_c.value);
          return Opacity(
            opacity: t,
            child: FractionalTranslation(
              translation:
                  Offset(DsHistoryCard.confirmShift * (1 - t), 0),
              child: child,
            ),
          );
        },
...
/// named afterwards — which is also why [DsCommand.shouldFilter] is off here:
...
/// rows 352 × 48.7.)*
class DsHistorySearch extends StatefulWidget {
  const DsHistorySearch({
    super.key,
  ...
  final List<DsConversationSummary> conversations;
  final bool open;
  ...
  }) partition(List<DsConversationSummary> conversations, String query) {
    final String trimmed = query.trim().toLowerCase();
    final List<DsConversationSummary> byNewest =
        List<DsConversationSummary>.of(conversations)
          ..sort((DsConversationSummary a, DsConversationSummary b) =>
              b.updatedAt.compareTo(a.updatedAt));
    if (trimmed.isEmpty) {
      return (
        pinned: byNewest.where((DsConversationSummary c) => c.pinned).toList(),
        recent: byNewest
            .where((DsConversationSummary c) => !c.pinned)
            .take(DsHistorySearch.recent)
            .toList(),
        results: const <DsConversationSummary>[],
      );
    }
    return (
      pinned: const <DsConversationSummary>[],
      recent: const <DsConversationSummary>[],
      results: byNewest
          .where((DsConversationSummary c) =>
  ...
  @override
  State<DsHistorySearch> createState() => _DsHistorySearchState();
}

class _DsHistorySearchState extends State<DsHistorySearch> {
  final GlobalKey<DsModalPortalState> _portal = GlobalKey<DsModalPortalState>();
  ...
  @override
  void didUpdateWidget(DsHistorySearch old) {
    super.didUpdateWidget(old);
  ...
  Widget build(BuildContext context) {
    ...
    final ({
      ...
      List<DsConversationSummary> recent,
      List<DsConversationSummary> results,
    }) split = DsHistorySearch.partition(widget.conversations, widget.query);

    final List<DsCommandGroup> groups = <DsCommandGroup>[
      if (split.pinned.isNotEmpty)
        DsCommandGroup(heading: 'Pinned', items: _rows(split.pinned)),
      if (split.recent.isNotEmpty)
        DsCommandGroup(heading: 'Recent', items: _rows(split.recent)),
      if (split.results.isNotEmpty)
        DsCommandGroup(
          heading: DsHistorySearch.matchHeading(split.results.length),
    ...
    return DsModalPortal(
      ...
      content: (BuildContext context, VoidCallback close) => Padding(
        ...
        padding: EdgeInsets.only(
          top: MediaQuery.sizeOf(context).height * DsHistorySearch.topFraction,
        ),
        child: ConstrainedBox(
          constraints: BoxConstraints(maxWidth: DsDialogContent.maxWidth),
          child: DsMachineSurface(
            ...
            fill: theme.popover,
            child: ClipRRect(
              borderRadius: BorderRadius.circular(DsDialogContent.radius),
              child: DsCommand(
                inDialog: true,
                shouldFilter: false,
                controller: _input,
  ...
  List<DsCommandItem> _rows(List<DsConversationSummary> list) =>
      <DsCommandItem>[
        for (final DsConversationSummary c in list)
          DsCommandItem(
            value: c.id,
            label: c.title,
            subtitle: c.preview,
            meta: dsRelativeTimeOf(context, c.updatedAt),
            lucideIcon: c.pinned ? DsLucide.pin : DsLucide.messageSquare,
            iconTone: c.pinned ? DsIconTone.action : DsIconTone.muted,
...
class DsChatHistory extends StatefulWidget {
  const DsChatHistory({
    super.key,
    required this.store,
  ...
  final DsConversationStore store;
  ...
  /// arrives as its own prop and defaults to [DsConversationStore.open].
  ...
  static double get width => DsContainers.sm;

  /// `EXIT_MS` — *"must match `--duration-base` in globals.css"*.
  static Duration get exit => DsDurations.base;

  /// `anim-panel-in` — `translateX(-100%) → none` over `--duration-overlay`.
  static Duration get panelIn => DsDurations.overlay;

  @override
  State<DsChatHistory> createState() => _DsChatHistoryState();
}

class _DsChatHistoryState extends State<DsChatHistory> {
  ...
  void _remove(String id) {
    ...
    Future<void>.delayed(dsAnimationDuration(context, DsChatHistory.exit), () {
      if (!mounted) return;
      ...
      children: <Widget>[
        OverlayPortal(
          ...
          child: DsButton(
            ...
            child: const DsIcon.lucide(DsLucide.panelLeft),
          ),
        ),
        DsHistorySearch(
              ...
              Positioned.fill(
                child: _FadeIn(
                  child: _TapRegion(
                    ...
                    child: ColoredBox(color: DsTheme.of(context).scrim),
                  ),
                ),
              ),
              Positioned(
                ...
                bottom: 0,
                left: 0,
                width: rect.width < DsChatHistory.width
                    ? rect.width
                    : DsChatHistory.width,
  ...
  Widget _drawerBody(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final DsConversationStore store = widget.store;
    ...
    final List<DsConversationSummary> byNewest =
        List<DsConversationSummary>.of(store.conversations)
          ..sort((DsConversationSummary a, DsConversationSummary b) =>
              b.updatedAt.compareTo(a.updatedAt));
    final List<DsConversationSummary> pinned =
        byNewest.where((DsConversationSummary c) => c.pinned).toList();
    final List<DsConversationSummary> rest =
        byNewest.where((DsConversationSummary c) => !c.pinned).toList();
    _flip.reconcile(<String>[
      for (final DsConversationSummary c in <DsConversationSummary>[
        ...pinned,
        ...rest,
      ])
        c.id,
      ...
      children: <Widget>[
        ...
        Expanded(
          child: SingleChildScrollView(
            padding: EdgeInsets.fromLTRB(ds(3), 0, ds(3), ds(3)),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: <Widget>[
                    ...
                    _TapRegion(
                      ...
                      child: DsItem(
                        ...
                        content: const DsItemContent(
                          children: <Widget>[DsItemTitle('New chat')],
                        ),
                      ),
                    ),
                    ...?widget.nav,
                ...
                if (store.error != null) ...<Widget>[
                  ...
                  DsAlert(
                    ...
                    icon: const DsIcon.lucide(DsLucide.octagonX),
                    title: 'History is unavailable',
                    description: store.error,
                  ),
                ],
                if (store.error == null && store.conversations.isEmpty) ...<Widget>[
                  SizedBox(height: ds(4)),
                  DsEmpty(
                    children: <Widget>[
                      DsEmptyHeader(
                        children: <Widget>[
                          ...
                          // while [DsEmptyMedia] takes a curated glyph and
                          ...
                          DsEmptyTitle(store.isLoading
                              ? 'Loading conversations'
                              : 'No conversations yet'),
                          DsEmptyDescription(store.isLoading
  ...
  Widget _section(
    DsThemeData theme,
    String? heading,
    List<DsConversationSummary> rows,
    DsConversationStore store,
  ) =>
      Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: <Widget>[
          if (heading != null) ...<Widget>[
            Padding(
              padding: EdgeInsets.symmetric(horizontal: ds(1)),
              child: DsText(heading, DsType.section),
            ),
          ...
          DsItemGroup(
            gapOverride: ds(1),
            children: <Widget>[
              for (final DsConversationSummary c in rows)
                DsHistoryCard(
...
class _FadeInState extends State<_FadeIn> with SingleTickerProviderStateMixin {
  ...
  void didChangeDependencies() {
    ...
    if (_started) return;
    _started = true;
    _c.duration = dsAnimationDuration(context, DsDurations.overlay);
    if (_c.duration == Duration.zero) {
      _c.value = 1;
    } else {
      _c.forward(from: 0);
  ...
  Widget build(BuildContext context) => FadeTransition(
        opacity: CurvedAnimation(parent: _c, curve: DsCurves.out),
        child: widget.child,
      );
}
...
class _PanelInState extends State<_PanelIn> with SingleTickerProviderStateMixin {
  late final AnimationController _c = AnimationController(vsync: this);
  ...
  void didChangeDependencies() {
    ...
    if (_started) return;
    _started = true;
    _c.duration = dsAnimationDuration(context, DsChatHistory.panelIn);
    if (_c.duration == Duration.zero) {
      _c.value = 1;
    } else {
      _c.forward(from: 0);
  ...
  Widget build(BuildContext context) => SlideTransition(
        ...
        ).animate(CurvedAnimation(parent: _c, curve: DsCurves.out)),
        child: widget.child,
      );
}
...
///
/// The drawer's empty state swaps a [DsSpinner] in while the store loads, and
/// [DsEmptyMedia] takes a curated [DsIconGlyph] and nothing else. Everything
