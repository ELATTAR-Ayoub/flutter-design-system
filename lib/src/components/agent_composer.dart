...
///     is a [DragTarget] over `List<DsAgentAttachment>` — same three moments,
///     same `border-agent bg-agent/8`, same [dsFormatBytes] refusal message
///     against [kDsMaxFileBytes], with attachments where the reference has
///     `File`s.
...
/// The composer.
class DsAgentComposer extends StatefulWidget {
  const DsAgentComposer({
    super.key,
  ...
  final ValueChanged<List<DsAgentAttachment>>? onAttach;
  ...
  static double get maxRowsPx => ds(50);
  ...
  static EdgeInsets get controlInsets =>
      EdgeInsets.only(left: ds(2), right: ds(2), bottom: ds(2));

  /// `gap-1` between the controls.
  static double get controlGap => ds(1);

  /// `p-3` on the file tray.
  static double get trayPadding => ds(3);

  /// The inline-block descent under the input — drift 1.
  static double get inlineGap => ds(1.5);

  /// `size-8` — the send, stop and plus controls are all one rung.
  static DsButtonSize get controlSize => DsButtonSize.iconSm;

  /// `size-4` on the send arrow — the base class list's, not the `size-8`
  /// rung's.
  static double get sendGlyphSize => DsButton.iconPxFor(DsButtonSize.icon);

  /// `size-3.5` on the stop square, written explicitly at the call site.
  static double get stopGlyphSize => DsButton.iconPxFor(DsButtonSize.iconSm);
  ...
  static double get messageTopGap => ds(2);

  @override
  State<DsAgentComposer> createState() => _DsAgentComposerState();
}

class _DsAgentComposerState extends State<DsAgentComposer> {
  ...
  List<DsAgentAttachment> get _attachments =>
      widget.attachments ?? const <DsAgentAttachment>[];
  ...
  @override
  void didUpdateWidget(DsAgentComposer old) {
  ...
  void _onValueChanged() {
    ...
    setState(() => _caret = selection.isValid
  ...
  String? get _query => (widget.commands?.isNotEmpty ?? false)
      ? dsSlashQuery(_value, _caret)
      : null;

  List<DsAgentCommand> _matchesFor(String? query) => query == null
      ? const <DsAgentCommand>[]
      : dsFilterCommands(
          widget.commands ?? const <DsAgentCommand>[], query);
  ...
  /// their own argument."*
  void _applyCommand(DsAgentCommand command) {
  ...
  void _takeFiles(List<DsAgentAttachment> files) {
    final ValueChanged<List<DsAgentAttachment>>? onAttach = widget.onAttach;
    if (onAttach == null) return;

    final List<DsAgentAttachment> accepted = <DsAgentAttachment>[];
    final List<String> refused = <String>[];
    for (final DsAgentAttachment file in files) {
      if (file.size > kDsMaxFileBytes) {
        refused.add(file.name);
      } else {
        accepted.add(file);
      }
    }
    setState(() => _rejected = refused.isNotEmpty
        ? '${refused.join(", ")} — over the '
            '${dsFormatBytes(kDsMaxFileBytes)} limit'
        : null);
  ...
  KeyEventResult _onKey(FocusNode node, KeyEvent event) {
    ...
    final List<DsAgentCommand> matches = _matchesFor(query);
    final bool paletteOpen = query != null && matches.isNotEmpty;
    final LogicalKeyboardKey key = event.logicalKey;
    final bool enter = key == LogicalKeyboardKey.enter ||
  ...
  Widget build(BuildContext context) {
    ...
    final bool canSend = !widget.disabled &&
        (_value.trim().isNotEmpty || _attachments.isNotEmpty);

    final Widget shell = DsMachineSurface(
      ...
      fill: _dragging
          ? theme.agent
              .withValues(alpha: DsAgentComposer.dragFillAlpha)
          : theme.card,
      border: Border.all(
      ...
      child: Column(
        ...
        children: <Widget>[
          if (_attachments.isNotEmpty)
            Container(
              padding: EdgeInsets.all(DsAgentComposer.trayPadding),
          ...
          _ComposerInput(
            ...
            placeholder: _dragging
                ? DsAgentComposer.dropPlaceholder
                : (widget.placeholder ??
                    DsAgentComposer.defaultPlaceholder),
          ),
          // Drift 1 — the line box under an `inline-block` textarea.
          SizedBox(height: DsAgentComposer.inlineGap),
          Padding(
            padding: DsAgentComposer.controlInsets,
            child: Row(
              ...
              children: <Widget>[
                ...
                DsAgentAttachMenu(
                  onPickFiles:
                      widget.onAttach != null ? _pickFiles : null,
                  commands: widget.commands,
                  onRunCommand: _applyCommand,
                  disabled: widget.disabled,
                ),
                SizedBox(width: DsAgentComposer.controlGap),
                ...
                if (widget.busy && widget.onStop != null)
                  DsButton(
                    ...
                    size: DsAgentComposer.controlSize,
                    label: 'Stop',
                    onPressed: widget.onStop,
                    child: DsIcon.lucide(
                      DsLucide.square,
                      sizePx: DsAgentComposer.stopGlyphSize,
                    ),
                  )
                else
                  DsButton(
                    ...
                    child: DsIcon.lucide(
                      DsLucide.arrowUp,
                      sizePx: DsAgentComposer.sendGlyphSize,
                    ),
                  ),
              ],
            ),
    ...
    final Widget dropTarget = widget.onAttach == null
        ? body
        : DragTarget<List<DsAgentAttachment>>(
            onWillAcceptWithDetails: (
              DragTargetDetails<List<DsAgentAttachment>> details,
            ) {
              setState(() => _dragging = true);
              return true;
            },
            onLeave: (List<DsAgentAttachment>? _) =>
            ...
            builder: (
              ...
              List<List<DsAgentAttachment>?> candidate,
...
class _Message extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        padding: EdgeInsets.only(top: DsAgentComposer.messageTopGap),
        child: DsText(
          text,
          DsType.caption,
          color: DsTheme.of(context).destructiveInk,
        ),
      );
}

/// The `<textarea>`: `.type-body`, transparent, `outline-none`, `resize-none`,
/// growing to [DsAgentComposer.maxRowsPx] and then scrolling.
...
class _ComposerInputState extends State<_ComposerInput> {
  ...
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final TextStyle style = DsText.styleOf(context, DsType.body);
    ...
    body = ConstrainedBox(
      constraints: BoxConstraints(maxHeight: DsAgentComposer.maxRowsPx),
      child: Padding(padding: DsAgentComposer.inputInsets, child: body),
    );
    ...
    return Opacity(
      opacity:
          widget.enabled ? 1 : DsAgentComposer.disabledInputOpacity,
      child: Semantics(
        textField: true,
        multiline: true,
        label: DsAgentComposer.inputLabel,
...
class _RenderComposerStack extends RenderBox
  ...
  void performLayout() {
    ...
    (overlay.parentData! as StackParentData).offset =
        Offset(0, -(overlay.size.height + _gap));
  }
