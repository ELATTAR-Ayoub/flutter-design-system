...
/// `toggleVariants(default/default)` — i.e. a [DsToggle] — + the two-class
...
/// as its first child — and [DsSlidingPillGroup] already paints it first, so
...
/// [DsSlidingPillGroup] is the transcription of all of that, including the
...
/// item, and [DsSlidingPillGroup] documents an out-of-range `activeIndex` as
...
/// [DsSlidingPillGroup] is positional, it measures the child *at an index*, so
...
/// through [DsSlidingPillGroup] that nothing else in the port exercises.
///
...
class DsToggleGroup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget group = DsSlidingPillGroup(
      ...
      children: <Widget>[
        for (int i = 0; i < items.length; i++)
          DsToggle(
            ...
            // re-asserting it: `DsToggle` always asks for `!pressed`, so the
            ...
            // through [DsText] here would restate all three.
