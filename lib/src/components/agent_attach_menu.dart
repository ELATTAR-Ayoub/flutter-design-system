...
///
/// This file composes `menu.dart`'s [DsMenuSurface] and [DsMenuPointerDown]
/// over [DsPopover] rather than [DsDropdownMenu], and the reason is the row:
...
/// not a shape [DsMenuItem] has a slot for. The chrome, the placement, the
...
class DsAgentAttachMenu extends StatelessWidget {
  const DsAgentAttachMenu({
    super.key,
    this.onPickFiles,
    this.commands,
  ...
  final ValueChanged<DsAgentCommand> onRunCommand;

  final bool disabled;

  /// `size-8` on the trigger — `size="icon"` asks for 40 and twMerge keeps 32.
  static DsButtonSize get triggerSize => DsButtonSize.iconSm;

  /// `w-80` on the content.
  static double get width => ds(80);

  /// `max-h-96`.
  static double get maxHeight => ds(96);
  ...
  static double get rowRadius => DsRadii.md;
  ...
  List<DsAgentCommand> get _skills =>
      (commands ?? const <DsAgentCommand>[])
          .where((DsAgentCommand c) => c.group == DsAgentCommandGroup.skill)
          .toList();

  @override
  Widget build(BuildContext context) {
    final List<DsAgentCommand> skills = _skills;
...
class _AttachMenu extends StatefulWidget {
  ...
  final VoidCallback? onPickFiles;
  final List<DsAgentCommand> skills;
  final ValueChanged<DsAgentCommand> onRunCommand;
  final bool disabled;
  ...
  Widget build(BuildContext context) {
    ...
    return DsPopover(
      ...
      anchor: DsMenuPointerDown(
        ...
        child: DsMenuTriggerScope(
          open: _isOpen,
          child: DsButton(
            variant: DsButtonVariant.ghost,
            size: DsAgentAttachMenu.triggerSize,
            ...
            child: DsIcon.lucide(
              DsLucide.plus,
              sizePx: DsAgentAttachMenu.glyphSize,
            ),
          ),
        ),
      ),
      content: (BuildContext context, DsPopoverAnchorMetrics metrics) {
        final double pad = DsMenu.contentPadding;
        return SizedBox(
          width: DsAgentAttachMenu.width,
          child: DsMenuSurface(
            child: ConstrainedBox(
              constraints:
                  BoxConstraints(maxHeight: DsAgentAttachMenu.maxHeight),
              // `overflow-y-auto`.
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    if (hasFiles)
                      Padding(
                        ...
                        child: _MenuRow(
                          ...
                          stroke: DsIcon.strokeFor(
                            DsAgentAttachMenu.glyphSize,
                          ),
                          title: 'Photos & files',
                    ...
                    if (hasSkills) ...<Widget>[
                      ...
                      for (final DsAgentCommand command in widget.skills)
                        Padding(
                          padding: EdgeInsets.symmetric(horizontal: pad),
                          child: _MenuRow(
                            // `command.icon ?? SparklesIcon`.
                            glyph: command.icon ?? DsLucide.sparkles,
                            stroke: DsAgentSlashPalette.lucideStroke,
                            title: command.label,
                            hint: command.hint,
                            truncateTitle: true,
                            onSelect: () =>
        ...
        child: Semantics(
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              ...
              color: _highlighted ? theme.accent : dsTransparent,
              borderRadius:
                  BorderRadius.circular(DsAgentAttachMenu.rowRadius),
            ),
            child: Padding(
              padding: DsAgentAttachMenu.rowInsets,
              child: Row(
                ...
                children: <Widget>[
                  ...
                  SizedBox(width: DsAgentAttachMenu.rowGap),
                  Flexible(
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        // [DsAgentAttachMenu.rowLinesHaveNoGap].
                        if (widget.hint != null)
                          DsText(
                            widget.hint!,
                            DsType.caption,
