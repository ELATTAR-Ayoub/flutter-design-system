...
/// | the **open list** | **DIVERGENT.** The port mounts its own [DsSelectMenu] — `SelectContent`'s surface, rows, tick and keyboard — where the OS would have drawn its picker. `<option class="bg-[Canvas] text-[CanvasText]">` asks for the platform's system colours; the port paints `--popover` |
///
...
/// committing. `DsSelect` — a Radix menu — opens on the arrows instead, and the
...
/// | property | value |
/// |---|---|
/// | height | **32px** (`h-8`) — the `DsSelect` beside it is 40. selects-map drift 8 |
...
/// list is a [DsSelectChild] list and [DsSelectGroup] is one of its members.
library;
...
class DsNativeSelect<T> extends StatefulWidget {
  ...
  /// same switch, as [DsSelect.expand].
  final bool expand;

  /// An explicit measure, which beats both [expand] states — [DsSelect.width]'s
  ...
  static double get menuOffset => ds(1);

  @override
  State<DsNativeSelect<T>> createState() => _DsNativeSelectState<T>();
}

class _DsNativeSelectState<T> extends State<DsNativeSelect<T>> {
  ...
  FocusNode get _focusNode =>
      widget.focusNode ??
      _scope?.focusNode ??
      (_ownedFocusNode ??= FocusNode(debugLabel: 'DsNativeSelect'));
  ...
  /// flattens them.
  List<DsSelectOption<T>> get _flat {
    final List<DsSelectOption<T>> flat = <DsSelectOption<T>>[];
    for (final DsSelectChild<T> child in widget.options) {
      switch (child) {
  ...
  int get _selectedIndex {
    final List<DsSelectOption<T>> flat = _flat;
    final int i =
        flat.indexWhere((DsSelectOption<T> o) => o.value == widget.value);
    return i < 0 ? 0 : i;
  }
  ...
  /// The open list's highlight, which does wrap — it is [DsSelectMenu], and the
  /// menu is Radix's.
  void _move(int step) {
    final List<DsSelectOption<T>> flat = _flat;
  ...
  Color _fill(DsThemeData theme) {
    if (theme.kind == DsThemeKind.light) return dsTransparent;
    return theme.input.withValues(
      alpha: _hovered ? _darkHoverFillAlpha : _darkFillAlpha,
    );
  }

  Color _border(DsThemeData theme) {
    if (_invalid) {
  ...
  Widget build(BuildContext context) {
    ...
    // else it could show.
    final DsSelectOption<T>? shown =
        flat.isEmpty ? null : flat[_selectedIndex];
        ...
        children: <Widget>[
          ...
          Positioned.fill(
            child: Align(
              ...
              child: Padding(
                padding: EdgeInsets.only(right: ds(2.5)),
                child: const DsIcon(
    ...
    // picker appears. `DsPopover` supplies the positioner and the collision
    // flip; `DsSelectMenu` supplies everything inside it.
    control = DsPopover(
      open: _open,
      side: DsPopoverSide.bottom,
      align: DsPopoverAlign.start,
      sideOffset: DsNativeSelect.menuOffset,
