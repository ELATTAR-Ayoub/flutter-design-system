...
/// | `rounded-sm` | `--radius-sm` **6px** ([DsRadii.sm]) |
/// | `bg-muted` | — |
/// | `px-1` | 4px |
/// | `font-sans text-xs font-medium` | 12px / 500 — [DsComponentType.kbdKey] |
...
/// `DsShadows.key`, `DsShadows.keyDown` and `DsPress` in this package, and the
...
/// exposed rather than applied — the same arrangement `DsButton.gapFor` uses
/// for the same reason.
class DsKbd extends StatelessWidget {
  const DsKbd(this.text, {super.key});
  ...
  /// `px-1` — 4px.
  static double get paddingX => ds(1);
  ...
  Widget build(BuildContext context) {
    ...
    final Widget key = DsMachineSurface(
      ...
      fill: theme.muted,
      child: Padding(
        padding: EdgeInsets.symmetric(horizontal: DsKbd.paddingX),
      ...
      child: SelectionContainer.disabled(
        child: SizedBox(
          ...
          child: ConstrainedBox(
            constraints: BoxConstraints(minWidth: DsKbd.minWidth),
            child: key,
          ),
        ),
      ),
...
class DsKbdGroup extends StatelessWidget {
  const DsKbdGroup({super.key, required this.children});

  /// The keys, in order — `[DsKbd('Ctrl'), DsKbd('K')]`.
      ...
      child: Row(
        ...
        children: <Widget>[
          for (int i = 0; i < children.length; i++) ...<Widget>[
            if (i > 0) SizedBox(width: DsKbdGroup.gap),
            children[i],
          ],
        ],
      ),
