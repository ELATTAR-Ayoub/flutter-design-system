...
///
/// Ruling F7: *"Validator = dependency-free `DsRule<T>` lists with first-issue /
...
/// the sentence it renders. That is [DsRule]. There is no object schema, no
...
///    `too_small` *and* `invalid_format`, in that order. So [DsRules.check]
...
///    is [DsIssueMode], and it is the entire difference between the account
...
/// `DsFieldError` too — same operation, same place.
library;

/// A predicate over one field's value. `true` means the value passes.
typedef DsRuleTest<T> = bool Function(T value);
...
class DsRule<T> {
  const DsRule(this.test, this.message);

  /// Passes when this returns `true`.
  final DsRuleTest<T> test;
  ...
  static DsRule<String> minLength(int min, String message) =>
      DsRule<String>((String v) => v.length >= min, message);

  /// `z.string().max(n, msg)`.
  static DsRule<String> maxLength(int max, String message) =>
      DsRule<String>((String v) => v.length <= max, message);
  ...
  static DsRule<String> pattern(RegExp expression, String message) =>
      DsRule<String>(expression.hasMatch, message);

  /// `z.email(msg)` — one rule, one predicate, [emailPattern].
  static DsRule<String> email(String message) =>
      DsRule<String>(emailPattern.hasMatch, message);
  ...
  static DsRule<bool> accepted(String message) =>
      DsRule<bool>((bool v) => v, message);
  ...
  static DsRule<T?> oneOf<T>(List<T> allowed, String message) =>
      DsRule<T?>((T? v) => v != null && allowed.contains(v), message);
...
class DsRules {
  ...
  /// one entry under [DsIssueMode.first].
  ///
  ...
  static List<String> check<T>(
    T value,
    List<DsRule<T>> rules, {
    DsIssueMode mode = DsIssueMode.first,
  }) {
    final List<String> issues = <String>[];
    for (final DsRule<T> rule in rules) {
      final String? issue = rule.issue(value);
      if (issue == null) continue;
      issues.add(issue);
      if (mode == DsIssueMode.first) break;
    }
    return issues;
  }
