...
/// drift 12, and the port renders [DsRadii.xl].
///
/// ## Measured geometry
///
...
///
/// ## Why this file does not build on `DsPopover`
///
...
/// `DsSelect` and `DsInputGroupAlign`. What this file does reuse from wave A2 is
/// the input-group *recipe*; it cannot reuse the widget, because [DsInputGroup]
...
///   label split ([DsComponentType.menuHeading]).
...
/// One `CommandGroup`.
class DsCommandGroup {
  const DsCommandGroup({
    required this.items,
    this.heading,
    this.separatorBefore = false,
  });

  final List<DsCommandItem> items;

  /// `heading` — rendered `px-3 py-2` at [DsComponentType.menuHeading].
  ///
...
class DsCommand extends StatefulWidget {
  const DsCommand({
    super.key,
  ...
  final List<DsCommandGroup> groups;
  ...
  /// cmdk's `filter` prop. Defaults to [dsCommandScore]; return 0 to hide.
  ...
  /// cmdk's `onValueChange`, carrying the selected row's [DsCommandItem.searchValue].
  ...
  static double get listMaxHeight => ds(72);

  /// `h-8!` — the input group, at 32px rather than the family's 40.
  static double get inputHeight => ds(8);
  ...
  static double get emptyHeight {
    final DsTypeSpec spec = DsComponentType.sheetBody;
    return (spec.size ?? 0) * (spec.height ?? 1) + ds(6) * 2;
  }
  ...
  @override
  State<DsCommand> createState() => _DsCommandState();
}

class _DsCommandState extends State<DsCommand> {
  ...
  FocusNode get _focusNode =>
      widget.focusNode ??
      (_ownedFocusNode ??= FocusNode(debugLabel: 'DsCommand'));
  ...
  @override
  void didUpdateWidget(DsCommand old) {
    ...
    if (old.groups != widget.groups && _visibleItems.isNotEmpty) {
      final List<DsCommandItem> visible = _visibleItems;
      if (!visible.any((DsCommandItem i) => i.searchValue == _value)) {
        _selectFirstItem();
      }
    }
  }
  ...
  double _score(DsCommandItem item) {
    final double Function(String, String, List<String>) filter =
        widget.filter ?? dsCommandScore;
  ...
  List<_ResolvedGroup> get _resolved {
    if (_search.isEmpty || !widget.shouldFilter) {
      return widget.groups
          .map((DsCommandGroup g) => _ResolvedGroup(g, g.items))
          .toList();
    }

    final List<_ResolvedGroup> out = <_ResolvedGroup>[];
    for (final DsCommandGroup group in widget.groups) {
      final List<({DsCommandItem item, double score, int order})> scored =
          <({DsCommandItem item, double score, int order})>[];
      for (int i = 0; i < group.items.length; i++) {
        final DsCommandItem item = group.items[i];
      ...
      scored.sort((
        ({DsCommandItem item, double score, int order}) a,
        ({DsCommandItem item, double score, int order}) b,
      ) {
      ...
      out.add(
        _ResolvedGroup(
          ...
          scored
              .map(
                (({DsCommandItem item, double score, int order}) e) => e.item,
              )
              .toList(),
        ),
      );
  ...
  List<DsCommandItem> get _visibleItems => <DsCommandItem>[
    for (final _ResolvedGroup g in _resolved) ...g.items,
  ];

  /// `getValidItems()` — `[cmdk-item=""]:not([aria-disabled="true"])`.
  List<DsCommandItem> get _validItems =>
      _visibleItems.where((DsCommandItem i) => i.enabled).toList();
  ...
  void _selectFirstItemInner() {
    final List<DsCommandItem> valid = _validItems;
  ...
  void _move(int step) {
    ...
    final int at = valid.indexWhere(
      (DsCommandItem i) => i.searchValue == _value,
    );
    int next = at + step;
    if (widget.loop) {
      next = next < 0
  ...
  void _moveByGroup(int step) {
    ...
    final int at = groups.indexWhere(
      (_ResolvedGroup g) =>
          g.items.any((DsCommandItem i) => i.searchValue == _value),
    );
    if (at < 0) return _move(step);
    final int target = at + step;
    if (target < 0 || target >= groups.length) return _move(step);
    final Iterable<DsCommandItem> valid = groups[target].items.where(
      (DsCommandItem i) => i.enabled,
    );
  ...
  void _commit(DsCommandItem item) {
  ...
  KeyEventResult _onKey(FocusNode node, KeyEvent event) {
    ...
    if (key == LogicalKeyboardKey.enter ||
      ...
      final DsCommandItem? selected = _selectedItem;
  ...
  DsCommandItem? get _selectedItem {
    for (final DsCommandItem item in _visibleItems) {
  ...
  Widget build(BuildContext context) {
    ...
    _rowKeys.removeWhere(
      (String value, GlobalKey _) =>
          !_visibleItems.any((DsCommandItem i) => i.searchValue == value),
    );
      ...
      rows.add(
        _inset(
          _CommandGroupBlock(
            ...
            children: <Widget>[
              for (final DsCommandItem item in group.items)
                _CommandRow(
    ...
    list = ConstrainedBox(
      constraints: BoxConstraints(maxHeight: DsCommand.listMaxHeight),
      ...
      children: <Widget>[
        ...
        Padding(
          padding: EdgeInsets.only(
            left: DsCommand.padding,
            right: DsCommand.padding,
            top: DsCommand.padding,
          ),
          child: _CommandInput(
            theme: theme,
        ...
        SizedBox(height: DsCommand.padding),
      ],
    );
  ...
  Widget _inset(Widget child) => Padding(
    padding: EdgeInsets.symmetric(horizontal: DsCommand.padding),
    child: child,
  );
}
...
///
/// Not a [DsInputGroup]: that widget fixes the 40px pill, the `--card` fill and
...
class _CommandInput extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Color fill = theme.input.withValues(alpha: DsCommand.inputFillAlpha);
    ...
    Widget group = Row(
      ...
      children: <Widget>[
        ...
        Padding(
          padding: EdgeInsets.only(
            left: ds(2),
            // `py-1.5`, the same inset [DsInputGroupAddon] carries.
            top: DsInputGroupAddon.insetY,
            bottom: DsInputGroupAddon.insetY,
          ),
          child: Opacity(
            opacity: DsCommand.searchGlyphOpacity,
            child: const DsIcon(
              DsIconGlyph.search,
    ...
    group = DecoratedBox(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: fill, width: DsWidths.hairline),
      ),
      child: group,
    );
...
class _CommandRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    row = Opacity(
      opacity: item.enabled ? 1 : DsCommand.disabledOpacity,
      child: row,
    );
...
class _CommandEmpty extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.symmetric(vertical: ds(6)),
      child: DsText(
        label,
        DsComponentType.sheetBody,
