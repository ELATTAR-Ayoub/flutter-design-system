...
/// `DsSlidingPillGroup` makes. The pill deliberately **skips** its first
...
/// tree and contributes no name of its own. Put it inside a `DsButton` as the
/// child, never beside one.
class DsIconSwap extends StatefulWidget {
  const DsIconSwap({
    super.key,
    required this.icons,
  ...
  /// [DsSwapRoll.travelFor] of *that* — 25.6px at 16, 32px at 20.
  ///
  ...
  @override
  State<DsIconSwap> createState() => _DsIconSwapState();
}

class _DsIconSwapState extends State<DsIconSwap> with TickerProviderStateMixin {
  /// The squash's delay and the squash itself on **one** clock.
  ///
  /// `DsKeyframePlayer` has no delay parameter and a second controller started
  ...
  static final Duration _squashRun = DsSwapRoll.squashDelay + DsJelly.duration;

  /// Where `yuki-jelly`'s own 0% sits on that clock — 150/750 = 0.2.
  static final double _squashStart =
      DsSwapRoll.squashDelay.inMicroseconds / _squashRun.inMicroseconds;
  ...
  /// The strip position it is heading for — the resolved [DsIconSwap.activeIndex].
  late int _to;
  ...
  int get _active =>
      DsIconSwap.resolveIndex(widget.activeIndex, widget.icons.length);
  ...
  @override
  void didUpdateWidget(DsIconSwap old) {
  ...
  /// to honouring it.
  void _readMotion() {
    _roll.duration = dsAnimationDuration(context, DsSwapRoll.duration);
    _squash.duration = dsAnimationDuration(context, _squashRun);
  ...
  double get _rollT => DsSwapRoll.curve.transform(_roll.value);
  ...
  Widget build(BuildContext context) {
    // Re-read on every pass, the way `DsPress` and `DsSlidingPillGroup` do:
    // the OS switch can flip between two frames of a running roll.
    _roll.duration = dsAnimationDuration(context, DsSwapRoll.duration);
    _squash.duration = dsAnimationDuration(context, _squashRun);

    final double step = DsSwapRoll.travelFor(widget.cell);
  ...
  Widget _squashed(Widget icon) => AnimatedBuilder(
        animation: _squash,
        builder: (BuildContext context, Widget? child) {
          final Offset scale = DsJelly.scale.transform(_jellyT);
