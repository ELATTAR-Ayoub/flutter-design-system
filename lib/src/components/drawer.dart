...
class DsDrawer extends StatefulWidget {
  const DsDrawer({super.key, required this.trigger, required this.content});
...
class _DsDrawerState extends State<DsDrawer> {
  ...
  Widget build(BuildContext context) => DsModalPortal(
        ...
        overlayDuration: DsDurations.drawer,
        overlayCurve: DsCurves.vaul,
        transition: (
...
class _DrawerTransition extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) {
          final double t = DsCurves.vaul.transform(animation.value.clamp(0, 1));
...
class DsDrawerContent extends StatelessWidget {
  const DsDrawerContent({super.key, required this.children});

  /// `flex h-auto flex-col` with **no gap**.
  final List<Widget> children;

  /// `mt-24` — the strip of page the drawer may never cover.
  static double get topGutter => ds(24);
    ...
    return ConstrainedBox(
      ...
      child: DecoratedBox(
        ...
        child: ClipRRect(
          borderRadius: shape,
          child: Padding(
            padding: EdgeInsets.only(top: DsWidths.hairline),
            child: DefaultTextStyle(
              ...
              child: Column(
                ...
                children: <Widget>[const DsDrawerHandle(), ...children],
              ),
            ),
          ),
        ),
...
/// bottom direction.
class DsDrawerHandle extends StatelessWidget {
  const DsDrawerHandle({super.key});

  /// `w-24`.
  static double get width => ds(24);

  /// `h-1`.
  static double get height => ds(1);
...
/// direction.
class DsDrawerHeader extends StatelessWidget {
  const DsDrawerHeader({super.key, required this.children});
...
class DsDrawerFooter extends StatelessWidget {
  const DsDrawerFooter({super.key, required this.children});
...
class DsDrawerTitle extends StatelessWidget {
  const DsDrawerTitle(this.text, {super.key});
...
class DsDrawerDescription extends StatelessWidget {
  const DsDrawerDescription(this.text, {super.key});
