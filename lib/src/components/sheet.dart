...
///
/// **CORRECTED, and the correction is measured.** This shipped as `ds(10)` —
...
/// **28.8px**, not 40. Both the mobile-nav route below and [DsSheetContent]
/// read it from here.
...
// than a route (see `DsModalPortal`'s doc for why a route cannot be used).
//
...
enum DsSheetSide {
  ...
  bool get isHorizontal =>
      this == DsSheetSide.left || this == DsSheetSide.right;
}

/// `Sheet` — trigger, portal, overlay, panel.
///
/// Named with the `Panel` suffix free: [DsSheet] above is the mobile nav's
/// static opener and keeps its name, because `example/lib/shell.dart` calls it
/// and the shell is not this family's file.
class DsSheetOverlay extends StatelessWidget {
  const DsSheetOverlay({
    super.key,
...
/// spells out.
class DsSheetTransition extends StatelessWidget {
  const DsSheetTransition({
    super.key,
...
class DsSheetContent extends StatelessWidget {
  const DsSheetContent({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < children.length; i++) {
      ...
      if (i > 0) {
        ...
        if (child is DsSheetFooter) rows.add(const Spacer());
        ...
        child: DecoratedBox(
          decoration: BoxDecoration(
            ...
            border: switch (side) {
              ...
              DsSheetSide.top => Border(bottom: edge),
              DsSheetSide.bottom => Border(top: edge),
            },
            boxShadow: DsShadows.tailwindLg.outerShadows(theme),
          ),
          child: Padding(
            ...
            padding: switch (side) {
              DsSheetSide.left => EdgeInsets.only(right: DsWidths.hairline),
              DsSheetSide.right => EdgeInsets.only(left: DsWidths.hairline),
              DsSheetSide.top => EdgeInsets.only(bottom: DsWidths.hairline),
              DsSheetSide.bottom => EdgeInsets.only(top: DsWidths.hairline),
            },
            child: DefaultTextStyle(
              style: DsText.styleOf(
                context,
...
class DsSheetContentGroup extends InheritedWidget {
  const DsSheetContentGroup({
    super.key,
    required this.showCloseButton,
  ...
  static DsSheetContentGroup? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<DsSheetContentGroup>();

  @override
  bool updateShouldNotify(DsSheetContentGroup old) =>
...
class DsSheetHeader extends StatelessWidget {
  const DsSheetHeader({super.key, required this.children});
  ...
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final bool close =
        DsSheetContentGroup.maybeOf(context)?.showCloseButton ?? true;
...
class DsSheetFooter extends StatelessWidget {
  const DsSheetFooter({super.key, required this.children});
...
class DsSheetTitle extends StatelessWidget {
  const DsSheetTitle(this.text, {super.key});
...
class DsSheetDescription extends StatelessWidget {
  const DsSheetDescription(this.text, {super.key});
