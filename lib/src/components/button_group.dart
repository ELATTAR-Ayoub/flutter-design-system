...
/// | class | line | effect |
/// |---|---|---|
/// | `flex w-fit items-stretch` | 8 | a shrink-to-fit row whose children stretch to the tallest — this is the whole reason a height-less [DsButtonGroupText] comes out 40px |
...
/// the left end keeps the child's own radius — a `rounded-pill` [DsButton]
/// leaves a 20px stadium on a 40px control — while the right end is always
/// 12px.
///
/// ## Corners, and why a clip alone cannot make one
///
/// CSS restyles a child from its parent. Flutter cannot: [DsButton] paints
/// `BorderRadius.circular(DsRadii.pill)` in its own build (`button.dart:471`)
...
///
/// Members this file declares are not bled at all. [DsButtonGroupText] is told
/// its shape through `_DsSlot`, an [InheritedWidget] standing in for the
/// cascade, and paints it directly — corners, dropped left border and all;
/// [DsButtonGroupSeparator] is a 1px rule with no corners and no border, so
...
/// (or, for a [DsButtonGroupText], because it was told not to paint one). Two
...
/// group can hold — `--input` for an outline [DsButton], `--destructive/25` for
/// a destructive one, transparent for the other five variants and `--border`
/// for a [DsButtonGroupText] (which paints its own). Group B's first seam
...
///   resting colour.
/// * **Gradient surfaces are sampled from a wider box.** `sheen-action` and
///   `foil-value` ramp across the member; the ramp is laid out across the
...
/// [DsButtonVariant.destructive] (`button.tsx`, and `button.dart:115`).
///
...
/// it. A member that cannot (any [DsButton]) is wrapped in a `_BledSlot`
/// instead, which does the reading on its behalf.
class _DsSlot extends InheritedWidget {
  const _DsSlot({required this.shape, required super.child});

  final _SlotShape shape;

  static _SlotShape? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<_DsSlot>()?.shape;

  @override
  bool updateShouldNotify(_DsSlot oldWidget) => oldWidget.shape != shape;
}

/// A row of controls joined into one.
class DsButtonGroup extends StatelessWidget {
  const DsButtonGroup({super.key, required this.children});

  /// The members, in source order — buttons, [DsButtonGroupText] cells and
  /// [DsButtonGroupSeparator] rules.
  ///
  ...
  /// rules declare them — `DsRadii.pill` is 999 here, and clamps to half the
  ...
  static bool _hasDataSlot(Widget child) => child is! DsButtonGroupText;
  ...
  /// member. A trailing [DsButtonGroupText] is invisible to the selector, so
  ...
  static double _ownRadius(Widget child) {
    if (child is DsButton) return DsRadii.pill;
    if (child is DsButtonGroupText) return DsRadii.lg;
  ...
  static _SlotShape _shapeOf(List<Widget> children, int index) {
    ...
    return _SlotShape(
      ...
      // last one.
      right: forced ? DsRadii.lg : (last ? own : 0),
      leftBorder: first,
    );
  }

  /// The resting `border-color` of a [DsButton] variant — the colour of the
  ...
  static Color _frameOf(DsThemeData theme, DsButtonVariant variant) {
    switch (variant) {
  ...
  Widget _member(DsThemeData theme, int index) {
    final Widget child = children[index];
    final _SlotShape shape = _shapeOf(children, index);
    if (child is DsButtonGroupText) {
      return _DsSlot(shape: shape, child: child);
    }
    if (child is DsButton) {
      return _BledSlot(
        shape: shape,
...
/// [DsButton]. That frame is a stroke rather than a [BoxDecoration] border for
...
/// reaches straight past it to the last Button. [DsButtonGroup] reproduces
/// that, including the case where the reach is visibly wrong.
class DsButtonGroupText extends StatelessWidget {
  const DsButtonGroupText(this.text, {super.key, this.numeric = false});
  ...
  /// [DsComponentType.buttonGroupNum], transcribed once in the foundation layer
  /// rather than re-derived here.
  final bool numeric;

  /// `px-2.5` — 10px.
  static double get paddingX => ds(2.5);
  ...
  Widget build(BuildContext context) {
    ...
    // with a full border.
    final _SlotShape shape = _DsSlot.maybeOf(context) ??
        const _SlotShape(
          left: DsRadii.lg,
          right: DsRadii.lg,
          leftBorder: true,
        ...
        child: Padding(
          ...
          padding: EdgeInsets.fromLTRB(
            DsButtonGroupText.paddingX +
                (shape.leftBorder ? DsWidths.hairline : 0),
            DsWidths.hairline,
            DsButtonGroupText.paddingX + DsWidths.hairline,
            DsWidths.hairline,
          ),
          ...
          child: Center(
            ...
            child: DsText(
              text,
              numeric
                  ? DsComponentType.buttonGroupNum
                  : DsComponentType.buttonGroupText,
              maxLines: 1,
              softWrap: false,
            ),
          ),
...
///
/// `w-px` (**1px**, [DsWidths.hairline]) + `self-stretch` + `my-px` (**1px of
...
class DsButtonGroupSeparator extends StatelessWidget {
  const DsButtonGroupSeparator({super.key});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      // (row height − 2) tall.
      padding: EdgeInsets.symmetric(vertical: DsWidths.hairline),
      child: SizedBox(
        width: DsWidths.hairline,
        child: ColoredBox(color: DsTheme.of(context).input),
      ),
    );
  }
}
...
void _dsStrokeFrame(
  ...
  if (color.a == 0) return;
  final RRect r = shape.deflate(DsWidths.hairline / 2);
  if (r.width <= 0 || r.height <= 0) return;

  final Path path = Path();
  if (leftEnd) _dsAddEnd(path, r, left: true);
  if (rightEnd) _dsAddEnd(path, r, left: false);
  if (sides) {
    path
...
void _dsAddEnd(Path path, RRect r, {required bool left}) {
...
/// [DsButtonGroupText]'s border — every run of it, minus the left one when
...
class _Frame extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) => _dsStrokeFrame(
        canvas,
...
class _BledSlot extends SingleChildRenderObjectWidget {
  ...
  /// The member's own resting `border-color` — see `DsButtonGroup._frameOf`.
  final Color frame;
