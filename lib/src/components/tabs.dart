...
/// it is the same [DsSlidingPillGroup] here and the same single definition of
/// the squash.
///
...
/// [DsDurations.transitionDefault]); the 250ms it happens to run at is the
...
enum DsTabsVariant {
  ...
  /// [DsToggleVariant.standard] already carries.
  standard,
...
///
/// A **model**, not a widget, for [DsToggleGroupItem]'s reason: the travelling
...
@immutable
class DsTabItem {
  const DsTabItem({required this.label, this.content});
...
/// The port's convention across `DsSelect`, `DsCalendar` and `DsToggleGroup` is
...
class DsTabs extends StatelessWidget {
  const DsTabs({
    ...
    this.variant = DsTabsVariant.standard,
  });

  final List<DsTabItem> items;

  /// Which tab is active. Radix's `value`, resolved to an index for
  /// [DsSlidingPillGroup]'s positional substrate — [DsToggleGroup]'s own
  ...
  static double get trackPadding => ds(1);

  /// `gap-1` on `standard`, `gap-2` on `line`.
  static double gapFor(DsTabsVariant variant) =>
      variant == DsTabsVariant.line ? ds(2) : ds(1);

  EdgeInsets get _trackInset => variant == DsTabsVariant.line
  ...
  Widget build(BuildContext context) {
    ...
    final bool line = variant == DsTabsVariant.line;
    ...
    final Widget track = SizedBox(
      ...
      child: DsSlidingPillGroup(
        ...
        // [DsSlidingPillGroup.jellyAlignment].
...
class _DsTabsTrigger extends StatefulWidget {
  const _DsTabsTrigger({
    required this.label,
  ...
  @override
  State<_DsTabsTrigger> createState() => _DsTabsTriggerState();
}

class _DsTabsTriggerState extends State<_DsTabsTrigger> {
  bool _hovered = false;
  ...
  Color _ink(DsThemeData theme) {
    if (widget.active) {
      return widget.variant == DsTabsVariant.line
          ? theme.foreground
        ...
        child: Semantics(
          ...
          child: SizedBox(
            ...
            child: TweenAnimationBuilder<Color?>(
              ...
              curve: DsCurves.out,
              builder: (BuildContext context, Color? ink, Widget? _) => DsMachineSurface(
                ...
                child: Padding(
                  padding: EdgeInsets.symmetric(
                    horizontal: DsTabs.triggerPaddingX,
                  ),
                  child: Center(
