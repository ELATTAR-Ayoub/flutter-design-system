...
/// through [DsIconSwap] for **1600ms**, and a toast reports the outcome. It
...
/// reading"*. The port composes it on [DsModalPortal] — the dialogs family's
/// own portal, overlay and jelly transition — rather than on `DsDialogContent`,
...
enum DsAttachmentSize {
  ...
  String get label => this == DsAttachmentSize.md ? 'default' : name;
}

/// `orientation` — a row with a 160px floor, or a 96px tile that widens to 120
/// once it carries a title.
enum DsAttachmentOrientation { horizontal, vertical }

/// `AttachmentMedia variant`.
enum DsAttachmentMediaVariant {
  /// The default.
  icon,
...
/// What an [DsAttachment] publishes to the slots inside it — the port of
/// `group/attachment` plus its three `data-*` attributes.
class DsAttachmentScope extends InheritedWidget {
  const DsAttachmentScope({
    super.key,
    required this.state,
    required this.size,
  ...
  final DsAttachmentState state;
  final DsAttachmentSize size;
  final DsAttachmentOrientation orientation;

  static DsAttachmentScope of(BuildContext context) {
    final DsAttachmentScope? found =
        context.dependOnInheritedWidgetOfExactType<DsAttachmentScope>();
    assert(found != null, 'No DsAttachment above this slot.');
    return found!;
  }

  @override
  bool updateShouldNotify(DsAttachmentScope old) =>
      old.state != state ||
      old.size != size ||
...
/// The card.
class DsAttachment extends StatefulWidget {
  const DsAttachment({
    super.key,
    required this.media,
    this.content,
    this.actions,
    this.state = DsAttachmentState.done,
    this.size = DsAttachmentSize.md,
    this.orientation = DsAttachmentOrientation.horizontal,
  });

  /// `p-2` / `p-1.5` / `p-1` — the rule that wins; see the library note.
  static double paddingFor(DsAttachmentSize size) => switch (size) {
        DsAttachmentSize.md => ds(2),
        DsAttachmentSize.sm => ds(1.5),
        DsAttachmentSize.xs => ds(1),
      };

  /// `gap-2` / `gap-2.5` / `gap-1.5`. `sm`'s is the widest of the three, which
  /// is the reference's own arrangement.
  static double gapFor(DsAttachmentSize size) => switch (size) {
        DsAttachmentSize.md => ds(2),
        DsAttachmentSize.sm => ds(2.5),
        DsAttachmentSize.xs => ds(1.5),
      };

  /// `rounded-xl`, `rounded-lg` on `xs`.
  static double radiusFor(DsAttachmentSize size) =>
      size == DsAttachmentSize.xs ? DsRadii.lg : DsRadii.xl;

  /// `min-w-40` — the horizontal floor.
  static double get horizontalMinWidth => ds(40);

  /// `w-24`, widening to `w-30` once there is content.
  static double get verticalWidth => ds(24);
  static double get verticalWidthWithContent => ds(30);
  ...
  @override
  State<DsAttachment> createState() => _DsAttachmentState();
}

class _DsAttachmentState extends State<DsAttachment> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool vertical =
        widget.orientation == DsAttachmentOrientation.vertical;
    final double gap = DsAttachment.gapFor(widget.size);
    ...
    row = Padding(
      padding: EdgeInsets.all(DsAttachment.paddingFor(widget.size)),
      child: row,
    );
    ...
    if (widget.actions != null && vertical) {
      ...
      row = Stack(
        ...
        children: <Widget>[
          row,
          PositionedDirectional(top: ds(3), end: ds(3), child: widget.actions!),
        ],
      );
    }

    final Color borderColor = widget.state == DsAttachmentState.error
        ? theme.destructive
            .withValues(alpha: DsAttachment.errorBorderAlpha)
        : theme.border;

    Widget card = _DashedBorderBox(
      radius: DsAttachment.radiusFor(widget.size),
      color: borderColor,
      // `data-[state=idle]:border-dashed`.
      dashed: widget.state == DsAttachmentState.idle,
      fill: theme.card,
      ring: _focusWithin
          ? theme.ring.withValues(alpha: DsAttachment.focusRingAlpha)
          : null,
      child: row,
    );
    ...
    card = vertical
        ? SizedBox(
            width: widget.content == null
                ? DsAttachment.verticalWidth
                : DsAttachment.verticalWidthWithContent,
            child: card,
          )
        : ConstrainedBox(
            constraints:
                BoxConstraints(minWidth: DsAttachment.horizontalMinWidth),
            child: card,
          );

    return DsAttachmentScope(
      state: widget.state,
      size: widget.size,
      ...
      child: DefaultTextStyle.merge(
        ...
        style: DsText.styleOf(
          context,
          widget.size == DsAttachmentSize.md
              ? DsComponentType.attachmentTitle
              : DsComponentType.attachmentTitleSm,
  ...
  Widget build(BuildContext context) {
    ...
    Widget box = DecoratedBox(
      decoration: BoxDecoration(
        ...
        border: dashed
            ? null
            : Border.all(color: color, width: DsWidths.hairline),
        ...
        boxShadow: ring == null
            ...
            : DsShadowSpec(<DsShadowLayer>[
              ...
              ]).outerShadows(DsTheme.of(context)),
      ),
      child: Padding(
...
class _DashPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double half = DsWidths.hairline / 2;
    final Path path = Path()
      ..addRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(half, half, size.width - 2 * half,
              size.height - 2 * half),
    ...
    for (final ui.PathMetric metric in path.computeMetrics()) {
      ...
      while (at < metric.length) {
        final double end =
            math.min(at + _DashedBorderBox.dash, metric.length);
...
class DsAttachmentMedia extends StatelessWidget {
  const DsAttachmentMedia({
    super.key,
    required this.child,
    this.variant = DsAttachmentMediaVariant.icon,
    this.preview,
    this.previewName,
  ...
  static double? wellFor(DsAttachmentSize size) => switch (size) {
        DsAttachmentSize.md => ds(10),
        DsAttachmentSize.sm => ds(8),
        DsAttachmentSize.xs => ds(7),
      };

  /// `rounded-lg`, `rounded-md` on `xs`.
  static double radiusFor(DsAttachmentSize size) =>
      size == DsAttachmentSize.xs ? DsRadii.md : DsRadii.lg;
  ...
  static double glyphFor(
    DsAttachmentSize size,
    DsAttachmentOrientation orientation,
  ) {
    if (size == DsAttachmentSize.xs) return 14;
    if (orientation == DsAttachmentOrientation.vertical) return 24;
    return 16;
  }
  ...
  static double get previewMaxWidth => ds(192);
  ...
  static double get previewCloseInset => ds(3);

  final Widget child;
  final DsAttachmentMediaVariant variant;

  /// The full-size media, and the port of `src` + `preview`.
  ///
  /// Supplying one makes the well expandable: an [DsAttachmentTrigger] covers
  /// it at `cursor: zoom-in`, and pressing it opens the media over the dimmed
  /// page through the shared [DsModalPortal] — a dialog and not an alert
  ...
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final DsAttachmentScope scope = DsAttachmentScope.of(context);
    final bool error = scope.state == DsAttachmentState.error;
    final bool vertical =
        scope.orientation == DsAttachmentOrientation.vertical;

    // `group-data-[orientation=vertical]:w-full` is overruled by the size rules
    // on `sm` and `xs`, which are written after it.
    final double? width = vertical && scope.size == DsAttachmentSize.md
        ? null
        : wellFor(scope.size);

    Widget content = child;
    if (variant == DsAttachmentMediaVariant.image) {
      final bool dim = scope.state != DsAttachmentState.done &&
          scope.state != DsAttachmentState.idle;
      content = Opacity(
    ...
    // `IntrinsicHeight` — which is how `DsGrid` measures every state cell — a
...
///
/// Composed on [DsModalPortal] rather than on `DsDialogContent`, because this
  ...
  Widget build(BuildContext context) {
    ...
    return ConstrainedBox(
      constraints: BoxConstraints(
        maxWidth: DsAttachmentMedia.previewMaxWidth,
      ),
      child: Semantics(
        container: true,
...
/// card is vertical.
class DsAttachmentContent extends StatelessWidget {
  const DsAttachmentContent({super.key, required this.title, this.description});

  /// `group-data-[orientation=vertical]/attachment:px-1`.
  static double get verticalInset => ds(1);
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.symmetric(
        horizontal: vertical ? verticalInset : 0,
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        mainAxisSize: MainAxisSize.min,
...
class DsAttachmentTitle extends StatelessWidget {
  const DsAttachmentTitle(this.text, {super.key});
  ...
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final DsAttachmentScope scope = DsAttachmentScope.of(context);
    final bool shimmering = scope.state == DsAttachmentState.uploading ||
        scope.state == DsAttachmentState.processing;

    final Widget label = DsText(
      text,
      scope.size == DsAttachmentSize.md
          ? DsComponentType.attachmentTitle
          : DsComponentType.attachmentTitleSm,
    ...
    return DsShimmerText(child: label);
  }
}
...
class DsShimmerText extends StatefulWidget {
  const DsShimmerText({super.key, required this.child});
  ...
  static double get spreadPx => ds(10);
  ...
  @override
  State<DsShimmerText> createState() => _DsShimmerTextState();
}

class _DsShimmerTextState extends State<DsShimmerText>
    with SingleTickerProviderStateMixin {
  late final AnimationController _c = AnimationController(
    vsync: this,
    duration: DsShimmerText.period,
  );
  ...
  void didChangeDependencies() {
    ...
    if (dsAnimationDuration(context, DsShimmerText.period) == Duration.zero) {
      _c.stop();
      _c.value = 0;
  ...
  Widget build(BuildContext context) {
    ...
    final Color highlight = theme.kind == DsThemeKind.dark
        ...
        : base.withValues(alpha: DsShimmerText.lightHighlightAlpha);
    final Color mid = Color.lerp(highlight, base, 0.5)!;
    final double fontSize =
        DsText.styleOf(context, DsComponentType.attachmentTitleSm).fontSize ??
            12;
    ...
    final double spread =
        DsShimmerText.spreadChars * fontSize * 0.6455 + DsShimmerText.spreadPx;
      ...
      builder: (BuildContext context, Widget? child) => ShaderMask(
        ...
        shaderCallback: (Rect bounds) {
          ...
          final double radians =
              DsShimmerText.angleDegrees * math.pi / 180;
          final Offset axis =
...
class DsAttachmentDescription extends StatelessWidget {
  const DsAttachmentDescription(this.text, {super.key});

  /// `mt-0.5` — 2px.
  static double get topGap => ds(0.5);
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: DsText(
        ...
        color: scope.state == DsAttachmentState.error
...
class DsAttachmentActions extends StatelessWidget {
  const DsAttachmentActions({super.key, required this.children});

  /// `group-data-[orientation=vertical]/attachment:gap-1`.
  static double get verticalGap => ds(1);
  ...
  Widget build(BuildContext context) {
    final DsAttachmentScope scope = DsAttachmentScope.of(context);
    final double gap =
        scope.orientation == DsAttachmentOrientation.vertical ? verticalGap : 0;
...
///
/// A ghost `icon-xs` [DsButton], so it needs a label of its own. Pass
/// [downloadName] and it becomes the save control: the glyph rolls to a check
/// through [DsIconSwap] for [savingWindow], and [onDownload] is where the toast
/// is fired from.
class DsAttachmentAction extends StatefulWidget {
  const DsAttachmentAction({
    super.key,
    this.child,
    this.onPressed,
    this.label,
  ...
  static const Duration savingWindow = DsDurations.attachmentSaving;
  ...
  @override
  State<DsAttachmentAction> createState() => _DsAttachmentActionState();
}

class _DsAttachmentActionState extends State<DsAttachmentAction> {
  bool _saving = false;
  ...
  void _press() {
    ...
    if (name != null) {
      ...
      Future<void>.delayed(DsAttachmentAction.savingWindow, () {
  ...
  @override
  Widget build(BuildContext context) {
    final double px = DsButton.iconPxFor(DsButtonSize.iconXs);

    final Widget glyph = widget.child ??
        DsIconSwap(
          activeIndex: _saving ? 1 : 0,
          window: px,
          cell: px,
          icons: <Widget>[
            DsIcon.lucide(DsLucide.download, sizePx: px),
            DsIcon.lucide(DsLucide.check, sizePx: px),
          ],
        );
...
class DsAttachmentTrigger extends StatelessWidget {
  const DsAttachmentTrigger({
    super.key,
...
class DsAttachmentGroup extends StatefulWidget {
  const DsAttachmentGroup({super.key, required this.children});

  /// `gap-3`.
  static double get gap => ds(3);

  /// `py-1`.
  static double get paddingY => ds(1);

  /// `scroll-px-1`.
  static double get scrollPadding => ds(1);

  final List<Widget> children;

  @override
  State<DsAttachmentGroup> createState() => _DsAttachmentGroupState();
}

class _DsAttachmentGroupState extends State<DsAttachmentGroup> {
  ...
  @override
  void didUpdateWidget(DsAttachmentGroup old) {
  ...
  void _snap() {
    ...
    for (final GlobalKey key in _keys) {
      ...
      final double target = (p.pixels +
              box.localToGlobal(Offset.zero, ancestor: viewport).dx -
              DsAttachmentGroup.scrollPadding)
  ...
  Widget build(BuildContext context) {
    final Widget row = SingleChildScrollView(
      ...
      padding: EdgeInsets.symmetric(vertical: DsAttachmentGroup.paddingY),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          for (int i = 0; i < widget.children.length; i++) ...<Widget>[
            if (i > 0) SizedBox(width: DsAttachmentGroup.gap),
...
/// `scroll-fade-x` — the same scroll-driven mask `DsScrollFade` runs on the
...
class _ScrollFadeX extends StatelessWidget {
  ...
  static double get fadeCap => ds(10);

  /// `--scroll-fade-reveal`.
  static double get reveal => ds(24);
  ...
  /// [DsCurves.cssEaseInOut] over [reveal] px.
  static (double start, double end) fadesFor({
    ...
    return (
      full * DsCurves.cssEaseInOut.transform(sT),
      full * (1 - DsCurves.cssEaseInOut.transform(eT)),
    );
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        final double offset =
            controller.hasClients ? controller.position.pixels : 0;
        final double max = controller.hasClients
            ? controller.position.maxScrollExtent -
                controller.position.minScrollExtent
            : 0;
        final (double start, double end) =
            fadesFor(width: width, offset: offset, max: max);
        // Always mounted, even at zero fade — see `DsScrollFade`'s own note:
