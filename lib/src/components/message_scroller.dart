...
///   ([DsCurves.cssEaseInOut], *not* `--ease-in-out`).
///
/// So the fade is at full height for the first 302 of this specimen's 398px of
/// travel and closes over the last 96. [DsScrollFade] reproduces exactly that,
...
/// | leg | measured |
/// |---|---|
/// | hide | `scale 1 → 0.95` and `translate-y-0 → 100%` cut in one frame; opacity 1 → 0 over **250ms** on [DsCurves.curveIn] (`data-[active=false]:ease-in`) |
/// | show | the same two cut back; opacity 0 → 1 over **250ms** on [DsCurves.out] |
///
...
enum DsScrollPosition {
  ...
  /// The item marked [DsMessageScrollerItem.scrollAnchor], falling back to
  /// [end] when there is none.
  lastAnchor,
}

/// Which edge a [DsMessageScrollerButton] travels to.
...
class DsMessageScrollerController extends ChangeNotifier {
  DsMessageScrollerController({
    this.autoScroll = false,
    this.defaultScrollPosition = DsScrollPosition.end,
  ...
  final DsScrollPosition defaultScrollPosition;
  ...
  /// The offsets of every registered [DsMessageScrollerItem], by message id.
  ...
  bool scrollable(DsScrollDirection direction) {
    if (!scroll.hasClients) return direction == DsScrollDirection.end;
    final ScrollPosition p = scroll.position;
    return switch (direction) {
      DsScrollDirection.start => p.pixels - p.minScrollExtent >= scrollEdgeThreshold,
      DsScrollDirection.end => p.maxScrollExtent - p.pixels >= scrollEdgeThreshold,
    };
  }
  ...
  /// [DsScrollPosition.lastAnchor] restores to.
  ...
  Future<void> scrollToEnd() =>
      _smoothTo(scroll.position.maxScrollExtent);

  /// `scrollToStart()`.
  Future<void> scrollToStart() =>
      _smoothTo(scroll.position.minScrollExtent);
  ...
  /// within a frame, so the duration is [DsDurations.frame] × `√distance` and
  /// the shape is [DsCurves.cssEase].
  ///
  ...
  Future<void> _smoothTo(double target) async {
    ...
    await scroll.position.animateTo(
      target,
      duration: DsDurations.frame * math.sqrt(distance),
  ...
  void followNewContent() {
    ...
    if (!scrollable(DsScrollDirection.end)) {
...
/// Publishes a [DsMessageScrollerController] to the subtree — the port of
/// `MessageScrollerProvider`.
class DsMessageScrollerProvider extends InheritedNotifier<DsMessageScrollerController> {
  const DsMessageScrollerProvider({
    super.key,
    required DsMessageScrollerController controller,
    required super.child,
  }) : super(notifier: controller);

  static DsMessageScrollerController of(BuildContext context) {
    final DsMessageScrollerProvider? found = context
        .dependOnInheritedWidgetOfExactType<DsMessageScrollerProvider>();
    assert(found != null, 'No DsMessageScrollerProvider above this widget.');
...
class DsMessageScroller extends StatelessWidget {
  const DsMessageScroller({
    super.key,
  ...
  /// [DsMessageScrollerButton], absolutely positioned against this frame.
  final Widget? button;
...
class DsMessageScrollerViewport extends StatefulWidget {
  const DsMessageScrollerViewport({
    super.key,
    required this.child,
  ...
  static double get thumbThickness => ds(2);
  ...
  @override
  State<DsMessageScrollerViewport> createState() =>
      _DsMessageScrollerViewportState();
}

class _DsMessageScrollerViewportState extends State<DsMessageScrollerViewport> {
  bool _restored = false;

  @override
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final DsMessageScrollerController c = DsMessageScrollerProvider.of(context);
      ...
      child: SingleChildScrollView(
        ...
        child: Padding(
          ...
          padding: const EdgeInsetsDirectional.only(
            end: DsMessageScrollerViewport.gutter,
          ),
          child: widget.child,
        ),
      ),
    ...
    scroller = RawScrollbar(
      ...
      thumbColor: c.autoscrolling ? dsTransparent : theme.border,
      thickness: DsMessageScrollerViewport.thumbThickness,
    ...
    return Semantics(
      ...
      child: DsScrollFade(controller: c, child: scroller),
    );
  }
}
...
class DsScrollFade extends StatelessWidget {
  const DsScrollFade({
    super.key,
  ...
  final DsMessageScrollerController controller;
  final Widget child;
  ...
  static double fadeFor({
    ...
    return full * (1 - DsCurves.cssEaseInOut.transform(t));
  }
...
class DsMessageScrollerContent extends StatelessWidget {
  const DsMessageScrollerContent({
    super.key,
...
class DsMessageScrollerItem extends StatefulWidget {
  const DsMessageScrollerItem({
    super.key,
    required this.child,
    this.messageId,
  ...
  static double get assumedOffscreenHeight => ds(40);
  ...
  /// [DsScrollPosition.lastAnchor].
  final bool scrollAnchor;

  @override
  State<DsMessageScrollerItem> createState() => _DsMessageScrollerItemState();
}

class _DsMessageScrollerItemState extends State<DsMessageScrollerItem> {
  @override
  Widget build(BuildContext context) {
    ...
    if (widget.messageId != null || widget.scrollAnchor) {
      final DsMessageScrollerController c =
          DsMessageScrollerProvider.of(context);
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted || !c.scroll.hasClients) return;
...
///
/// Renders a [DsButton] and hides itself the moment that direction has nowhere
...
class DsMessageScrollerButton extends StatelessWidget {
  const DsMessageScrollerButton({
    super.key,
    this.direction = DsScrollDirection.end,
  });

  /// `data-[direction=end]:bottom-4` / `data-[direction=start]:top-4`.
  static double get inset => ds(4);

  /// `data-[active=false]:scale-95`.
  static const double inactiveScale = 0.95;

  final DsScrollDirection direction;
  ...
  Widget build(BuildContext context) {
    ...
    final double size = DsButton.heightFor(DsButtonSize.iconSm);
    ...
    Widget button = DsButton(
      ...
      label: direction == DsScrollDirection.end
          ? 'Scroll to end'
          : 'Scroll to start',
      onPressed: active
          ? () => direction == DsScrollDirection.end
              ? c.scrollToEnd()
              : c.scrollToStart()
          : null,
      child: Transform.rotate(
        // `data-[direction=start]:[&_svg]:rotate-180`.
        angle: direction == DsScrollDirection.start ? math.pi : 0,
        child: DsIcon.lucide(
          DsLucide.arrowDown,
    ...
    button = Transform.translate(
      ...
      child: Transform.scale(
        ...
        child: AnimatedOpacity(
          ...
          curve: active ? DsCurves.out : DsCurves.curveIn,
          child: button,
        ),
      ),
    );
    ...
    return Positioned(
      top: direction == DsScrollDirection.start ? inset : null,
      bottom: direction == DsScrollDirection.end ? inset : null,
      left: 0,
      right: 0,
