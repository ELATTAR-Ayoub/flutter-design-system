...
/// [DsComponentType.navMenuTrigger].
///
...
///     [DsPopoverOriginModel.selfCenter].
...
/// mounts it through [DsPopover] instead, which is the port's one mechanism for
...
/// [DsToaster.unmountDelay]'s argument: putting a third-party runtime constant
...
class DsNavigationMenuItem {
  ...
  /// beside it.
  const DsNavigationMenuItem.link({required this.label, this.onTap})
    : content = null,
      _isTrigger = false;
...
/// The bar.
class DsNavigationMenu extends StatefulWidget {
  const DsNavigationMenu({
    super.key,
    required this.items,
    this.viewport = true,
  ...
  final List<DsNavigationMenuItem> items;
  ...
  /// `gap-1` on the list.
  static double get listGap => ds(1);

  /// `h-10` on the trigger.
  static double get triggerHeight => ds(10);

  /// `px-4` on the trigger.
  static double get triggerPaddingX => ds(4);

  /// `gap-1.5` between a trigger's label and its chevron.
  static double get triggerGap => ds(1.5);

  /// `size="sm"` on the chevron.
  static double get chevronPx => DsIcon.pxFor(DsIconSize.sm);

  /// `mt-2` — the gap between the bar and the panel.
  static double get panelOffset => ds(2);

  /// `p-2` inside the panel.
  static double get panelPadding => ds(2);
  ...
  static double get indicatorHeight => ds(2);

  /// `size-2` — the square that becomes the caret once it is rotated.
  static double get caretSize => ds(2);

  @override
  State<DsNavigationMenu> createState() => _DsNavigationMenuState();
}

class _DsNavigationMenuState extends State<DsNavigationMenu> {
  ...
  @override
  void didUpdateWidget(DsNavigationMenu old) {
  ...
  Widget _panelBody(BuildContext context, Widget content) {
    ...
    return DsPopoverSurface(
      child: Padding(
        padding: EdgeInsets.all(DsNavigationMenu.panelPadding),
  ...
  Widget build(BuildContext context) {
    ...
    final Widget list = Row(
      ...
      children: <Widget>[
        for (int i = 0; i < widget.items.length; i++) ...<Widget>[
          if (i > 0) SizedBox(width: DsNavigationMenu.listGap),
    ...
    return DsPopover(
      ...
      // [DsPopoverBarrier.none] carries for a submenu, and this menu has no
  ...
  Widget _indicator(BuildContext context) {
    final int? open = _open;
    return DsNavigationMenuIndicator(
      width: (open != null && open < _widths.length) ? _widths[open] : 0,
    );
  }

  Widget _item(BuildContext context, int index) {
    final DsNavigationMenuItem item = widget.items[index];
    if (!item._isTrigger) {
      return _DsNavigationMenuTrigger(
        label: item.label,
        open: false,
        chevron: false,
        onTap: item.onTap,
    ...
    final Widget trigger = _DsNavigationMenuTrigger(
      label: item.label,
      open: _open == index,
      chevron: true,
    ...
    return DsPopover(
      open: _open == index,
      align: DsPopoverAlign.start,
      sideOffset: DsNavigationMenu.panelOffset,
      origin: DsPopoverOriginModel.selfCenter,
      slideSides: const <DsPopoverSide>{},
      barrier: DsPopoverBarrier.none,
      anchor: trigger,
...
/// shipped."*
///
/// **Where it sits is drift 1 on [DsNavigationMenu]**: Radix pins it to the
...
/// follows the open trigger. Mounted by [DsNavigationMenu] when its
...
class DsNavigationMenuIndicator extends StatelessWidget {
  const DsNavigationMenuIndicator({super.key, required this.width});
  ...
  @override
  Widget build(BuildContext context) {
    final double caret = DsNavigationMenu.caretSize;
    return SizedBox(
      width: width,
      height: DsNavigationMenu.indicatorHeight,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        // square is turned.
        borderRadius: BorderRadius.only(topLeft: Radius.circular(DsRadii.sm)),
        border: Border.all(
...
/// `ring-1 ring-foreground/10` — the same rim [DsPopoverSurface] wears.
const double _caretRingAlpha = 0.10;

/// The 40px pill: `navigationMenuTriggerStyle()`, with or without a chevron.
class _DsNavigationMenuTrigger extends StatefulWidget {
  const _DsNavigationMenuTrigger({
    required this.label,
    required this.open,
  ...
  @override
  State<_DsNavigationMenuTrigger> createState() =>
      _DsNavigationMenuTriggerState();
}

class _DsNavigationMenuTriggerState extends State<_DsNavigationMenuTrigger> {
  bool _hovered = false;
    ...
    return MouseRegion(
      ...
      child: Semantics(
        ...
        child: DsPress(
          onTap: widget.onTap,
          child: SizedBox(
            height: DsNavigationMenu.triggerHeight,
            ...
            child: Container(
              decoration: BoxDecoration(
                ...
                color: lit ? theme.secondary : dsTransparent,
              ...
              child: Padding(
                padding: EdgeInsets.symmetric(
                  horizontal: DsNavigationMenu.triggerPaddingX,
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: <Widget>[
                    ...
                    if (widget.chevron) ...<Widget>[
                      SizedBox(width: DsNavigationMenu.triggerGap),
    ...
    return DefaultTextStyle.merge(
      ...
      child: TweenAnimationBuilder<double>(
        ...
        child: DsIcon(
          ...
          sizePx: DsNavigationMenu.chevronPx,
...
/// string is not `true`"*.
class DsNavigationMenuLink extends StatefulWidget {
  const DsNavigationMenuLink({
    super.key,
    required this.child,
    this.active = false,
    this.onTap,
  ...
  static double get iconPx => ds(4);

  @override
  State<DsNavigationMenuLink> createState() => _DsNavigationMenuLinkState();
}

class _DsNavigationMenuLinkState extends State<DsNavigationMenuLink> {
  bool _hovered = false;
    ...
    return MouseRegion(
      ...
      child: Semantics(
        ...
        child: DsPress(
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              color: lit ? theme.accent : dsTransparent,
              borderRadius: BorderRadius.circular(DsRadii.md),
            ),
            child: Padding(
              padding: EdgeInsets.symmetric(
                horizontal: DsNavigationMenuLink.paddingX,
                vertical: DsNavigationMenuLink.paddingY,
              ),
