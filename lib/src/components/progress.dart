...
/// off the end. [DsProgress] therefore translates, exactly as written.
///
...
enum DsProgressTone {
  ...
  String get label => this == DsProgressTone.normal ? 'default' : name;

  /// `bg-<x>-ink` — the fill.
  Color inkOf(DsThemeData theme) => switch (this) {
        DsProgressTone.normal => theme.actionInk,
        DsProgressTone.value => theme.valueInk,
        DsProgressTone.success => theme.successInk,
        DsProgressTone.warning => theme.warningInk,
        DsProgressTone.destructive => theme.destructiveInk,
      };
  ...
  /// shadow.
  DsShadowSpec get shadow => switch (this) {
        DsProgressTone.normal => DsShadows.btnPrimary,
        DsProgressTone.value => DsShadows.btnValue,
...
class DsProgress extends StatelessWidget {
  ...
  static double get height => ds(2.5);
  ...
  /// is [DsDurations.transitionDefault], not [DsDurations.base] — two
  /// declarations that agree at 250ms today. Measured on the live page as
  /// `transform 250ms cubic-bezier(0.22, 1, 0.36, 1)`.
  static Duration get transition => DsDurations.transitionDefault;
        ...
        child: DsMachineSurface(
          ...
          child: ClipRRect(
            ...
            child: _AnimatedFractionalTranslation(
              ...
              duration: dsAnimationDuration(context, transition),
              curve: DsCurves.out,
...
///
/// Private: it is [DsProgress]'s own plumbing, not a component, and the barrel
