...
const double _railPadding = DsWidths.hairline;
...
class DsScrollArea extends StatefulWidget {
  const DsScrollArea({
    super.key,
    this.borderRadius,
  ...
  @override
  State<DsScrollArea> createState() => _DsScrollAreaState();
}

class _DsScrollAreaState extends State<DsScrollArea> {
...
class _Viewport extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // query — and a `ScrollArea` inside a `DsGrid` is asked for one, because
...
class _RailState extends State<_Rail> {
  ...
  double _thumbLength(double track, ScrollPosition position) {
  ...
  void _jumpTo(double pixels, ScrollPosition position) {
...
/// starts inside a [DsScrollArea] does not also move the page.
///
...
/// to the parent.
class DsScrollAreaBehavior extends ScrollBehavior {
  const DsScrollAreaBehavior();
