...
///     [DsResizablePanel.defaultSize] is a weight, [DsResizablePanel.minSize]
///     is pixels.
...
const String dsResizableHandleFocusLabel = 'DsResizableHandle';

/// One panel's declaration.
class DsResizablePanel {
...
/// pair.
class DsResizablePanelGroup extends StatefulWidget {
  const DsResizablePanelGroup({
    super.key,
  ...
  final List<DsResizablePanel> panels;
  ...
  @override
  State<DsResizablePanelGroup> createState() => _DsResizablePanelGroupState();
}

class _DsResizablePanelGroupState extends State<DsResizablePanelGroup> {
  ...
  List<double> _normalised() {
    final double total = widget.panels.fold(
      0,
      (double a, DsResizablePanel p) => a + p.defaultSize,
    );
    if (total <= 0) {
    ...
    return <double>[
      for (final DsResizablePanel p in widget.panels)
