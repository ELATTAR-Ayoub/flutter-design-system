...
({double min, double max}) dsChartNiceDomain(
  double min,
  double max, {
  int tickCount = 5,
  bool allowDecimals = true,
}) {
  final List<double> ticks = dsChartNiceTicks(
    min,
    max,
    tickCount: tickCount,
...
List<double> _tickOfSingleValue(double value, int tickCount, bool decimals) {
  ...
  return <double>[
    for (int i = 0; i < tickCount; i++) _snap(middle + (i - middleIndex) * step),
  ];
}
...
class DsLinearScale {
  ...
  factory DsLinearScale.nice({
    ...
    if (lo == hi) hi = lo + 1;
    final ({double min, double max}) d =
        dsChartNiceDomain(lo, hi, tickCount: tickCount);
    return DsLinearScale(
      domainMin: d.min,
      domainMax: d.max,
  ...
  List<double> ticks({int tickCount = 5}) =>
      dsChartNiceTicks(domainMin, domainMax, tickCount: tickCount);
}

/* ── Bar layout ──────────────────────────────────────────────────────────── */

/// One `Bar`'s offset inside its band, and its width.
typedef DsBarSlot = ({double offset, double size});
...
List<DsBarSlot> dsBarSlots({
  required double bandSize,
  required int barCount,
  double barCategoryGapRatio = 0.1,
  double barGap = 4,
}) {
  if (barCount < 1) return const <DsBarSlot>[];
...
RRect dsBarRRect(Rect rect, List<double> radii) {
...
enum DsCurveType { linear, natural, monotone, step }
...
Path dsCurvePath(List<Offset> points, DsCurveType type) {
...
Path dsAreaPath(List<Offset> top, List<Offset> base, DsCurveType type) {
...
void _emitCurve(
  ...
  switch (type) {
    case DsCurveType.linear:
      start(pts.first);
      for (int i = 1; i < pts.length; i++) {
        path.lineTo(pts[i].dx, pts[i].dy);
      }
    case DsCurveType.step:
      start(pts.first);
      for (int i = 1; i < pts.length; i++) {
    ...
    case DsCurveType.natural:
      _emitNatural(path, pts, start);
    case DsCurveType.monotone:
...
void _emitMonotoneX(Path path, List<Offset> pts, void Function(Offset) start) {
  ...
  double slope3(double x2, double y2) {
    ...
    final double v = (sign(s0) + sign(s1)) *
...
/// instead.
Offset dsPolarToCartesian(double cx, double cy, double radius, double angle) {
...
double dsPolarMaxRadius(double width, double height, {double inset = 10}) =>
...
Path dsSectorPath({
  ...
  final Path path = Path();
  final Offset outerStart = dsPolarToCartesian(cx, cy, outerRadius, startAngle);
  path.moveTo(outerStart.dx, outerStart.dy);
  _arc(path, cx, cy, outerRadius, startAngle, tempEnd);
  if (innerRadius > 0) {
    final Offset innerEnd = dsPolarToCartesian(cx, cy, innerRadius, tempEnd);
...
Path dsSectorWithCornerPath({
  ...
  final _Tangent so = _tangentCircle(
      cx, cy, outerRadius, startAngle, sign, false, cornerRadius);
  final _Tangent eo = _tangentCircle(
      cx, cy, outerRadius, endAngle, -sign, false, cornerRadius);
  final double outerArc =
      (startAngle - endAngle).abs() - so.theta - eo.theta;
  if (outerArc < 0) {
    return dsSectorPath(
      cx: cx,
      cy: cy,
  ...
  if (innerRadius > 0) {
    final _Tangent si = _tangentCircle(
        cx, cy, innerRadius, startAngle, sign, true, cornerRadius);
    final _Tangent ei = _tangentCircle(
        cx, cy, innerRadius, endAngle, -sign, true, cornerRadius);
    final double innerArc =
        (startAngle - endAngle).abs() - si.theta - ei.theta;
    if (innerArc < 0) {
      path.lineTo(cx, cy);
      path.close();
      return path;
...
_Tangent _tangentCircle(
  ...
  final double theta =
      math.asin(cornerRadius / centerRadius) * 180 / math.pi;
  final double centerAngle = angle + sign * theta;
  final Offset circleTangency =
      dsPolarToCartesian(cx, cy, radius, centerAngle);
  final Offset lineTangency = dsPolarToCartesian(
    cx,
    cy,
...
void _arc(
  ...
  // same statement `dsPolarToCartesian` already makes.
...
Path dsPolarPolygonPath(List<Offset> points) {
  final Path path = Path();
