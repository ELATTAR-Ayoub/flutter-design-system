...
///    reads the variable directly. [DsToolChip] runs
...
// consumer of the library rather than just for this file. [DsAgentMessage]
...
const int dsMaxJsonChars = 4000;

/* ── Message ─────────────────────────────────────────────────────────────── */

/// `UserMessage` — the bubble.
class DsUserMessage extends StatelessWidget {
  const DsUserMessage({
    super.key,
    required this.turn,
    this.imageBuilder,
  ...
  /// `px-4 py-3`.
  static double get padX => ds(4);
  static double get padY => ds(3);
            ...
            if (turn.text.isNotEmpty)
              ConstrainedBox(
                ...
                child: Container(
                  ...
                  child: DsAgentMarkdown(text: turn.text),
                ),
              ),
...
class DsAgentMessage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final String text = dsStripProtocol(turn.text);
          ...
          DefaultTextStyle.merge(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                if (turn.streaming) const DsTypingCursor(),
              ],
            ),
          ),
        if (turn.attachments.isNotEmpty) ...<Widget>[
...
class DsTypingCursor extends StatefulWidget {
  const DsTypingCursor({super.key});

  /// `h-4`.
  static double get markHeight => ds(4);

  /// `w-px`.
  static const double markWidth = DsWidths.hairline;

  /// `ml-1`.
  static double get inset => ds(1);

  @override
  State<DsTypingCursor> createState() => _DsTypingCursorState();
}

class _DsTypingCursorState extends State<DsTypingCursor>
  ...
  void didChangeDependencies() {
    ...
    if (dsAnimationDuration(context, DsPulseLive.duration) == Duration.zero) {
      _c.stop();
      _c.value = 0;
  ...
  Widget build(BuildContext context) {
    ...
    final double lineBox =
        (ambient.fontSize ?? DsType.body.size!) * (ambient.height ?? 1);
    ...
    final double halfLeading =
        ((lineBox - dsContentAreaHeight(ambient)) / 2).clamp(0, lineBox);
        ...
        child: Padding(
          ...
          child: AnimatedBuilder(
            ...
            builder: (BuildContext context, Widget? _) => CustomPaint(
              size: Size(
                DsTypingCursor.markWidth,
                DsTypingCursor.markHeight,
              ),
...
/// [DsPulseLive] draws for the live dot. Both halves read off the same
...
class _CaretPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final double spread =
        DsPulseLive.ringSpread * DsPulseLive.ringPhase.transform(phase);
    if (spread > 0) {
      canvas.drawRect(
        Rect.fromLTWH(0, 0, size.width, size.height).inflate(spread),
        Paint()..color = DsPulseLive.ringColorAt(phase),
      );
    }
    canvas.drawRect(
      ...
      Paint()
        ..color = mark.withValues(
          alpha: mark.a * DsPulseLive.dotOpacityAt(phase),
        ),
    );
  }
...
class _SpinState extends State<_Spin> with SingleTickerProviderStateMixin {
  late final AnimationController _c =
      AnimationController(vsync: this, duration: DsDurations.spin);

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (dsAnimationDuration(context, DsDurations.spin) == Duration.zero) {
      _c.stop();
      _c.value = 0;
    } else if (!_c.isAnimating) {
      _c.repeat();
...
class _StatusIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (status == DsAgentTurnStatus.running) {
      ...
      return _Spin(
        child: DefaultTextStyle.merge(
          ...
          child: DsIcon.lucide(
            retry ? DsLucide.rotateCw : DsLucide.loaderCircle,
            sizePx: px,
    ...
    if (status == DsAgentTurnStatus.error) {
      return DsIcon.lucide(
...
/// available.
class DsToolChip extends StatefulWidget {
  const DsToolChip({
    super.key,
    required this.turn,
    this.toolStates,
  ...
  static double get panelPad => ds(3);
  static double get panelGap => ds(3);
  static const double panelFillAlpha = 0.40;

  /// `space-y-1` inside one labelled detail.
  static double get detailGap => ds(1);

  /// `max-h-64` on a raw value.
  static double get valueMaxHeight => ds(64);

  final DsToolTurn turn;
  final DsToolStateMap? toolStates;

  /// The caller's generative renderer for this tool's result, if it has one.
  final Widget Function(DsToolTurn)? renderResult;

  final Widget Function(BuildContext, DsAgentAttachment)? imageBuilder;
  final void Function(String name)? onDownload;

  @override
  State<DsToolChip> createState() => _DsToolChipState();
}

class _DsToolChipState extends State<DsToolChip> {
  bool _open = false;

  @override
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final DsToolTurn turn = widget.turn;
    final DsAgentState? mapped = dsStateForTool(turn.name, widget.toolStates);
    final String label =
        mapped == null ? dsHumaniseToolName(turn.name) : mapped.label;
    final Widget? rendered = turn.status == DsAgentTurnStatus.ok &&
            widget.renderResult != null
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        DsButton(
          ...
          autoHeight: true,
          padding: EdgeInsets.symmetric(
            horizontal: DsToolChip.padX,
            vertical: DsToolChip.padY,
          ),
          surface: turn.status == DsAgentTurnStatus.error
              ? DsButtonSurface(
                  border: theme.destructive
                      .withValues(alpha: DsToolChip.errorRimAlpha),
                )
              : null,
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              SizedBox(width: DsToolChip.contentGap),
              Flexible(
                child: DsText(
              ...
              AnimatedRotation(
                ...
                child: DsIcon.lucide(
                  DsLucide.chevronRight,
                  sizePx: DsToolChip.chevronPx,
                  tone: DsIconTone.inherit,
                ),
              ),
            ],
        ...
        if (_open) ...<Widget>[
          SizedBox(height: DsToolChip.gap),
          Container(
            padding: EdgeInsets.all(DsToolChip.panelPad),
            decoration: BoxDecoration(
              color: theme.muted.withValues(alpha: DsToolChip.panelFillAlpha),
            ...
            child: Column(
              ...
              children: <Widget>[
                _Detail(
                  label: 'Tool',
                  child: DsText(turn.name, DsType.code),
                ),
                if (turn.params.isNotEmpty) ...<Widget>[
                  SizedBox(height: DsToolChip.panelGap),
                  _Detail(
                    label: 'Arguments',
...
class _Detail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      mainAxisSize: MainAxisSize.min,
      children: <Widget>[
        DsText(label, DsType.caption, color: theme.mutedForeground),
        SizedBox(height: DsToolChip.detailGap),
        child,
      ],
    );
  }
...
class _Json extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool clipped = text.length > dsMaxJsonChars;
    final String shown = clipped
        ? '${text.substring(0, dsMaxJsonChars)}\n… '
            '${text.length - dsMaxJsonChars} more characters'
        : text;

    return ConstrainedBox(
      constraints: BoxConstraints(maxHeight: DsToolChip.valueMaxHeight),
...
class DsActionChip extends StatelessWidget {
  const DsActionChip({super.key, required this.turn});
  ...
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final String label =
        '${dsHumaniseToolName(turn.action)}${_describeTarget(turn.params)}';
    final Color rim = switch (turn) {
      DsActionTurn(status: DsAgentTurnStatus.error) =>
        theme.destructive.withValues(alpha: rimAlpha),
      DsActionTurn(approval: DsApprovalOutcome.rejected) =>
        DsPalette.warning.withValues(alpha: rimAlpha),
      _ => theme.border,
    };
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            Flexible(
              child: DsText(
                turn.approval == DsApprovalOutcome.rejected
                    ? 'Declined: $label'
                    : label,
                DsType.chip,
                color: theme.mutedForeground,
...
class DsApprovalCard extends StatelessWidget {
  const DsApprovalCard({super.key, required this.approval, this.describe});

  /// `p-4` and `space-y-3`.
  static double get pad => ds(4);
  static double get gap => ds(3);

  /// `gap-3` between the glyph and the sentence, and `space-y-1` inside it.
  static double get headGap => ds(3);
  static double get headLineGap => ds(1);

  /// `mt-1 size-4` on the glyph.
  static double get glyphPx => ds(4);
  static double get glyphTop => ds(1);
  ...
  static double get paramsMaxHeight => ds(40);
  static double get paramsPad => ds(2);

  /// `gap-2` between Approve and Decline.
  static double get actionGap => ds(2);

  /// The reason Decline hands back, verbatim.
  static const String declineReason = 'The user declined this action.';

  final DsPendingApproval approval;
  ...
  static String defaultSentence(DsPendingApproval approval) {
    final Object? target = approval.params['url'] ??
        approval.params['path'] ??
        approval.params['selector'];
    final String suffix =
        target is String && target.isNotEmpty ? ' on $target' : '';
  ...
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final String sentence = describe?.call(approval.action, approval.params) ??
    ...
    return DsFadeUp(
      child: Semantics(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: DsPalette.warning.withValues(alpha: washAlpha),
            borderRadius: BorderRadius.circular(DsRadii.lg),
            border: Border.all(
              color: DsPalette.warning.withValues(alpha: rimAlpha),
              width: DsWidths.hairline,
            ),
          ),
          child: Column(
...
class DsFadeUp extends StatefulWidget {
  const DsFadeUp({super.key, required this.child});

  /// `translateY(10px)` at the `from` stop.
  static double get rise => ds(2.5);

  final Widget child;

  @override
  State<DsFadeUp> createState() => _DsFadeUpState();
}

class _DsFadeUpState extends State<DsFadeUp>
    with SingleTickerProviderStateMixin {
  late final AnimationController _c =
      AnimationController(vsync: this, duration: DsDurations.slow);

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (dsAnimationDuration(context, DsDurations.slow) == Duration.zero) {
      _c.value = 1;
        ...
        builder: (BuildContext context, Widget? child) {
          ...
          return Opacity(
            ...
            child: Transform.translate(
              offset: Offset(0, DsFadeUp.rise * (1 - t)),
              child: child,
            ),
          );
        },
...
class DsRowIn extends StatefulWidget {
  const DsRowIn({super.key, required this.index, required this.child});

  /// `translateX(-10px)` at the `from` stop.
  static double get slide => ds(2.5);

  /// `calc(var(--duration-tick) + var(--row-index) * var(--duration-tick) / 2)`
  /// — 80ms, then 40ms per row. Measured 0.08 / 0.12 / 0.16s on the three
  /// capability chips.
  static Duration delayFor(int index) => DsDurations.tick * (1 + index / 2);

  final int index;
  final Widget child;

  @override
  State<DsRowIn> createState() => _DsRowInState();
}

class _DsRowInState extends State<DsRowIn> with SingleTickerProviderStateMixin {
  ...
  late final AnimationController _c = AnimationController(
    vsync: this,
    duration: DsRowIn.delayFor(widget.index) + DsDurations.base,
  );
  ...
  late final CurvedAnimation _t = CurvedAnimation(
    parent: _c,
    curve: Interval(
      DsRowIn.delayFor(widget.index).inMicroseconds /
          (DsRowIn.delayFor(widget.index) + DsDurations.base).inMicroseconds,
      1,
      curve: DsCurves.out,
    ),
  );
  ...
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (dsAnimationDuration(context, DsDurations.base) == Duration.zero) {
      _c.value = 1;
    } else if (!_c.isAnimating && _c.value == 0) {
      _c.forward();
    }
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) {
          ...
          return Opacity(
            opacity: t,
            child: Transform.translate(
              offset: Offset(-DsRowIn.slide * (1 - t), 0),
              child: child,
            ),
          );
        },
...
@immutable
class DsAgentCapability {
  const DsAgentCapability({
    required this.id,
    required this.label,
    this.hint,
    this.glyph,
...
class DsWelcomeCard extends StatelessWidget {
  const DsWelcomeCard({
    super.key,
    this.name,
    this.blurb,
    this.capabilities = const <DsAgentCapability>[],
  ...
  static double get outerPadY => ds(4);

  /// `max-w-md` on the column.
  static double get columnWidth => DsContainers.md;

  /// `size-lg` on the avatar — 80px.
  static double get avatarPx => ds(20);

  /// `mt-3` / `mt-1` / `mt-4` / `mt-4`, and `gap-2` / `gap-px` inside the two
  /// lists.
  static double get nameTop => ds(3);
  static double get blurbTop => ds(1);
  static double get listTop => ds(4);
  static double get gridGap => ds(2);
  static double get suggestionGap => DsWidths.hairline;

  /// `size-3.5` on a capability's glyph, and `gap-1.5` beside it (the `sm`
  /// button rung's own).
  static double get capabilityGlyphPx => ds(3.5);
  ...
  final List<DsAgentCapability> capabilities;
  ...
  /// than sending.
  final void Function(DsAgentCapability capability)? onUseCapability;
  ...
  static final DsTypeSpec capabilityLabel = DsTypeSpec(
    family: DsFonts.sans,
    size: DsType.small.size,
    height: DsType.caption.height,
    wght: 500,
  );

  /// `type-small` under the same rung — 13px / 19.5px / 500.
  static final DsTypeSpec suggestionLabel = DsTypeSpec(
    family: DsFonts.sans,
    size: DsType.small.size,
  ...
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final List<DsAgentCapability> shown =
        capabilities.take(maxCapabilities).toList(growable: false);

    return Padding(
      padding: EdgeInsets.symmetric(vertical: outerPadY),
      child: Center(
        child: DsSpringUpEntrance(
          child: SizedBox(
            width: columnWidth,
            child: Column(
              ...
              children: <Widget>[
                ...
                if (blurb != null) ...<Widget>[
                  ...
                  DsText(blurb!, DsType.small, align: TextAlign.center),
                ],
...
class _CapabilityGrid extends StatelessWidget {
  ...
  final List<DsAgentCapability> capabilities;
  final bool disabled;
  final void Function(DsAgentCapability)? onUse;

  @override
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final double gap = DsWelcomeCard.gridGap;
    final int rows = (capabilities.length + 1) ~/ 2;

    Widget chip(int index) {
      final DsAgentCapability capability = capabilities[index];
      ...
      return DsRowIn(
        ...
        child: DsButton(
          ...
          // `hover:text-foreground`, and [DsButtonSurface] carried no
          ...
          child: Row(
            ...
            children: <Widget>[
              DefaultTextStyle.merge(
                ...
                child: DsIcon.lucide(
                  capability.glyph ?? DsLucide.sparkles,
                  sizePx: DsWelcomeCard.capabilityGlyphPx,
                  tone: DsIconTone.inherit,
                ),
              ),
...
class DsSpringUpEntrance extends StatefulWidget {
  const DsSpringUpEntrance({super.key, required this.child});

  final Widget child;

  @override
  State<DsSpringUpEntrance> createState() => _DsSpringUpEntranceState();
}

class _DsSpringUpEntranceState extends State<DsSpringUpEntrance>
    with SingleTickerProviderStateMixin {
  late final AnimationController _c =
      AnimationController(vsync: this, duration: DsSpringUp.duration);

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (dsAnimationDuration(context, DsSpringUp.duration) == Duration.zero) {
      _c.value = 1;
    } else if (!_c.isAnimating && _c.value == 0) {
      _c.forward();
    }
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) => Opacity(
          opacity: DsSpringUp.opacity.transform(_c.value),
          child: Transform.translate(
            offset: Offset(0, DsSpringUp.translateY.transform(_c.value)),
            child: child,
          ),
        ),
        child: widget.child,
...
class DsPopInEntrance extends StatefulWidget {
  const DsPopInEntrance({super.key, required this.child});

  final Widget child;

  @override
  State<DsPopInEntrance> createState() => _DsPopInEntranceState();
}

class _DsPopInEntranceState extends State<DsPopInEntrance>
    with SingleTickerProviderStateMixin {
  late final AnimationController _c =
      AnimationController(vsync: this, duration: DsPopIn.duration);

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (dsAnimationDuration(context, DsPopIn.duration) == Duration.zero) {
      _c.value = 1;
    } else if (!_c.isAnimating && _c.value == 0) {
      _c.forward();
    }
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) {
          ...
          final Offset scale = DsPopIn.scale.transform(_c.value);
          return Opacity(
            opacity: DsPopIn.opacity.transform(_c.value),
