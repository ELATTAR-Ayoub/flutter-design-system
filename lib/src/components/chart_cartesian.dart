...
class DsChartMargin {
  ...
  /// `DsChartMargin(left: 12, right: 12)` gets zeros on the other two axes,
  /// which is exactly what React's shallow `defaultProps` merge does and what
  /// the measured plot boxes above show.
  static const DsChartMargin standard =
      DsChartMargin(top: 5, right: 5, bottom: 5, left: 5);
...
/// point.
enum DsChartLayout { horizontal, vertical }

/// `XAxis` / `YAxis` — `type={"number" | "category"}`.
enum DsChartAxisType { category, number }
...
class DsChartAxis {
  const DsChartAxis({
    this.dataKey,
    this.type = DsChartAxisType.category,
    this.tickLine = true,
    this.axisLine = true,
    this.tickMargin = 0,
    this.minTickGap = 5,
...
@immutable
class DsChartGrid {
  const DsChartGrid({this.horizontal = true, this.vertical = true});
...
enum DsChartSeriesKind { area, bar, line }

/// A dot on a `Line` or `Radar` vertex.
@immutable
class DsChartDot {
  const DsChartDot({this.radius = 3, this.fill, this.stroke, this.strokeWidth = 1});
...
@immutable
class DsChartLabelList {
  const DsChartLabelList({
    this.dataKey,
    this.position = DsChartLabelPosition.top,
    this.offset = 5,
    this.color,
    this.formatter,
  });
  ...
  final String? dataKey;
  final DsChartLabelPosition position;
  final double offset;
...
enum DsChartLabelPosition { top, right, insideLeft, insideEnd }

/// One drawn series.
@immutable
class DsChartSeriesSpec {
  const DsChartSeriesSpec({
    required this.kind,
    ...
    this.fillOpacity = 1,
    this.strokeWidth = 1,
    this.curve = DsCurveType.linear,
    this.stackId,
    this.radii,
    this.cellFills,
    this.dot,
    this.labels = const <DsChartLabelList>[],
    this.gradient,
  });

  final DsChartSeriesKind kind;
  final String dataKey;
  final Color? stroke;
  final Color? fill;
  ...
  final DsChartDot? dot;
  final List<DsChartLabelList> labels;
...
class DsChartTooltipSpec {
  ...
  final String Function(String label, List<DsChartTooltipItem> items)?
      labelFormatter;
  final Widget Function(BuildContext, DsChartTooltipItem, int)? formatter;
  final double? width;
}

/// `ChartLegend content={<ChartLegendContent />}`.
@immutable
class DsChartLegendSpec {
  const DsChartLegendSpec({
    this.align = DsChartLegendAlign.bottom,
    this.nameKey,
    this.wrap = false,
    this.gap,
    this.offset = 0,
  ...
  final DsChartLegendAlign align;
  final String? nameKey;
  final bool wrap;
  final double? gap;
  ...
  static double get reservedHeight => ds(3) + 16;
}
...
class DsCartesianChart extends StatefulWidget {
  const DsCartesianChart({
    super.key,
    required this.data,
    required this.series,
    this.margin = DsChartMargin.standard,
    this.layout = DsChartLayout.horizontal,
    this.grid,
    this.xAxis,
    this.yAxis,
    this.legend,
  ...
  final List<DsChartSeriesSpec> series;
  final DsChartMargin margin;
  final DsChartLayout layout;
  final DsChartGrid? grid;
  final DsChartAxis? xAxis;
  final DsChartAxis? yAxis;
  ...
  @override
  State<DsCartesianChart> createState() => _DsCartesianChartState();
}

class _DsCartesianChartState extends State<DsCartesianChart>
    with SingleTickerProviderStateMixin {
  late final AnimationController _entrance = AnimationController(
    vsync: this,
    duration: DsChartMotion.duration,
  );
  ...
  Widget _legend(
    ...
    final DsChartLegendSpec spec = widget.legend!;
    return Positioned(
      left: layout.frame.left,
      width: layout.frame.width,
      bottom: spec.align == DsChartLegendAlign.bottom ? spec.offset : null,
      top: spec.align == DsChartLegendAlign.top ? spec.offset : null,
      child: DsChartLegendContent(
        ...
        items: <DsChartLegendItem>[
          for (final DsChartSeriesSpec s in widget.series)
            DsChartLegendItem(
  ...
  Widget _tooltip(
    ...
    final List<DsChartTooltipItem> items = <DsChartTooltipItem>[
      for (final DsChartSeriesSpec s in widget.series)
        DsChartTooltipItem(
          name: s.dataKey,
          dataKey: s.dataKey,
    ...
    return Positioned(
      left: math.min(
        math.max(x + ds(3), 0),
        math.max(layout.size.width - DsChartTooltipContent.minWidth, 0),
      ),
...
class _CartesianLayout {
  ...
  static _CartesianLayout resolve({
    ...
    required DsCartesianChart widget,
    required Size size,
  }) {
    final bool horizontal = widget.layout == DsChartLayout.horizontal;
    final DsChartAxis? xAxis = widget.xAxis;
    final DsChartAxis? yAxis = widget.yAxis;

    final double legendReserve = widget.legend != null &&
            widget.legend!.align == DsChartLegendAlign.bottom
        ? DsChartLegendSpec.reservedHeight
        : 0;
    final double legendTopReserve = widget.legend != null &&
            widget.legend!.align == DsChartLegendAlign.top
        ? DsChartLegendSpec.reservedHeight
        : 0;

    final double left = widget.margin.left +
        (yAxis != null && !yAxis.hide
            ? (yAxis.size ?? DsChartAxis.defaultYSize)
            : 0);
    final double right = widget.margin.right;
    final double top = widget.margin.top + legendTopReserve;
    final double bottom = widget.margin.bottom +
        (xAxis != null && !xAxis.hide
            ? (xAxis.size ?? DsChartAxis.defaultXSize)
            : 0) +
        legendReserve;
    ...
    final DsChartAxis? categoryAxis = horizontal ? xAxis : yAxis;
    final DsChartAxis? valueAxis = horizontal ? yAxis : xAxis;
    final String categoryKey = categoryAxis?.dataKey ??
        (widget.data.isEmpty ? '' : widget.data.first.keys.first);
    final int n = widget.data.length;

    final bool hasBars = widget.series
        .any((DsChartSeriesSpec s) => s.kind == DsChartSeriesKind.bar);

    // ── the category scale ────────────────────────────────────────────────
    final double catStart = horizontal ? frame.left : frame.top;
    final double catExtent = horizontal ? frame.width : frame.height;
    final DsBandScale band =
        DsBandScale(count: n, start: catStart, extent: catExtent);
    final DsPointScale point =
        DsPointScale(count: n, start: catStart, extent: catExtent);
    ...
    final DsLinearScale value = DsLinearScale.nice(
      dataMin: stacks.min,
      dataMax: stacks.max,
    ...
    final DsChartGrid? grid = widget.grid;
    if (grid != null) {
      ...
      if (grid.horizontal) {
        for (final double v in vTicks) {
          ...
          if (horizontal) {
            gridLines.add(_GridLine(
                Offset(frame.left, y), Offset(frame.right, y)));
          }
        }
        if (!horizontal) {
          for (int i = 0; i < n; i++) {
    ...
    if (xAxis != null && !xAxis.hide) {
      final double baseline = frame.bottom +
          DsChartAxis.tickSize +
          xAxis.tickMargin +
          _capOffset;
      if (horizontal) {
        xTicks.addAll(_preserveEndTicks(
          context: context,
          labels: <String>[
            for (int i = 0; i < n; i++)
    ...
    if (yAxis != null && !yAxis.hide) {
      final double x =
          frame.left - DsChartAxis.tickSize - yAxis.tickMargin;
      if (horizontal) {
        for (final double v in value.ticks()) {
          yTicks.add(_AxisTick(
    ...
    final List<DsBarRect> bars = <DsBarRect>[];
    final List<_DotShape> dots = <_DotShape>[];
    final List<_SeriesLabel> labels = <_SeriesLabel>[];

    final List<DsChartSeriesSpec> barSeries = widget.series
        .where((DsChartSeriesSpec s) => s.kind == DsChartSeriesKind.bar)
        .toList();
    final Map<String?, int> barStackColumn = <String?, int>{};
    int barColumns = 0;
    for (final DsChartSeriesSpec s in barSeries) {
    ...
    final List<DsBarSlot> slots =
        dsBarSlots(bandSize: band.bandwidth, barCount: math.max(barColumns, 1));

    for (final DsChartSeriesSpec s in widget.series) {
      final List<double> lo = stacks.lower(s.dataKey);
      final List<double> hi = stacks.upper(s.dataKey);
      switch (s.kind) {
        case DsChartSeriesKind.area:
          ...
          areas.add(_AreaShape(
            fill: dsAreaPath(topPts, basePts, s.curve),
            stroke: dsCurvePath(topPts, s.curve),
            spec: s,
          ));
        case DsChartSeriesKind.line:
          final List<Offset> pts = <Offset>[
            for (int i = 0; i < n; i++)
              Offset(categoryCoords[i], value.scale(hi[i])),
          ];
          lines.add(_LineShape(path: dsCurvePath(pts, s.curve), spec: s));
          if (s.dot != null) {
            for (int i = 0; i < n; i++) {
          ...
          _collectLabels(labels, s, widget.data, pts, hi, DsChartLayout.horizontal);
        case DsChartSeriesKind.bar:
          final int column = barStackColumn[s.stackId ?? s.dataKey] ?? 0;
          final DsBarSlot slot =
              slots[math.min(column, slots.length - 1)];
          for (int i = 0; i < n; i++) {
            ...
            final Rect rect = horizontal
                ? Rect.fromLTRB(start, math.min(a, b), start + slot.size,
                    math.max(a, b))
                : Rect.fromLTRB(math.min(a, b), start, math.max(a, b),
                    start + slot.size);
            bars.add(DsBarRect(
              rect: rect,
          ...
          _collectLabels(
            ...
            <Offset>[
              for (int i = 0; i < n; i++)
                horizontal
                    ? Offset(band.bandStart(i) + slot.offset + slot.size / 2,
                        value.scale(hi[i]))
                    : Offset(value.scale(hi[i]),
                        band.bandStart(i) + slot.offset + slot.size / 2),
            ],
            hi,
            widget.layout,
            barStart: <double>[
              for (int i = 0; i < n; i++) value.scale(lo[i]),
            ],
          );
      }
    }
  ...
  static String _format(String Function(Object?)? f, Object? v) =>
      f != null ? f(v) : (v is num ? dsChartNumber(v) : '${v ?? ''}');
  ...
  static void _collectLabels(
    ...
    List<Offset> anchors,
    List<double> values,
    DsChartLayout layout, {
    List<double>? barStart,
  }) {
    for (final DsChartLabelList spec in s.labels) {
      for (int i = 0; i < anchors.length; i++) {
        final Object? raw =
            spec.dataKey != null ? data[i][spec.dataKey] : values[i];
        final String text = spec.formatter != null
            ? spec.formatter!(raw)
            : (raw is num ? dsChartNumber(raw) : '${raw ?? ''}');
        switch (spec.position) {
          case DsChartLabelPosition.top:
            out.add(_SeriesLabel(
              text: text,
              x: anchors[i].dx,
          ...
          case DsChartLabelPosition.right:
            out.add(_SeriesLabel(
              text: text,
          ...
          case DsChartLabelPosition.insideLeft:
            out.add(_SeriesLabel(
              text: text,
          ...
          case DsChartLabelPosition.insideEnd:
            out.add(_SeriesLabel(
              text: text,
              x: anchors[i].dx - spec.offset,
              baseline: anchors[i].dy + _midOffset,
...
class _Stacks {
  ...
  static _Stacks build(DsCartesianChart widget) {
    final int n = widget.data.length;
    ...
    if (widget.stackOffsetExpand) {
      for (final DsChartSeriesSpec s in widget.series) {
        ...
        final List<double> t =
            totals.putIfAbsent(key, () => List<double>.filled(n, 0));
        for (int i = 0; i < n; i++) {
...
class _CartesianPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final Paint grid = Paint()
      ..color = theme.border.withValues(alpha: DsChartGrid.strokeAlpha)
    ...
    if (cursor && activeIndex != null && layout.bars.isNotEmpty) {
      ...
      canvas.drawRect(
        Rect.fromLTRB(x - half, layout.frame.top, x + half, layout.frame.bottom),
    ...
    for (final _AreaShape area in layout.areas) {
      ...
      if (area.spec.gradient != null) {
        ...
        fill.color = (area.spec.fill ?? theme.foreground)
            .withValues(alpha: area.spec.fillOpacity);
      }
    ...
    for (final DsBarRect bar in layout.bars) {
      final Rect rect = _grow(bar, t);
      if (rect.isEmpty) continue;
      canvas.drawRRect(
        dsBarRRect(rect, bar.radii),
        Paint()..color = bar.color,
      );
    }
  }

  Rect _grow(DsBarRect bar, double t) {
...
class _TickLabel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Widget text = DsText(tick.text, DsChartText.xs, color: color);
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          if (anchor == _TickAnchor.middle)
            _CenteredAt(x: tick.x, child: _BaselineBox(child: text))
            ...
            _EndedAt(x: tick.x, child: _BaselineBox(child: text)),
        ],
      ),
    );
  }
...
/// The painter behind `DsBarRect`, exposed so the package test can pin a bar's
/// rendered pixels without standing a whole chart up.
class DsBarSeriesPainter extends CustomPainter {
  const DsBarSeriesPainter({required this.bars});

  final List<DsBarRect> bars;

  @override
  void paint(Canvas canvas, Size size) {
    for (final DsBarRect bar in bars) {
      canvas.drawRRect(dsBarRRect(bar.rect, bar.radii), Paint()..color = bar.color);
    }
  }

  @override
  bool shouldRepaint(DsBarSeriesPainter old) => old.bars != bars;
}
