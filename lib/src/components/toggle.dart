...
/// opposite of what [DsButton] does:
///
...
///   is the travelling pill in `DsToggleGroup`, not this.
///
/// The base class list, resolved (`toggle.tsx` L10):
///
/// | class | value |
/// |---|---|
/// | `group/toggle inline-flex items-center justify-center gap-1` | hugs its content; 4px between an icon and a label ([DsToggle.gap]) |
...
///   [DsToggleVariant.standard]: that variant is `bg-transparent` with no
///   border utility at all, so its box has zero border-width. It is live on
///   [DsToggleVariant.outline], which brings `border border-input` with it.
library;
...
enum DsToggleVariant {
  ...
  /// inner width and height on each side. [DsButton]'s base list carries
...
/// `disabled:opacity-50`.
///
/// Deliberately not [DsButton]'s 45%: the two components declare their own,
...
/// What the control paints in a given state — the same shape as `DsButton`'s
...
class _ToggleSkin {
  ...
  /// [DsToggleVariant.outline] — [DsToggleVariant.standard] has no border box
...
/// [inExclusiveGroup]) exist for `DsToggleGroup` alone. That is not a
...
class DsToggle extends StatefulWidget {
  const DsToggle({
    ...
    this.onChanged,
    this.variant = DsToggleVariant.standard,
  ...
  /// and re-resolving the class through [DsText] here would drop both.
  final Widget child;
  ...
  /// `h-7` / `h-8` / `h-9`.
  static double heightFor(DsToggleSize size) => switch (size) {
        DsToggleSize.sm => ds(7),
        DsToggleSize.md => ds(8),
        DsToggleSize.lg => ds(9),
      };
  ...
  static double minWidthFor(DsToggleSize size) => switch (size) {
        DsToggleSize.sm => ds(7),
        DsToggleSize.md => ds(8),
        DsToggleSize.lg => ds(9),
      };
  ...
  /// [DsToggleSize.sm]) need a child carrying `data-icon`, which nothing in
  ...
  ///
  /// Exposed rather than applied, exactly as [DsButton.gapFor] is: this
  ...
  /// `--radius-md` is [DsRadii.md], and the ceiling the CSS writes as a raw
  /// length is numerically [DsRadii.lg] — the radius every other size carries.
  ...
  static double radiusFor(DsToggleSize size) => switch (size) {
        DsToggleSize.sm => math.min(DsRadii.md, DsRadii.lg),
        DsToggleSize.md || DsToggleSize.lg => DsRadii.lg,
      };

  /// What `[&_svg:not([class*='size-'])]:size-*` would force an icon child to.
  ///
  /// A CSS descendant selector restyles a glyph it did not create; [DsIcon]
  /// takes its px up front, so this component cannot reach into its own child
  /// to resize it. The rule is exposed instead: pass
  /// `DsIcon(..., size: DsToggle.iconSizeFor(size))` and the child lands on
  /// the same 16px (14px on [DsToggleSize.sm]) the selector would have forced.
  ///
  ...
  static DsIconSize iconSizeFor(DsToggleSize size) => switch (size) {
        DsToggleSize.sm => DsIconSize.sm,
        DsToggleSize.md || DsToggleSize.lg => DsIconSize.md,
      };
...
class _DsToggleState extends State<DsToggle> {
  ...
  /// pointer surface neither — the identical hand-wiring `DsButton` carries,
  ...
  _ToggleSkin _skin(DsThemeData theme) {
    final bool bordered = widget.variant == DsToggleVariant.outline;
    return _ToggleSkin(
      fill: widget.pressed
  ...
  Widget build(BuildContext context) {
    ...
    final BorderRadius radius =
        BorderRadius.circular(DsToggle.radiusFor(widget.size));
    ...
    Widget toggle = _TransitionAllColors(
      ...
      builder: (
        ...
        return DsMachineSurface(
          spec: DsButton.withFocusRing(DsShadows.none, ring),
          radius: radius,
          fill: fill,
          ...
          // never spends.
          border: bordered
              ? Border.all(color: border, width: DsWidths.hairline)
              : null,
          child: Padding(
            // Just `px-*`: `DsMachineSurface` has already inset this child by
            // the border's width, the way `box-sizing: border-box` does.
            padding: EdgeInsets.symmetric(horizontal: DsToggle.paddingX),
            child: Center(
    ...
    toggle = SizedBox(height: DsToggle.heightFor(widget.size), child: toggle);
    ...
    toggle = ConstrainedBox(
      constraints: BoxConstraints(minWidth: DsToggle.minWidthFor(widget.size)),
      child: toggle,
    );

    // No `DsPress`, and no pointer-down tracking either: the class list has no
...
/// same trade `DsButton`'s `_SpringColors` makes, and for the same reason: one
