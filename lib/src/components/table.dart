...
/// [DsTableColumnWidth], whose `flex` is its own `maxIntrinsicWidth`: Flutter's
...
///   is [DsDurations.transitionDefault]. The hover fill is `bg-muted/50`; a
...
///   13px and the resolved 18.5714px line box — see [DsComponentType.tableHead]
...
@immutable
class DsTableColumnWidth extends TableColumnWidth {
  const DsTableColumnWidth();
  ...
  @override
  String toString() => 'DsTableColumnWidth()';
}
...
enum DsTableAlign {
  ...
  Alignment get alignment =>
      this == DsTableAlign.start ? Alignment.centerLeft : Alignment.centerRight;
}

/// One `<th>` or `<td>`.
class DsTableCellSpec {
  const DsTableCellSpec({
    required this.child,
    this.align = DsTableAlign.start,
    this.checkbox = false,
  });

  final Widget child;
  final DsTableAlign align;
...
/// One `<tr>` in the body.
class DsTableRowSpec {
  const DsTableRowSpec({required this.cells, this.selected = false})
      : span = null,
        spanHeight = null;
  ...
  /// what the assertion in [DsTable] enforces.
  const DsTableRowSpec.span(Widget this.span, {this.spanHeight})
      : cells = const <DsTableCellSpec>[],
        selected = false;

  final List<DsTableCellSpec> cells;
...
class DsTable extends StatefulWidget {
  ...
  final List<DsTableCellSpec> header;

  final List<DsTableRowSpec> rows;
  ...
  static double get headerHeight => ds(10);

  /// `px-2` on a `th`, `p-2` on a `td`.
  static double get cellPadding => ds(2);

  /// `mt-4` on the caption.
  static double get captionGap => ds(4);

  /// The rule between two rows.
  static double get ruleWidth => DsWidths.hairline;

  /// The half of that rule the last row keeps as layout and does not paint.
  static double get collapsedRemainder => DsWidths.hairline / 2;
...
class _DsTableState extends State<DsTable> {
  ...
  Color? _fill(DsThemeData theme, DsTableRowSpec row, int index) {
  ...
  Widget build(BuildContext context) {
    ...
    final List<DsTableRowSpec> rows = widget.rows;
    final bool spanning = rows.any((DsTableRowSpec r) => r.isSpan);
    assert(
    ...
    final List<Widget> stack = <Widget>[
      Table(
        defaultColumnWidth: const DsTableColumnWidth(),
        defaultVerticalAlignment: TableCellVerticalAlignment.intrinsicHeight,
        children: <TableRow>[
          TableRow(
            children: <Widget>[
              for (final DsTableCellSpec cell in widget.header)
          ...
          if (!spanning)
            for (int i = 0; i < rows.length; i++)
              TableRow(
                children: <Widget>[
                  for (final DsTableCellSpec cell in rows[i].cells)
                    _BodyCell(
                      spec: cell,
      ...
      if (spanning)
        SizedBox(
          ...
          child: Padding(
            padding: EdgeInsets.all(DsTable.cellPadding),
      ...
      if (widget.caption != null) ...<Widget>[
        SizedBox(height: DsTable.captionGap),
...
class _HeaderCell extends StatelessWidget {
  ...
  final DsTableCellSpec spec;

  @override
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    return Container(
      height: DsTable.headerHeight,
      alignment: spec.align.alignment,
      padding: EdgeInsets.only(
        left: DsTable.cellPadding,
        right: spec.checkbox ? 0 : DsTable.cellPadding,
      ),
      decoration: BoxDecoration(
        border: Border(
          bottom: BorderSide(color: theme.border, width: DsTable.ruleWidth),
        ),
      ),
...
class _BodyCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final EdgeInsets padding = EdgeInsets.fromLTRB(
      DsTable.cellPadding,
      DsTable.cellPadding,
      spec.checkbox ? 0 : DsTable.cellPadding,
      DsTable.cellPadding,
    );
    ...
    final EdgeInsets box = last && !alone
        ? padding.copyWith(bottom: padding.bottom + DsTable.collapsedRemainder)
        : padding;
    ...
    return MouseRegion(
      ...
      child: TweenAnimationBuilder<Color?>(
        ...
        duration: dsAnimationDuration(context, dsTableHoverDuration),
        curve: dsTableHoverCurve,
...
Duration get dsTableHoverDuration => DsDurations.transitionDefault;

/// The curve that transition runs on.
Curve get dsTableHoverCurve => DsCurves.out;
