...
/// | addon button | **24 × 24**, radius **7px** ([DsRadii.addonButton]), `p-0`, ghost, and `data-pressed:bg-transparent` — the one button in the system whose press cancels its own fill | — |
...
///
/// That is why [DsPopover] wraps the input rather than the group: the anchor
/// box is the measured thing, not the visible pill.
///
/// | property | value | source |
/// |---|---|---|
/// | width | `--anchor-width` + `ds(7)`, min-width beating width | *(measured 372)* |
/// | radius / fill | 12px, `--popover` | *(measured)* |
/// | elevation | `shadow-md` + 1px `--foreground`/10 ring — [DsPopoverSurface] | — |
...
/// | animation | **0.32s `cubic-bezier(0.22, 1, 0.36, 1)`** | *(measured — `--duration-overlay` on `--ease-out`, and [DsDurations.overlay]'s first consumer)* |
/// | list | `p-1`, cap `min(ds(72) − ds(9), available − ds(9))` = 252, scrollbar hidden | *(measured: `p-1`, and the popup's own `max-h` tracking `--available-height`)* |
...
/// `String(inputValue).trim()` (`AriaCombobox.js:195`). [dsCollatorContains] is
...
/// dependency may be added for it. [dsCollatorContains] folds case and strips
...
typedef DsComboboxItem<T> = DsSelectOption<T>;
...
bool dsCollatorContains(String label, String query) {
...
class _DsComboboxState<T> extends State<DsCombobox<T>> {
  ...
  FocusNode get _focusNode =>
      widget.focusNode ??
      _scope?.focusNode ??
      (_ownedFocusNode ??= FocusNode(debugLabel: 'DsCombobox'));
  ...
  String? _labelOf(T? value) {
    for (final DsComboboxItem<T> item in widget.items) {
  ...
  List<DsComboboxItem<T>> get _visible {
    ...
    final bool Function(String, String) match =
        widget.filter ?? dsCollatorContains;
    return widget.items
        .where((DsComboboxItem<T> i) => match(i.label, query))
        .toList();
  }
  ...
  void _commit(int index) {
    final List<DsComboboxItem<T>> visible = _visible;
    if (index < 0 || index >= visible.length) return;
    final DsComboboxItem<T> item = visible[index];
  ...
  Widget build(BuildContext context) {
    ...
    Widget group = DsInputGroup(
      ...
      endAddon: DsInputGroupAddon(
        ...
        // not a `DsInputGroupButton`, so the inference cannot see it — this is
...
class _ComboboxPopupState<T> extends State<_ComboboxPopup<T>> {
  ...
  void _reveal() {
    ...
    final double top =
        ds(1) + widget.highlighted * DsCombobox.itemHeight - ds(1);
    final double bottom = top + DsCombobox.itemHeight + ds(1) * 2;
        ...
        child: ClipRRect(
          ...
          child: ConstrainedBox(
            ...
            child: SingleChildScrollView(
              ...
              padding: EdgeInsets.all(empty ? 0 : ds(1)),
              child: empty
