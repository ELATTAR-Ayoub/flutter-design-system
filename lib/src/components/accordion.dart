...
class DsAccordionItem {
  const DsAccordionItem({required this.title, required this.content});
...
class DsAccordion extends StatelessWidget {
  const DsAccordion({
    super.key,
    required this.items,
  ...
  /// [DsTabs]' reason — the list is positional and an `indexWhere` miss would
...
class _DsAccordionTrigger extends StatefulWidget {
  const _DsAccordionTrigger({
    required this.title,
  ...
  @override
  State<_DsAccordionTrigger> createState() => _DsAccordionTriggerState();
}

class _DsAccordionTriggerState extends State<_DsAccordionTrigger> {
  bool _hovered = false;
  bool _focused = false;
        ...
        child: GestureDetector(
          ...
          child: Focus(
            ...
            child: TweenAnimationBuilder<Color?>(
              tween: ColorTween(end: _focused ? theme.ring : dsTransparent),
              duration: transition,
              curve: DsCurves.out,
              builder: (BuildContext context, Color? border, Widget? _) {
                return DsMachineSurface(
                  ...
                  // 38.57.
                  border: Border.all(
                    color: border ?? dsTransparent,
                    width: DsWidths.hairline,
                  ),
                  child: Padding(
                    padding: EdgeInsets.symmetric(
                      vertical: DsAccordion.triggerPaddingY,
                    ),
                    child: Row(
                      ...
                      children: <Widget>[
                        Expanded(
                          child: Builder(
                            builder: (BuildContext context) {
                              ...
                              // that has to ask for [DsLineBox] by hand, where
                              // [DsText] would have brought it.
                        ...
                        DsIcon(
                          widget.expanded
                              ? DsIconGlyph.chevronUp
                              : DsIconGlyph.chevronDown,
                          sizePx: DsAccordion.chevronPx,
