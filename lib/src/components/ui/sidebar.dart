...
/// row ladder, the paddings and the press feel all come from [Button], and
...
/// | menu row hover | `--secondary` reached over 250ms on `--ease-spring`, overshooting to rgb(43,43,46) past a rgb(39,39,42) target — [Button]'s own colour spring, untouched |
///
...
///     figures and −0.01em survive. See [TextStyles.sidebarMenuBadge].
...
///     opposite outcome. See [TextStyles.avatarFallback].
///  3. **`SidebarGroupLabel`'s `px-3 pr-10` never renders on the page.** Every
///     label on it is inside a [SidebarCollapsibleGroup], which passes
...
///     job and can. Reproduced: [SidebarRail] takes no focus.
...
///     149.5px list item that contains it (measured). [SidebarMenu] takes
...
enum SidebarSide {
  ...
  bool get isLeft => this == SidebarSide.left;
}
...
enum SidebarMenuButtonSize {
  ...
  /// Button's own ladder."*
  ButtonSize get button => switch (this) {
    SidebarMenuButtonSize.sm => ButtonSize.xs,
    SidebarMenuButtonSize.md => ButtonSize.sm,
    SidebarMenuButtonSize.lg => ButtonSize.lg,
  };
}

/// `SidebarMenuSubButton size`.
enum SidebarMenuSubButtonSize {
  /// → `Button` `xs`.
  sm,
  ...
  ButtonSize get button => switch (this) {
    SidebarMenuSubButtonSize.sm => ButtonSize.xs,
    SidebarMenuSubButtonSize.md => ButtonSize.sm,
  };
}
...
class SidebarScope extends InheritedWidget {
  ...
  /// [SidebarProvider.variant] for why a *provider* knows this at all.
  ...
  static SidebarScope of(BuildContext context) {
    final SidebarScope? scope = maybeOf(context);
    assert(
      scope != null,
      'SidebarScope.of must be used within a '
      'SidebarProvider.',
    );
    return scope!;
  }
  ...
  @override
  bool updateShouldNotify(SidebarScope old) =>
      old.open != open ||
...
/// The three `data-*` attributes [Sidebar] writes on itself, published to the
/// regions inside it.
///
...
class SidebarChrome extends InheritedWidget {
  const SidebarChrome({
    super.key,
    required this.side,
    required this.variant,
  ...
  bool get iconMode => collapsible == SidebarCollapsible.icon;

  static SidebarChrome? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<SidebarChrome>();

  /// False outside a [Sidebar], which is what an element carrying no
  /// `data-collapsible` resolves to.
  static bool iconModeOf(BuildContext context) =>
      maybeOf(context)?.iconMode ?? false;

  @override
  bool updateShouldNotify(SidebarChrome old) =>
      old.side != side ||
      old.variant != variant ||
...
class SidebarProvider extends StatefulWidget {
  ...
  /// The wrapper's flex children — a [Sidebar] and usually a
  /// [SidebarInset].
  ...
  /// site passes the same value here and to its [Sidebar]; they must agree,
...
class _SidebarProviderState extends State<SidebarProvider> {
  ...
  bool _isMobile(BuildContext context) =>
      SidebarProvider.isMobileWidth(MediaQuery.sizeOf(context).width);
  ...
  bool _onKey(KeyEvent event) {
    ...
    if (event.logicalKey != SidebarProvider.shortcut) return false;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.variant == SidebarVariant.inset) {
...
/// [collapsible] is [SidebarCollapsible.none], a `Sheet` on mobile, and the
...
class Sidebar extends StatelessWidget {
  ...
  /// Must match [SidebarProvider.variant] — see that field.
  final SidebarVariant variant;
  ...
  ///
  /// [SidebarCollapsible.none] only, and it is a page override rather than a
  ...
  /// on both edges. 64px.
  static double get insetIconGap => LayoutWidths.sidebarCollapsed + space(4);

  /// `w-[calc(var(--sidebar-width-icon)+(--spacing(4))+2px)]` — the same
  /// measure for the **container**, plus the two hairlines. 66px.
  static double get insetIconWidth => insetIconGap + BorderWidths.hairline * 2;

  /// `p-2` on the container under `floating` / `inset`.
  static double get framePadding => space(2);

  /// `w-4` — the rail's own width.
  static double get railWidth => space(4);

  @override
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    final SidebarScope scope = SidebarScope.of(context);
    ...
    // `:has()` on the panel's own children — a [SidebarRail] among them is
    ...
    // to its column.
    final bool hasRail = children.any((Widget w) => w is SidebarRail);

    final bool collapsed = scope.collapsed;
    final SidebarCollapsible? mode = collapsed ? collapsible : null;
    final bool iconMode = mode == SidebarCollapsible.icon;
    final bool offcanvas = mode == SidebarCollapsible.offcanvas;
    final bool framed = variant != SidebarVariant.sidebar;

    final double gapWidth = offcanvas
        ? 0
        : iconMode
        ? (framed ? insetIconGap : LayoutWidths.sidebarCollapsed)
        : LayoutWidths.sidebar;
    final double containerWidth = iconMode
        ? (framed ? insetIconWidth : LayoutWidths.sidebarCollapsed)
        : LayoutWidths.sidebar;
    // `data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]`
    // — the panel keeps its width and leaves.
    final double slide = offcanvas ? -LayoutWidths.sidebar : 0;
    // `group-data-[side=left]:border-r` / `border-l`, on the plain variant only.
    final double border = framed ? 0 : BorderWidths.hairline;
  ...
  Widget _container(ThemeTokens theme, {required bool framed}) {
    final bool floating = variant == SidebarVariant.floating;
    ...
    return DecoratedBox(
      decoration: BoxDecoration(
        border: Border(
          right: side.isLeft
              ? BorderSide(color: theme.border, width: BorderWidths.hairline)
              : BorderSide.none,
          left: side.isLeft
              ? BorderSide.none
              : BorderSide(color: theme.border, width: BorderWidths.hairline),
      ...
      child: Padding(
        padding: EdgeInsets.only(
          left: side.isLeft ? 0 : BorderWidths.hairline,
          right: side.isLeft ? BorderWidths.hairline : 0,
        ),
        child: inner,
      ),
    );
  ...
  Widget _mobile(BuildContext context, ThemeTokens theme, SidebarScope scope) {
    ...
    return SidebarChrome(
      ...
      child: Semantics(
        ...
        container: true,
        child: SheetContent(
          side: side.isLeft ? SheetSide.left : SheetSide.right,
          // `[&>button]:hidden`.
...
///
/// Placement is [Sidebar]'s (the strip has to know where the container's
/// edge currently is); this is the control itself, and it is what a call site
/// composes into the panel. Rendering it outside a [Sidebar] renders nothing,
...
class SidebarRail extends StatelessWidget {
  ...
  /// measure as a border."*
  static double get hairline => space(0.5);

  @override
  Widget build(BuildContext context) => const SizedBox.shrink();
}

/// The rail as [Sidebar] actually paints it, in the slot it computed.
...
class _RailSlotState extends State<_RailSlot> {
  ...
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    final SidebarScope scope = SidebarScope.of(context);
    final SidebarChrome? chrome = SidebarChrome.maybeOf(context);
      ...
      child: GestureDetector(
        ...
        child: Semantics(
          ...
          child: AnimatedContainer(
            ...
            curve: MotionCurves.outFlex,
            color: _hovered && widget.offcanvas ? theme.sidebar : transparent,
            child: Align(
              ...
              alignment: widget.offcanvas
                  ...
                  : Alignment.center,
              child: SizedBox(
                width: SidebarRail.hairline,
                child: ColoredBox(
                  color: _hovered ? theme.sidebarBorder : transparent,
                ),
              ),
            ),
          ),
...
class SidebarTrigger extends StatelessWidget {
  const SidebarTrigger({super.key, this.onPressed});
...
class SidebarInset extends StatelessWidget {
  ...
  static double get margin => space(2);
  ...
  Widget build(BuildContext context) {
    ...
    if (scope.variant == SidebarVariant.inset) {
...
EdgeInsets _regionPadding(BuildContext context) =>
    EdgeInsets.all(SidebarChrome.iconModeOf(context) ? space(2) : space(3));

/// `SidebarHeader` — `flex flex-col gap-2 p-3`.
class SidebarHeader extends StatelessWidget {
  const SidebarHeader({super.key, required this.children});
...
/// column puts a [SidebarContent] between them, and that is what takes the
/// slack.
class SidebarFooter extends StatelessWidget {
  const SidebarFooter({super.key, required this.children});
  ...
  Widget build(BuildContext context) => Padding(
    padding: _regionPadding(context),
    child: _GappedColumn(gap: SidebarHeader.gap, children: children),
  );
}
...
class SidebarContent extends StatelessWidget {
  const SidebarContent({super.key, this.children = const <Widget>[]});
  ...
  Widget build(BuildContext context) {
    ...
    return Expanded(
      child: SidebarChrome.iconModeOf(context)
...
class SidebarSeparator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    final bool icon = SidebarChrome.iconModeOf(context);
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: icon ? space(2) : space(3)),
      child: SizedBox(
...
class SidebarGroup extends StatelessWidget {
  const SidebarGroup({super.key, required this.children});
...
class SidebarGroupContent extends StatelessWidget {
  const SidebarGroupContent({super.key, required this.child});
...
class SidebarGroupLabel extends StatelessWidget {
  const SidebarGroupLabel(this.label, {super.key, this.padding});

  final String label;

  /// Overrides `px-3 pr-10`.
  ///
  /// [SidebarCollapsibleGroup] passes [EdgeInsets.zero] — its `px-0`, which
  ...
  Widget build(BuildContext context) {
    ...
    final Widget row = SizedBox(
      height: height,
      child: Padding(
        padding: padding ?? EdgeInsets.only(left: space(3), right: space(10)),
        child: Align(
...
/// padding."* Positioned by [SidebarCollapsibleGroup], which is the only
/// thing that holds one.
class SidebarGroupAction extends StatelessWidget {
...
class SidebarCollapsibleGroup extends StatefulWidget {
  ...
  /// A [SidebarGroupAction], anchored `top-4 right-3` in the group.
  final Widget? action;

  final bool defaultOpen;

  /// `h-6` on the trigger.
  static double get triggerHeight => space(6);

  /// `h-px`, and `h-1` while closed.
  static double get lineOpen => BorderWidths.hairline;
  static double get lineClosed => space(1);

  @override
  State<SidebarCollapsibleGroup> createState() =>
      _SidebarCollapsibleGroupState();
}

class _SidebarCollapsibleGroupState extends State<SidebarCollapsibleGroup> {
  late bool _open = widget.defaultOpen;
  ...
  Widget build(BuildContext context) {
    ...
    final Widget line = TweenAnimationBuilder<double>(
      tween: Tween<double>(
            ...
            ? SidebarCollapsibleGroup.lineOpen
            : SidebarCollapsibleGroup.lineClosed,
      ),
      duration: effectiveMotionDuration(context, MotionDurations.normal),
    ...
    final Widget trigger = Button(
      ...
      surface: const ButtonStyleRecipe(
        fill: transparent,
    ...
    final Widget header = Row(
      ...
      children: <Widget>[
        Flexible(
          child: SidebarGroupLabel(widget.label, padding: EdgeInsets.zero),
        ),
        ...
        Expanded(
          child: Padding(
            padding: EdgeInsets.only(
              left: space(2),
              right: hasAction ? space(8) : space(2),
            ),
            child: SizedBox(
              height: SidebarCollapsibleGroup.triggerHeight,
              child: trigger,
            ),
          ),
        ),
        ...
        Stack(
          children: <Widget>[
            ...
            if (hasAction)
              ...
              Positioned(top: space(1), right: 0, child: widget.action!),
          ],
        ),
        Unfold(
          open: _open,
          child: SidebarGroupContent(child: widget.child),
        ),
      ],
    );
  }
...
/// What a [SidebarMenuItem] tells the row inside it.
///
...
///
/// **Why this is not [ActiveIndicator].** That widget lays its options out
...
class _SidebarMenuState extends State<SidebarMenu>
  ...
  late final AnimationController _jelly = AnimationController(
    vsync: this,
    duration: MotionDurations.stateChange,
  );
  ...
  /// Called from [SidebarMenuButton]'s own lifecycle — a plain list append,
  ...
  Widget build(BuildContext context) {
    ...
    _jelly.duration = effectiveMotionDuration(
      context,
    ...
    for (int i = 0; i < widget.children.length; i++) {
      if (i > 0) rows.add(SizedBox(height: SidebarMenu.gap));
            ...
            AnimatedPositioned(
              ...
              child: IgnorePointer(
                child: AnimatedOpacity(
                  ...
                  duration: travels
                      ? effectiveMotionDuration(context, MotionDurations.fast)
                      : Duration.zero,
                  curve: MotionCurves.enter,
...
class SidebarMenuItem extends StatelessWidget {
  ...
  /// The row — a [SidebarMenuButton], or a trigger wrapping one.
  final Widget button;

  /// A [SidebarMenuAction], `absolute top-1/2 right-1`.
  final Widget? action;

  /// A [SidebarMenuBadge], `absolute top-1/2 right-2`.
  final Widget? badge;

  /// A [SidebarMenuSub], which is in flow **under** the row and makes the
...
class SidebarMenuButton extends StatefulWidget {
  const SidebarMenuButton({
    ...
    this.size = SidebarMenuButtonSize.md,
    this.tooltip,
    this.label,
    this.onPressed,
    this.expanded = false,
  ...
  /// The row's content — normally a [SidebarMenuRow].
  final Widget child;
...
class _SidebarMenuButtonState extends State<SidebarMenuButton> {
  ...
  @override
  void didUpdateWidget(SidebarMenuButton old) {
  ...
  void _sync(_SidebarMenuState? menu) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool ghost = widget.variant == ButtonVariant.ghost;
    final ButtonStyleRecipe? surface = !ghost
        ? null
        : icon
        ? const ButtonStyleRecipe(fill: transparent, hoverFill: transparent)
        : widget.isActive
        ? ButtonStyleRecipe(
            fill: transparent,
            hoverFill: transparent,
    ...
    if (icon) {
      ...
      button = SizedBox(
        height: SidebarMenuButton.iconSize,
        child: ConstrainedBox(
          constraints: BoxConstraints(maxWidth: SidebarMenuButton.iconSize),
          child: button,
        ),
      );
    }
...
class SidebarMenuRow extends StatelessWidget {
  ...
  /// The glyph, already sized by the caller from [Button.iconPxFor].
  ...
  Widget build(BuildContext context) {
    final bool icon = SidebarChrome.iconModeOf(context);
    final double gap = Button.gapFor(size.button);
...
/// per row is 7.5px down a page of them. [LineBox] gives the paragraph its
/// CSS height back.
///
/// The style is read from the ambient [DefaultTextStyle] rather than passed:
/// what a row's label is typed as is the *rung's* business, and [Button] has
/// already resolved it by the time this builds.
class SidebarMenuLabel extends StatelessWidget {
  const SidebarMenuLabel(this.text, {super.key});
...
/// [TextStyles.sidebarMenuBadge] (drift 1).
...
class SidebarMenuSkeleton extends StatelessWidget {
  const SidebarMenuSkeleton({super.key, this.showIcon = false, this.seed = ''});
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: Padding(
      ...
      child: Row(
        children: <Widget>[
          if (showIcon) ...<Widget>[
            Skeleton(width: space(4), height: space(4), radius: Radii.md),
...
class SidebarMenuSub extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    if (SidebarChrome.iconModeOf(context)) return const SizedBox.shrink();
    ...
    return Padding(
      ...
      child: DecoratedBox(
        ...
        child: Padding(
          padding: EdgeInsets.only(
            left: space(3) - BorderWidths.hairline,
            right: space(3),
            top: space(1),
            bottom: space(1),
          ),
          child: _GappedColumn(gap: space(1), children: children),
        ),
      ),
    );
  }
...
class SidebarMenuSubItem extends StatelessWidget {
  const SidebarMenuSubItem({super.key, required this.child});
...
class SidebarMenuSubButton extends StatelessWidget {
  const SidebarMenuSubButton({
    ...
    this.isActive = false,
    this.variant = ButtonVariant.ghost,
    this.size = SidebarMenuSubButtonSize.md,
    this.onPressed,
  });
  ...
  final bool isActive;
  final ButtonVariant variant;
  final SidebarMenuSubButtonSize size;
  final VoidCallback? onPressed;
  ...
  Widget build(BuildContext context) {
    ...
    final bool ghost = variant == ButtonVariant.ghost;
    return Button(
      variant: variant,
      size: size.button,
      radius: BorderRadius.circular(Radii.lg),
