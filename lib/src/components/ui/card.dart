...
class Card extends StatelessWidget {
  const Card({super.key, required this.children, this.fill, this.ringColor});
  ...
  static double get spacing => space(4);

  /// `rounded-xl`.
  static double get radius => Radii.xl;

  /// `ring-1`.
  static double get ringWidth => BorderWidths.hairline;

  /// The default ring colour.
  static Color ringOf(ThemeTokens theme) =>
  ...
  Widget build(BuildContext context) {
    ...
    // `has-data-[slot=card-footer]:pb-0`.
    final bool footer = children.isNotEmpty && children.last is CardFooter;

    return DecoratedBox(
      decoration: BoxDecoration(
        borderRadius: shape,
        boxShadow: ShadowStyle(<ShadowLayer>[
          ShadowLayer(0, 0, 0, ringWidth, (ThemeTokens _) => ring),
        ]).outerShadows(theme),
      ),
      child: ClipRRect(
        borderRadius: shape,
...
class CardHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: Card.spacing),
      child: action == null
          ? column
          : Row(
...
class CardTitle extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => StyledText(text, TextStyles.cardTitle);
}
...
class CardFooter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    return Container(
      padding: EdgeInsets.all(Card.spacing),
      decoration: BoxDecoration(
