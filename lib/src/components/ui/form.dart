...
/// objects. So the id half of this file lives in `FieldScope` (see
...
/// `"onTouched"` anywhere — so [ValidateMode] has two members and not four.
///
...
/// (`forms-map.md` §3.4).
///
/// [Form.focusFirstError] focuses the first invalid field **whatever it is**.
...
/// `{ shouldFocus: true }` is not passed — and [Form.setError] matches that.
library;
...
abstract class FormFieldBase extends ChangeNotifier {
  FormFieldBase({required this.name});
  ...
  /// The node a failed submit lands on, and the one a `FieldLabel` focuses.
  ///
  ...
  /// The form's re-validation hook. Set when the field joins a [Form].
...
class FormField<T> extends FormFieldBase {
  FormField({
    required super.name,
    required T initialValue,
    List<ValidationRule<T>>? rules,
    this.issueMode = IssueMode.first,
  }) : _initialValue = initialValue,
       _value = initialValue,
       rules = rules ?? <ValidationRule<T>>[];

  /// The schema, in declaration order. Zod evaluates every check in that order
  /// without aborting, and so does [Validators.check].
  ...
  @override
  List<String> issues() => Validators.check<T>(_value, rules, mode: issueMode);
...
/// A [FormField] that owns the [TextEditingController] its control edits.
///
...
class TextFormField extends FormField<String> {
  ...
  /// Hand this to `Input.controller` / `Textarea.controller`.
...
/// [ListenableBuilder] and passes each field's state into its own `Field`.
class Form extends ChangeNotifier {
  Form({
    required this.fields,
    this.mode = ValidateMode.onSubmit,
    this.reValidateMode = ValidateMode.onChange,
  }) {
  ...
  /// `formState.isSubmitting` — what `Button.loading` reads.
  bool get isSubmitting => _submitting;

  /// `formState.isValid`, computed from what is currently stored rather than
  /// re-asked.
  bool get isValid => fields.every((FormFieldBase f) => !f.invalid);

  /// The field named [name], or a thrown [StateError] naming what is declared.
  FormFieldBase operator [](String name) => fields.firstWhere(
    (FormFieldBase f) => f.name == name,
    orElse: () => throw StateError(
      'No field named "$name". This form declares: '
      '${fields.map((FormFieldBase f) => f.name).join(', ')}.',
    ),
  );

  /// [name] as its typed self — `form.field<String>('handle').value`.
  FormField<T> field<T>(String name) => this[name] as FormField<T>;

  /// The text field named [name], for its controller.
  TextFormField text(String name) => this[name] as TextFormField;
