...
/// | `outline-none` | — |
/// | `transition-[box-shadow,border-color,background-color] duration-base ease-out` | 250ms on `--ease-out` — from `--default-transition-duration`; `duration-base` emits nothing ([MotionDurations.normal]) |
...
/// wrapper paint everything. So [Input] grows a [Input.bare] mode and a
/// [Input.padding] override — the two things the strip list amounts to — and
/// nothing resembling a prefix or suffix slot. `InputGroup` is the socket.
///
...
/// seeded value.
///
/// USER-ORDERED MOBILE ADAPTATION: this file also owns [FieldVisibility], the
...
// `rendering.dart` for [RenderAbstractViewport], which [FieldVisibility]
...
/// variant**, so this alpha is both themes — unlike `InputGroup`, which rings
...
class Input extends StatefulWidget {
  ...
  ///
  /// A [FieldScope]'s node wins over the owned one and loses to this — the
  ...
  ///
  /// ORed with the scope's, so a `Field` carrying an error marks its control
  ...
  /// contributes only what it alone declares. [TextStyles.inputNumber] and
  /// [TextStyles.inputSerial] are those two collapses already resolved in
  /// the foundation layer; passing [TextStyles.numberBase] here would render 15px and
  /// be wrong by two pixels.
  ///
  /// Defaults to [TextStyles.bodyCompact] — the bare `text-sm`, 13/400.
  final TextStyleToken? textSpec;

  /// The accessible name, for a field with no visible `<label>`.
  ///
  /// A [FieldScope] supplies it from the visible `FieldLabel`, which is the
...
class _InputState extends State<Input> {
  ...
  /// [FieldScope], or the first fall-through to the owned one — and every one
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyleToken spec = widget.textSpec ?? TextStyles.bodyCompact;
    ...
    final TextStyle textStyle = StyledText.styleOf(context, spec);
    ...
    final EdgeInsetsGeometry padding = widget.padding ?? Input.insets;
    ...
    // USER-ORDERED MOBILE ADAPTATION — see [FieldVisibility]. Outermost, so
    // what gets revealed is the whole control including its socket, and free on
    // every desktop frame.
    return FieldVisibility(focusNode: _focusNode, child: field);
  }
}

/// The pill, its permanent socket, and the one ring that ever joins it.
///
/// Split out of [Input] so `Textarea` can wear the identical recipe at a
...
class FieldSurfaceRecipe extends StatelessWidget {
  const FieldSurfaceRecipe({
    super.key,
  ...
  /// them overrides it.
  ///
  /// Was 0.30 on `Input` while the stylesheet and the example app's own
  ...
  /// Overrides `bg-card` — see [Input.fill].
  final Color? fill;

  /// `shadow-none`: the socket goes, the ring stays — see [Input.flat].
  final bool flat;
  ...
  Widget build(BuildContext context) {
    ...
    final Color ring = invalid
        ...
        // Not `transparent`: a ring fading out through black would tint
      ...
      builder: (BuildContext context, Color? b, Widget? _) =>
          TweenAnimationBuilder<Color?>(
            ...
            builder: (BuildContext context, Color? r, Widget? _) {
              ...
              final ShadowStyle base = flat ? Shadows.none : Shadows.inset;
              final ShadowStyle spec = ringColor.a == 0
                  ? base
                  : Button.withFocusRing(base, ringColor);
...
/// family — [Input], `Textarea`, `InputOtp`, the agent composer — wraps
...
/// ancestor has already lifted clear of the keyboard — [AgentConsole]'s own
...
class FieldVisibility extends StatefulWidget {
  ...
  /// [FieldScope]'s, the one it owns — passes whichever won, so the reveal
  ...
  ///
  /// [MotionDurations.fast] rather than the transition default: the OS keyboard is
  /// already sliding up over it, and a field that arrives first reads as having
  /// been in the clear all along.
  static Duration get travel => MotionDurations.fast;
...
class _FieldVisibilityState extends State<FieldVisibility> {
  ...
  @override
  void didUpdateWidget(FieldVisibility old) {
    super.didUpdateWidget(old);
  ...
  void _revealIn(ScrollableState scrollable, RenderObject target) {
    ...
    final double margin = FieldVisibility.margin;
    ...
    final Duration travel = effectiveMotionDuration(
      context,
      FieldVisibility.travel,
    );
    ...
    position.animateTo(landing, duration: travel, curve: MotionCurves.enter);
  }
...
/// [FieldSurfaceRecipe] at the pill radius, with the field's own padding inside it.
class _Socket extends StatelessWidget {
  ...
  /// [Input.radius] — null keeps the pill.
  final BorderRadius? radius;

  @override
  Widget build(BuildContext context) {
    return FieldSurfaceRecipe(
      radius: radius ?? BorderRadius.circular(Radii.full),
      focused: focused,
      invalid: invalid,
      fill: fill,
      flat: flat,
