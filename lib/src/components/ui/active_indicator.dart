...
class _ActiveIndicatorState extends State<ActiveIndicator>
  ...
  /// first frame only — [build] re-reads it through [effectiveMotionDuration] on
  /// every pass, the way `Press` does, so reduced motion stills the squash
  ...
  @override
  void didUpdateWidget(ActiveIndicator old) {
        ...
        AnimatedPositioned(
          ...
          duration: travels
              ? effectiveMotionDuration(
                  context,
                  widget.moveDuration ?? MotionDurations.normal,
                )
              : Duration.zero,
