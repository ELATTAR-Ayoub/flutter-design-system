...
class EmptyTitle extends StatelessWidget {
  ...
  ///
  /// Built from [TextStyles.buttonLabel] plus one override rather than
  ...
  ///    `Fonts.heading` and `Fonts.sans` are both `"Inter Local"`
  ...
  ///    [TextStyles.h1] — the one spec in the type layer that transcribes exactly
  ///    that token — rather than typed here, so a retune of the token carries.
  ///
  /// FOLLOW-UP: fold this into a `TextStyles.emptyTitle` when
  /// `typography.dart` is next open to this phase.
  static TextStyle styleOf(BuildContext context, {Color? color}) {
    final TextStyleToken spec = TextStyles.buttonLabel;
    return StyledText.styleOf(
      context,
      spec,
      color: color,
    ).copyWith(letterSpacing: TextStyles.h1.tracking! * spec.size!);
  }
...
class EmptyDescription extends StatelessWidget {
  ...
  /// [TextStyles.textareaBody] carries.
  ///
  ...
  static TextStyleToken get spec => TextStyles.textareaBody;
