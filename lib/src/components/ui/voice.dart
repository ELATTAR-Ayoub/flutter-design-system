...
/// |---|---|
/// | `voice/visualisers.tsx` | [LiveWaveform], [BarVisualizer] |
/// | `voice/pickers.tsx` | [MicControl] |
///
...
/// buffer that call would have filled — [LiveWaveform.samples] and
/// [BarVisualizer.spectrum] — and everything downstream of the read is the
...
/// and twelve bars at their 0.06 floor. [BarVisualizer.active] is the one
...
class _WaveformPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint paint = Paint()
      ...
      ..strokeWidth = LiveWaveform.strokeWidth
    ...
    for (int i = 0; i < view.length; i++) {
      ...
      final double y = mid + view[i] * mid * LiveWaveform.amplitude;
      if (i == 0) {
        path.moveTo(x, y);
      } else {
        path.lineTo(x, y);
...
class _BarVisualizerState extends State<BarVisualizer>
  ...
  @override
  void didUpdateWidget(BarVisualizer old) {
  ...
  /// signal or [BarVisualizer.active] can move the bars produces the same
...
class _BarsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double barWidth = (w - BarVisualizer.gap * (bars - 1)) / bars;
    ...
    for (int i = 0; i < bars; i++) {
      smoothed[i] =
          smoothed[i] * BarVisualizer.smoothingKeep +
          levels[i] * BarVisualizer.smoothingTake;
      final double value = math.max(BarVisualizer.floor, smoothed[i]);
      final double barHeight = value * h;
      final double x = i * (barWidth + BarVisualizer.gap);
  ...
  List<double> _levels() {
    ...
    if (view != null && view.isNotEmpty) {
      ...
      return <double>[
        for (int i = 0; i < bars; i++)
          () {
            final int from =
                (count * math.pow(i / bars, BarVisualizer.bandExponent))
                    .floor();
            final int to = math.max(
              from + 1,
              (count * math.pow((i + 1) / bars, BarVisualizer.bandExponent))
                  .floor(),
            );
            double sum = 0;
            ...
            return math
                .min(1, sum / (to - from) / BarVisualizer.bandDivisor)
                .toDouble();
          }(),
      ];
    }
    if (active) {
      ...
      return <double>[
        for (int i = 0; i < bars; i++)
          math.max(
            BarVisualizer.activeFloor,
            (math.sin(
                  t * BarVisualizer.activeRate + i * BarVisualizer.activePhase,
                )).abs() *
                BarVisualizer.activeScale *
                (1 - (i / bars - 0.5).abs()),
          ),
      ];
    }
...
/// is documented here and not built — the [MicControl] surface is exactly the
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        color: listening
            ? theme.agentAccent.withValues(alpha: liveFillAlpha)
            : null,
        border: Border.all(
          color: listening
              ? theme.agentAccent.withValues(alpha: liveBorderAlpha)
              : theme.border,
...
class _MicButtonState extends State<_MicButton>
  ...
  void _sync() {
    _pulse.duration = effectiveMotionDuration(
      context,
      MotionDurations.pulseLive,
    );
  ...
  Widget build(BuildContext context) {
    ...
    // `text-agent` while live. [IconTone.inherit] is `text-current`, so the
    ...
    return AnimatedBuilder(
      ...
      builder: (BuildContext context, Widget? child) {
        ...
        final double eased = MotionCurves.move.transform(leg.clamp(0.0, 1.0));
        return Opacity(
          opacity: 1 - eased * _pulseOpacityDrop,
          child: DecoratedBox(
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(Radii.full),
              boxShadow: Shadows.pulseLiveRing(eased).outerShadows(theme),
            ),
            child: child,
          ),
        );
  ...
  /// `className="size-4"` on the icon, against [ButtonSize.iconSm]'s 14.
