...
/// | leg | measured |
/// |---|---|
/// | hide | `scale 1 → 0.95` and `translate-y-0 → 100%` cut in one frame; opacity 1 → 0 over **250ms** on [MotionCurves.exit] (`data-[active=false]:ease-in`) |
/// | show | the same two cut back; opacity 0 → 1 over **250ms** on [MotionCurves.enter] |
///
...
enum ScrollPosition {
  ...
  /// The item marked [MessageScrollerItem.scrollAnchor], falling back to
  /// [end] when there is none.
  lastAnchor,
}

/// Which edge a [MessageScrollerButton] travels to.
...
class MessageScrollerController extends ChangeNotifier {
  MessageScrollerController({
    this.autoScroll = false,
    this.defaultScrollPosition = ScrollPosition.end,
  ...
  final ScrollPosition defaultScrollPosition;
  ...
  /// The offsets of every registered [MessageScrollerItem], by message id.
  ...
  bool scrollable(ScrollDirection direction) {
    if (!scroll.hasClients) return direction == ScrollDirection.end;
    final flutter.ScrollPosition p = scroll.position;
  ...
  /// [ScrollPosition.lastAnchor] restores to.
  ...
  void followNewContent() {
    ...
    if (!scrollable(ScrollDirection.end)) {
...
/// Publishes a [MessageScrollerController] to the subtree — the port of
/// `MessageScrollerProvider`.
class MessageScrollerProvider
    extends InheritedNotifier<MessageScrollerController> {
  const MessageScrollerProvider({
    super.key,
    required MessageScrollerController controller,
    required super.child,
  }) : super(notifier: controller);

  static MessageScrollerController of(BuildContext context) {
    final MessageScrollerProvider? found = context
        .dependOnInheritedWidgetOfExactType<MessageScrollerProvider>();
    assert(found != null, 'No MessageScrollerProvider above this widget.');
...
class MessageScroller extends StatelessWidget {
  const MessageScroller({super.key, required this.viewport, this.button});

  final Widget viewport;

  /// [MessageScrollerButton], absolutely positioned against this frame.
  final Widget? button;
...
class MessageScrollerViewport extends StatefulWidget {
  ...
  @override
  State<MessageScrollerViewport> createState() =>
      _MessageScrollerViewportState();
}

class _MessageScrollerViewportState extends State<MessageScrollerViewport> {
  bool _restored = false;

  @override
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    final MessageScrollerController c = MessageScrollerProvider.of(context);
    ...
    if (!_restored) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        switch (c.defaultScrollPosition) {
          ...
          case ScrollPosition.lastAnchor:
      ...
      child: SingleChildScrollView(
        ...
        child: Padding(
          ...
          padding: const EdgeInsetsDirectional.only(
            end: MessageScrollerViewport.gutter,
          ),
          child: widget.child,
        ),
      ),
    ...
    scroller = RawScrollbar(
      ...
      thumbColor: c.autoscrolling ? transparent : theme.border,
      thickness: MessageScrollerViewport.thumbThickness,
    ...
    return Semantics(
      ...
      child: ScrollFade(controller: c, child: scroller),
    );
  }
}
...
class ScrollFade extends StatelessWidget {
  const ScrollFade({super.key, required this.controller, required this.child});
  ...
  final MessageScrollerController controller;
  final Widget child;
...
class MessageScrollerContent extends StatelessWidget {
...
class MessageScrollerItem extends StatefulWidget {
  ...
  static double get assumedOffscreenHeight => space(40);
  ...
  /// [ScrollPosition.lastAnchor].
  final bool scrollAnchor;

  @override
  State<MessageScrollerItem> createState() => _MessageScrollerItemState();
}

class _MessageScrollerItemState extends State<MessageScrollerItem> {
  @override
  Widget build(BuildContext context) {
...
///
/// Renders a [Button] and hides itself the moment that direction has nowhere
...
class MessageScrollerButton extends StatelessWidget {
  const MessageScrollerButton({
    super.key,
    this.direction = ScrollDirection.end,
  });

  /// `data-[direction=end]:bottom-4` / `data-[direction=start]:top-4`.
  static double get inset => space(4);
  ...
  Widget build(BuildContext context) {
    ...
    final double size = Button.heightFor(ButtonSize.iconSm);
    ...
    Widget button = Button(
      ...
      label: direction == ScrollDirection.end
          ? 'Scroll to end'
          : 'Scroll to start',
      onPressed: active
      ...
      child: Transform.rotate(
        ...
        angle: direction == ScrollDirection.start ? math.pi : 0,
        child: Icon.lucide(
          Lucide.arrowDown,
    ...
    button = Transform.translate(
      ...
      child: Transform.scale(
        ...
        child: AnimatedOpacity(
          ...
          curve: active ? MotionCurves.enter : MotionCurves.exit,
          child: button,
        ),
      ),
    );
    ...
    return Positioned(
      top: direction == ScrollDirection.start ? inset : null,
      bottom: direction == ScrollDirection.end ? inset : null,
      left: 0,
      right: 0,
