...
/// `ActiveIndicator` makes. The pill deliberately **skips** its first
...
/// tree and contributes no name of its own. Put it inside a `Button` as the
...
class _IconSwapState extends State<IconSwap> with TickerProviderStateMixin {
  /// The squash's delay and the squash itself on **one** clock.
  ///
  /// `KeyframePlayer` has no delay parameter and a second controller started
  ...
  /// The strip position it is heading for — the resolved [IconSwap.activeIndex].
  late int _to;
  ...
  int get _active =>
      IconSwap.resolveIndex(widget.activeIndex, widget.icons.length);
  ...
  @override
  void didUpdateWidget(IconSwap old) {
