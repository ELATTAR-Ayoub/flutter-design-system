...
///
/// Ruling F7: *"Validator = dependency-free `ValidationRule<T>` lists with first-issue /
...
/// the sentence it renders. That is [ValidationRule]. There is no object schema, no
...
///    `too_small` *and* `invalid_format`, in that order. So [Validators.check]
...
///    is [IssueMode], and it is the entire difference between the account
...
/// `FieldError` too — same operation, same place.
library;
...
class ValidationRule<T> {
  const ValidationRule(this.test, this.message);

  /// Passes when this returns `true`.
  final ValidationTest<T> test;
  ...
  static ValidationRule<String> minLength(int min, String message) =>
      ValidationRule<String>((String v) => v.length >= min, message);

  /// `z.string().max(n, msg)`.
  static ValidationRule<String> maxLength(int max, String message) =>
      ValidationRule<String>((String v) => v.length <= max, message);
  ...
  static ValidationRule<String> pattern(RegExp expression, String message) =>
      ValidationRule<String>(expression.hasMatch, message);

  /// `z.email(msg)` — one rule, one predicate, [emailPattern].
  static ValidationRule<String> email(String message) =>
      ValidationRule<String>(emailPattern.hasMatch, message);
  ...
  static ValidationRule<bool> accepted(String message) =>
      ValidationRule<bool>((bool v) => v, message);
  ...
  static ValidationRule<T?> oneOf<T>(List<T> allowed, String message) =>
      ValidationRule<T?>((T? v) => v != null && allowed.contains(v), message);
...
class Validators {
  ...
  /// one entry under [IssueMode.first].
  ///
  ...
  static List<String> check<T>(
    ...
    IssueMode mode = IssueMode.first,
  }) {
    final List<String> issues = <String>[];
    for (final ValidationRule<T> rule in rules) {
      final String? issue = rule.issue(value);
      if (issue == null) continue;
      issues.add(issue);
      if (mode == IssueMode.first) break;
    }
    return issues;
  }
