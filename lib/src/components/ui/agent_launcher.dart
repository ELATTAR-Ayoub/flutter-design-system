...
/// named beside it. [AgentLauncher] reproduces the snap.
///
/// The button's own `hover:border-agent/50` springs the way every `Button`
...
///
/// GAP CLOSED — `hover:border-agent/50` is painted. It was not: [ButtonStyleRecipe]
...
/// site now passes [AgentLauncher.hoverRimAlpha] of `--agent` through it. The
...
class AgentLauncher extends StatefulWidget {
  ...
  /// The face on the button. Null takes [AgentAvatarRegistry.renderer], the
  ...
  static double get dialogMaxWidth => space(320);

  /// `h-[min(88vh,52rem)]`.
  static const double dialogHeightFraction = 0.88;
  static double get dialogMaxHeight => space(208);
  ...
  /// [CompactDialogLayout.clampSize] takes the last word — 292.5 x 609 — and the
  ...
  static Size dialogSize(Size viewport) {
    ...
    return CompactDialogLayout.clampSize(
      Size(
  ...
  @override
  State<AgentLauncher> createState() => _AgentLauncherState();
}

class _AgentLauncherState extends State<AgentLauncher> {
  bool _hovered = false;
  ...
  Widget _button(BuildContext context, VoidCallback open) {
    ...
    return SizedBox.square(
      ...
      child: Button(
        ...
        label: widget.label,
        onPressed: open,
        child: AgentFace(state: AgentState.idle, avatar: widget.avatar),
      ),
    );
  }
}
...
class _FixedState extends State<_Fixed> {
  ...
  Widget build(BuildContext context) => flutter.OverlayPortal(
    ...
    overlayChildBuilder: (BuildContext context) => Positioned(
      right: AgentLauncher.inset,
      bottom: AgentLauncher.inset,
      child: widget.child,
    ),
...
class _Label extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return IgnorePointer(
      child: Padding(
        padding: EdgeInsets.only(right: AgentLauncher.labelGap),
...
class _Dialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    final Size box = AgentLauncher.dialogSize(MediaQuery.sizeOf(context));
    ...
    return Semantics(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: <BoxShadow>[
            ...DialogContent.ringSpec.outerShadows(theme),
            ...Shadows.xl.outerShadows(theme),
          ],
        ),
        child: child,
      ),
