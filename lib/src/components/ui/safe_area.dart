...
/// ([SafeArea]); a scroll view keeps scrolling under both bars and pays the
...
/// [SafeArea] is how a customer app inherits the fix without knowing it
...
/// [SafeArea] returns its child **unwrapped**, so no [Padding] and no
...
/// hands down, so nesting is safe: a [SafeArea] inside a sheet that already
/// paid the bottom bar reads zero and adds nothing.
///
/// ```dart
/// DecoratedBox(                       // paints to the screen edge
///   decoration: const BoxDecoration(/* … */),
///   child: SafeArea(bottom: false, child: Row(children: controls)),
/// )
/// ```
class SafeArea extends StatelessWidget {
  ...
  /// pairs this with a `SafeArea(bottom: false)` around the bar's contents,
