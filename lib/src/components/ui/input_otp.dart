...
// [FieldVisibility] — the field family's one keyboard-avoidance hook, which
...
/// follows `InputGroup`'s theme-split, not the bare field's flat 20.
...
class InputOtp extends StatefulWidget {
  ...
  static double get separatorWidth => Icon.pxFor(IconSize.md);
...
class _InputOtpState extends State<InputOtp> {
  ...
  @override
  void didUpdateWidget(InputOtp old) {
  ...
  Widget build(BuildContext context) {
    ...
    for (int g = 0; g < widget.groups.length; g++) {
      if (g > 0) strip.add(const InputOtpSeparator());
    ...
    field = SizedBox(
      width: InputOtp.widthFor(widget.groups),
      height: InputOtp.slotSize,
      child: field,
    );
    ...
    // [FieldVisibility]. A one-time code arrives while the keyboard is open by
  ...
  Widget _group({
    ...
    final double width = count * InputOtp.slotSize;
        ...
        children: <Widget>[
          for (final int i in order)
            PositionedDirectional(
              start: (i - first) * InputOtp.slotSize,
              top: 0,
              width: InputOtp.slotSize,
              height: InputOtp.slotSize,
              child: InputOtpSlot(
                char: _charAt(i),
                active: _isActive(i),
    ...
    if (invalid) {
      final double alpha = ThemeScope.kindOf(context) == ResolvedColorMode.dark
...
class InputOtpSlot extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    final bool dark = ThemeScope.kindOf(context) == ResolvedColorMode.dark;
    ...
    final BorderRadius radius = BorderRadius.horizontal(
      ...
      left: Radius.circular(first ? Radii.lg : 0),
      right: Radius.circular(last ? Radii.lg : 0),
    );
  ...
  Widget _content(BuildContext context, ThemeTokens theme) {
    return Stack(
      alignment: Alignment.center,
      children: <Widget>[
        if (char != null)
          // `text-sm` — Inter 13/400 at `--foreground`. Not mono; see drift 13.
          StyledText(char!, TextStyles.bodySmall, color: theme.foreground),
...
class _FakeCaret extends StatelessWidget {
  ...
  static double get height => space(4);
...
class InputOtpSeparator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return const ExcludeSemantics(
      ...
      // `--foreground` — and turns with a `Field`'s invalid colouring, the
      // same as `currentColor` does.
      child: Icon(IconGlyph.minus),
    );
  }
}
