...
/// [Popover.animateOut] false is that missing twin.
///
...
@immutable
class MenubarMenu {
  const MenubarMenu({required this.label, required this.children});
...
class Menubar extends StatefulWidget {
  ...
  /// `p-1`.
  static double get padding => space(1);

  /// `gap-0.5` — 2px.
  static double get gap => space(0.5);

  /// `px-3` on a trigger.
  static double get triggerPaddingX => space(3);

  /// `sideOffset={8}`.
  static double get sideOffset => space(2);
  ...
  static double get alignOffset => -space(1);
  ...
  Widget build(BuildContext context) {
    ...
    return Focus(
      ...
      child: Container(
        ...
        padding: EdgeInsets.all(Menubar.padding),
          ...
          child: Row(
            ...
            children: <Widget>[
              for (int i = 0; i < widget.menus.length; i++) ...<Widget>[
                if (i > 0) SizedBox(width: Menubar.gap),
                _MenubarTrigger(
                  menu: widget.menus[i],
                  open: _open == i,
  ...
  Widget build(BuildContext context) {
    ...
    final Widget trigger = DecoratedBox(
      ...
      child: SizedBox(
        height: Menubar.height,
        child: Padding(
          padding: EdgeInsets.symmetric(horizontal: Menubar.triggerPaddingX),
          child: Center(
            widthFactor: 1,
            child: StyledText(
              widget.menu.label,
    ...
    return Popover(
      ...
      content: (BuildContext context, PopoverAnchorMetrics metrics) =>
          ...
          // is: [Popover] places the leading edges flush and this moves them.
          Transform.translate(
            offset: Offset(Menubar.alignOffset, 0),
            child: MenuContent(
