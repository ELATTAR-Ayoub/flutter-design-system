...
/// |---|---|
/// | `avatar/cube.tsx` | [AgentCube], [AgentCubeSpec], [CubeScene] |
/// | `avatar/scenes.ts` | [agentCubeScene], the nineteen recipes |
/// | `avatar/types.ts` | [AgentAvatarSize] and [AgentAvatar]'s four props |
/// | `avatar/cube-avatar.tsx` | [AgentAvatar], the crossfade, the idle cube |
/// | `globals.css` L3109–3177 | [AgentCubeKeyframes], the fourteen tables |
/// | `globals.css` L720–731 / L914–932 | [AgentCubeTokens] |
///
...
/// cube resolves its own phase out of it — the same mechanism `action_feedback.dart`
...
/// in the scene flashes at full opacity on the first frame."* [AgentCubeMotion
...
/// [MotionDurations.shimmerText] because the page-local copy needs a token to read.
library;
...
class AgentCube {
  ...
  /// recipe."*
  static Rect viewBoxOf(List<AgentCubeSpec> cubes) {
    double minX = double.infinity;
    double maxX = double.negativeInfinity;
    double minY = double.infinity;
    double maxY = double.negativeInfinity;

    for (final AgentCubeSpec cube in cubes) {
...
//
// `AgentCubeTokens.of(BuildContext)` did **not** travel, and its two call
// sites read `ThemeScope.of(context).cube` instead — it was a second resolver for
// what the theme scope already does, which is the same tidy-up
// `feedback_surface.dart` made when its five knobs moved.
//
// What did NOT move, and why: [AgentCubeFaces] below mixes an accent cube's
...
class AgentCubeFaces {
  const AgentCubeFaces({
    required this.top,
    required this.left,
    required this.right,
  ...
  /// [AgentCube.dash] on the ghost, null everywhere else.
  ...
  static AgentCubeFaces neutral(AgentCubeTokens t) => AgentCubeFaces(
    top: t.top,
    left: t.left,
    right: t.right,
    stroke: t.stroke,
  ...
  static AgentCubeFaces error(AgentCubeTokens t) => AgentCubeFaces(
    top: t.errorTop,
    left: t.errorLeft,
    right: t.errorRight,
  ...
  static AgentCubeFaces accent(AgentCubeTokens t, Color accent) =>
      AgentCubeFaces(
        top: OklabColor.mix(accent, t.top, accentTopMix),
        left: accent,
        right: OklabColor.mix(accent, t.accentShade, accentRightMix),
        stroke: OklabColor.mix(accent, t.stroke, accentStrokeMix),
      );

  /// *"The 'missing box' — an accent cube at 45%, dashed. Reads as a slot to
  /// fill."*
  static AgentCubeFaces ghost(AgentCubeTokens t, Color accent) {
    final AgentCubeFaces lit = AgentCubeFaces.accent(t, accent);
    return AgentCubeFaces(
      ...
      stroke: OklabColor.mix(accent, t.ghostInk, ghostStrokeMix),
      dash: AgentCube.dash,
    );
  }

  /// `faces(cube)` — red beats dashed beats accent beats neutral.
  static AgentCubeFaces forCube(
    AgentCubeSpec cube,
    AgentCubeTokens tokens,
    Color accent,
  ) {
    if (cube.red) return AgentCubeFaces.error(tokens);
    if (cube.dashed) return AgentCubeFaces.ghost(tokens, accent);
    if (cube.accent) return AgentCubeFaces.accent(tokens, accent);
    return AgentCubeFaces.neutral(tokens);
  }
}
...
/// class.
class AgentCubeKeyframes {
  const AgentCubeKeyframes._();

  /// `var(--ease-in-out)`, every table except [AgentCubeKeyframe.spin3d].
  ...
  static final Map<AgentCubeKeyframe, Animatable<Offset>?>
  _translate = <AgentCubeKeyframe, Animatable<Offset>?>{
    // 0%,100% { translateY(0) } 50% { translateY(-5px) }
    AgentCubeKeyframe.bob: _y(<(double, double)>[(0, 0), (50, -5), (100, 0)]),
    // 0%,55%,100% { 0 } 20%,35% { -11px }
    AgentCubeKeyframe.rise: _y(<(double, double)>[
      (0, 0),
      (20, -11),
      (35, -11),
      (55, 0),
    ...
    AgentCubeKeyframe.appear: _y(<(double, double)>[(0, 8), (10, 0)]),
    // 0% { translateY(-24px) } 18% { translateY(0) }
    AgentCubeKeyframe.drop: _y(<(double, double)>[(0, -24), (18, 0)]),
    AgentCubeKeyframe.glide: Keyframes.offsets(<KeyframeStop<Offset>>[
      const KeyframeStop<Offset>(0, Offset.zero),
      const KeyframeStop<Offset>(100, glideTravel),
    ], curve: curve),
    AgentCubeKeyframe.blinkfade: null,
    AgentCubeKeyframe.blinkslow: null,
    // 0%,100% { 0 } 45%,60% { -6.5px }
    AgentCubeKeyframe.lift: _y(<(double, double)>[
      (0, 0),
      (45, -6.5),
      (60, -6.5),
      (100, 0),
    ]),
    AgentCubeKeyframe.lift2: _y(<(double, double)>[
      (0, 0),
      (45, -13),
      (60, -13),
      (100, 0),
    ]),
    AgentCubeKeyframe.settle: _y(<(double, double)>[
      (0, 0),
      (50, -3),
      (100, 0),
    ]),
    // 0%,60%,100% { 0 } 25%,40% { -20px }
    AgentCubeKeyframe.pull: _y(<(double, double)>[
      (0, 0),
      (25, -20),
      (40, -20),
      (60, 0),
    ...
    AgentCubeKeyframe.shake: _y(<(double, double)>[
      (0, 0),
      (20, -2.5),
      (40, 2.5),
      (60, -2),
    ...
    AgentCubeKeyframe.bounce: _y(<(double, double)>[
      (0, 0),
      (35, -16),
      (55, 0),
      (70, -6),
    ...
    AgentCubeKeyframe.spin3d: null,
  };

  static final Map<AgentCubeKeyframe, Animatable<double>?>
  _opacity = <AgentCubeKeyframe, Animatable<double>?>{
    // 0% 0 → 10% 1 → 93% 1 → 95% 0 → 100% 0. It ENDS at zero on purpose:
    // *"cubes arrive one by one and then hard-cut back, Game Boy logo style."*
    AgentCubeKeyframe.appear: _a(<(double, double)>[
      (0, 0),
      (10, 1),
      (93, 1),
      (95, 0),
    ...
    AgentCubeKeyframe.drop: _a(<(double, double)>[
      (0, 0),
      (18, 1),
      (93, 1),
      (95, 0),
    ...
    // 0%,100% { 0.15 } 50% { 0.95 }
    AgentCubeKeyframe.blinkfade: _a(<(double, double)>[
      (0, 0.15),
      (50, 0.95),
      (100, 0.15),
    ]),
    // 0%,100% { 1 } 50% { 0.25 }
    AgentCubeKeyframe.blinkslow: _a(<(double, double)>[
      (0, 1),
      (50, 0.25),
      (100, 1),
    ]),
...
class AgentCubeMotion {
  const AgentCubeMotion(
    this.name,
    this.seconds, {
    this.delay = 0,
  ...
  const AgentCubeMotion.appear(double seconds, double delay)
    : this(AgentCubeKeyframe.appear, seconds, delay: delay, startsHidden: true);

  /// `drop(seconds, delay, timing)`.
  const AgentCubeMotion.drop(double seconds, double delay)
    : this(AgentCubeKeyframe.drop, seconds, delay: delay, startsHidden: true);

  final AgentCubeKeyframe name;
  ...
  ({double opacity, Offset translate}) sampleAt(double elapsed) {
    ...
    return (
      opacity: AgentCubeKeyframes.opacityAt(name, t),
      translate: AgentCubeKeyframes.translateAt(name, t),
    );
  }
}
...
class AgentCubeSpec {
  ...
  final AgentCubeMotion? motion;

  /// *"A second wrapper, for scenes that compose two motions (glide + bob)."*
  final AgentCubeMotion? outer;

  AgentCubeSpec dividedBy(double speed) => AgentCubeSpec(
    x: x,
    y: y,
    z: z,
    accent: accent,
...
class AgentCubeScene {
  const AgentCubeScene({required this.cubes, required this.width});

  final List<AgentCubeSpec> cubes;
...
///
/// [AgentState.idle] has no entry, and the reference says why: *"it is a real
...
AgentCubeScene agentCubeScene(AgentState state, {double speed = 1}) {
  assert(
    state != AgentState.idle,
    'idle is not an isometric scene — AgentAvatar draws it directly',
  );
  final AgentCubeScene scene = _scenes[state]!();
  if (speed == 1) return scene;
  return AgentCubeScene(
    cubes: <AgentCubeSpec>[
      for (final AgentCubeSpec cube in scene.cubes) cube.dividedBy(speed),
    ],
    width: scene.width,
  );
}

final Map<AgentState, AgentCubeScene Function()>
_scenes = <AgentState, AgentCubeScene Function()>{
  /// *"Five cubes waiting their turn, bobbing in sequence."*
  AgentState.queued: () => AgentCubeScene(
    width: 190,
    cubes: <AgentCubeSpec>[
      for (int x = 0; x < 5; x += 1)
        AgentCubeSpec(
          ...
          y: 0,
          accent: x == 2,
          motion: AgentCubeMotion(AgentCubeKeyframe.bob, 2.2, delay: x * 0.22),
        ),
    ],
  ),
  ...
  /// last."*
  AgentState.planning: () {
    final List<AgentCubeSpec> cubes = <AgentCubeSpec>[];
    for (int x = 0; x < 4; x += 1) {
      for (int y = 0; y < 2; y += 1) {
        for (int z = 0; z <= x; z += 1) {
          cubes.add(
            AgentCubeSpec(
              ...
              z: z.toDouble(),
              accent: z == x,
              motion: AgentCubeMotion.appear(3.6, x * 0.35 + z * 0.12),
            ),
          );
        }
      }
    }
    return AgentCubeScene(width: 160, cubes: cubes);
  },
        ...
        cubes.add(
          x == 1 && y == 1
              ? AgentCubeSpec(
                  ...
                  boost: _boost,
                  accent: true,
                  motion: const AgentCubeMotion(AgentCubeKeyframe.pull, 2.6),
                )
              : AgentCubeSpec(x: x.toDouble(), y: y.toDouble()),
        );
      }
    }
    return AgentCubeScene(width: _w, cubes: cubes);
  },
  ...
  AgentState.ingesting: () {
    ...
    for (int x = 0; x < 2; x += 1) {
      for (int y = 0; y < 2; y += 1) {
        cubes.add(AgentCubeSpec(x: x.toDouble(), y: y.toDouble()));
        cubes.add(
          AgentCubeSpec(
            x: x.toDouble(),
            y: y.toDouble(),
            z: 1,
            accent: true,
            motion: AgentCubeMotion.drop(2.8, (x * 2 + y) * 0.4),
          ),
        );
      }
    }
    return AgentCubeScene(width: 120, cubes: cubes);
  },
      ...
      for (int y = 0; y < 4; y += 1) {
        cubes.add(
          AgentCubeSpec(
            ...
            y: y.toDouble(),
            accent: x + y == 3,
            motion: AgentCubeMotion(
              AgentCubeKeyframe.settle,
              1.6,
              delay: (x + y) * 0.15,
            ),
          ),
  ...
  AgentState.delegating: () {
    ...
    cubes.add(
      const AgentCubeSpec(
        ...
        boost: _boost,
        accent: true,
        outer: AgentCubeMotion(AgentCubeKeyframe.glide, 2.2, alternate: true),
      ),
    );
    return AgentCubeScene(width: 190, cubes: cubes);
  },
        ...
        cubes.add(
          x == 2 && y == 2
              ? AgentCubeSpec(
                  ...
                  boost: _boost,
                  dashed: true,
                  motion: const AgentCubeMotion(
                    AgentCubeKeyframe.blinkslow,
                    2.2,
                  ),
                )
              : AgentCubeSpec(x: x.toDouble(), y: y.toDouble()),
  ...
  AgentState.retrying: () {
    ...
    cubes.add(
      const AgentCubeSpec(
        ...
        boost: _boost,
        accent: true,
        motion: AgentCubeMotion(AgentCubeKeyframe.bounce, 2),
      ),
    );
    return AgentCubeScene(width: 120, cubes: cubes);
  },
        ...
        cubes.add(
          x == 1 && y == 1
              ? AgentCubeSpec(
                  ...
                  y: y.toDouble(),
                  red: true,
                  motion: const AgentCubeMotion(AgentCubeKeyframe.shake, 1.4),
                )
              : AgentCubeSpec(x: x.toDouble(), y: y.toDouble()),
        );
      }
  ...
  AgentState.summarizing: () {
    ...
    for (int i = 0; i < _ring.length; i += 1) {
      final (double x, double y) = _ring[i];
      cubes.add(
        AgentCubeSpec(x: x, y: y, motion: AgentCubeMotion.appear(4, i * 0.26)),
      );
    }
    cubes.add(
      const AgentCubeSpec(
        ...
        y: 1,
        accent: true,
        motion: AgentCubeMotion.appear(4, 2.1),
      ),
    );
    return AgentCubeScene(width: _w, cubes: cubes);
  },
      ...
      for (int y = 0; y < 4; y += 1) {
        ...
        cubes.add(
          AgentCubeSpec(
            ...
            motion: delay == null
                ? null
                : AgentCubeMotion(AgentCubeKeyframe.rise, 3, delay: delay),
          ),
        );
      }
    }
        ...
        for (int y = 0; y < 2; y += 1) {
          cubes.add(
            AgentCubeSpec(
              x: x.toDouble(),
              y: y.toDouble(),
              motion: const AgentCubeMotion(AgentCubeKeyframe.settle, 2.4),
            ),
          );
        }
      }
    ...
    for (final (double x, double y, double delay) in <(double, double, double)>[
      ...
      cubes.add(
        AgentCubeSpec(
          ...
          y: y,
          accent: true,
          motion: AgentCubeMotion.appear(3, delay),
        ),
      );
    }
    return AgentCubeScene(width: 196, cubes: cubes);
  },
  ...
  AgentState.searching: () {
    ...
    cubes.add(
      const AgentCubeSpec(
        ...
        boost: _boost,
        accent: true,
        outer: AgentCubeMotion(AgentCubeKeyframe.glide, 2.6, alternate: true),
        motion: AgentCubeMotion(AgentCubeKeyframe.bob, 1.3),
      ),
    );
    return AgentCubeScene(width: _w, cubes: cubes);
  },
  ...
  AgentState.reading: () {
          ...
          cubes.add(
            AgentCubeSpec(
              ...
              motion: z == 0
                  ? null
                  : AgentCubeMotion(
                      z == 1 ? AgentCubeKeyframe.lift : AgentCubeKeyframe.lift2,
                      2.2,
                    ),
            ),
          );
    ...
    return AgentCubeScene(width: 150, cubes: cubes);
  },
      ...
      for (int x = 0; x < 4; x += 1) {
        cubes.add(
          AgentCubeSpec(
            ...
            motion: row == 2
                ? null
                : AgentCubeMotion.appear(3.2, (1 - row) * 0.5 + x * 0.12),
          ),
        );
      }
    }
      ...
      for (int x = 0; x < 4; x += 1) {
        cubes.add(
          AgentCubeSpec(
            ...
            y: y.toDouble(),
            accent: y == 2 && x == 3,
            motion: AgentCubeMotion.appear(3.4, (y * 4 + x) * 0.13),
          ),
        );
      }
    }
        ...
        for (int z = 0; z < 2; z += 1) {
          cubes.add(
            AgentCubeSpec(
              ...
              z: z.toDouble(),
              accent: z == 1,
              motion: const AgentCubeMotion(AgentCubeKeyframe.settle, 3.2),
            ),
          );
        }
      }
    }
    return AgentCubeScene(width: 110, cubes: cubes);
  },
};
...
class CubeScene extends StatelessWidget {
  const CubeScene({
    super.key,
    required this.scene,
    required this.width,
  ...
  Widget build(BuildContext context) {
    final Rect viewBox = AgentCube.viewBoxOf(scene.cubes);
    ...
    return SizedBox(
      ...
      child: CustomPaint(
        painter: _CubeScenePainter(
          cubes: AgentCube.sorted(scene.cubes),
          viewBox: viewBox,
          tokens: ThemeScope.of(context).cube,
          accent: accent,
          elapsed: elapsed,
          frozen: frozen,
        ),
...
class _CubeScenePainter extends CustomPainter {
  ...
  void _paintCube(Canvas canvas, AgentCubeSpec cube, double scale) {
    ...
    final Offset origin =
        AgentCube.iso(cube.x, cube.y, cube.z) +
        outer.translate +
        inner.translate;

    final AgentCubeFaces faces = AgentCubeFaces.forCube(cube, tokens, accent);
    ...
    if (layered) {
      canvas.saveLayer(
        ...
        Rect.fromCenter(
          center: Offset.zero,
          width: AgentCube.halfWidth * 4,
          height: AgentCube.cubeHeight * 6,
        ),
        Paint()..color = transparent.withValues(alpha: opacity),
      );
    }
    ...
    _paintFace(canvas, AgentCube.leftFace, faces.left, faces, scale);
    _paintFace(canvas, AgentCube.rightFace, faces.right, faces, scale);
    _paintFace(canvas, AgentCube.topFace, faces.top, faces, scale);
  ...
  void _paintFace(
    ...
    final Paint stroke = Paint()
      ...
      ..strokeWidth = AgentCube.strokeWidth
...
class _IdleCube extends StatelessWidget {
  ...
  double get face => AgentCube.idleFace(scale);
    ...
    return SizedBox(
      ...
      child: CustomPaint(
        painter: _IdleCubePainter(
          face: size,
          perspective: AgentCube.idlePerspective(scale),
...
class _IdleCubePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final AgentCubeFaces lit = AgentCubeFaces.accent(tokens, accent);
...
/// hero"*.
///
/// Named `AgentAvatarSize` rather than `AvatarSize` because the base
...
enum AgentAvatarSize {
  ...
  const AgentAvatarSize(this._box, this.scale);
...
class AgentAvatar extends StatefulWidget {
  ...
  /// [AgentState.idle], as the reference's own signature does.
  ...
  static Duration get crossfade => MotionDurations.fast;

  /// `agent-cube-spin3d 9s linear infinite` — idle's own clock, and the one
  /// duration in the family that is not a scene recipe's.
  static const AgentCubeMotion spin = AgentCubeMotion(
    AgentCubeKeyframe.spin3d,
    9,
  );

  @override
  State<AgentAvatar> createState() => _CubeAvatarState();
}

class _CubeAvatarState extends State<AgentAvatar>
    with TickerProviderStateMixin {
  /// A bare [Ticker] rather than an [AnimationController], for the reason
  /// `ActionFeedback` states: a scene runs up to three periods and twenty
  ...
  late final AnimationController _fade = AnimationController(
    vsync: this,
    duration: AgentAvatar.crossfade,
  );
  ...
  void didChangeDependencies() {
    ...
    final bool frozen =
        effectiveMotionDuration(context, AgentAvatar.crossfade) ==
        Duration.zero;
  ...
  @override
  void didUpdateWidget(AgentAvatar old) {
  ...
  Widget _scene(AgentState state, Color accent, double elapsed) {
    if (state == AgentState.idle) {
      return _IdleCube(
        scale: widget.size.scale,
        accent: accent,
        motion: AgentAvatar.spin.dividedBy(widget.speed),
        elapsed: elapsed,
        frozen: _frozen,
      );
    }
    final AgentCubeScene scene = agentCubeScene(state, speed: widget.speed);
    return CubeScene(
      scene: scene,
  ...
  Widget build(BuildContext context) {
    ...
    final Color accent = widget.accent ?? ThemeScope.of(context).agentAccent;
    final AgentState? outgoing = _outgoing;
            ...
            _loose(
              AnimatedBuilder(
                ...
                builder: (BuildContext context, Widget? child) => Opacity(
                  opacity: MotionCurves.enter.transform(_fade.value),
                  child: child,
                ),
