...
///
/// `Press` is deliberately **not** used here. §3.9 of the audit traces a live
...
///
/// Two variants are not a flat fill. `default` wears `action-feedback` and
/// `premium` wears `premium-surface` — a gradient ramp plus two blended
/// pseudo-layers each — so those two route their surface through
/// [ActionFeedback] / [PremiumSurface] instead of [Surface]. The rest of
...
enum ButtonVariant {
  /// `variant="default"` — `action-feedback bg-primary text-primary-foreground
  ...
  /// `premium-surface font-semibold text-value-foreground shadow-btn-value
  ...
  /// contradicts — see the drift note on [Button].
  secondary,
...
enum ButtonSize {
  ...
  /// [ButtonVariant.primary] carries, and for the same reason.
  md,
...
/// `focus-visible:ring-ring/50` on every variant but [ButtonVariant.destructive].
...
class _ButtonSkin {
  ...
  /// [ButtonStyleRecipe] applied over this skin.
  _ButtonSkin withSurface(ButtonStyleRecipe? surface, {required bool hovered}) {
...
/// utility does.
class ButtonStyleRecipe {
  const ButtonStyleRecipe({
    this.fill,
    this.hoverFill,
    this.border,
    this.hoverBorder,
...
/// `shadow-btn-primary`"*. In fact only [ButtonVariant.outline] carries
/// `shadow-btn`; [ButtonVariant.secondary] and [ButtonVariant.destructive]
/// declare no shadow class at all, and [ButtonVariant.premium] carries
...
class Button extends StatefulWidget {
  const Button({
    ...
    this.emphasis = ButtonEmphasis.none,
    this.loading = false,
    this.onPressed,
    this.label,
    this.focusNode,
  ...
  /// Utilities appended to the variant's class list — see [ButtonStyleRecipe].
  ///
  ...
  final ButtonStyleRecipe? surface;
  ...
  /// The uppercase treatment. See [ButtonEmphasis.caps].
  ...
  /// Flutter runs out of vocabulary, and [_ButtonState.build] records why.
  ///
  ...
  /// the button grows by [Spinner.px] + [gapFor] = **24px on the `default`
  ...
  static double heightFor(ButtonSize size) => switch (size) {
    ButtonSize.xs || ButtonSize.iconXs => space(6),
    ButtonSize.sm || ButtonSize.iconSm => space(8),
    ButtonSize.md || ButtonSize.icon => space(10),
    ButtonSize.lg || ButtonSize.iconLg => space(12),
    ButtonSize.xl => space(14),
  };
  ...
  static double gapFor(ButtonSize size) => switch (size) {
    ButtonSize.xs => space(1),
    ButtonSize.sm => space(1.5),
    ButtonSize.md => space(2),
    ButtonSize.lg || ButtonSize.xl => space(2.5),
    ButtonSize.iconXs ||
    ButtonSize.iconSm ||
    ButtonSize.icon ||
    ButtonSize.iconLg => 0,
  ...
  static double paddingXFor(ButtonSize size) => switch (size) {
    ButtonSize.xs => space(2.5),
    ButtonSize.sm => space(3.5),
    ButtonSize.md => space(4),
    ButtonSize.lg => space(6),
    ButtonSize.xl => space(8),
    ButtonSize.iconXs ||
    ButtonSize.iconSm ||
    ButtonSize.icon ||
    ButtonSize.iconLg => 0,
  ...
  /// reach into [child] and resize a [Icon], so the rung publishes the number
  ...
  static double iconPxFor(ButtonSize size) => switch (size) {
    ButtonSize.xs || ButtonSize.iconXs => 12,
    ButtonSize.sm || ButtonSize.iconSm => 14,
    ButtonSize.md || ButtonSize.lg || ButtonSize.icon => 16,
  ...
  static bool isSquare(ButtonSize size) =>
  ...
  /// front of it — the shared focus-ring helper both this widget and `Input`
  /// reach for.
  ///
  ...
  /// first-listed `box-shadow` layer on top and [ShadowStyle.outerShadows]
  ...
  static ShadowStyle withFocusRing(
    ShadowStyle spec,
    Color ring, {
    double progress = 1,
...
class _ButtonState extends State<Button> {
  ...
  ///
  /// [Button.expanded] enters the table as `lit` — the three variants that
  ...
  _ButtonSkin _skin(ThemeTokens theme) {
    ...
    switch (widget.variant) {
      case ButtonVariant.primary:
        return _ButtonSkin(
          // `bg-primary` under the `action-feedback` ramp, which is opaque and
          // covers it — carried anyway because the class list carries it.
          fill: theme.primary,
          border: transparent,
          content: theme.primaryForeground,
          shadow: _pressed ? Shadows.controlPressed : Shadows.controlPrimary,
      ...
      case ButtonVariant.outline:
        return _ButtonSkin(
          ...
          shadow: _pressed ? Shadows.controlPressed : Shadows.control,
          ring: theme.ring.withValues(alpha: _focusRingAlpha),
        );
  ...
  Color _focusBorder(ThemeTokens theme) =>
      widget.variant == ButtonVariant.destructive
  ...
  /// 24px of [Button.loading]'s drift comes from: 16 for the glyph, 8 for
  /// `gap-2`.
  Widget _content() {
    final Widget label = widget.emphasis == ButtonEmphasis.caps
        ? _caps(widget.child)
        : widget.child;
  ...
  /// [Surface] directly. `default` and `premium` paint a gradient
  /// `background-image` plus two blended pseudo-layers, which is what
  /// [ActionFeedback] and [PremiumSurface] are; they take the same shadow spec,
  /// radius and border and splice the ramp in where CSS puts it.
  Widget _surface({
    required ShadowStyle spec,
  ...
  Widget build(BuildContext context) {
    ...
    final double height = Button.heightFor(widget.size);
    final bool square = Button.isSquare(widget.size);
    final BorderRadius radius =
        widget.radius ?? BorderRadius.circular(Radii.full);
    ...
    Widget button = TweenAnimationBuilder<double>(
      ...
      builder: (BuildContext context, double ringT, Widget? _) {
        ...
        final ShadowStyle spec = ringT <= 0
            ? skin.shadow
            : Button.withFocusRing(skin.shadow, skin.ring, progress: ringT);
        return _SpringColors(
          ...
          builder: (BuildContext context, Color fill, Color border, Color ink) {
            final TextStyleToken? typeSpec = Button.typeFor(
              widget.size,
              widget.emphasis,
            );
            ...
            TextStyle style = typeSpec == null
                ...
                : StyledText.styleOf(context, typeSpec, color: ink);
              ...
              child: Padding(
                ...
                padding:
                    ...
                    EdgeInsets.symmetric(
                      horizontal: Button.paddingXFor(widget.size),
                    ),
    ...
    // scale at all, at any hold length. See [Button.suppressPressScale].
    //
