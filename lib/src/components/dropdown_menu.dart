...
/// [DsPopover]. What is left here is the trigger contract and the four numbers
...
/// | `max-h-(--radix-…-available-height)` | | capped at the room the chosen side leaves, which is what [DsPopover] already constrains its child to |
///
...
/// over. [DsButton] carried no flag for it while `button.dart` belonged to
/// another wave; it carries [DsButton.suppressPressScale] now, every trigger
/// passes it, and [DsDropdownMenu.pressScaleSuppressed] pins that it does.
///
...
/// pointer moved away while the menu stays open. [DsMenuTriggerScope] publishes
/// the open flag and [DsButton.expanded] resolves it, so the fill is now held
...
/// call site passes [DsButton.suppressPressScale] itself; `aria-expanded`
/// changes with a menu the call site does not own, so it comes from here.
class DsMenuTriggerScope extends InheritedWidget {
  const DsMenuTriggerScope({
    super.key,
    required this.open,
  ...
  static bool openOf(BuildContext context) =>
      context
          .dependOnInheritedWidgetOfExactType<DsMenuTriggerScope>()
          ?.open ??
      false;

  @override
  bool updateShouldNotify(DsMenuTriggerScope oldWidget) =>
      oldWidget.open != open;
}

/// A `DropdownMenu` — [trigger], and [children] under it.
///
/// Open state is a prop, the way [DsPopover]'s is and the way Radix's `open`
/// is: the caller owns the boolean. [DsDropdownMenu] is the *uncontrolled*
/// spelling — it owns the boolean itself, because every instance on the menus
/// page is uncontrolled and a caller that wants control can drive [DsPopover]
/// and [DsMenuContent] directly, which is what this widget is.
class DsDropdownMenu extends StatefulWidget {
  ...
  static double get sideOffset => ds(1);
  ...
  /// the port's. True, as it is on [DsDatePicker] — the same attribute, one
  /// component over.
  ...
  /// is a call-site fact: [DsDropdownMenu] cannot suppress anything itself, so
  /// this is only true for as long as every trigger keeps passing
  /// [DsButton.suppressPressScale].
  ...
  Widget build(BuildContext context) {
    return DsPopover(
      ...
      anchor: DsMenuPointerDown(
        ...
        // site authored.
        child: DsMenuTriggerScope(open: _isOpen, child: widget.trigger),
      ),
      content: (BuildContext context, DsPopoverAnchorMetrics metrics) =>
          DsMenuContent(
        children: widget.children,
        width: widget.width,
        minWidth: DsMenu.minWidthDropdown,
        onClose: _close,
      ),
    );
  }
