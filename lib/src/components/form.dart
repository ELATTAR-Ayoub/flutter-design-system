...
/// objects. So the id half of this file lives in `DsFieldScope` (see
...
/// `"onTouched"` anywhere — so [DsValidateMode] has two members and not four.
///
...
/// (`forms-map.md` §3.4).
///
/// [DsForm.focusFirstError] focuses the first invalid field **whatever it is**.
...
/// `{ shouldFocus: true }` is not passed — and [DsForm.setError] matches that.
library;
...
abstract class DsFormFieldBase extends ChangeNotifier {
  DsFormFieldBase({required this.name});
  ...
  /// The node a failed submit lands on, and the one a `DsFieldLabel` focuses.
  ///
  ...
  /// The form's re-validation hook. Set when the field joins a [DsForm].
...
class DsFormField<T> extends DsFormFieldBase {
  DsFormField({
    required super.name,
    required T initialValue,
    List<DsRule<T>>? rules,
    this.issueMode = DsIssueMode.first,
  })  : _initialValue = initialValue,
        _value = initialValue,
        rules = rules ?? <DsRule<T>>[];

  /// The schema, in declaration order. Zod evaluates every check in that order
  /// without aborting, and so does [DsRules.check].
  final List<DsRule<T>> rules;

  /// `criteriaMode` — one message or all of them.
  final DsIssueMode issueMode;
  ...
  @override
  List<String> issues() =>
      DsRules.check<T>(_value, rules, mode: issueMode);
...
/// A [DsFormField] that owns the [TextEditingController] its control edits.
///
...
class DsTextFormField extends DsFormField<String> {
  ...
  /// Hand this to `DsInput.controller` / `DsTextarea.controller`.
...
/// [ListenableBuilder] and passes each field's state into its own `DsField`.
class DsForm extends ChangeNotifier {
  DsForm({
    required this.fields,
    this.mode = DsValidateMode.onSubmit,
    this.reValidateMode = DsValidateMode.onChange,
  }) {
    for (final DsFormFieldBase field in fields) {
      field._onEdit = () => _onEdit(field);
    }
  }

  /// **Registration order**, which is the order [focusFirstError] walks.
  final List<DsFormFieldBase> fields;

  /// When the question is first asked.
  final DsValidateMode mode;

  /// When it is asked again, once the form has been submitted.
  final DsValidateMode reValidateMode;
  ...
  /// `formState.isSubmitting` — what `DsButton.loading` reads.
  bool get isSubmitting => _submitting;

  /// `formState.isValid`, computed from what is currently stored rather than
  /// re-asked.
  bool get isValid => fields.every((DsFormFieldBase f) => !f.invalid);

  /// The field named [name], or a thrown [StateError] naming what is declared.
  DsFormFieldBase operator [](String name) => fields.firstWhere(
        (DsFormFieldBase f) => f.name == name,
        orElse: () => throw StateError(
          'No field named "$name". This form declares: '
          '${fields.map((DsFormFieldBase f) => f.name).join(', ')}.',
        ),
      );

  /// [name] as its typed self — `form.field<String>('handle').value`.
  DsFormField<T> field<T>(String name) => this[name] as DsFormField<T>;

  /// The text field named [name], for its controller.
  DsTextFormField text(String name) => this[name] as DsTextFormField;
  ...
  void _onEdit(DsFormFieldBase field) {
    // `mode` decides the first ask; once the form has been submitted,
    // `reValidateMode` decides every one after it.
    final DsValidateMode active =
        _submitCount == 0 ? mode : reValidateMode;
    if (active == DsValidateMode.onChange) field.validate();
    notifyListeners();
  }
