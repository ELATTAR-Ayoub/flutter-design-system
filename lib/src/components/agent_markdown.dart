...
/// contract, which is the part that actually needs porting: [dsSafeHref] refuses
...
///    [_MdType.body] and [DsAgentCodeBlock].
...
class _MdType {
  ...
  static final DsTypeSpec body = DsTypeSpec(
    family: DsFonts.sans,
    size: DsType.body.size,
    height: relaxed,
    wght: 400,
  );

  /// `<strong className="font-semibold">` inside the body.
  static final DsTypeSpec strong = DsTypeSpec(
    family: DsFonts.sans,
    size: DsType.body.size,
    height: relaxed,
    wght: 600,
  ...
  static final DsTypeSpec emphasis = DsTypeSpec(
    family: DsFonts.sans,
    size: DsType.body.size,
    height: relaxed,
    wght: 400,
...
@immutable
class DsMarkdownOrderedItem {
  const DsMarkdownOrderedItem(this.n, this.text);
...
enum DsMarkdownAlign { left, right, center }

/// The block vocabulary `parse()` produces.
sealed class DsMarkdownBlock {
  const DsMarkdownBlock();
}

class DsMarkdownParagraph extends DsMarkdownBlock {
  DsMarkdownParagraph(this.lines);
  final List<String> lines;
}

class DsMarkdownBullets extends DsMarkdownBlock {
  DsMarkdownBullets(this.items);
  final List<String> items;
}

class DsMarkdownNumbers extends DsMarkdownBlock {
  DsMarkdownNumbers(this.items);
  final List<DsMarkdownOrderedItem> items;
}

class DsMarkdownHeading extends DsMarkdownBlock {
  const DsMarkdownHeading(this.level, this.text);
  final int level;
  final String text;
}

class DsMarkdownFence extends DsMarkdownBlock {
  const DsMarkdownFence(this.lang, this.lines);
  final String lang;
  final List<String> lines;
}

class DsMarkdownQuote extends DsMarkdownBlock {
  DsMarkdownQuote(this.lines);
  final List<String> lines;
}

class DsMarkdownTable extends DsMarkdownBlock {
  const DsMarkdownTable(this.head, this.rows, this.align);
  final List<String> head;
  final List<List<String>> rows;
  final List<DsMarkdownAlign> align;
}
...
List<DsMarkdownAlign> _alignments(String delimiter) =>
    _cells(delimiter).map((String c) {
      final bool left = c.startsWith(':');
      final bool right = c.endsWith(':');
      if (left && right) return DsMarkdownAlign.center;
      if (right) return DsMarkdownAlign.right;
      return DsMarkdownAlign.left;
    }).toList(growable: false);

/// Group lines into blocks. A blank line closes whatever is open.
List<DsMarkdownBlock> dsParseMarkdown(String source) {
  final List<DsMarkdownBlock> blocks = <DsMarkdownBlock>[];
  DsMarkdownBlock? open;
  void close() {
  ...
  for (int i = 0; i < lines.length; i += 1) {
    ...
    if (fence != null) {
      ...
      blocks.add(DsMarkdownFence(fence[1] ?? '', body));
      continue;
    }
    ...
    if (heading != null) {
      close();
      blocks.add(DsMarkdownHeading(heading[1]!.length, heading[2]!));
      continue;
    }
    ...
    if (line.contains('|') &&
      ...
      final List<DsMarkdownAlign> align = _alignments(lines[i + 1]);
      ...
      i -= 1;
      blocks.add(DsMarkdownTable(head, rows, align));
      continue;
    }

    final RegExpMatch? quote = _quote.firstMatch(line);
    if (quote != null) {
      if (open is! DsMarkdownQuote) close();
      if (open is! DsMarkdownQuote) open = DsMarkdownQuote(<String>[]);
      (open! as DsMarkdownQuote).lines.add(quote[1]!);
      continue;
    }

    final RegExpMatch? bullet = _bullet.firstMatch(line);
    if (bullet != null) {
      if (open is! DsMarkdownBullets) close();
      if (open is! DsMarkdownBullets) open = DsMarkdownBullets(<String>[]);
      (open! as DsMarkdownBullets).items.add(bullet[1]!);
      continue;
    }

    final RegExpMatch? numbered = _number.firstMatch(line);
    if (numbered != null) {
      if (open is! DsMarkdownNumbers) close();
      if (open is! DsMarkdownNumbers) {
        open = DsMarkdownNumbers(<DsMarkdownOrderedItem>[]);
      }
      (open! as DsMarkdownNumbers).items.add(
            DsMarkdownOrderedItem(int.parse(numbered[1]!), numbered[2]!),
          );
      continue;
    }
    ...
    if (open is DsMarkdownBullets) {
      final List<String> items = (open! as DsMarkdownBullets).items;
      items[items.length - 1] = '${items[items.length - 1]} ${line.trim()}';
      continue;
    }
    if (open is DsMarkdownNumbers) {
      final List<DsMarkdownOrderedItem> items =
          (open! as DsMarkdownNumbers).items;
      final DsMarkdownOrderedItem last = items[items.length - 1];
      items[items.length - 1] =
          DsMarkdownOrderedItem(last.n, '${last.text} ${line.trim()}');
      continue;
    }

    if (open is! DsMarkdownParagraph) close();
    if (open is! DsMarkdownParagraph) open = DsMarkdownParagraph(<String>[]);
    (open! as DsMarkdownParagraph).lines.add(line.trim());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return DsInlineBox(
      ...
      child: Container(
        ...
        child: DsText(text, DsType.code, color: theme.foreground, inline: true),
      ),
    );
  }
}
...
InlineSpan dsRenderMarkdownInline(BuildContext context, String text) {
  final DsThemeData theme = DsTheme.of(context);
  final List<InlineSpan> out = <InlineSpan>[];
  int last = 0;

  TextStyle style(DsTypeSpec spec, {Color? color}) =>
      DsText.styleOf(context, spec, color: color);
  ...
  for (final RegExpMatch match in _inline.allMatches(text)) {
    ...
    if (code != null) {
      ...
      final String? href =
          dsSafeHref(anchor.substring(split + 2, anchor.length - 1));
      ...
      final String? href = dsSafeHref(bare);
...
/// return null`.
class DsAgentMarkdown extends StatelessWidget {
  const DsAgentMarkdown({super.key, required this.text, this.textAlign});
  ...
  static double get blockGap => ds(2);

  /// `pl-6` on both list forms.
  static double get listInset => ds(6);

  /// `space-y-1` between list items.
  static double get itemGap => ds(1);

  /// `pl-4` on a blockquote, over a 2px left rule.
  static double get quoteInset => ds(4);

  /// `border-l-2`.
  static double get quoteRule => ds(0.5);

  /// `border-agent/40`.
  static const double quoteRuleAlpha = 0.40;

  /// `px-3 py-2` in every table cell.
  static double get cellPadX => ds(3);
  static double get cellPadY => ds(2);

  /// `mt-1` on a heading.
  static double get headingTop => ds(1);

  @override
  Widget build(BuildContext context) {
    final List<DsMarkdownBlock> blocks = dsParseMarkdown(text);
    if (blocks.isEmpty) return const SizedBox.shrink();

    final DsThemeData theme = DsTheme.of(context);

    return DefaultTextStyle(
      style: DsText.styleOf(context, _MdType.body, color: theme.foreground),
      child: Column(
  ...
  Widget _block(
    ...
    DsThemeData theme,
      ...
      switch (block) {
        ...
        // a message.
        DsMarkdownHeading(:final String text) => Padding(
            padding: EdgeInsets.only(top: headingTop),
            child: DsRichText(
              dsRenderMarkdownInline(context, text),
              DsType.h4,
              color: theme.foreground,
              align: textAlign,
            ),
          ),
        DsMarkdownFence(:final String lang, :final List<String> lines) =>
          DecoratedBox(
            decoration: BoxDecoration(
            ...
            child: Padding(
              ...
              child: ClipRRect(
                borderRadius:
                    BorderRadius.circular(DsRadii.md - DsWidths.hairline),
                child: DsAgentCodeBlock(
                  code: lines.join('\n'),
                  language: lang,
                ),
              ),
        ...
        DsMarkdownQuote(:final List<String> lines) => IntrinsicHeight(
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              mainAxisSize: MainAxisSize.min,
              children: <Widget>[
                ...
                Expanded(
                  child: DsRichText(
                    dsRenderMarkdownInline(context, lines.join(' ')),
                    _MdType.quote,
        ...
        DsMarkdownBullets(:final List<String> items) => _list(
            context,
            theme,
            <_ListRow>[
              for (final String item in items) (marker: '•', text: item),
            ],
            markerWeight: null,
          ),
        // `value` rather than relying on position: a list split across blocks
        // must still read 1, 2, 3 rather than 1, 1, 1.
        DsMarkdownNumbers(:final List<DsMarkdownOrderedItem> items) => _list(
            context,
            theme,
            <_ListRow>[
              for (final DsMarkdownOrderedItem item in items)
                (marker: '${item.n}.', text: item.text),
            ],
            markerWeight: DsType.small,
          ),
        DsMarkdownTable() => _table(context, theme, block),
        DsMarkdownParagraph(:final List<String> lines) => DsRichText(
            dsRenderMarkdownInline(context, lines.join(' ')),
            _MdType.body,
            color: theme.foreground,
            align: textAlign,
  ...
  Widget _list(
    ...
    DsThemeData theme,
    List<_ListRow> rows, {
    required DsTypeSpec? markerWeight,
  }) {
    // `marker:font-medium` on the ordered form only; both markers take
    // `--muted-foreground`.
    final TextStyle marker = DsText.styleOf(
      context,
    ...
    return Column(
      ...
      children: <Widget>[
        for (int i = 0; i < rows.length; i += 1)
          Padding(
            padding:
                EdgeInsets.only(bottom: i == rows.length - 1 ? 0 : itemGap),
            child: Row(
              ...
              children: <Widget>[
                ...
                Expanded(
                  child: DsRichText(
                    dsRenderMarkdownInline(context, rows[i].text),
                    _MdType.body,
                    color: theme.foreground,
                    align: textAlign,
                  ),
  ...
  Widget _table(
    BuildContext context,
    DsThemeData theme,
    DsMarkdownTable block,
  ) {
    DsMarkdownAlign alignOf(int j) =>
        j < block.align.length ? block.align[j] : DsMarkdownAlign.left;

    TextAlign flutterAlign(DsMarkdownAlign a) => switch (a) {
          DsMarkdownAlign.left => TextAlign.left,
          DsMarkdownAlign.right => TextAlign.right,
          DsMarkdownAlign.center => TextAlign.center,
        };
    ...
    const double half = DsWidths.hairline / 2;
    Widget cell(
      Widget child, {
              ...
              TableRow(
                ...
                children: <Widget>[
                  for (int j = 0; j < block.head.length; j += 1)
                    cell(
                      DsRichText(
                        dsRenderMarkdownInline(context, block.head[j]),
                        DsType.label,
                        color: theme.mutedForeground,
...
const Map<String, String> dsLanguageAliases = <String, String>{
  'bash': 'bash',
  'css': 'css',
  'js': 'javascript',
...
class DsPrismPalette {
  const DsPrismPalette._();

  /// `pre[class*="language-"] { background: #1e1e1e }`.
  static const Color ground =
      Color(0xFF1E1E1E); // allow-hardcoded: vscDarkPlus ground

  /// The theme's plain-text colour.
  static const Color plain =
      Color(0xFFD4D4D4); // allow-hardcoded: vscDarkPlus plain

  static const Color keyword =
      Color(0xFF569CD6); // allow-hardcoded: vscDarkPlus keyword
  static const Color string =
      Color(0xFFCE9178); // allow-hardcoded: vscDarkPlus string
  static const Color number =
      Color(0xFFB5CEA8); // allow-hardcoded: vscDarkPlus number
  static const Color function =
      Color(0xFFDCDCAA); // allow-hardcoded: vscDarkPlus function
  static const Color comment =
      Color(0xFF6A9955); // allow-hardcoded: vscDarkPlus comment
  static const Color type =
      Color(0xFF4EC9B0); // allow-hardcoded: vscDarkPlus class-name
...
@immutable
class DsCodeToken {
  const DsCodeToken(this.text, this.color);
  final String text;
  final Color color;
}
...
/// language over a Prism-tokenised body in [DsPrismPalette].
class DsAgentCodeBlock extends StatelessWidget {
  const DsAgentCodeBlock({super.key, required this.code, this.language});

  final String code;
  final String? language;

  /// `p-3` on the un-highlighted form.
  static double get plainPadding => ds(3);

  /// `px-3 py-2` on the header strip.
  static double get stripPadX => ds(3);
  static double get stripPadY => ds(2);

  /// The normalised language, or null when the fence named one this file does
  /// not register.
  static String? normalise(String? language) {
    if (language == null || language.isEmpty) return null;
    return dsLanguageAliases[language.toLowerCase()];
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (normalized == null) {
      return Container(
        ...
        child: _Scroller(
          child: DsPreformattedCode(code: code, color: theme.foreground),
        ),
      );
    }
    ...
    final double strut =
        DsText.styleOf(context, _MdType.body).fontSize! * _MdType.relaxed;
    ...
    return ColoredBox(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Container(
            color: DsPrismPalette.ground,
            margin: const EdgeInsets.symmetric(
              vertical: DsPrismPalette.margin,
            ),
            padding: const EdgeInsets.all(DsPrismPalette.padding),
...
/// use a plain [DsText] instead.
class DsPreformattedCode extends StatelessWidget {
  const DsPreformattedCode({super.key, required this.code, this.color});
...
class _HighlightedCode extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final List<List<DsCodeToken>> lines = dsTokenise(code, language);
    final TextStyle base = DsText.styleOf(
      context,
      DsType.code,
      color: DsPrismPalette.plain,
    );
    ...
    return Column(
      ...
      mainAxisSize: MainAxisSize.min,
      children: <Widget>[
        for (final List<DsCodeToken> line in lines)
          SizedBox(
            height: DsPrismPalette.lineHeight,
            child: Align(
              alignment: Alignment.centerLeft,
              child: Text.rich(
                TextSpan(
                  children: <InlineSpan>[
                    for (final DsCodeToken token in line)
                      TextSpan(
                        text: token.text,
...
List<List<DsCodeToken>> dsTokenise(String code, String language) =>
    <List<DsCodeToken>>[
...
const Set<String> _sqlKeywords = <String>{
  'and', 'as', 'asc', 'by', 'delete', 'desc', 'distinct', 'from', 'group',
  'having', 'inner', 'insert', 'into', 'join', 'left', 'limit', 'not', 'null',
  'on', 'or', 'order', 'outer', 'right', 'select', 'set', 'update', 'values',
  'where',
};
...
List<DsCodeToken> _tokeniseLine(String line, String language) {
  ...
  final List<DsCodeToken> out = <DsCodeToken>[];
  final StringBuffer plain = StringBuffer();

  void flush() {
    if (plain.isEmpty) return;
    out.add(DsCodeToken(plain.toString(), DsPrismPalette.plain));
    plain.clear();
  }
  ...
  while (i < line.length) {
    ...
    if ((hash && c == '#') ||
      ...
      flush();
      out.add(DsCodeToken(line.substring(i), DsPrismPalette.comment));
      return out;
    }
    ...
    if (_digit.hasMatch(c)) {
      ...
      out.add(DsCodeToken(line.substring(start, i), DsPrismPalette.number));
      continue;
    }
    ...
    if (_identStart.hasMatch(c)) {
      ...
      if (keywords.contains(word)) {
        out.add(DsCodeToken(word, DsPrismPalette.keyword));
      } else if (i < line.length && line[i] == '(') {
        out.add(DsCodeToken(word, DsPrismPalette.function));
      } else if (word.isNotEmpty &&
          word[0].toUpperCase() == word[0] &&
          word[0].toLowerCase() != word[0]) {
        out.add(DsCodeToken(word, DsPrismPalette.type));
      } else {
        out.add(DsCodeToken(word, DsPrismPalette.plain));
      }
      continue;
    }
...
List<DsCodeToken> _tokeniseMarkdownLine(String line) {
  if (_mdFence.hasMatch(line) || _mdHeading.hasMatch(line)) {
    return <DsCodeToken>[DsCodeToken(line, DsPrismPalette.keyword)];
  }
  final List<DsCodeToken> out = <DsCodeToken>[];
  int last = 0;
  for (final RegExpMatch m in _mdEmphasis.allMatches(line)) {
    if (m.start > last) {
      out.add(
        DsCodeToken(line.substring(last, m.start), DsPrismPalette.plain),
      );
    }
    out.add(DsCodeToken(m[0]!, DsPrismPalette.string));
    last = m.end;
  }
  if (last < line.length) {
    out.add(DsCodeToken(line.substring(last), DsPrismPalette.plain));
  }
  return out;
}
