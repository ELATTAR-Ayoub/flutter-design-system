...
/// |---|---|
/// | `voice/visualisers.tsx` | [DsLiveWaveform], [DsBarVisualizer] |
/// | `voice/pickers.tsx` | [DsMicControl] |
///
...
/// buffer that call would have filled — [DsLiveWaveform.samples] and
/// [DsBarVisualizer.spectrum] — and everything downstream of the read is the
...
/// and twelve bars at their 0.06 floor. [DsBarVisualizer.active] is the one
...
class DsLiveWaveform extends StatefulWidget {
  const DsLiveWaveform({
    super.key,
    this.samples,
  ...
  @override
  State<DsLiveWaveform> createState() => _DsLiveWaveformState();
}

class _DsLiveWaveformState extends State<DsLiveWaveform> {
  @override
...
class _WaveformPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint paint = Paint()
      ...
      ..strokeWidth = DsLiveWaveform.strokeWidth
    ...
    for (int i = 0; i < view.length; i++) {
      ...
      final double y = mid + view[i] * mid * DsLiveWaveform.amplitude;
      if (i == 0) {
        path.moveTo(x, y);
      } else {
        path.lineTo(x, y);
...
class DsBarVisualizer extends StatefulWidget {
  ...
  @override
  State<DsBarVisualizer> createState() => _DsBarVisualizerState();
}

class _DsBarVisualizerState extends State<DsBarVisualizer>
  ...
  @override
  void didUpdateWidget(DsBarVisualizer old) {
  ...
  /// signal or [DsBarVisualizer.active] can move the bars produces the same
  ...
  void _play() {
    ...
    final bool wants =
        !_stilled && (widget.active || widget.spectrum != null);
...
class _BarsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double barWidth =
        (w - DsBarVisualizer.gap * (bars - 1)) / bars;

    final List<double> levels = _levels();
    final Paint paint = Paint()..color = ink;

    for (int i = 0; i < bars; i++) {
      smoothed[i] = smoothed[i] * DsBarVisualizer.smoothingKeep +
          levels[i] * DsBarVisualizer.smoothingTake;
      final double value = math.max(DsBarVisualizer.floor, smoothed[i]);
      final double barHeight = value * h;
      final double x = i * (barWidth + DsBarVisualizer.gap);
  ...
  List<double> _levels() {
    ...
    if (view != null && view.isNotEmpty) {
      ...
      return <double>[
        for (int i = 0; i < bars; i++)
          () {
            final int from = (count * math.pow(i / bars, DsBarVisualizer.bandExponent))
                .floor();
            final int to = math.max(
              from + 1,
              (count * math.pow((i + 1) / bars, DsBarVisualizer.bandExponent))
                  .floor(),
            );
            double sum = 0;
            ...
            return math.min(
              1,
              sum / (to - from) / DsBarVisualizer.bandDivisor,
            ).toDouble();
          }(),
      ];
    }
    if (active) {
      ...
      return <double>[
        for (int i = 0; i < bars; i++)
          math.max(
            DsBarVisualizer.activeFloor,
            (math.sin(t * DsBarVisualizer.activeRate +
                        i * DsBarVisualizer.activePhase))
                    .abs() *
                DsBarVisualizer.activeScale *
                (1 - (i / bars - 0.5).abs()),
          ),
      ];
    }
...
/// is documented here and not built — the [DsMicControl] surface is exactly the
/// surface the reference page exercises. The precedent is ruling S2's dormant
/// `data-vertical` branch: recorded, not built.
class DsMicControl extends StatelessWidget {
  const DsMicControl({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        color: listening
            ? theme.agent.withValues(alpha: liveFillAlpha)
            : null,
        border: Border.all(
          color: listening
...
class _MicButtonState extends State<_MicButton>
  ...
  late final AnimationController _pulse = AnimationController(
    vsync: this,
    duration: DsDurations.pulseLive,
  );
  ...
  void _sync() {
    _pulse.duration = dsAnimationDuration(context, DsDurations.pulseLive);
  ...
  Widget build(BuildContext context) {
    ...
    final Widget button = DsButton(
      ...
      child: const DsIcon.lucide(DsLucide.mic, sizePx: _glyphPx),
    );

    if (!widget.listening) return button;

    // `text-agent` while live. [DsIconTone.inherit] is `text-current`, so the
    ...
    return AnimatedBuilder(
      ...
      builder: (BuildContext context, Widget? child) {
        ...
        final double eased = DsCurves.inOut.transform(leg.clamp(0.0, 1.0));
        return Opacity(
          opacity: 1 - eased * _pulseOpacityDrop,
          child: DecoratedBox(
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(DsRadii.pill),
              boxShadow: DsShadows.pulseLiveRing(eased).outerShadows(theme),
            ),
            child: child,
          ),
        );
  ...
  /// `className="size-4"` on the icon, against [DsButtonSize.iconSm]'s 14.
