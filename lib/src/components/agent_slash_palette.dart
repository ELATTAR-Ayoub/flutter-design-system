...
class DsAgentCommand {
  ...
  final String label;
  final String? hint;
  final DsAgentCommandGroup group;
  ...
  ///
  /// A [DsLucideGlyph] rather than the icons page's curated [DsIconGlyph]: the
...
/// Escape fail to close the palette — see [dsSlashQuery] and the composer's
/// drift register.
...
String? dsSlashQuery(String value, int caret) {
...
List<DsAgentCommand> dsFilterCommands(
  List<DsAgentCommand> commands,
  String query,
) {
  final String q = query.trim().toLowerCase();
  if (q.isEmpty) return commands;
  return commands
      .where((DsAgentCommand c) =>
...
class _Group {
  ...
  final String title;
  final List<DsAgentCommand> items;
}
...
/// down to their bugs (see [DsAgentSlashPalette.scrollsGroupsNotRows]).
class DsAgentSlashPalette extends StatelessWidget {
  const DsAgentSlashPalette({
    super.key,
  ...
  /// Already filtered — the composer runs [dsFilterCommands].
  final List<DsAgentCommand> commands;

  final int activeIndex;
  final ValueChanged<DsAgentCommand> onSelect;
  final ValueChanged<int> onHover;

  /// `max-h-64`.
  static double get maxHeight => ds(64);

  /// `mb-2` — the gap between the palette's bottom edge and the composer's top.
  static double get bottomGap => ds(2);
  ...
  static Duration get entrance => DsDurations.slow;
  ...
  static EdgeInsets get headingInsets =>
      EdgeInsets.fromLTRB(ds(3), ds(3), ds(3), ds(1));
  ...
  static double get lucideStroke => DsIcon.strokeFor(DsIconPaths.viewBox);
  ...
  List<_Group> get _groups => <_Group>[
        _Group(
          ...
          commands
              .where((DsAgentCommand c) =>
                  c.group == DsAgentCommandGroup.skill)
              .toList(),
        ),
        _Group(
          'Commands',
          commands
              .where((DsAgentCommand c) =>
                  c.group == DsAgentCommandGroup.command)
              .toList(),
        ),
...
class _PaletteBoxState extends State<_PaletteBox> {
  ...
  /// [DsAgentSlashPalette.scrollsGroupsNotRows].
  ...
  Widget build(BuildContext context) {
      ...
      rows.add(
        KeyedSubtree(
          ...
          child: Column(
            ...
            children: <Widget>[
              Padding(
                padding: DsAgentSlashPalette.headingInsets,
                child: DsText(
                  group.title,
                  DsType.caption,
              ...
              for (final DsAgentCommand command in group.items)
                Builder(
    ...
    final Widget box = DecoratedBox(
      decoration: BoxDecoration(
        ...
        boxShadow: DsShadows.e3.outerShadows(theme),
      ),
      child: ClipRRect(
        ...
        child: Padding(
          ...
          child: ConstrainedBox(
            constraints: BoxConstraints(
              maxHeight:
                  DsAgentSlashPalette.maxHeight - DsWidths.hairline * 2,
            ),
    ...
    return TweenAnimationBuilder<double>(
      ...
      duration:
          dsAnimationDuration(context, DsAgentSlashPalette.entrance),
      curve: DsCurves.out,
      builder: (BuildContext context, double t, Widget? child) => Opacity(
        opacity: t,
        child: Transform.translate(
          offset: Offset(0, DsAgentSlashPalette.rise * (1 - t)),
          child: child,
        ),
      ),
      child: box,
...
class _PaletteRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final DsThemeData theme = DsTheme.of(context);
    final DsLucideGlyph? glyph = command.icon;
      ...
      child: MouseRegion(
        ...
        child: Listener(
          ...
          child: ColoredBox(
            ...
            color: active ? theme.accent : dsTransparent,
            child: Padding(
              padding: DsAgentSlashPalette.rowInsets,
              child: Row(
                // `items-start`.
                  ...
                  if (glyph != null) ...<Widget>[
                    Padding(
                      padding: EdgeInsets.only(
                        top: DsAgentSlashPalette.glyphTopInset,
                      ),
                      // `text-agent` — a colour utility on the glyph, which
                      // `DsIconTone.inherit` reads off the ambient text style
                      // exactly as `text-current` does.
                      child: DefaultTextStyle.merge(
                        style: TextStyle(color: theme.agent),
                        child: DsIcon.lucide(
                          glyph,
                          sizePx: DsAgentSlashPalette.glyphSize,
                          strokeOverride: DsAgentSlashPalette.lucideStroke,
                        ),
                      ),
                    ),
                    SizedBox(width: DsAgentSlashPalette.rowGap),
                  ],
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        if (command.hint != null) ...<Widget>[
                          SizedBox(height: DsAgentSlashPalette.lineGap),
                          DsText(
                            command.hint!,
                            DsType.caption,
                            color: theme.mutedForeground,
