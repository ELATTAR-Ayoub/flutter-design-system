...
/// the placement comes from [dsPopoverPlacement] and the paint from
/// [DsPopoverSurface] — the same two objects the combobox and the date picker
/// use. What it cannot borrow is [DsPopover] itself: that widget lays a
...
class DsHoverCard extends StatefulWidget {
  const DsHoverCard({
    super.key,
    required this.trigger,
    required this.content,
    this.width,
    this.openDelay = DsDurations.hoverCardOpenDelay,
    this.closeDelay = DsDurations.hoverCardCloseDelay,
  });

  /// `HoverCardTrigger asChild`.
  final Widget trigger;

  /// `HoverCardContent`'s children — laid out under [DsHoverCardContent]'s
  /// `p-2.5`.
  ...
  static double get defaultWidth => ds(72);
  ...
  @override
  State<DsHoverCard> createState() => _DsHoverCardState();
}

class _DsHoverCardState extends State<DsHoverCard>
    with SingleTickerProviderStateMixin {
  final OverlayPortalController _portal = OverlayPortalController();
  final GlobalKey _anchorKey = GlobalKey();

  /// Built in [initState], never lazily — see `DsPopover`'s note.
      ...
      child: CustomSingleChildLayout(
        ...
        child: MouseRegion(
          ...
          child: _HoverCardTransition(
            ...
            child: SizedBox(
              width: widget.width ?? DsHoverCard.defaultWidth,
              child: DsHoverCardContent(child: widget.content),
            ),
          ),
        ),
      ),
...
class _HoverCardLayout extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    final DsPopoverPlacement placement = dsPopoverPlacement(
      ...
      content: childSize,
      viewport: size,
      sideOffset: DsHoverCard.sideOffset,
    );
    onPlaced(placement);
...
class DsHoverCardContent extends StatelessWidget {
  const DsHoverCardContent({super.key, required this.child});

  final Widget child;

  /// `p-2.5`.
  static double get padding => ds(2.5);
        ...
        builder: (BuildContext context, Widget? child) {
          ...
          return Opacity(
            ...
            child: Transform.translate(
              offset: Offset(0, entering ? -DsHoverCard.slide * (1 - t) : 0),
