...
///   from [DsThemeKind] against [DsPalette] (ruling S3's precedent: raw-palette
///   access is this port's pattern where a token is not on [DsThemeData]).
///
...
///   [DsVoiceOrb.level], a plain listenable, for the reason in the divergence
///   note below.
///
...
///     [DsVoiceOrb.level] is that number. Everything downstream of it, from the
...
class DsOrbProgram {
  DsOrbProgram._(this.program, this.noise);
  ...
  static Future<DsOrbProgram?>? _pending;
  static DsOrbProgram? _loaded;
  ...
  static DsOrbProgram? get loaded => _loaded;

  /// Compiles and decodes on first call; every later call gets the same future.
  static Future<DsOrbProgram?> load() {
    if (_pending != null) return _pending!;
    return _pending = _load();
  }

  static Future<DsOrbProgram?> _load() async {
    try {
      ...
      final ByteData bytes = await _firstOf(
        rootBundle.load,
        <String>[_noiseAsset, _bareNoiseAsset],
      );
      ...
      _lastError = null;
      return _loaded = DsOrbProgram._(program, frame.image);
    } catch (error, stack) {
      _lastError = error;
...
class DsVoiceOrb extends StatefulWidget {
  const DsVoiceOrb({
    super.key,
    this.state = DsOrbState.idle,
    this.level,
  ...
  static List<double> offsetsForSeed(int seed) =>
      _DsVoiceOrbState._makeOffsets(seed).toList();

  @override
  State<DsVoiceOrb> createState() => _DsVoiceOrbState();
}

class _DsVoiceOrbState extends State<DsVoiceOrb>
  ...
  bool _stilled = false;
  DsOrbProgram? _resources = DsOrbProgram.loaded;
  ...
  void initState() {
    super.initState();
    if (_resources == null) {
      DsOrbProgram.load().then((DsOrbProgram? loaded) {
  ...
  @override
  void didUpdateWidget(DsVoiceOrb old) {
  ...
  void didChangeDependencies() {
    ...
    // The same question [dsAnimationDuration] answers for every other looper in
  ...
  void _tick(Duration elapsed) {
    ...
    final bool listening = widget.state == DsOrbState.listening;
    ...
    final double targetSpeed = _speedFloor +
        (1 - math.pow(_curOut - 1, 2).toDouble()) * _speedRange;
  ...
  Widget build(BuildContext context) {
    ...
    final DsThemeKind kind = DsTheme.kindOf(context);
    ...
    return RepaintBoundary(
      child: CustomPaint(
        ...
        painter: _OrbPainter(
          ...
          from: from,
          to: to,
          inverted: kind == DsThemeKind.dark,
        ),
      ),
    );
  }
  ...
  /// uses."* Both stops are ramp members, so they read off [DsPalette] rather
  /// than adding two fields to [DsThemeData] that nothing else would consume.
  static (Color, Color) _orbStops(DsThemeKind kind) => switch (kind) {
        DsThemeKind.light => (DsPalette.action, DsPalette.actionDark),
        DsThemeKind.dark => (DsPalette.actionBright, DsPalette.action),
      };
...
class _OrbPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final double side = size.shortestSide * DsVoiceOrb.discFraction;
    if (side <= 0) return;
