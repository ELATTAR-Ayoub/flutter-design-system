...
///                                     var(--page-glow) 0%, transparent 64%);
...
//
// allow-hardcoded: body page-glow gradient geometry (globals.css L985–989)
...
class DsPageGlow extends StatelessWidget {
  const DsPageGlow({super.key, this.child});
...
class _PageGlowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Rect ellipse = DsPageGlow.debugEllipse(size);
    canvas.drawRect(
      box,
      Paint()
  ...
  /// stop contributes no colour — so the ramp stays `--page-glow` all the way
  ...
  static Float64List _ellipseMatrix(Rect ellipse) => Float64List.fromList(
        <double>[
