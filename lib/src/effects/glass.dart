...
///
/// **That failure mode does not reproduce in Flutter.** [DsMachineSurface]
/// takes one [DsShadowSpec] and paints every layer of it, so a caller passing
...
/// nothing here clips [DsGlassPanel.child]. The two clips in this file are
...
Color _panelRing(DsThemeData t) =>
    DsOklab.mix(t.foreground, dsTransparent, _panelRimAlpha);

Color _controlRing(DsThemeData t) =>
    DsOklab.mix(t.foreground, dsTransparent, _controlRimAlpha);
...
/// twice sigma and which [DsShadowLayer.blurRadius] converts. So `--blur-xl`
...
final ui.ImageFilter _glassBackdrop = ui.ImageFilter.compose(
  outer: dsSaturate(_panelSaturation),
  inner: ui.ImageFilter.blur(sigmaX: DsBlurs.xl, sigmaY: DsBlurs.xl),
);
...
const DsShadowSpec _controlSpec = DsShadowSpec(<DsShadowLayer>[
  _topHighlight,
  _controlRingLayer,
]);
...
double _shadowReach(DsShadowLayer layer) =>
...
Path _outsideShape(RRect shape, List<DsShadowLayer> ambient) {
  double reach = 0;
  for (final DsShadowLayer layer in ambient) {
...
///
/// So the ambient is lifted out of [DsMachineSurface] and painted here, under a
...
class _AmbientShadowPainter extends CustomPainter {
  ...
  final List<DsShadowLayer> ambient;
...
///
/// **Why that is a [Stack] and not one [DsMachineSurface].** Two of those
/// layers cannot go where `DsMachineSurface` puts them. The blurred backdrop
...
/// `DsMachineSurface` is exactly what only it can do: the inset layers, over
/// the content, in CSS's order.
class _DsGlassSurface extends StatelessWidget {
  const _DsGlassSurface({
    required this.spec,
    required this.fill,
  ...
  /// Resolved against the live theme, like every [DsShadowLayer] colour.
  final Color Function(DsThemeData) fill;
  ...
  Widget build(BuildContext context) {
    ...
    if (filter == null) {
      ...
      // also declares no outer shadow — so one `DsMachineSurface` is the whole
      ...
      assert(
        spec.layers.every((DsShadowLayer l) => l.inset),
    ...
    final List<DsShadowLayer> ambient = <DsShadowLayer>[
      for (final DsShadowLayer layer in spec.layers)
    ...
    return Stack(
      ...
      children: <Widget>[
        ...
        // `DsMachineSurface` can paint. It carries no fill and no outer layers
        // here; both are already on the canvas beneath it.
        DsMachineSurface(
          spec: DsShadowSpec(spec.insetLayers),
          radius: radius,
          child: content,
        ),
      ],
...
class DsGlassPanel extends StatelessWidget {
  ...
  ///
  /// Exposed for the same reason as [DsMachineSurface.debugInsetRing]: this is
...
///
/// Byte-identical to [DsGlassPanel] except the ambient layer is
...
class DsGlassPanelDeep extends StatelessWidget {
  const DsGlassPanelDeep({
    super.key,
    required this.radius,
    this.padding,
    required this.child,
  ...
  /// [DsGlassPanel.debugShadow] with `--shadow-e4`'s layers in place of
  /// `--shadow-e2`'s. The first two layers are the same objects.
  @visibleForTesting
  static DsShadowSpec get debugShadow => _panelDeepSpec;

  /// See [DsGlassPanel.debugAmbientClip] — the same clip, a deeper ambient.
  @visibleForTesting
  static Path debugAmbientClip(RRect shape, List<DsShadowLayer> ambient) =>
      _outsideShape(shape, ambient);

  /// The composed `blur(24px) saturate(1.5)`, shared with [DsGlassPanel].
  @visibleForTesting
  static ui.ImageFilter? get debugBackdrop => _glassBackdrop;
...
/// [DsMachineSurface] and nothing else.
class DsGlassControl extends StatelessWidget {
  const DsGlassControl({
    super.key,
    required this.radius,
    this.padding,
    required this.child,
  ...
  @visibleForTesting
  static DsShadowSpec get debugShadow => _controlSpec;
  ...
  @visibleForTesting
  static Color debugFill(DsThemeData theme) => _controlFill(theme);
