...
void main() {
  testWidgets(
    'enabled renders the child at full opacity and lets taps through',
  ...
  testWidgets('disabled fades to the token and blocks the child', (
    WidgetTester t,
  ) async {
    int taps = 0;
    await t.pumpWidget(
  ...
  testWidgets('disabled with blockPointer false keeps the child hittable', (
    WidgetTester t,
  ) async {
    int taps = 0;
    await t.pumpWidget(
  ...
  testWidgets('onDisabledTap fires only while disabled', (
    WidgetTester t,
  ) async {
    int hits = 0;
    Widget build(bool disabled) => _host(
      Disabled(
        disabled: disabled,
        onDisabledTap: () => hits++,
        child: const SizedBox(width: 80, height: 40),
  ...
  testWidgets('no reason means no Tooltip in the tree', (WidgetTester t) async {
    await t.pumpWidget(
      _host(
        const Disabled(disabled: true, child: SizedBox(width: 80, height: 40)),
      ),
    );
  ...
  testWidgets('a reason shows on hover while disabled', (WidgetTester t) async {
    ...
    await t.pump(
      MotionDurations.tooltipShowDelay + MotionDurations.overlayEnter,
    );
    await t.pumpAndSettle();
    expect(find.text('Finish the form first'), findsOneWidget);
  });

  testWidgets('a reason shows on touch tap while disabled', (
    WidgetTester t,
  ) async {
    await t.pumpWidget(
      _host(
    ...
    final TestGesture finger = await t.createGesture(
  ...
  testWidgets('a reason on an enabled control shows nothing', (
    WidgetTester t,
  ) async {
    await t.pumpWidget(
      _host(
  ...
  testWidgets('toggling disabled keeps the child State alive', (
    WidgetTester t,
  ) async {
    Widget build(bool disabled) =>
        _host(Disabled(disabled: disabled, child: const _CounterChild()));

    await t.pumpWidget(build(false));
    final _CounterChildState state = t.state<_CounterChildState>(
      find.byType(_CounterChild),
    );
    state.bump();
    ...
    await t.pumpWidget(build(true));
    await t.pumpAndSettle();
    expect(t.state<_CounterChildState>(find.byType(_CounterChild)).count, 1);
