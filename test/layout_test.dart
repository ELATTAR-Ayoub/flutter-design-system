...
void main() {
      ...
      (WidgetTester tester) async {
        for (final (double ratio, double height) pair in <(double, double)>[
          ...
          await _pump(
            ...
            SizedBox(
              width: 327.33,
              child: DsAspectRatio(ratio: pair.$1, child: const SizedBox()),
            ),
          );
          final RenderBox box = tester.renderObject<RenderBox>(
            find.byType(DsAspectRatio),
          );
  ...
  group('DsScrollArea', () {
    ...
    testWidgets('there is no rail at rest, one on hover, and none again '
      ...
      await _pump(
        tester,
        SizedBox(width: 480, height: 254, child: DsScrollArea(child: rows(14))),
      );
      Finder thumb() => find.descendant(
        of: find.byType(DsScrollArea),
      ...
      final TestGesture gesture = await _hover(
        tester,
        tester.getCenter(find.byType(DsScrollArea)),
      );
    ...
    testWidgets('re-entering inside the delay cancels the pending hide', (
      ...
      await gesture.moveTo(tester.getCenter(find.byType(DsScrollArea)));
    ...
    testWidgets('the rail is a 10px lane with a 7px thumb, sized by the '
      ...
      await _pump(
        ...
        SizedBox(
          width: 480,
          height: 254,
          child: DsScrollArea(child: SizedBox(height: 622, child: rows(1))),
        ),
      );
      await _hover(tester, tester.getCenter(find.byType(DsScrollArea)));
      ...
      expect(thumbBox.size.width, closeTo(ds(2.5) - ds(0.75), 0.01));
      ...
      final double right = thumbBox
          ...
          .dx;
      expect(right, closeTo(area.size.width - DsWidths.hairline, 0.01));
    });
  ...
  group('DsCarouselController', () {
    DsCarouselController page() => DsCarouselController()
      ..setMetrics(
        viewSize: 1046,
    ...
    test('trimSnaps collapses the page\'s six slides into four stops', () {
      final DsCarouselController c = page();
  ...
  group('DsResizablePanelGroup', () {
    ...
    testWidgets('the keyboard steps 5 points, Home floors and End maxes', (
      ...
      final FocusNode node = tester.binding.focusManager.rootScope.descendants
          .firstWhere(
            (FocusNode n) => n.debugLabel == dsResizableHandleFocusLabel,
          );
      node.requestFocus();
      await tester.pump();
