...
/// `DsSlider` — the fourth control family, and the first genuinely new painter
/// since phase 3.
///
...
double get _travel => _panel - DsSlider.thumbSize;

/// The track's content box: the root minus its 1px border on each side.
double get _content => _panel - 2 * DsWidths.hairline;
...
/// Every [DsMachineSurface] the slider paints, in tree order: the track, then
/// the range inside it, then one per thumb.
List<DsMachineSurface> surfaces(WidgetTester t) => t
...
Rect surfaceRect(WidgetTester t, int index) {
  ...
  final RenderBox root = t.renderObject<RenderBox>(find.byType(DsSlider));
...
/// `DsShadowSpec` claims and what lands on the canvas are two different
...
List<FocusNode> thumbNodes(WidgetTester t) => t
    .widgetList<Focus>(
      find.descendant(of: find.byType(DsSlider), matching: find.byType(Focus)),
    )
...
Offset thumbCentre(WidgetTester t, int index) =>
    t.getTopLeft(find.byType(DsSlider)) +
    surfaceRect(t, 2 + index).center;
...
void main() {
  ...
  group('geometry', () {
    testWidgets('the root IS the track: 10px, and the knobs overflow it',
      ...
      final Size root = t.getSize(find.byType(DsSlider));
      expect(root.height, DsSlider.trackHeight,
      ...
      expect(surfaceRect(t, 0).height, DsSlider.trackHeight,
          reason: 'track `grow` + `data-horizontal:h-2.5`');
      final Rect thumb = surfaceRect(t, 2);
      expect(thumb.size, Size(DsSlider.thumbSize, DsSlider.thumbSize));
      expect(thumb.top, closeTo(-(DsSlider.thumbSize - DsSlider.trackHeight) / 2, 0.01),
    ...
    testWidgets('the range is a percentage of the TRACK CONTENT box',
      ...
      expect(range.left - DsWidths.hairline, closeTo(10 / 500 * _content, _tolerance),
          reason: 'left = 10/500 x 446 = 8.92, probed 8.90625');
      expect(range.width, closeTo(230 / 500 * _content, _tolerance),
          reason: 'width = 230/500 x 446 = 205.16, probed 205.19');
      expect(range.height, DsSlider.trackHeight - 2 * DsWidths.hairline,
    ...
    testWidgets('a single value fills from the start of the channel',
      ...
      final Rect range = surfaceRect(t, 1);
      expect(range.left, closeTo(DsWidths.hairline, _tolerance),
          reason: 'one thumb means the fill starts at the minimum');
      expect(range.width, closeTo(25 / 100 * _content, _tolerance),
    ...
    testWidgets('the thumb travels the ROOT less its own width',
      ...
      for (final (List<double> values, double width, double max, int index,
              double want) probe
        ...
        await t.pumpWidget(host(slider(
          values: probe.$1,
          width: probe.$2,
          max: probe.$3,
          step: 5,
        ...
        expect(thumb.left, closeTo(probe.$5, _tolerance),
    ...
    testWidgets('the two spaces disagree everywhere but the midpoint',
      ...
      expect(
        ...
        closeTo(surfaceRect(t, 2).left + DsSlider.thumbSize / 2, _tolerance),
        reason: 'at 50% the fill ends exactly under the knob\'s centre',
      );

      await t.pumpWidget(host(slider(values: <double>[25])));
      final double fillEnd = surfaceRect(t, 1).right;
      final double knobCentre = surfaceRect(t, 2).left + DsSlider.thumbSize / 2;
      expect((fillEnd - knobCentre).abs(), greaterThan(_tolerance),
  ...
  group('rasterised', () {
    for (final DsThemeMode mode in <DsThemeMode>[
      ...
      final DsThemeData theme =
          mode == DsThemeMode.light ? DsThemeData.light : DsThemeData.dark;
      ...
      testWidgets('$mode: the channel is a sunken socket, not a flat fill',
        ...
        expect(column.length, DsSlider.trackHeight.round());
        ...
        expect(_luma(column[2]), lessThan(_luma(column[7])),
            reason: '$mode: the sunken band is at the top, as a positive dy '
                'puts it — an inverted socket paints the complement of this');
        expect(_nearest(column[7], <Color>[theme.muted, theme.card]),
            theme.muted,
  ...
  group('the raised/recessed pair', () {
    testWidgets('sunken track, lit fill, raised knob', (WidgetTester t) async {
      await t.pumpWidget(host(slider(values: <double>[40])));
      final DsThemeData theme = DsTheme.of(t.element(find.byType(DsSlider)));
      final List<DsMachineSurface> painted = surfaces(t);

      expect(painted[0].spec, DsShadows.pressed, reason: 'track `shadow-pressed`');
      expect(painted[0].fill, theme.muted);
      expect(borderOf(painted[0]), theme.input, reason: '`border-input`');

      expect(painted[1].spec, DsShadows.btnPrimary,
          reason: 'range `shadow-btn-primary` — the blue glow');
      expect(painted[1].fill, theme.actionInk);
      expect(painted[1].radius, BorderRadius.zero,
          reason: 'probed `border-radius: 0px`; the channel clips its ends');

      expect(painted[2].fill, theme.foreground, reason: 'thumb `bg-foreground`');
      expect(borderOf(painted[2]), theme.input);
      expect(painted[2].spec.layers.length, DsShadows.btn.layers.length + 1,
  ...
  group('the ring springs and the scale snaps', () {
    testWidgets('a focused thumb prepends ring-3 at half alpha',
      ...
      expect(ringOf(surfaces(t)[2], theme).a, 0,
          reason: 'at rest the ring is its own hue at zero alpha');

      await focusThumb(t, 0);
      expect(thumbNodes(t)[0].hasFocus, isTrue,
          reason: 'the thumb has to actually hold focus for this to mean '
              'anything');
      await t.pump(DsDurations.transitionDefault);

      final Color ring = ringOf(surfaces(t)[2], theme);
      expect(ring.a, closeTo(0.5, 0.01),
          reason: 'probed `oklab(... / 0.5) 0px 0px 0px 3px` on the focused '
              'thumb, in front of all four --shadow-btn layers');
      expect(ring.r, closeTo(theme.ring.r, 0.01), reason: '`ring-ring/50`');
    });

    testWidgets('the ring TWEENS — it is in transition-[transform,box-shadow]',
        (WidgetTester t) async {
      await t.pumpWidget(host(slider(values: <double>[40])));
      final DsThemeData theme = DsTheme.of(t.element(find.byType(DsSlider)));
      ...
      await t.pump(DsDurations.transitionDefault ~/ 8);
      ...
      await t.pump(DsDurations.transitionDefault ~/ 2 -
          DsDurations.transitionDefault ~/ 8);
      expect(ringOf(surfaces(t)[2], theme).a, greaterThan(0.5),
          reason: 'the spring overshoots its target before settling back');

      await t.pump(DsDurations.transitionDefault);
      expect(ringOf(surfaces(t)[2], theme).a, closeTo(0.5, 0.01),
  ...
  group('pointer', () {
    testWidgets('a press maps against the ROOT width, not the thumb travel',
      ...
      final Offset origin = t.getTopLeft(find.byType(DsSlider));
      ...
      await t.pump();
      expect(valuesOf(t).single, 29,
          reason: 'the pointer maps against the root; the knob renders '
              'against the travel — the same disagreement as the fill');
    });

    testWidgets('a track press grabs the NEAREST thumb', (WidgetTester t) async {
      await t.pumpWidget(host(
        const _Live(initial: <double>[10, 240], max: 500, step: 5),
      ));
      final Offset origin = t.getTopLeft(find.byType(DsSlider));

      // Probed: [10, 240], clicked 30% of the way between the knobs ->
      // [85, 240]. The low thumb moves; the high one is untouched.
      final double lowCentre = 10 / 500 * _travel + DsSlider.thumbSize / 2;
      final double highCentre = 240 / 500 * _travel + DsSlider.thumbSize / 2;
      await t.tapAt(
    ...
    testWidgets('a drag tracks the pointer with no tween',
        (WidgetTester t) async {
      await t.pumpWidget(host(const _Live(initial: <double>[25])));
      final Offset start = t.getTopLeft(find.byType(DsSlider)) +
          Offset(25 / 100 * _travel + DsSlider.thumbSize / 2, 5);
      ...
      await t.pump(DsDurations.transitionDefault);
      expect(valuesOf(t).single, afterOneFrame,
    ...
    testWidgets('thumbs stop at each other and may coincide',
      ...
      await t.pumpWidget(host(
        const _Live(initial: <double>[0, 500], max: 500, step: 5),
      ));
      final Offset origin = t.getTopLeft(find.byType(DsSlider));

      final TestGesture gesture =
          await t.startGesture(origin + Offset(DsSlider.thumbSize / 2, 5));
      await t.pump();
      await gesture.moveTo(origin + const Offset(_panel + 60, 5));
      await t.pump();
      expect(valuesOf(t), <double>[500, 500],
    ...
    testWidgets('every emitted value is a whole number of steps',
      ...
      final Offset origin = t.getTopLeft(find.byType(DsSlider));
      final TestGesture gesture =
          await t.startGesture(origin + const Offset(20, 5));
      for (int i = 1; i <= 12; i++) {
        await gesture.moveBy(const Offset(9, 0));
        await t.pump();
        expect(valuesOf(t)[0] % 5, 0,
  ...
  group('keyboard', () {
    ...
    testWidgets('each thumb owns its own focus and its own keys',
      ...
      await t.sendKeyEvent(LogicalKeyboardKey.arrowRight);
      await t.pump();
      expect(valuesOf(t), <double>[21, 71], reason: 'and the second is its own');
    });
  });
  ...
  group('disabled', () {
    testWidgets('the root dims once and the knob does not dim again',
        (WidgetTester t) async {
      await t.pumpWidget(host(
        slider(values: <double>[40], enabled: false, live: false),
      ));
    ...
    testWidgets('a press moves nothing', (WidgetTester t) async {
      ...
      await t.pumpWidget(host(
        const _Live(initial: <double>[40], enabled: false),
      ));
      await t.tapAt(t.getTopLeft(find.byType(DsSlider)) +
          const Offset(_panel * 0.9, 5));
      await t.pump();
    ...
    testWidgets('a null onChanged is inoperable too', (WidgetTester t) async {
      ...
      final Opacity dimming = t.widget<Opacity>(find.descendant(
        of: find.byType(DsSlider),
        matching: find.byType(Opacity),
      ));
  ...
  group('contract', () {
    ...
    testWidgets('it does not jelly', (WidgetTester t) async {
      ...
      expect(
        find.descendant(
          of: find.byType(DsSlider),
          matching: find.byType(DsJellyReplay),
        ),
        findsNothing,
      );
    });
    ...
    testWidgets('each thumb announces itself as a slider',
      ...
      await t.pumpWidget(host(
        slider(values: <double>[20, 70], label: 'Price range'),
      ));
      expect(
        find.bySemanticsLabel('Price range'),
        findsNWidgets(2),
        reason: 'Radix puts role="slider" on each thumb, so a range '
