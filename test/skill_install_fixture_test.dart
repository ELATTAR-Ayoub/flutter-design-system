...
String sha256Hex(List<int> message) {
  ...
  final ByteData lengthBytes = ByteData(8)
    ..setUint64(0, bitLength, Endian.big);
...
void main() {
  ...
  final Directory sourceDir =
      Directory('${_repoRoot.path}/skills/$_skillName');
    ...
    test(
      ...
      () {
        ...
        final Directory fixtureRoot =
            Directory.systemTemp.createTempSync('elattar-skill-install-');
        addTearDown(() => fixtureRoot.deleteSync(recursive: true));

        final Directory claudeSkillsDir =
            Directory('${fixtureRoot.path}/.claude/skills')
              ..createSync(recursive: true);

        final Directory siblingSkillDir =
            Directory('${claudeSkillsDir.path}/some-other-skill')
              ..createSync(recursive: true);
        ...
        final Directory installedDir =
            Directory('${claudeSkillsDir.path}/$_skillName');
        ...
        expect(installedFiles, equals(publishedFiles),
            reason: 'the installed tree\'s file list diverges from '
        ...
        for (final String relative in publishedFiles) {
          final String sourceHash =
              sha256Hex(File('${sourceDir.path}/$relative').readAsBytesSync());
          final String installedHash = sha256Hex(
              File('${installedDir.path}/$relative').readAsBytesSync());
          expect(installedHash, equals(sourceHash),
              reason: '$relative differs (sha256 mismatch) between '
                  'skills/$_skillName and the installed copy');
        }

        // Install must not have disturbed the sibling skill.
        expect(siblingFile.readAsStringSync(), siblingContent,
            reason: 'installing $_skillName altered an unrelated, '
        ...
        expect(claudeSkillsDir.existsSync(), isTrue,
            reason: 'removal deleted ~/.claude/skills/ itself, not just '
        ...
        final List<String> remainingEntries = claudeSkillsDir
            .listSync()
            .map((FileSystemEntity e) =>
                e.uri.pathSegments.where((String s) => s.isNotEmpty).last)
            .toList()
          ..sort();
        expect(remainingEntries, equals(<String>['some-other-skill']),
            reason: 'removal left orphaned entries under '
