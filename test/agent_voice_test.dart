/// The voice family — `DsVoiceOrb`, `DsLiveWaveform`, `DsBarVisualizer`,
/// `DsMicControl`.
///
...
Future<void> _pumpOrb(
  WidgetTester tester, {
  DsOrbState state = DsOrbState.idle,
  bool reduced = true,
...
void main() {
  ...
  group('shaders/orb.frag', () {
    test('the example app copy has not drifted', () {
      ...
      expect(app.existsSync(), isTrue,
          reason: 'example/shaders/orb.frag is the gallery copy — without '
              'it the orb silently paints nothing in a web build');
      expect(app.readAsBytesSync(), package.readAsBytesSync(),
    ...
    test('it carries no operator SkSL rejects', () {
      ...
      expect(code.where((String line) => line.contains(' % ')), isEmpty,
  ...
  group('splitmix32', () {
    test('reproduces the browser sequence bit for bit', () {
      ...
      final List<double> actual = DsVoiceOrb.offsetsForSeed(_seed);
    ...
    test('every draw lands inside one full turn', () {
      for (final int seed in <int>[0, 1, 0xFFFFFFFF, 987654321]) {
        for (final double offset in DsVoiceOrb.offsetsForSeed(seed)) {
    ...
    test('a seed is a seed — the same one twice gives the same phases', () {
      expect(DsVoiceOrb.offsetsForSeed(7), DsVoiceOrb.offsetsForSeed(7));
      expect(DsVoiceOrb.offsetsForSeed(7),
          isNot(equals(DsVoiceOrb.offsetsForSeed(8))));
    });
  });

  /* ── The disc ──────────────────────────────────────────────────────────── */

  group('DsVoiceOrb geometry', () {
    test('the disc fraction is the measured one', () {
      ...
      expect(DsVoiceOrb.discFraction, closeTo(7 / 7.673269879789604, 1e-9));
      // And what the browser actually rendered: 102px of a 112px canvas.
      expect((DsVoiceOrb.discFraction * 112).round(), 102);
    });

    testWidgets('takes exactly the box it is given', (WidgetTester tester) async {
      ...
      expect(tester.getSize(find.byType(DsVoiceOrb)), const Size(112, 112));
    });
  ...
  group('DsVoiceOrb rendered pixels', () {
    setUp(DsOrbProgram.resetForTest);
    ...
    testWidgets('the shader asset is reachable and compiles',
      ...
      final DsOrbProgram? loaded =
          await tester.runAsync<DsOrbProgram?>(() => DsOrbProgram.load());
      _drainStageError(tester);
      expect(loaded, isNotNull,
          reason: 'the bundle is present, so shaders/orb.frag failing to load '
    ...
    testWidgets('paints a disc: opaque at the centre, clear at the corners',
      ...
      final _Raster raster =
          await _raster(tester, find.byType(RepaintBoundary).last);
    ...
    testWidgets('the disc edge lands where the reference measured it',
      ...
      expect(last - first + 1, closeTo(102, 2),
    ...
    testWidgets('dark inverts the ramp rather than recolouring it',
      ...
      Future<_Raster> render(DsThemeMode mode) async {
        await tester.pumpWidget(
          DsTheme(
            ...
            child: const MediaQuery(
              ...
              child: Directionality(
                textDirection: TextDirection.ltr,
                child: Center(
                  child: DsVoiceOrb(size: _orbSize, seed: _seed),
                ),
              ),
            ),
          ),
      ...
      final _Raster light = await render(DsThemeMode.light);
      final _Raster dark = await render(DsThemeMode.dark);
      const int mid = _orbSize ~/ 2;
  ...
  group('DsLiveWaveform', () {
    testWidgets('defaults are the component\'s own', (WidgetTester tester) async {
      expect(DsLiveWaveform.defaultWidth, 120);
      expect(DsLiveWaveform.defaultHeight, 28);
      expect(DsLiveWaveform.strokeWidth, 1.5);

      await tester.pumpWidget(_host(
        const DsLiveWaveform(width: 320, height: 48),
      ));
      expect(tester.getSize(find.byType(DsLiveWaveform)), const Size(320, 48));
    });

    testWidgets('no stream draws a flat line at the midpoint, and nothing else',
        (WidgetTester tester) async {
      await tester.pumpWidget(_host(
        const DsLiveWaveform(width: 320, height: 48),
      ));
      ...
      expect(raster.alphaAt(raster.width ~/ 2, inkRow), greaterThan(150),
          reason: 'the resting line is drawn');
      expect(raster.alphaAt(raster.width ~/ 2, inkRow - 1), greaterThan(150),
    ...
    testWidgets('a signal is drawn at 0.9 of the half height',
      ...
      await tester.pumpWidget(_host(
        DsLiveWaveform(samples: samples, width: 320, height: 48),
      ));
      final _Raster raster =
          await _raster(tester, find.byType(RepaintBoundary).last);
      ...
      expect(raster.alphaAt(160, 24), 0,
  ...
  group('DsBarVisualizer', () {
    test('the reference constants', () {
      expect(DsBarVisualizer.defaultBars, 12);
      expect(DsBarVisualizer.defaultWidth, 96);
      expect(DsBarVisualizer.defaultHeight, 24);
      expect(DsBarVisualizer.gap, 2);
      expect(DsBarVisualizer.floor, 0.06);
    ...
    testWidgets('at rest, twelve bars sit at the floor',
        (WidgetTester tester) async {
      await tester.pumpWidget(_host(const DsBarVisualizer()));
      final _Raster raster =
          await _raster(tester, find.byType(RepaintBoundary).last);
    ...
    testWidgets('active oscillates without an analyser; idle does not',
        (WidgetTester tester) async {
      await tester.pumpWidget(_host(const DsBarVisualizer(active: true)));
      await tester.pump(const Duration(milliseconds: 120));
      final _Raster moving =
          await _raster(tester, find.byType(RepaintBoundary).last);
      await tester.pump(const Duration(milliseconds: 120));
      final _Raster later =
          await _raster(tester, find.byType(RepaintBoundary).last);
      expect(moving.bytes, isNot(equals(later.bytes)),
      ...
      final _Raster stillLater =
          await _raster(tester, find.byType(RepaintBoundary).last);
      expect(still.bytes, equals(stillLater.bytes),
    ...
    testWidgets('reduced motion stops the oscillator',
        (WidgetTester tester) async {
      await tester.pumpWidget(_host(
        const DsBarVisualizer(active: true),
        reduced: true,
      ));
      final _Raster first =
          await _raster(tester, find.byType(RepaintBoundary).last);
      await tester.pump(const Duration(milliseconds: 400));
      final _Raster second =
          await _raster(tester, find.byType(RepaintBoundary).last);
  ...
  group('DsMicControl', () {
    testWidgets('one pill, 34 x 34, with a single half',
        (WidgetTester tester) async {
      await tester.pumpWidget(_host(
        DsMicControl(listening: false, onToggle: () {}),
      ));
      ...
      expect(tester.getSize(find.byType(DsMicControl)), const Size(34, 34));
      expect(find.byType(DsButton), findsOneWidget);
      expect(tester.getSize(find.byType(DsButton)), const Size(32, 32));
    });
    ...
    testWidgets('the accessible name is the verb, and it flips',
      ...
      expect(
        tester.widget<DsButton>(find.byType(DsButton)).label,
        'Dictate',
      );

      await tester.pumpWidget(_host(
        DsMicControl(listening: true, onToggle: () {}),
      ));
      expect(
        tester.widget<DsButton>(find.byType(DsButton)).label,
        'Stop dictation',
    ...
    testWidgets('going live tints the pill and nothing else changes size',
      ...
      expect(liveEdge[2], greaterThan(restEdge[2]),
          reason: 'the border goes agent-coloured');
    });

    testWidgets('a null handler leaves the control inert',
        (WidgetTester tester) async {
      await tester.pumpWidget(_host(const DsMicControl(listening: false)));
    ...
    testWidgets('disabled beats a handler', (WidgetTester tester) async {
      await tester.pumpWidget(_host(
        DsMicControl(listening: false, disabled: true, onToggle: () {}),
      ));
      expect(tester.widget<DsButton>(find.byType(DsButton)).onPressed, isNull);
    });
  });
...
Widget _host(Widget child, {bool reduced = false}) => DsTheme(
      controller: DsThemeController(mode: DsThemeMode.light),
      child: MediaQuery(
        data: MediaQueryData(disableAnimations: reduced),
        child: Directionality(
