...
/// One hand-transcribed element as `tag attr attr …`.
///
/// Deliberately the same shape as [_generatedSignature] so the two models can
/// be compared as strings — every attribute either appears in both or the
/// comparison fails.
String _handSignature(DsIconElement element) => switch (element) {
...
/// The same, for the generated model.
String _generatedSignature(DsLucideNode node) => switch (node) {
      DsLucidePath(:final String d) => 'path $d',
      DsLucideLine(
        :final double x1,
        :final double y1,
        :final double x2,
      ...
      DsLucidePolyline(:final List<Offset> points) =>
        'polyline ${_points(points)}',
      DsLucidePolygon(:final List<Offset> points) =>
...
void main() {
  ...
  group('the generated set reproduces the hand transcription', () {
    ...
    test('element for element, character for character', () {
      ...
      for (final DsIconGlyph glyph in DsIconGlyph.values) {
        ...
        final List<String> hand =
            DsIconPaths.elements[glyph]!.map(_handSignature).toList();
        final List<String> mechanical =
            generated.nodes.map(_generatedSignature).toList();
    ...
    test('the details the hand transcription was written to catch survive', () {
      String d(String module, int i) =>
          (dsLucideByName[module]!.nodes[i] as DsLucidePath).d;
      ...
      expect(_generatedSignature(dsLucideByName['circle-x']!.nodes[0]),
          _generatedSignature(dsLucideByName['circle-check']!.nodes[0]));
      ...
      final DsLucideCircle dot =
          dsLucideByName['tag']!.nodes[1] as DsLucideCircle;
  ...
  group('the whole package', () {
    ...
    test('save-off is an upstream defect, reproduced deliberately', () {
      ...
      // for the port is recorded here because it is real: `_GlyphPainter` in
      // `icon.dart` scales but does not clip, so this one glyph would paint
      // outside its own box and overlap whatever sits to its right. That is a
      // renderer decision (clip to the viewBox, as the browser does), not a
      // data decision, and it is noted in `tool/README.md`.
      final DsLucideGlyph glyph = DsLucide.saveOff;
      expect(glyph.nodes, hasLength(7));
      expect((glyph.nodes[5] as DsLucidePath).d, 'M29.5 11.5s5 5 4 5');
  ...
  group('primitives new to the generated set', () {
    test('ellipse is a real ellipse, not a circle', () {
      ...
      final DsLucideEllipse lid =
          dsLucideByName['database']!.nodes.first as DsLucideEllipse;
      ...
      final List<DsLucideEllipse> all = <DsLucideEllipse>[
        for (final DsLucideGlyph g in dsLucideByName.values)
          ...g.nodes.whereType<DsLucideEllipse>(),
      ];
      expect(all, hasLength(16));
      for (final DsLucideEllipse e in all) {
    ...
    test('polygon closes and polyline does not', () {
      ...
      final List<String> polygons = <String>[
        for (final MapEntry<String, DsLucideGlyph> e in dsLucideByName.entries)
          if (e.value.nodes.any((DsLucideNode n) => n is DsLucidePolygon))
            e.key,
      ];
      expect(polygons, unorderedEquals(<String>['navigation', 'navigation-2']));

      final Path closed = Path();
      (dsLucideByName['navigation']!.nodes.single as DsLucidePolygon)
          .addTo(closed);
      expect(closed.computeMetrics().single.isClosed, isTrue);

      final Path open = Path();
      (dsLucideByName['package']!.nodes[2] as DsLucidePolyline).addTo(open);
      ...
      final DsLucidePolyline mailbox = dsLucideByName['mailbox']!
          .nodes
          .whereType<DsLucidePolyline>()
          .single;
    ...
    test('a rect with no `rx` takes `ry`, and with neither is square', () {
      ...
      final DsLucideRect rounded = dsLucideByName['arrow-down-0-1']!
          .nodes
          .whereType<DsLucideRect>()
          .single;
      ...
      final DsLucideRect square = dsLucideByName['spray-can']!
          .nodes
          .whereType<DsLucideRect>()
          .single;
