...
void main() {
    ...
    for (final ColorMode mode in <ColorMode>[ColorMode.light, ColorMode.dark]) {
      ...
      testWidgets('$mode: the interior is the card token, the edges are inset', (
        ...
        for (int y = 1; y < 7; y++) {
          expect(
            _luma(column[y]),
            lessThanOrEqualTo(_luma(column[y + 1])),
            reason: '$mode: the falloff is monotonic at y=$y',
          );
        }
        expect(
          _luma(column[column.length - 2]),
          lessThan(_luma(theme.card)),
        ...
        expect(
          column[20],
          isNot(collapsed(theme)),
          reason:
              '$mode: the inset ring collapsed to a full fill — the '
  ...
  group('Form', () {
    ...
    testWidgets(
      'focus-on-error lands on the first invalid field — every type',
  ...
  group('InputGroup', () {
    ...
    EdgeInsetsDirectional controlInsets(WidgetTester t) =>
        t
                .widget<Padding>(
                  find
                      .descendant(
  ...
  group('Field', () {
    ...
    testWidgets('a horizontal field\'s control adopts it observably', (
      ...
      expect(
        ...
        same(node),
        reason:
            'the form\'s node has to land ON the control, or a failed '
    ...
    testWidgets('the visible label is not announced twice', (
      ...
      expect(
        ...
        'Email',
        reason:
            'once — a correct <label for> names the input, it is not '
            'read as a separate string beside it',
      );
      handle.dispose();
    });

    testWidgets('tapping the label focuses the control', (
      WidgetTester t,
    ) async {
      final FocusNode node = FocusNode();
      addTearDown(node.dispose);
    ...
    testWidgets('the three line-heights RENDER at three different boxes', (
      ...
      for (final MapEntry<String, Finder> part in parts.entries) {
        ...
        expect(
          rendered(part.value),
          closeTo(declared(part.value), 1e-3),
  ...
  group('FieldVisibility — a focused field is never behind the keyboard', () {
    ...
    testWidgets(
      'focusing one deep in a scroller lifts it clear of the keyboard',
      (WidgetTester t) async {
        final FocusNode node = FocusNode();
