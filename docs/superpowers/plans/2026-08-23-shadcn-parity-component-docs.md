...
- Tokens only: no `Colors.`, no `Color(`, no `TextStyle(`, no bare `Text(` for prose, no hardcoded numeric layout, no raw `Duration(`. `Text()` as a `Button` child is the established convention and is fine.
- `Type.label` uppercases its text. A `find.text` on a case-carrying identifier rendered in a label will fail. Use `Type.small` or `Type.code` for identifiers.
...
| calendar | calendar | |
| carousel | carousel | our page also covers user_menu and marker |
| checkbox | checkbox | |
