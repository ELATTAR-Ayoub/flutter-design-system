...
> `DsDurations.transitionDefault` (250ms) unless globals.css reads the var
...
| S2 | `DsSlider` horizontal-only to full parity; the `data-vertical:*` branch is dormant — recorded, not built (the asChild precedent). |
| S3 | `bg-action/12` derives from `DsPalette.action` directly (raw-palette access is the port's pattern); no new `DsThemeData` field; doc-comment that `--primary`≡`--color-action` today but the aliases can diverge. |
| S4 | The three unpinnable matrix states become documented package props: `DsCheckbox(inert:)` (controlled-no-handler — full opacity, no pointer), `DsSelectionControl(forceFocusRing:)` (gallery still — real focus is exclusive; the B8/Q1 precedent), `DsFieldLabel(spec:)` override (the `font-normal` site). `selects`/`feedback` reuse the same three. |
...
| # | Ruling |
|---|---|
| F1 | Bloom drift+hover composition: builder's design freedom within the measured triples; `DsBloomCosmic` may become stateful; painter rule applies in full. |
...
| F5 | Sonner's unnamed `ease` = CSS standard `ease` `(.25,.1,.25,1)` → `DsCurves.cssEase`, doc-commented to sonner's stylesheet. New `DsDurations.sway`/`swayAlt` members only if not already staged (map says staged — verify). 18s/11s land under their names. |
| F6 | `_referenceHeight['feedback'] = 6106` accepted (toaster is fixed-position; pristine safe). |
| F7 | `DsStarfield` lives in `lib/src/effects/`; the sparkle path joins `icon_paths.dart` as an off-set addition (single-writer rule: the feedback builder owns icon files this phase). |
| F8 | `toast.promise` builds (it is on the page). |
| F9 | Close both stale in-file records (DsToastType.glyph KNOWN GAP; bloom theme-var note) — rider on the feedback builder. |
...
| L4 | `DsSelect.expand` stays; optional `width` added, wins when set. |
| L5 | Disabled cell's empty `SelectContent`: nothing-opens is the parity — reproduce. |
| L6 | `DsNativeSelect` = option (c): faithful 32px no-socket closed control, the port's own menu, divergence recorded prominently. **First by-construction divergence in the port — flagged for the user in the phase report.** |
...
| L8 | `DsComponentType.menuLabel` + `menuShortcut` (+ `menuHeading` at weight 500 if the probe confirms); no new `DsType` rung. |
| L9 | `commandScore` ports verbatim (filter AND re-sort — row order is visible fidelity). |
| L10 | `DsDateFormat` in `foundation/` — twelve strings, no `intl`. |
| L11 | `DsPopover`: side/align/offset + collision flip; no arrow; no nested portals. |
...
- **A1 — selection.** `lib/src/components/slider.dart` (new, S2), the three
  ...
  register 15. Kit: `DsStateGrid` maps cols 6/3 already — no kit grant.
- **A2 — selects core.** Six `DsSelect` deltas (incl. the `_placement()`
  fix for grouped lists), `popover.dart`, `native_select.dart`,
  `combobox.dart` (+ `DsIconGlyph.calendar` NO — calendar glyph waits for
...
- **A3 — feedback effects.** `DsStarfield` (+ sparkle path in
  `icon_paths.dart` — A3 is this phase's icon writer), bloom drift+hover
  (F1), `AlertAction` + F10 lane, `DsProgress`, `DsSkeleton`, the
  `DsEmpty` family (with the never-paints dashed border and the defeated
...
**Wave C.** `calendar.dart` + `DsDateFormat` + `?clock=` seam (+ L3 probe
