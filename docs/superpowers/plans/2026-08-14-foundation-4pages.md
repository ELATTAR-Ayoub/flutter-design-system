...
- Default theme dark; three-way mode light/system/dark; `.light`/`.dark` equivalents are `ThemeData.light/.dark`.
...
class Oklab { static Color mix(Color a, Color b, double tA);    // oklab interpolation
                static Color fromOklchRelative(...); }            // chroma-reduction gamut mapping
class Palette {  // raw ramps + state — theme-independent, from globals.css @theme static
...
class ThemeData {
  ...
  final Color background, foreground, card, cardForeground, popover, popoverForeground,
    ...
    actionText, premiumText, successText, warningText, infoText, destructiveText,
    destructive, destructiveForeground, scrim;
  final Color ink1, ink2, ink3, ink4, rim, rimStrong, wall;       // shadow ink family
  static final ThemeData light, dark;                            // values = globals.css theme blocks
}

// foundation/spacing.dart
double el(num n);                       // n * 4.0  (Tailwind spacing unit)
class Widths { static const double shell=1680, content=1080, page=1200, prose=720,
  rail=240, sidebar=256, sidebarIcon=48, sidebarMobile=288, siteHeader=64, scrollOffset=96; }
class Radii { static const double xs=2, sm=6, md=10, lg=12, xl=16, xl2=20, xl3=24, xl4=32, pill=999; }
class Breakpoints { static const double sm=640, md=768, lg=1024, xl=1280; }

// foundation/typography.dart
class Fonts { static const String sans='InterLocal', mono='GeistMono', accent='Redaction35'; }
enum TypeColor { none, foreground, muted }   // the class's own color, per globals.css
class TypeSpec {
  ...
  final bool uppercase, tabular; final TypeColor defaultColor; final FontStyle fontStyle;
  TextStyle resolve(double fontSize, Color color);                // resolved TextStyle
}
class Type {   // one spec per web .type-* class — values from globals-map §5
  static double displaySize(double vw);   // clamp(44, 4.4vw, 64)
  static double h1Size(double vw);        // clamp(32, 2.8vw, 40)
  static final TypeSpec display, h1, h2, h3, h4, lead, body, small, nav, navSm, chip,
...
class ShadowLayer { final double dx, dy, blur, spread; final bool inset;
  final Color Function(ThemeData) color; }
class ShadowSpec { final List<ShadowLayer> layers;
  List<BoxShadow> outerShadows(ThemeData t);  bool get hasInset; }
class Shadows { static final ShadowSpec e1, e2, e3, e4, key, keyDown, pressed,
  btn, btnPrimary, btnValue, btnDown, chip, glowAction, glowValue; }

// foundation/motion.dart
class Durations { static const Duration tick=..80ms, fast=150, base=250, slow=400,
  overlay=320, jelly=420, reward=550, bloom=1000; }
class Curves { static const Cubic spring=Cubic(.34,1.56,.64,1), out=Cubic(.22,1,.36,1),
  curveIn=Cubic(.7,0,.84,0), inOut=Cubic(.65,0,.35,1), settle=Cubic(.16,1,.3,1),
  standard=Cubic(.4,0,.2,1), outFlex=Cubic(.05,.6,.4,.9); }

// theme_scope.dart
enum ThemeMode { light, system, dark }
class ThemeController extends ChangeNotifier {
  ThemeMode mode; void setMode(ThemeMode m);
  ThemeKind resolve(Brightness platformBrightness); }
class Theme extends InheritedNotifier<ThemeController> {
  static ThemeData of(BuildContext c);          // resolved current theme (subscribes)
  static ThemeController controllerOf(BuildContext c); }
class Text extends StatelessWidget {            // renders a TypeSpec; uppercases when flagged
  Text(String text, TypeSpec spec, {Color? color, double? fontSize, TextAlign? align,
    int? maxLines, TextOverflow? overflow}); }

// effects/
class PageGlow extends StatelessWidget {}       // radial-gradient(120% 90% at 62% 34%, pageGlow, transparent 64%)
class MachineSurface extends StatelessWidget {  // paints ShadowSpec incl. inset layers around/inside child
  MachineSurface({required ShadowSpec spec, required BorderRadius radius,
    Color? fill, BoxBorder? border, required Widget child}); }

// motion/
class Press extends StatefulWidget {            // 40ms scale-in on press, base/spring back
  Press({double scale = 0.94, required Widget child, VoidCallback? onTap}); }
class Lift extends StatefulWidget {             // hover: translateY(-3), e3 shadow, border color swap
  Lift({required Widget Function(BuildContext, bool hovered) builder}); }
class SlidingPill { /* see Task 4: SlidingPillGroup */ }

// components/
enum IconGlyph { menu, x, sun, monitor, moon, arrowLeft, arrowRight, check }
enum IconSize { xs, sm, md, lg, xl, xl2, xl3 }  // 12/14/16/20/24/32/40 px
enum IconTone { normal, muted, subtle, action, value, success, warning, info, error, inherit }
class Icon extends StatelessWidget {
  Icon(IconGlyph g, {IconSize size=IconSize.md, IconTone tone=IconTone.inherit,
    double? sizePx, double? strokeOverride}); } // strokeWidth = clamp web formula (2.4/2.4/2.4/2.4/2.0/1.5/1.6)
enum ButtonVariant { outline, ghost }           // phase-1 subset; API open for the other 5
enum ButtonSize { sm, md, icon, iconSm }
class Button extends StatelessWidget {
  Button({required Widget child, ButtonVariant variant, ButtonSize size, VoidCallback? onPressed}); }
class Sheet { static Future<void> showLeft(BuildContext c, {required WidgetBuilder builder,
...
class Category { final String slug, title, blurb; final List<String> contents; }
class Group { final String id, title, href, blurb; final List<Category> categories; }
final List<Group> elGroups;                     // foundations/base/agent/site — ALL entries verbatim
String categoryHref(Group g, Category c);
...
// Owns: Logo, version pill, tagline, ThemeToggle, NavTree, Sheet mobile nav, page glow, thin scrollbar,
...
class PageHeader { (eyebrow, title, blurb, contents?) }
class Section { (id, title, description?, child) }         // registers anchor key for #prose scroll
class Panel { (label?, note?, child, flush=false, bodyPadding?) }
class Meta { (items: List<(String k, InlineSpan v)>) }
class Code { (String text) }                                // inline code chip
class DoDont { (dos: List<String>, donts: List<String>) }
class Note { (tone: action|value|error, title?, child) }    // title ALWAYS renders mutedForeground
class IndexGrid/IndexCard { (href, title, blurb, contents) }
class PageFootNav { (groupId, slug) }

// example/lib/token_swatch.dart
class TokenRegistry { static Color resolve(String cssName, ThemeData t); static String printedValue(String cssName, ThemeKind k); }
double elContrastRatio(Color fg, Color bg);                   // WCAG, alpha-composited, 0.03928 threshold
class ContrastBadge { (String token, {String against = '--background'}) }  // "Contrast X.X:1 · VERDICT"
class TokenSwatch { (token, name, use, measure=true) }  class TokenSwatchList { (rows) }
```
...
**Files:** Create `lib/src/design_system/foundation/colors.dart`, `theme.dart`, `spacing.dart`; Test `test/foundation_colors_test.dart`.
**Interfaces:** Produces `elHsl`, `Oklab`, `Palette`, `ThemeData.light/.dark`, `el()`, `Widths`, `Radii`, `Breakpoints` exactly as the API Contract. Values: `globals-map.md` §3a/3c/3d/3h/3i (read it in full).
**Steps:**
- [ ] Failing tests first: `elHsl(213,94,78)` == `Color(0xFF92C2FC)`; `elHsl(217,91,53)` == `0xFF1A6EF4`; `elHsl(224,76,33)` == `0xFF143694` (documented rasterizations); `ThemeData.dark.background == elHsl(240,10,3.9)`; `ThemeData.light.mutedForeground == elHsl(240,4,40)`; dark `actionText == Palette.actionBright`; light `actionText == Palette.actionDark`; dark `ink2` == black 50%; light `ink2` == `elHsl(240,20,20,0.07)`; `el(6) == 24`; `Radii.xl == 16`.
- [ ] Implement every token from the maps (ALL of §3h/§3i incl. pageGlow, ring per theme, scrim, chart tokens may be omitted this phase — but sidebar aliases, messageAccent via `Oklab` relative-color math ARE included). Run tests → pass. Commit `feat(foundation): color ramps, theme blocks, spacing, radii`.

### Task 3: Foundation — typography, motion, shadows

**Files:** Create `lib/src/design_system/foundation/typography.dart`, `motion.dart`, `shadows.dart`; Test `test/foundation_type_motion_test.dart`.
**Interfaces:** Produces `Fonts`, `TypeSpec`, `Type.*`, `Durations`, `Curves`, `ShadowLayer/Spec`, `Shadows.*` per API Contract. Values: `globals-map.md` §3b/3e/3f/§5 (every .type-* row), `shared-map.md` §7.
**Steps:**
- [ ] Failing tests: `Type.h2.weight` renders wght 650 via `variations`; `Type.numSm` → mono 12px height 1.2 weight 600 tabular tracking −0.01em; `Type.label.uppercase == true` && size 11 && tracking 0.16; `Type.displaySize(1000) == 44` (4.4vw of 1000 = 44 → min edge), `Type.displaySize(1600) == 64` (70.4 clamped to 64), `Type.h1Size(1440) == 40*…` (2.8vw of 1440 = 40.32 → clamp 40); `Durations.base == 250ms`; `Curves.spring == Cubic(.34,1.56,.64,1)`; `Shadows.lg` layer values (0,2,4 ink2 + 0,14,28,−8 ink3); `Shadows.control.hasInset == true`; `Shadows.glowAction` = 0 0 0 1 actionBright@45% + 0 10 34 −8 action@60%.
...
### Task 4: Theme scope, Text, effects, motion widgets

**Files:** Create `lib/src/design_system/foundation/theme_scope.dart`, `lib/src/components/ui/background_effect.dart`, `lib/src/components/ui/surface.dart`, `lib/src/components/ui/press.dart`, `lib/src/components/ui/hover_builder.dart`, `lib/src/components/ui/active_indicator.dart`; Test `test/theme_scope_test.dart`, `test/machine_surface_test.dart`.
**Interfaces:** Consumes Tasks 2–3. Produces `Theme`, `ThemeController`, `ThemeMode`, `Text`, `PageGlow`, `MachineSurface`, `Press`, `Lift`, and `SlidingPillGroup`:
```dart
class SlidingPillGroup extends StatefulWidget {
  // children laid out in a row; activeIndex drives one travelling pill behind them.
  // Travel: position/size 250ms Curves.spring, opacity 150ms out; hidden until first layout;
  // first placement without animation; every arrival replays jelly:
  // TweenSequence scale (x,y): (1,1)→30%(1.18,0.82)→45%(0.88,1.12)→60%(1.06,0.94)→78%(0.98,1.02)→100%(1,1), 600ms, Curves.out.
  SlidingPillGroup({required int activeIndex, required Widget pill, required List<Widget> children,
    EdgeInsets padding, double gap});
}
```
**Steps:**
- [ ] Failing widget tests: `Theme` with mode dark resolves `ThemeData.dark`; `setMode(light)` notifies and rebuilds a dependent; `Text('Remaining supply', Type.label)` renders `REMAINING SUPPLY` uppercase; system mode follows `MediaQuery.platformBrightness`. MachineSurface golden-lite: paints without error for `Shadows.control` (inset) and e3 (outer) inside a 100×40 RRect.
- [ ] Implement. MachineSurface: outer layers via BoxShadow list; inset layers painted in a CustomPainter clipped to the RRect using difference-path ring + MaskFilter blur with `Shadow.convertRadiusToSigma` (recorded decision). Press: Listener down → AnimatedScale 40ms linear to scale; up/cancel → 250ms spring back. Lift: MouseRegion hover flag; builder decides transform/shadow/border (250ms out for transform+shadow, 250ms standard for border per `lift` utility). Run → pass. Commit `feat: theme scope, Text, page glow, machine surface, press/lift/sliding pill`.

### Task 5: Icon (embedded Lucide), Button, Sheet

**Files:** Create `lib/src/components/ui/icon.dart`, `lib/src/components/ui/icon_paths.dart`, `lib/src/components/ui/button.dart`, `lib/src/components/ui/sheet.dart`; Test `test/components_test.dart`.
**Interfaces:** Consumes Tasks 2–4. Produces `Icon`, `IconGlyph/Size/Tone`, `Button`, `ButtonVariant/Size`, `Sheet` per API Contract.
**Steps:**
...
- [ ] Failing tests: Icon md renders 16×16; stroke for `IconSize.xl` == 2.0 and `xl2` == 1.5; tone muted resolves `mutedForeground`.
- [ ] Button per `shared-map.md` §5 Button: pill radius, border transparent default; outline = border input + card fill + `Shadows.control` (MachineSurface) + hover muted; ghost = mutedForeground text, hover secondary bg + foreground text; sizes sm h-32 / md h-40 / icon 40×40 / icon-sm 32×32; press feel = Press at 0.95 + 80ms active transitions (btn-spring). Sheet.showLeft: barrier `background@15%` + BackdropFilter blur 4 (blur-xs), panel width param, slide-in-from-left 40px + fade 320ms `Curves.out`, dismiss reverses faster (250ms inOut).
- [ ] Run tests → pass. Commit `feat(components): Icon with embedded lucide paths, Button outline/ghost, Sheet`.
...
- [ ] Shell per maps: 64px sticky header (85% bg + blur 24 over content), Logo (28px action tile, chevron `M6 15.5 12 6l6 9.5` primaryForeground stroke 2.4 square caps + underline `M6 19h12` valueBright, wordmark `ELATTAR` + `ELATTAR` premiumText, press effect), version pill `Design System v0.1` (micro, pill border, px-10 py-4; ≥640 only), tagline `Desktop-first · 1440 frame · Light & dark` (micro; ≥768 only), ThemeToggle (radiogroup: muted pill container p-2 gap-1px; 3 options 28×28, icons 14px stroke 2 — direct, not Icon formula; SlidingPillGroup pill = card bg + e1); sidebar 240px ≥1024 (border-r, px-24 pt-40, NavTree: group label `type-label`, active group actionText; items `type-nav` py-8 pl-16 border-l, active = action border + action@12% bg + foreground; hover border input + foreground); burger <1024 opens Sheet 288px with Logo header + NavTree; main px-24 py-48 (≥1024 px-48) with centered 1080 column; PageGlow behind everything; thin scrollbar; shell width cap 1680 centered.
- [ ] main.dart: Theme root (default dark), WidgetsApp/MaterialApp(debug banner off, DefaultSelectionStyle selection action@35%), route switching: known routes → pages (placeholder for all but the 4), title `Elattar's Design System`.
