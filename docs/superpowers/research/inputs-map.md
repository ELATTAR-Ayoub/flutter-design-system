...
- `components\el\kit.tsx` — `Code`, `DoDont`, `PageHeader`, `Section`, `Meta`, `Note`, `PageFootNav`, `Panel`, **`StateCell`, `StateGrid`** (all 10 used; this is the first page in the corpus to use `StateGrid`).
...
**No fill mode is declared**, so under `prefers-reduced-motion: reduce` (the blanket rule at `globals.css:2534–2542` collapses duration to 0.01ms and iterations to 1) the caret reverts to the element's own resting style — **opacity 1, steady, visible**. In port terms: `KeyframeFill.none`.

`anim-caret` is the **only `anim-*` utility the motion page does not demonstrate**, which is exactly why `lib\src\components\ui\keyframes.dart` transcribes eleven tables and not twelve. See §16.2.
...
| `aria-label` on an unlabelled field | the 8 state cells + the password input | `Semantics(label:)` — `Input.label` already does this (`input.dart:270–271`) |
...
Recommended composite: one `Field` that owns label + control + description/error and publishes a single merged semantics node — `textField: true`, `label` = the visible label, `hint` = description or error, `validationResult` = invalid when the control is, and `liveRegion` on the error subtree only. That reproduces what a screen reader announces from the web page without inventing an id graph Flutter cannot express.
...
Both at `size="default"` / `emphasis="none"` — full anatomy in `shadows-map.md` §5.1–5.2. The premium button brings `premium-surface`'s two infinite animations onto this page (§13).
...
`MachineSurface` already accepts a `BoxBorder` and a full `BorderRadius` (`surface.dart:27–34`), and its box-sizing inset uses `border!.dimensions`, which is per-side. So `Border(top:…, right:…, bottom:…, left: BorderSide.none)` with `BorderRadius.only(topLeft:…, bottomLeft:…)` needs no change. The active slot's `z-10` is a paint-order concern: build the strip as a `Stack`, or reorder so the active slot paints last.
...
### 15.4 · `pulls-caret`
`KeyframePlayer(duration: 1000ms, repeat: true, fill: KeyframeFill.none)` over a two-stop opacity table evaluated through the existing `Steps(1)` curve — the same machinery `SignOn` uses. Because `steps(1, end)` means "hold, then snap", model it as a discrete lookup (`t < 0.5 → 1`, else `0`) rather than a tween, exactly as `SignOn.frameAt` does.
...
### 15.6 · Text metrics
`text-sm` beats `.type-num`'s size, so an input's mono value is **13px/600/tabular/−0.01em**, i.e. `Type.numSm`'s treatment at `Type.small`'s size. Neither existing spec is it. Either add an `inputNum` spec in `typography.dart` or derive one by `copyWith`-ing `numBase` down to 13 — but do it in the foundation layer, because the token guard forbids a bare `fontSize:` anywhere else.
...
| need | where |
|---|---|
| Pill field: 40px, `--input` border, `--card` fill, permanent `pressed` socket, focus ring @35% + border `--primary`@50%, 250ms `--ease-out`, real caret, placeholder, `disabled` @45%, `label` semantics | `lib\src\components\ui\input.dart` — `Input` (:62), `Input.height` (:95) |
| Inset painting, per-side border, partial radius, box-sizing inset | `lib\src\components\ui\surface.dart:27–94` |
| Focus-ring composition (3px, in front of the element's shadow) | `Button.withFocusRing` (`button.dart:241`) |
| All 7 button variants incl. `premium` + `ghost`, with foil and sheen | `button.dart:38–84`, `effects\premium_surface.dart`, `effects\action_feedback.dart` |
| `--shadow-pressed` and every other shadow token | `foundation\shadows.dart:197` |
| `input`, `card`, `border`, `ring`, `primary`, `destructive`, `destructiveText`, `premiumText`, `mutedForeground`, `radius`(=10) … — **nothing missing** | `foundation\theme.dart:94–268` |
| `Radii.lg` 12, `.pill` 999, `.sm` 6; `Widths.hairline`; `el(n)` | `foundation\spacing.dart:14, 66, 85–106` |
| `Durations.base` 250, `Curves.out` | `foundation\motion.dart:27, 216` |
| `Type.numBase` / `numSm` / `serial` / `label` / `small` / `caption`; tabular figures wired | `foundation\typography.dart:501, 490, 420, 387, 336` |
| `Steps(1, jump-end)`, `KeyframePlayer`, `KeyframeFill.none` | `motion\keyframes.dart:71, 260, 125` |
| Glyphs `eye`, `eyeOff`, `search`, `lock`, `percent`, `minus`, `check`, `x`, `arrowLeft`, `arrowRight` | `components\icon_paths.dart:105–140` |
| `IconSize.sm` (14) / `md` (16), `IconTone.subtle` | `components\icon.dart:30, 33, 76` |
| Kit: `PageHeader`, `Section`, `Panel`, `Meta`, `Code`, `DoDont`, `Note`, `Grid`, `PageFootNav` | `example\lib\kit.dart:54, 148, 242, 390, 448, 735, 834, 908, 1156` |
...
| Closest structural template | `example\lib\pages\shadows.dart` (its `#in-use` panel is the only existing `Input` call site, :429) |
...
| 2 | **`Textarea`** | 12px radius, `min-h 80`, `px-3.5 py-2.5`, `leading-relaxed` 1.625, auto-grow, same socket + focus/invalid recipe as `Input` |
| 3 | **`InputGroup` + `InputGroupAddon` / `Text` / `Button` / `Input`** | the whole §4.2 machine, including the 16→8 clearance rule and the −2px button pull |
| 4 | **`InputOtp`** (`Group`, `Slot`, `Separator`) | §6 whole; one hidden field driving painted 32px boxes |
...
| 6 | **`Field` / `FieldLabel` / `FieldDescription` / `FieldError` / `FieldGroup`** | §7.1 + the merged-semantics contract of §7.2 |
| 7 | **`StateGrid` / `StateCell`** in `example\lib\kit.dart` | nothing in the repo; the `gap-px` + `bg-border` hairline trick has no existing analogue |
| 8 | **`Input` API surface** — no addon slots, no `invalid`, no `readOnly`, no `radius`, no `size`, no `keyboardType`/`autofillHints`, no `obscureText`, no multiline, no `controller`-less value seeding beyond a controller | `input.dart:63–72`. Every §3 and §4 specimen needs at least one of these. |
| 9 | **`ButtonSize.xs`** (24px, 6px padding, 4px gap) **and a `radius` override on `Button`** | `button.dart` hardcodes `BorderRadius.circular(Radii.full)` at :471; `heightFor`/`gapFor`/`paddingXFor` (:201–226) are exhaustive switches |
| 10 | **A 7px radius token** | `calc(var(--radius) - 3px)`; `Radii` has 6 and 10, not 7 |
...
| 13 | **A `Durations` entry for the caret's 1000ms** | `bloom` is also 1000ms but means `--duration-bloom`; see open question 4 |
...
- `rounded-lg` → `Radii.lg`; the 7px addon-button radius needs a **new** `Radii` member, not a literal.
- The two new glyph path strings are guard-clean by construction (coordinates in a `d` string trip nothing).
- `pulls-caret`'s two stops and the 1000ms period need either a `Durations` member or an escape-hatch comment.
...
1. **The eyebrow says "Base" twice.** `PageHeader eyebrow={`${group.title} · Base`}` with `group.title` = "Base Components" → **"Base Components · Base"**. Every other page passes a plain group title.
...
1. **Scope of the component build.** This page needs five net-new components (`Textarea`, `InputGroup` family, `InputOtp`, `Field` family, `StateGrid`) plus a substantial widening of `Input`. **Recommendation:** build them as real, reusable package components in `lib\src\components\`, not page-local specimens — `forms`, `selects` and `selection` are the next three siblings and every one of them consumes `Field` and `InputGroup`. Budget accordingly rather than discovering it on the Forms page.
2. **`Input`'s API shape.** The page demands prefix/suffix slots, `invalid`, `readOnly`, `keyboardType`, `autofillHints`, `obscureText`, a value seed and a radius override. **Recommendation:** keep `Input` as the bare pill and put the addon machinery in `InputGroup` — that is exactly how the reference splits it (`InputGroupInput` *strips* the Input rather than extending it), and it keeps the shadows page's existing call site untouched.
3. **The Hover and Focus specimens (drifts 4 and 5).** Per the fidelity bar these ship as the reference renders them — cell 2 identical to cell 1, cell 3 painting a ring the component never produces. Confirm, because a Flutter `Input` with no way to force a fake focus appearance would need one added purely to reproduce a mistake.
4. **The caret's 1000ms.** `Durations.bloom` is already 1000ms but means `--duration-bloom`. Add `Durations.caret = 1000ms` (a token the CSS does not name either — it is an inline `1s`), reuse `bloom`, or hold it in the OTP file with an escape hatch? **Recommendation:** a named `caret` member, consistent with how `beatHover`/`glint`/`foilDrift` were handled for the shadows page.
5. **The 7px radius.** `calc(var(--radius) - 3px)` is derived, not a token. Add `Radii.addonButton = 7`, or expose `theme.radius` arithmetic at the call site? **Recommendation:** a derived getter next to `Radii` so the relationship to `--radius` = 10 survives a change to `--radius`, since that is what the CSS expresses.
...
10. **The selection-alpha bug (§16.2).** `Input` uses 0.30 where the reference and the rest of the port use 0.35. Fix it in this task, or spin it out? **Recommendation:** fix in place — it is one constant and the shadows page's own test does not assert it.
...
`Durations.transitionDefault` rather than `.base`. **No render change** - the
