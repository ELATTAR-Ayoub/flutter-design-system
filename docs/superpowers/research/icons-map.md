...
- `lib\ds\icons.ts` — `ICON_GROUPS` (four groups, 63 entries) and `ICON_COUNT` (a `reduce`, not a literal)
- `components\ds\kit.tsx` — `DsPageHeader`, `DsSection`, `Panel`, `Meta`, `Code`, `DoDont`, `Note`, `PageFootNav` (all eight used)
...
- `lib\src\components\icon.dart` — `DsIcon`, `DsIconSize`, `DsIconTone` — **already complete**
- `lib\src\components\icon_paths.dart` — `DsIconGlyph` (8 members), `DsIconElement` (4 subclasses), `_SvgPathParser` — **the gap**
- `test\icon_paths_test.dart` — the transcript guard
- `example\lib\kit.dart` — `DsPageHeader`/`DsSection`/`DsPanel`/`DsMeta`/`DsCode`/`DsDoDont`/`DsNote`/`DsPageFootNav`
...
`DsSection` shell (all sections): `<section id="…" class="mb-20">` (80px); heading block `mb-6`; `h2.type-h3.text-foreground` (21px/1.3, 600, −0.01em); description `p.type-small.mt-2.max-w-2xl` (13px/1.5, 400, muted).
...
So the optical claim in the copy is approximate rather than exact: rendered stroke still climbs 1.20 → 2.67 across the ladder. Port the ternary, not the claim. `DsIcon.strokeFor` already transcribes it and `test/components_test.dart:92–101` already pins all seven rungs with the reasoning in a comment.
...
⚠ **Key-name mapping for the Dart port.** The page prints the *object key*. `DsIconSize` spells the last two rungs `xl2`/`xl3` because Dart identifiers cannot start with a digit. The port must render **`2xl`** and **`3xl`**, not `xl2`/`xl3` — a label table, not `.name`.
...
⚠ **Key-name mapping.** `DsIconTone.normal` must render as **`default`** (Dart reserves `default`). Same label-table requirement as §3.
...
(The same mechanism, in the opposite direction, is why `DsIcon.sizePx` exists — the overview page's `<Icon size="md" className="size-5">` comes out 20px carrying md's 2.4 stroke. See `icon.dart:107–119`.)
...
> "The set is defined in `lib/ds/icons.ts`. Adding a glyph means adding it there with its single meaning — that file is the whitelist."
...
- `enum DsIconGlyph` — **8 members**: `menu, x, sun, monitor, moon, arrowLeft, arrowRight, check`.
- `sealed class DsIconElement` with **four** subclasses, one per lucide `__iconNode` node type:
  - `DsIconPathElement(String d)` — the `d` attribute character-for-character; parsed on every `addTo`.
  - `DsIconLineElement(x1, y1, x2, y2)` — `moveTo`/`lineTo`. (Constructor is point-then-point; lucide writes `x1, x2, y1, y2`, so the transcription reorders deliberately.)
  - `DsIconCircleElement(cx, cy, r)` — `Path.addOval`.
  - `DsIconRectElement(x, y, width, height, rx)` — `Path.addRRect`, uniform radius.
- `DsIconPaths.viewBox = 24`; `DsIconPaths.elements` is a `const Map<DsIconGlyph, List<DsIconElement>>`, in lucide's own element order (paint order **and** diff order against the `.mjs`).
- `DsIconPaths.pathFor(glyph)` returns a **fresh** `Path` per call — deliberate, because `Path` is mutable and a shared instance would let one painter corrupt every other icon.
...
`DsIcon.strokeFor(double px)` (`icon.dart:157–162`) is a literal transcription of the ternary, including the three named constants `_heavyStroke 2.4 / _lightStroke 1.6 / _heavyAbove 2.6 / _lightBelow 1.5` and a doc comment explaining that the middle branch is a **literal 2**, not `48/px`. `test/components_test.dart:92–101` pins all seven rungs, and comments the `1.5 < 1.5 == false` edge at 2xl. `DsIcon.strokeOverride` exists for the theme toggle, which renders lucide's default stroke 2 at 14px rather than the formula's 2.4.
...
1. **`polyline` is not implemented.** `Package` node 3 is `["polyline", { points: "3.29 7 12 12 20.71 7" }]`. `DsIconElement`'s own docstring already anticipates this ("lucide only ever emits `path`, `line`, `circle`, `rect`, `ellipse` and `polyline` nodes, and these four cover every glyph this package embeds. A new node type is a new subclass here, not a special case at the call site"). **Add `DsIconPolylineElement(List<Offset> points)`** → `moveTo` the first, `lineTo` the rest, **do not close** (that is `polygon`, which lucide does not emit). One glyph needs it; it is the file's own prescribed remedy.
...
| thing | where |
|---|---|
| `DsIcon`, the 7-rung ladder, `pxFor`, the tone map, `colorFor`, `label`→`Semantics`/`ExcludeSemantics` | `lib\src\components\icon.dart` |
...
| All eight kit primitives: `DsPageHeader`, `DsSection`, `DsPanel` (with `label` **and** `note`), `DsMeta`, `DsCode`, `DsDoDont`, `DsNote`, `DsPageFootNav` | `example\lib\kit.dart` |
| `DsGrid(base:1, sm:2, lg:3)` — exactly the tones and set grids | `example\lib\kit.dart:814` |
...
| `DsPageFootNav` empty-slot branch (`SizedBox.shrink()` inside `Expanded`) | `example\lib\kit.dart:1062` |
...
| `DsShadows.btn`, `.btnPrimary`, `.btnDown` | `lib\src\foundation\shadows.dart:204–237` |
| `DsPress` (250ms spring / 40ms in) and `DsThinScrollbar` | `lib\src\motion\press.dart`, `example\lib\shell.dart` |

### Missing — must be built

| # | thing | why the icons page needs it | size |
|---|---|---|---|
| 1 | **59 glyphs in `DsIconGlyph` + `DsIconPaths.elements`**, with 59 transcript assertions | §6 renders all 63; §3 needs `PackageOpen`; §4 needs `Search`; §5 needs `PackageOpen`, `Heart`, `Search`, `Trash2` | large, mechanical |
| 2 | **`DsIconPolylineElement`** | `Package` | tiny |
...
| 4 | **`DsButtonVariant.primary` / `.secondary` / `.destructive`** | §5 buttons 1, 2, 4. `button.dart:28–36` currently has **only `outline` and `ghost`**, and its header says the other five "drop in later — they need `sheen-action` and `foil-value`" | medium |
...
| 6 | **A multi-line code-block widget** (`<pre>`) | §2.1. `DsCode` is the **inline chip only** — there is no `pre` equivalent anywhere in `example\lib\kit.dart`. Needs: `DsType.code` at **1.625** leading (not `.type-code`'s 1.4), `--muted-foreground`, `--background` fill, 12px radius, 20px padding, horizontal scroll via the existing `DsThinScrollbar` | small |
| 7 | **`example\lib\pages\icons.dart`** + one `switch` arm in `main.dart` (`'$dsRoot/icons' => const IconsPage()`) + the `import` | the page itself | medium |
| 8 | **Label tables** `DsIconSize → "xs".."3xl"` and `DsIconTone → "default".."inherit"` | §3 and §4 print the *key*, and Dart spells two sizes and one tone differently | tiny |
...
**Build-order constraint:** items 2–3 change `DsIconElement`'s shape and `pathFor`'s return contract. Do them **before** item 1, or 59 transcriptions get rewritten.

**`token_guard_test.dart` applies — but read what it actually forbids.** `test\token_guard_test.dart` scans `lib` **and** `example/lib` as raw text, comments included, exempting only `lib/src/foundation/`. It is **not** a blanket numeric-literal scan: it is **nine specific patterns** — `Color(0x`, `Color.from`, `fontSize:\s*\d`, `letterSpacing:\s*-?\d`, `FontWeight.w\d`, `\bCurves.`, `Duration((milli|micro)seconds:\s*\d`, `BorderRadius.circular(\d`, `BoxShadow(`. Bare `0`/`0.0` and `dsTransparent` are always legal; the escape hatch is `// allow-hardcoded: <reason>` on the line.

Consequence for this batch: **glyph coordinates are safe.** `DsIconCircleElement(12, 12, 4)` matches no rule, which is why `icon_paths.dart` carries **zero** `allow-hardcoded` markers today and why 162 new elements need none either. What *will* trip the guard in the new page is typography and chrome: use `DsType.*` rather than `fontSize:`/`letterSpacing:`/`FontWeight.w*`, `DsRadii.*` through the existing kit rather than `BorderRadius.circular(12)`, `DsShadows.*`/`DsMachineSurface` rather than `BoxShadow(`, `DsCurves.*` rather than `Curves.*`, and `DsDurations.*` rather than `Duration(milliseconds: 250)` — the last two matter for the `sheen-action` beat in item 5.
