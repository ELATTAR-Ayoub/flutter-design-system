...
- `components\el\kit.tsx` — `PageHeader`, `Section`, `Panel`, `StateGrid` (:145), `StateCell` (:173), `Meta` (:202), `Code`, `DoDont`, `Note`, `PageFootNav`. **`StateGrid`/`StateCell`/`Meta` are used here and are not on any foundations page.**
...
- `components\ui\alert.tsx` — the server-error surface; carries `feedback-surface` + `starfield`.
...
4. Chips `ul.mt-7 flex flex-wrap gap-2`, each `type-chip rounded-pill border border-border bg-card px-3 py-1.5 text-muted-foreground`, from `category.contents`. Verbatim, in order:
   **Form · Validation · Field errors · Submit states · Server errors · Composed fields**
   Six chips, and they are exactly the six `Section` titles. They are `<li>`, **not links** — no anchor wiring.
...
Six sections, `PageFootNav` last. `Section` shell: `<section id class="mb-20">`; `<h2 class="type-h3">`; description `type-small mt-2 max-w-2xl`.
...
**Three different line-heights on three consecutive lines** (17.875 / 19.5 / 18.571). The field family uses Tailwind's `text-sm` ladder (`--text-sm--line-height` = `calc(1.25/0.875)` = 1.428571, never overridden), *not* `.type-small`'s 1.5. Porting these as `Type.small` would be wrong on two of the three.
...
| `htmlFor` ↔ `id` | `MergeSemantics` around label + control, or pass the label string into the control's `Semantics(label:)`. `Input.label` (`input.dart:92`) already does the latter and renders nothing visible — a visible `FieldLabel` must therefore feed it, not duplicate it. |
...
| `Slot` merging props onto five DOM shapes | a `Field` that takes a `child` and threads `(invalid, describedBy, focusNode)` down via `InheritedWidget` — the Flutter analogue of a Slot is context, not prop-merging. |
...
`feedback-surface` (`globals.css:1663–1847`) + a `starfield` span (`alert.tsx:85`, `globals.css:3397–3488`) are on every Alert. Deep dive belongs to a future `feedback-map`; the load-bearing facts for a forms port:
...
| need | where |
|---|---|
| `Button` — 7 variants (`primary premium secondary outline ghost destructive link`), 4 sizes (`sm md icon iconSm`), keyboard, disabled@0.45 + `IgnorePointer` | `lib\src\components\ui\button.dart:38`, `:90`, `:170`, `:562` |
| `Button.md` = **40px**, level with `Input.height` | `button.dart:201`, `input.dart:95` |
| **Focus-ring helper** `Button.withFocusRing(ShadowSpec, Color)` | `button.dart:241` |
| `Input` — pill, `--input` border, `--card` fill, permanent `Shadows.inset`, focus border `primary@50%` + ring `ring@35%`, 250ms `Curves.out`, own/inherited controller + focus node | `lib\src\components\ui\input.dart:62–272` |
| `Shadows.inset / btn / btnPrimary / e1 / e3` | `lib\src\design_system\foundation\shadows.dart:197 / 204 / 214 / 159 / 171` |
| `MachineSurface` (inset painting, CSS paint order) | `lib\src\components\ui\surface.dart:26` |
| `ThemeData` — `input`, `ring`, `destructive`, `destructiveText`, `popover`, `muted`, `accent` … (~102 fields) | `lib\src\design_system\foundation\theme.dart:30` |
| `Durations` / `Curves` / `Transforms` | `lib\src\design_system\foundation\motion.dart:14 / 206 / 173` |
| `Radii` (`sm md lg pill` + `xl4`), `Widths.hairline`, `el()` | `lib\src\design_system\foundation\spacing.dart:78 / 20` |
| `Type.*` 28 specs incl. `label`, `small`, `caption`, `micro`, `numSm`, `chip`, `code` | `lib\src\design_system\foundation\typography.dart:277–544` |
| `ComponentType.buttonLabel` / `.sheetBody` (13/400, what `Input` types with) | `typography.dart:239 / 247` |
| Kit: `PageHeader :54` · `Section :148` · `Panel :242` · `Code :448` · `DoDont :735` · `Note :834` (+`NoteTone`) · `Meta :390` · `Grid :908` · `PageFootNav :1156` | `example\lib\kit.dart` |
| Nav entry for `forms` — slug/title/blurb/6 contents, verbatim; siblings resolve | `example\lib\nav.dart:225–238`, `:747` |
| `IconGlyph`: `check`, `x`, `chevronDown`, `chevronUp`, `arrowLeft`, `arrowRight`, `eye`, `eyeOff`, `info`, `alertTriangle` | `lib\src\components\ui\icon_paths.dart:67–151` |
...
| # | missing | notes |
|---|---|---|
| 1 | **`Button.loading`** | **Confirmed absent.** No `loading` / `busy` / `pending` anywhere in `lib\`. The plan's Task A scoped variants only, so this is a genuine gap, and it is load-bearing: two of four forms and one of four state cells depend on it. Needs: prepend spinner, force-disable, and `Semantics` busy. |
| 2 | **A spinner** | Nothing in the package. Nearest are `Shimmer` (`lib\src\components\ui\keyframes.dart:868`) and `PulseLive` (`:940`) — neither rotates. Needs `pulls-spin`: 360° over **900ms**, **linear**, infinite, 16px, stroke 2.4. |
| 3 | **`loaderCircle` / `xCircle` glyphs** | `IconGlyph` has neither. `xCircle` is required by the server-error Alert; `loaderCircle` (lucide `Loader2`) by the spinner. Both are off-set additions like `rotateCcw` (`icon_paths.dart:148–150`) and must stay out of the icons page registry. |
| 4 | **`Input.invalid`** | `Input` has **no** invalid state, no destructive border, no destructive ring, no validation semantics. Add `invalid` → border `theme.destructive`, ring `destructive@20%`, `SemanticsValidationResult.invalid`, and make it **outrank focus** (§3.3). |
| 5 | **A field / form layer** | Nothing. Needs `FieldGroup` (column, 20px), `Field` (vertical 8px / horizontal), `FieldSet` + `FieldLegend`, `FieldLabel`, `FieldDescription` (with the `nth-last-2` −4px rule), **`FieldError`** (`liveRegion`, null when empty, string vs bulleted list), and the `data-invalid` colour cascade onto label + typed text. |
...
| 8 | **`Textarea`** | Nothing multiline. `Input` hard-pins `SizedBox(height: Input.height)` (`input.dart:249`) and `EditableText` defaults to one line. Needs `min-h-20` 80px floor, autogrow (`field-sizing: content`), `rounded-lg`, `px-3.5 py-2.5`, **lh 1.625**. |
| 9 | **`Checkbox`** | 20px, `rounded-sm`, `shadow-pressed` → `shadow-btn-primary`, hand-authored tick path `M5 12.5 10 17.5 19 7` at 14px stroke 3.2 (not lucide), indeterminate `M6 12h12`, `check-draw`/`dash-draw`/`jelly`, 44×36 hit area. |
| 10 | **`RadioGroup` / `RadioGroupItem`** | 20px circle, 8px dot at `shadow-e1`, `dot-pop` 320ms spring, `jelly`, 44×36 hit area, group gap. |
| 11 | **`Switch`** | 44×24 track / 20px thumb / 20px travel; track `shadow-pressed` ↔ `shadow-btn-primary`, thumb always `shadow-btn`; thumb on **spring**, track on **out**; `jelly`; 68×40 hit area. |
| 12 | **`Select`** | Trigger (40px pill, `shadow-pressed`, chevron 16/2.4) + portal content (`min-w-36`, `rounded-lg`, `shadow-md` **stock Tailwind, no token**, `ring-1 foreground/10`, `p-2`) + items + check indicator + keyboard nav. **No open/close animation** (§8.3). |
| 13 | **`Alert`** | 5 variants, 2-col grid, 16/14 padding, 12px radius, `role="alert"`. Plus `feedback-surface` (4 infinite animations, `screen`/`multiply` theme split, `oklch(from …)` relative colour) and `starfield`. **The single biggest new effect on this page.** |
| 14 | **A toast host** | `Toaster` bottom-right, 356px, `--shadow-e3`, `-ink` glyph, `feedback-surface`. Only `success` and `error` needed here. |
| 15 | **`StateGrid` / `StateCell`** | Absent repo-wide. Closest is the private `_SpecimenCell` (`example\lib\pages\shadows.dart:802`). Lift into `kit.dart` as `StateGrid(cols:)` / `StateCell(label:, note:)` — `gap-px` over a `--border` fill, `min-h-14` stage, `micro` label + `caption` note. |
...
Consequences here:
- The seven new animation timings (**150, 200, 280, 320, 600, 900, 1000 ms**) have no `--duration-*` token except 150 (`fast`), 250 (`base`) and 1000 (`bloom`). 200/280/320/600/900 need either new `Durations` members or escape hatches — same question as `shadows-map.md` Q3, now five entries larger.
- Checkbox `strokeWidth 3.2`, the tick path `M5 12.5 10 17.5 19 7`, `stroke-dasharray 22`/`12`, the jelly scale triples and `dot-pop`'s 1.35 are all literals. Put the glyph in `icon_paths.dart` (it is geometry, and that file already holds transcribed paths) and the scalars in `foundation\motion.dart` beside `Transforms`.
...
1. **Scope of the greenfield build.** This page needs a form controller, a validator, a field layer, and **six** new components (Textarea, Checkbox, Radio, Switch, Select, Alert) plus a spinner and a toast host. That is more new surface than the last three foundations pages combined, and Select/Alert/Toast are each properly the subject of a later page (`selects`, `feedback`). **Recommendation:** build the form/field/validator layer and `Button.loading` + spinner **fully**, since they are this page's actual subject; build Textarea/Checkbox/Radio/Switch to full parity (they are simple and `inputs`/`selection` will need them anyway); and build `Select` and `Alert` to the fidelity this page renders — a working menu and a static-bloom Alert — leaving the bloom's four infinite animations and the full Select variant matrix to `feedback` and `selects`. Confirm before I plan.
2. **`Button.loading` — build now?** It is missing, Task A did not mandate it, and two forms plus a state cell are unbuildable without it. **Recommendation: yes, now**, with the spinner, since it is a Button API change and doing it later means touching `button.dart` twice.
...
6. **Where do the new timings live?** 200 / 280 / 320 / 600 / 900 ms have no CSS token. **Recommendation:** add `Durations.dashDraw / checkDraw / dotPop / jelly / spin` in `foundation\motion.dart`, consistent with whatever was decided for `shadows-map` Q3 — one policy, not two.
7. **The validator's shape.** Zod is a schema library; Flutter has none in-repo and the port takes no dependencies. **Recommendation:** a tiny `Rule<T>` list per field (`(value) => String?`) with an "all issues" and a "first issue" collection mode — enough to express both `criteriaMode`s and nothing more. The Zod-4 email regex ships verbatim as one rule.
8. **Toast host placement.** The web mounts `<Toaster/>` once in the root layout. In Flutter this is an `Overlay` entry owned by the example app's shell, not by `lib\`. **Recommendation:** put `Toaster`'s widget in `lib\` and its mounting in `example\lib\shell.dart`, mirroring the split.
...
**Port impact, landed 2026-08-15.** `lib\src\components\ui\checkbox.dart` and
`lib\src\components\ui\radio.dart` ran their socket transition at
`Durations.fast` (150ms) where the browser runs 250ms. Both now use
`Durations.transitionDefault`, a new member of
`lib\src\design_system\foundation\motion.dart` that spells `--default-transition-duration`
separately from `--duration-base` so a retune of the token cannot silently move
the utilities. The three `pump(Durations.fast)` calls in
