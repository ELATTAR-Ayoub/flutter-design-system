...
- `components\ds\kit.tsx` — `DsPageHeader`, `DsSection`, `Panel`, `StateGrid` (:145), `StateCell` (:173), `Meta` (:202), `Code`, `DoDont`, `Note`, `PageFootNav`.
...
**Five chips, six sections, and the fifth chip names no section.** On `forms` the chips *were* the section titles. Here `Range Slider` is a chip with no `DsSection`, and `API`/`Rules` are sections with no chip. See §14 drift 1.
...
Six `DsSection`s, `PageFootNav` last. Shell: `<section id class="mb-20">`; `<h2 class="type-h3">`; description `type-small mt-2 max-w-2xl`.
...
| 5 | `api` | **API** | *(none — `DsSection` with no `description` prop)* |
| 6 | `rules` | **Rules** | *(none)* |
...
*(The 6-col row carries one 178.844 column — Chrome's sub-pixel distribution of 1080 − 5 gaps over six tracks. `DsGrid` must round the same way or the lattice drifts.)*
...
**`DsStateGrid` already supports `cols: 6` and `cols: 3`** (`example\lib\kit.dart:503`, the switch maps 2/3/4/5 and falls through to 6). Nothing to add.
...
**Cell 3 is controlled with no `onCheckedChange`.** Radix holds it at the prop value, so it never changes — but it carries no `disabled`, so it is **fully opaque and still focusable**. The port has no spelling for this: `DsCheckbox(onChanged: null)` drives `_enabled == false` in `DsSelectionControl` (`selection_control.dart:299`), which applies `Opacity(0.5)` **and** `IgnorePointer`. Rendering this cell today would dim a box the reference renders at full strength. **Gap 2 in §12.**
...
*(Ring composition, measured: `.shadow-pressed` @92704 sets `--tw-shadow`; `.ring-3` @94521 sets `--tw-ring-shadow`; both emit the same five-slot `box-shadow: var(--tw-inset-shadow), var(--tw-inset-ring-shadow), var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow)`. The ring is **added to** the socket, never replacing it — exactly what `DsButton.withFocusRing` already does.)*
...
The `RadioGroup` root is `grid w-full gap-2`, so inside a 138.83px stage it renders **228.75 × 20** *(probed — it fills the cell's content box and the item stays 20px)*. `DsRadioGroup` already reproduces this with loose constraints (`radio.dart:239–243`).
...
`FieldLabel` here is **weight 400** *(probed: `fontWeight: "400"`, colour `rgb(250,250,250)` = `--foreground`, 13px / 17.875px)* because of `className="font-normal"` overriding Label's `font-medium`. `DsFieldLabel` (`field.dart:503`) renders `DsText(text, DsComponentType.fieldLabel)` with **no weight override** — **gap 4 in §12**.
...
`type-h4` *(probed)*: **17px / 23.8px (1.4) / 600 / Inter Local**. *(Note: a second `.type-h4` exists at byte 262465 but it is scoped `.theme-reference .type-h4` — the DS page takes the one at 13723.)*
...
- `lib\src\components\checkbox.dart:238` — `duration: DsDurations.fast`
- `lib\src\components\radio.dart:418` — `duration: DsDurations.fast`
- `test\selection_feedback_test.dart:104, 217, 351` — `await t.pump(DsDurations.fast)`

**Both shipped painters run their colour/shadow transition at 150ms where the browser runs 250ms.** `DsSwitch` is unaffected (`DsDurations.base` == 250ms == the fallback, by coincidence). This is a phase-3 defect surfaced by this page, not a new-page requirement. **Gap 1 in §12; ruling requested in §15 Q1.**
...
```
All four already live in the port as `DsCheckDraw` / `DsDashDraw` / `DsDotPop` / `DsJelly` (`foundation\motion.dart:124, 130, 138, 71`) at the right numbers. **Only the two socket transitions are wrong.**

Reduced motion: `globals.css:2534–2542` collapses everything on `*, *::before, *::after`. The port routes this through `dsAnimationDuration(context, …)`, already wired in `DsSelectionControl` and `_Thumb`.
...
**All three of `DsType.h4`, `DsType.numBase`, `DsType.numSm` already exist** (`foundation\typography.dart:604`, `:794`, `:783`). No new type specs.
...
| need | where |
|---|---|
| `DsCheckbox` incl. tri-state, drawn tick/dash, `nextAfter` | `lib\src\components\checkbox.dart:98` |
| `DsRadioGroup` / `DsRadioGroupItem` incl. roving tabindex, arrow-wrap, dot-pop | `lib\src\components\radio.dart:102`, `:288` |
| `DsSwitch` incl. two sizes, recessed track / raised knob, spring thumb | `lib\src\components\switch.dart:83` |
| `DsSelectionControl`, `DsHitArea`, `DsJellyReplay` | `lib\src\components\selection_control.dart:217`, `:76`, `:144` |
| `DsFieldScope` / `DsFieldActivator` (label-for **activation**) | `lib\src\components\field.dart:95`, `:85` |
| `DsFieldGroup` (20px) · `DsFieldSet` (16/12 + legend special-case) · `DsFieldLegend` · `DsField` horizontal · `DsFieldDescription` | `field.dart:156`, `:190`, `:247`, `:291`, `:604` |
| `DsStateGrid` — **`cols: 6` and `cols: 3` already mapped** | `example\lib\kit.dart:503` |
| `DsStateCell`, `DsPanel`, `DsNote`(+tone), `DsMeta`, `DsDoDont`, `DsCode`, `DsPageHeader`, `DsSection`, `DsPageFootNav` | `example\lib\kit.dart:587, 242, 1124, 679, 1025, 737, 54, 148, 1446` |
| `DsShadows.pressed / btn / btnPrimary / e1` | `lib\src\foundation\shadows.dart` |
| `DsCheckDraw` 280 · `DsDashDraw` 200 · `DsDotPop` 320 · `DsJelly` 600 | `lib\src\foundation\motion.dart:124, 130, 138, 71` |
| `DsType.h4 / numBase / numSm / label / micro / caption / small` | `lib\src\foundation\typography.dart:604, 794, 783, 680, 723, 663, 629` |
| `theme.muted / mutedForeground / actionInk / valueInk / input / ring / primary / card / background / border` | `lib\src\foundation\theme.dart` |
| `DsPalette.action` = `hsl(217 91% 53%)` | `lib\src\foundation\colors.dart:311` |
...
| # | item | notes |
|---|---|---|
| 1 | **Checkbox and Radio socket duration is 150ms; the reference renders 250ms** | *(probed, §7.1)* Fix `checkbox.dart:238` and `radio.dart:418` to `DsDurations.base`, and the three `t.pump(DsDurations.fast)` calls at `test\selection_feedback_test.dart:104, 217, 351`. Also correct `forms-map.md` §8.3. **A painter-timing change → re-pin rendered pixels + keep the browser probe on record.** |
| 2 | **"Controlled, no handler, not disabled"** | Matrix cell 3 and bulk row 1. Today `DsCheckbox(onChanged: null)` → `_enabled == false` → `Opacity(0.5)` + `IgnorePointer` (`selection_control.dart:299, 411–414`). The reference paints opacity **1**. Needs a third state distinct from both "operable" and "disabled" — e.g. `DsCheckbox.onChanged: null` staying undimmed while `enabled: false` dims, which is the split `DsSwitch` already documents at `switch.dart:104–107` but `DsSelectionControl` collapses. |
| 3 | **A way to paint the focus ring without owning focus** | Two "Focus" cells on one page; Flutter has one focus. `DsSelectionControl` derives the ring from its private `_focused` (`:297`, `:324–341`) with no external override. Needs something like `focusRing: bool?` forcing `_borderTarget`/`_ringTarget`. Affects `DsCheckbox` and `DsRadioGroupItem` signatures. |
| 4 | **`DsFieldLabel` weight override** | Filter list passes `font-normal` → **400** *(probed)*. `field.dart:503` renders `DsText(text, DsComponentType.fieldLabel)` (500) with no hook. |
| 5 | **`DsSlider`** | The whole component — §6. Track 10px pill on `shadow-pressed`, Range 8px `--action-ink` on `shadow-btn-primary`, 20px thumbs on `shadow-btn`, single **and** range values, drag + keyboard, `hover:scale-110`, `active:scale-125`, `focus-visible:ring-3` at `ring/50`, 36 × 36 hit area, `data-disabled:opacity-50` on the root, **no jelly**, and the two-coordinate-space positioning formula. New painter → rendered-pixel pins + browser probe mandatory. |
| 6 | **A hover state in this family** | The withdrawal card hovers `border-input`; `DsSelectionControl` deliberately authors none. The hover belongs to the **card**, not the control, so it can live in the page or in a new card widget — but it is the first one. |
| 7 | **A selectable option card** | Whole-card target wrapping a radio, with `has-[[data-state=checked]]` border + tint. `DsFieldActivator` already carries "a tap on the label selects the item" for a radio in its own nested field (`radio.dart:394`), so the wiring exists; the **card skin** (16px pad, 12px gap, 12 radius, items-start, 250ms colour transition, hover) does not. |
| 8 | **A tinted-row gapped list** | Bulk header: `space-y-px` inside a bordered, radiused, **background-less** container, so the 1px seams show the *parent*'s fill. **`DsDividedList` (`example\lib\kit.dart:1580`) cannot be reused** — verified: it fills with `theme.card` and draws each seam as a `border-top` **on the row** (`divide-y` semantics). That paints a `--border` hairline over an opaque card; the reference paints nothing and lets `--background` show. Needs a sibling widget — transparent fill, 1px **gaps**, per-row background supplied by the caller. |
...
- **No new durations needed.** 250 is `DsDurations.base`; 280/200/320/600 already exist. The §12.2-1 fix *removes* a literal rather than adding one.
- **`DsSlider` adds three scalars**: `hover:scale-110` → 1.10, `active:scale-125` → 1.25, and `translateX(-50%)` → 0.5. The first two belong beside `DsTransforms` in `foundation\motion.dart` (the file already holds `DsDotPop`'s 1.35); the third is arithmetic, not a token.
- Slider sizes are all `ds()` multiples: track `h-2.5` = `ds(2.5)`, thumb `size-5` = `ds(5)`, hit `-inset-2` = `ds(2)`. Radius `pill` and `full` are `DsRadii`.
...
5. **Bulk header and option card — shared or page-local?** Both are compositions the reference builds inline out of `div`s and `Label`; neither is exported from `components\ui\`. `DsDividedList` is confirmed unusable for the first (§12.2-8, it is `divide-y` over an opaque card). **Recommendation: build both page-local in `example\lib\pages\selection.dart`.** The gapped-tinted list is one `Column` with `SizedBox(height: DsWidths.hairline)` seams over a transparent parent — small enough that lifting it into `kit.dart` before a second page wants it would be speculative. Confirm you would rather it stay local.
...
`components\ds\` and `app\design-system\` - 35 occurrences, all of them
...
- `lib\src\foundation\motion.dart` - new `DsDurations.transitionDefault`
...
- **Tests:** the three `pump(DsDurations.fast)` calls plus the two `DsSwitch`
...
(3), `DsFieldLabel` weight (4), `DsSlider` (5), the hover state (6), the option
