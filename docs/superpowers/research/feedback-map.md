...
px-4 py-3.5 text-left text-sm text-card-foreground bloom-cosmic
...
## 4 · `bloom-cosmic` — the full spec

`@utility bloom-cosmic` (`globals.css:1663–1847`). Two pseudo-element layers, painted on the **padding box**, both
...
(`bloom_cosmic.dart:393–430`, `:434–514`) inside a nested `saveLayer` for the mask.
...
The port's deferral, **verbatim** (`bloom_cosmic.dart:45–58`):
...
| Toast (live **and** preview) | `.bloom-cosmic [data-content]::before/::after` — **descendant, not child** (`:3403–3406`) | sonner owns the live markup and may wrap `[data-content]`; a `>` combinator would work on the preview and silently do nothing live |
...
| classes | `cn-toast bloom-cosmic` (`sonner.tsx:55`) | `cn-toast bloom-cosmic` (`toast-preview.tsx:32`) |
...
`lib\src\components\toaster.dart:41–50`:
...
> The bloom is `bloom-cosmic`, shared with Alert. Two layers drift at different speeds and rotate against each
