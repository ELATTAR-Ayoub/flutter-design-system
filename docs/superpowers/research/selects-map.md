...
**This is a delta map.** Phase 3 shipped `DsSelect` at *forms-page* fidelity — a working
...
- `app\design-system\components\base\selects\page.tsx` — the page. **`"use client"`**. 485 lines, no page-local components, three `useState` dates and one derived label. Nine `DsSection`s.
- `components\ds\kit.tsx` — `DsPageHeader` (:11), `DsSection` (:45), `Panel` (:77, **`note` prop at :79**), `StateGrid` (:145), `StateCell` (:173), `Meta` (:202), `Code` (:229), `DoDont` (:238), `Note` (:274), `PageFootNav` (:348).
...
`ds()` in the port is `n * 4.0` (`spacing.dart:14`) and `--spacing` is never redeclared, so
every `*-N` above ports as `ds(N)` with no conversion.
...
One top-level `Note` **before** the first section, then nine `DsSection`s, then `PageFootNav`.
`DsSection` shell: `<section id class="mb-20">`; `<h2 class="type-h3">`; description
...
`note` is used twice on this page and never on the forms page — the port's `DsPanel` already
...
## 3 · What phase 3 already shipped — the `DsSelect` delta
...
| Permanent `shadow-pressed` socket via `DsMachineSurface` | `select.dart:408–415` | `selection_feedback_test.dart:537–539` |
| `pl-4 pr-3.5`, `gap-2`, 13px `sheetBody`, chevron 16px muted | `select.dart:418`, `:426–444` | — |
| `transition-colors` on fill / border / ring at **250ms** `DsCurves.out` | `select.dart:378`, `:393–425` | — |
...
| Content `min-w-36`, `rounded-lg`, `DsShadows.tailwindMd` + 1px `foreground/10` ring, viewport `p-2` | `select.dart:88–96`, `:598–614` | `selection_feedback_test.dart:627` |
...
| `DsSelect.itemHeight` derived from the type spec, not measured | `select.dart:194–197` | `selection_feedback_test.dart:581` |
...
| `DsFieldScope` integration + label-for activation | `select.dart:218–227`, `:382` | `forms_page_test.dart:1043–1051` |

The row-height derivation is exact and already load-bearing:
`13 × (1.25/0.875) + ds(2)×2 = 18.5714 + 16 = **34.5714px**`, matching
...
| 4 | **A trigger width that is neither `w-fit` nor `w-full`** | page:166, 176, 186 (`className="w-40"`) | `DsSelect.expand` is a `bool`. `w-40` = 160px is a third case; twMerge kills `w-fit` before CSS sees it. |
| 5 | **Item-aligned placement over a heterogeneous list** | page:118–131 | `_placement()` (`select.dart:508–510`) computes the chosen row's centre as `ds(2) + (index + 0.5) × itemHeight`. With a 32px label, three rows, a 17px separator, a second 32px label and two rows, that arithmetic is wrong. It must become a running offset over a typed child list. **This is the sharpest technical delta on the page.** |
...
| 8 | **`aria-label` on a trigger with no visible label** | page:166, 176, 186 | `DsSelect.label` already exists (`select.dart:181`). No work — but the page is the first consumer. |
...
| `InputGroupButton size="icon-xs"` (`:76`) | `size-6 rounded-[calc(var(--radius)-3px)] p-0 has-[>svg]:p-0` | **24 × 24**, radius **7px** (`--radius: 10px`, *measured byte 242912*) — the port already has this as **`DsRadii.addonButton`** (`spacing.dart:125`) |
...
```
- `--cell-size` = `ds(7)` = **28px**; `--cell-radius` = `--radius-md` = **10px** *(measured, byte 3674)*.
...
The port has both: `DsType.numBase` (mono/15/1.2/600/−0.01/tabular, `typography.dart:794`) and
`DsComponentType.buttonLabel` (`:310`).
...
`DsDurations.base`, and treat any future `duration-fast` / `duration-tick` / `duration-slow`
...
`--duration-overlay` (320ms) already exists in the port as **`DsDurations.overlay`**
(`motion.dart:33`) and is currently unused — `DsSelect` deliberately does not animate.
Reduced motion collapses everything to `.01ms` (`globals.css:2534–2542`); the port routes this
through `dsAnimationDuration`.
...
There is **no `DsType` at 12px** in the port outside `DsType.numSm` (mono) and three
`DsComponentType` roles (`buttonLabelXs`, `buttonLabelCaps`, `kbdKey`) — §17 Q8.
...
`DsRadii.addonButton` (`spacing.dart:125`).

**Widths:** `--container-xs` 320 · `--container-sm` 384 · `--spacing` 4 · `w-40` 160 ·
`min-w-36` 144 · `max-h-72` 288 · combobox list cap 252 (`ds(72) − ds(9)`) ·
combobox popup min-width = anchor + `ds(7)` = +28.
...
on **all three** overlays. The port already carries it as `DsShadows.tailwindMd`
...
| need | where |
|---|---|
| **`DsSelect<T>`** — trigger, socket, states, keyboard, overlay menu, item-aligned placement, `expand`, field-scope wiring, 34.571px rows | `lib\src\components\select.dart:133` |
| `DsShadows.tailwindMd` (+ `tailwindLg`) | `lib\src\foundation\shadows.dart:168`, `:153` |
| `DsMachineSurface` (inset painting, CSS paint order, **`drawDRRect`**) | `lib\src\effects\machine_surface.dart:26`, `:205` |
| `DsInputGroup` + `DsInputGroupInput` / `Addon` / `Button` / `Align` | `lib\src\components\input_group.dart:124, 314, 387, 474, 534` |
| `DsInput`, `DsFieldSurface` | `lib\src\components\input.dart:83`, `:427` |
| Full field layer — `DsFieldScope`, `DsFieldGroup`, `DsField`, `DsFieldLabel`, `DsFieldDescription`, `DsFieldActivator` | `lib\src\components\field.dart:95, 156, 291, 503, 604, 85` |
| `DsButton` (7 variants incl. `outline`/`ghost`, sizes `sm`/`icon`), `btn-spring`, focus ring helper | `lib\src\components\button.dart:222` |
| `DsKbd` / `DsKbdGroup` | `lib\src\components\kbd.dart:54`, `:132` |
| `DsIcon` + glyphs `chevronDown/Up/Left/Right`, `check`, `x`, `search` | `icon.dart:135`; `icon_paths.dart:94–97, 77, 71, 88` |
| `DsRadii.addonButton` = 7 | `lib\src\foundation\spacing.dart:125` |
| `DsDurations.overlay` = 320ms, `DsCurves.out` / `.spring` | `lib\src\foundation\motion.dart:33`, `:266`, `:271` |
| `DsType.numBase` (mono 15/1.2/600), `DsType.numSm`, `DsComponentType.sheetBody` / `buttonLabel` | `typography.dart:794, 783, 451, 310` |
...
| Kit: `DsPanel(note:)`, `DsStateGrid`, `DsStateCell`, `DsNote`, `DsMeta`, `DsDoDont`, `DsCode`, `DsPageHeader`, `DsSection`, `DsPageFootNav` | `example\lib\kit.dart:242, 503, 587, 1124, 679, 1025, 737, 54, 148, 1446` |
...
| # | missing | scope | notes |
|---|---|---|---|
| 1 | **`DsIconGlyph.calendar`** | 1 path | The page's only glyph gap; blocks §5–§7. Case-insensitive grep for `calendar` across `lib\` returns zero. Must be classified as curated-63 vs off-set before it lands, because `test\icon_paths_test.dart` polices the icons-page registry. |
| 2 | **`DsSelect` groups / labels / separators** | §3.2 #1, #2 | New row kinds in the menu's child list. |
| 3 | **`DsSelect` scroll buttons + capped content** | §3.2 #3, #6 | 32px `bg-popover` caps above and below the padded viewport; the rail is replaced, not styled. |
| 4 | **`DsSelect` item-aligned over a heterogeneous list** | §3.2 #5 | `_placement()`'s uniform-row arithmetic must become a running offset. **The one real algorithmic change to shipped code.** |
| 5 | **`DsSelect` explicit trigger width** | §3.2 #4 | `expand: bool` → a width mode (`fit` / `fill` / `fixed(w)`), or a `width` param beside `expand`. API decision — §17 Q4. |
| 6 | **`DsPopover`** | new | Nothing in `lib\` generalizes an anchored overlay: `DsSelect`'s is private (`_SelectContent` `select.dart:579`), `DsSheet` uses a `PopupRoute` (`sheet.dart:141`). Needs side/align/sideOffset, collision handling, a corner transform-origin, and the 320ms enter/exit. **Both the Combobox and the Date Picker depend on it.** |
| 7 | **`DsNativeSelect`** | new | 32px, 12px radius, transparent, no socket. The *platform picker* part is §17 Q6. |
| 8 | **`DsCombobox`** | new | `DsInputGroup` chassis + `DsPopover` + a collator-equivalent filter + `data-highlighted` rows + an empty row. |
| 9 | **`DsCommand`** | new | `DsInputGroup` chassis (socket **off**, `--input` @30 % fill) + group headings + a no-margin separator + `--muted` highlight + shortcut column + first-item-selected-at-rest + the cmdk fuzzy score. §17 Q9. |
| 10 | **`DsCalendar`** | new, largest | Month grid, outside days, Sunday week start, `cccccc` weekday labels, month/year caption, nav buttons, single + range modes with the 16px `after:` bleeds, `today`, focus ring, keyboard. |
...
| 13 | **`main.dart` route arm** | 1 line | `'$dsRoot/components/base/selects' => const SelectsPage(),` immediately after the `forms` arm at `main.dart:167` — the switch deliberately follows nav order (`main.dart:153–155`). Today the route falls to `PlaceholderPage`. |
...
`\bheight:\s*[01](?!\d)`. Bare `0`/`0.0` and `dsTransparent` are always legal; the escape hatch
...
- Every geometry number is already expressible: cell size `ds(7)`, cell radius `DsRadii.md`,
  range bleed `ds(4)`, combobox cap `ds(72) − ds(9)`, popup min-width `anchor + ds(7)`,
  addon radius `DsRadii.addonButton`, overlay duration `DsDurations.overlay`. **No new
  `DsDurations` member is needed** — 320ms already exists as `overlay`.
...
For this page:
- The three overlays reuse the **existing** recipe — `DsMachineSurface` with a prepended 1px
  ring layer over `DsShadows.tailwindMd` (`select.dart:598–614`). **No new painter.**
- The Command input asks for `shadow-none`, i.e. `DsShadows.none` — the socket is *removed*, not
  restyled. No painter.
...
1. **Scope.** This page needs four brand-new components (`DsNativeSelect`, `DsCombobox`, `DsCommand`, `DsCalendar`), one new primitive (`DsPopover`), one new glyph, a date formatter, and six changes to shipped `DsSelect` code — one of them algorithmic. That is comparable to the whole of phase 3, and `DsCalendar` alone is bigger than any single component in the port. **Recommendation:** build `DsSelect`'s deferred surface + `DsPopover` + `DsNativeSelect` + `DsCombobox` first (they share the overlay and the input-group chassis and finish four of seven chips), then `DsCommand`, then `DsCalendar` last as its own wave, since §5/§6/§7 all collapse if the calendar is not right. Confirm the split before I plan.
...
4. **`DsSelect.expand` → a width mode?** The page needs three widths (`w-fit`, `*:w-full`, `w-40`) and `expand` is a `bool`. **Recommendation:** keep `expand` (it names a documented cascade, and `forms_page_test.dart:984` asserts it) and add an optional `width` that wins over both, rather than replacing a documented switch with an enum. Your call — this is public API on a shipped component.
