...
**In scope**
- Package layers: `foundation/` (all tokens), `effects/` (only what these surfaces need: machine shadows incl. inset, glows, page glow), `motion/` (press, lift, slide-pill + jelly, curves), `components/` (Icon, Button outline/ghost variants, Sheet for mobile nav).
...
**Out of scope**: shadows/motion/icons pages' real content, base/agent/site families, foil/sheen/feedback-surface/starfield effects (not rendered by these four pages — foil/bloom/sheen are only *named in copy* on colors §beyond), toasts, landing site.
...
flutter-design-system/               (git repo, package elattar_design_system)
  ...
  lib/src/foundation/                ← ONLY place literals are allowed
    colors.dart        raw ramps + state palette (Palette; theme-independent)
    theme.dart         ThemeData: every semantic token per theme (light/dark blocks of globals.css)
    typography.dart    Type: every .type-* class as a TextStyle factory (incl. clamp() rules)
    spacing.dart       ds4 unit + scale, widths (shell/content/page/prose...), radii
    shadows.dart       ShadowSpec (layers w/ inset flag) e1–e4, key, pressed, btn*, chip, glows + per-theme inks
  ...
  lib/src/components/ui/
    surface.dart   MachineSurface — paints ShadowSpec incl. inset layers (clip RRect, difference ring, MaskFilter)
    background_effect.dart         fixed-viewport radial gradient behind scroll content
  lib/src/components/ui/
    press.dart         Press: 40ms scale-in (0.94 default), 250ms spring-back
    lift.dart          Lift: hover translateY(-3) + e3 + border-color, 250ms out/standard
    active_indicator.dart  SlidingPill + jelly replay (yuki-jelly keyframes as TweenSequence)
  lib/src/components/
    icon.dart          Icon: 8 embedded Lucide 1.28.0 paths (Menu, X, Sun, Monitor, Moon, ArrowLeft, ArrowRight, Check),
                       size ladder 12/14/16/20/24/32/40, stroke 2.4/2.4/2.4/2.4/2/1.5/1.6 (web-computed), tones
    button.dart        Button: outline/ghost + icon sizes used by the shell (API shaped for all 7 variants later)
    sheet.dart         Sheet: left sheet (overlay bg-background/15 + blur-xs, panel slide+fade 320ms/out)
  lib/src/design_system/foundation/theme_scope.dart  Theme InheritedWidget + ThemeMode (light/system/dark) controller
  example/                     (the docs app)
    lib/main.dart              MaterialApp-free custom WidgetsApp/Router or minimal MaterialApp with Theme
    ...
    lib/kit.dart               PageHeader, Section, Panel, Meta, Code, DoDont, Note, IndexGrid/IndexCard, PageFootNav
...
Key contracts:
- `ThemeData` exposes every semantic token (`background, card, muted, accent, foreground, mutedForeground, border, input, primary, ring, actionText, premiumText, successText/…, pageGlow, ink1..4, rim, rimStrong, wall`) resolved per theme; raw ramps live on `Palette` and never flip.
- `Type` styles carry family/size/height/weight/tracking/uppercase-flag/tabular exactly as the web classes; call sites never restate numbers. `typeDisplay(context)`/`typeH1(context)` compute clamp() from viewport width.
- Colors with CSS `color-mix(in oklab, X n%, transparent)` resolve to X at n% alpha (mixing toward transparent preserves hue in premultiplied oklab); genuine two-color oklab mixes use an OKLab implementation in foundation (`Oklab.mix`). OKLCH relative-color tokens (`--bubble-tinted`) are computed, not hardcoded, with chroma-reduction gamut mapping (recorded decision).
...
| inset/mixed box-shadows (shadow-btn on outline button) | MachineSurface custom painter |
...
3. In-app browser at **1440-wide viewport**: for each page × {dark, light}: screenshot the reference and the port at the same scroll positions, section by section (header, each Section, foot-nav). Compare: layout metrics, type sizes/weights/tracking, colors, borders, shadows, badge values ("Contrast 13.5:1 · AAA" strings must match), copy verbatim.
