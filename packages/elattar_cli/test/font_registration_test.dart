...
///    `'zzz'`;
///  * leaving `DsFonts.package = 'elattar_design_system'` in installed source,
...
/// Families named by `DsFonts` in installed foundation source, e.g.
...
Set<String> _familiesAskedFor(String typographySource) {
  final int classStart = typographySource.indexOf('class DsFonts {');
  expect(classStart, greaterThanOrEqualTo(0), reason: 'DsFonts not found');
...
void main() {
  ...
  test('installed typography carries no package prefix', () {
    ...
    expect(typography(), contains('package: DsFonts.package'));
