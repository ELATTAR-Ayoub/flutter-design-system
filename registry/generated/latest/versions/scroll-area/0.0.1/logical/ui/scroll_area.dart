...
const double _railPadding = ElWidths.hairline;
...
class ElScrollArea extends StatefulWidget {
  const ElScrollArea({
    super.key,
    this.borderRadius,
  ...
  @override
  State<ElScrollArea> createState() => _ElScrollAreaState();
}

class _ElScrollAreaState extends State<ElScrollArea> {
...
class _Viewport extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // query — and a `ScrollArea` inside a `ElGrid` is asked for one, because
...
/// starts inside a [ElScrollArea] does not also move the page.
///
...
/// to the parent.
class ElScrollAreaBehavior extends ScrollBehavior {
  const ElScrollAreaBehavior();
