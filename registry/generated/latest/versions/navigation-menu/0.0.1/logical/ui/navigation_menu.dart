...
/// [ElComponentType.navMenuTrigger].
///
...
///     [ElPopoverOriginModel.selfCenter].
...
/// mounts it through [ElPopover] instead, which is the port's one mechanism for
...
/// [ElToaster.unmountDelay]'s argument: putting a third-party runtime constant
...
class ElNavigationMenuItem {
  ...
  /// beside it.
  const ElNavigationMenuItem.link({required this.label, this.onTap})
    : content = null,
      _isTrigger = false;
...
/// The bar.
class ElNavigationMenu extends StatefulWidget {
  const ElNavigationMenu({
    super.key,
    required this.items,
    this.viewport = true,
  ...
  final List<ElNavigationMenuItem> items;
  ...
  /// `gap-1` on the list.
  static double get listGap => el(1);

  /// `h-10` on the trigger.
  static double get triggerHeight => el(10);

  /// `px-4` on the trigger.
  static double get triggerPaddingX => el(4);

  /// `gap-1.5` between a trigger's label and its chevron.
  static double get triggerGap => el(1.5);

  /// `size="sm"` on the chevron.
  static double get chevronPx => ElIcon.pxFor(ElIconSize.sm);

  /// `mt-2` — the gap between the bar and the panel.
  static double get panelOffset => el(2);

  /// `p-2` inside the panel.
  static double get panelPadding => el(2);
  ...
  static double get indicatorHeight => el(2);

  /// `size-2` — the square that becomes the caret once it is rotated.
  static double get caretSize => el(2);

  @override
  State<ElNavigationMenu> createState() => _ElNavigationMenuState();
}

class _ElNavigationMenuState extends State<ElNavigationMenu> {
  ...
  @override
  void didUpdateWidget(ElNavigationMenu old) {
  ...
  Widget _panelBody(BuildContext context, Widget content) {
    ...
    return ElPopoverSurface(
      child: Padding(
        padding: EdgeInsets.all(ElNavigationMenu.panelPadding),
  ...
  Widget build(BuildContext context) {
    ...
    final Widget list = Row(
      ...
      children: <Widget>[
        for (int i = 0; i < widget.items.length; i++) ...<Widget>[
          if (i > 0) SizedBox(width: ElNavigationMenu.listGap),
    ...
    return ElPopover(
      ...
      // [ElPopoverBarrier.none] carries for a submenu, and this menu has no
  ...
  Widget _indicator(BuildContext context) {
    final int? open = _open;
    return ElNavigationMenuIndicator(
      width: (open != null && open < _widths.length) ? _widths[open] : 0,
    );
  }

  Widget _item(BuildContext context, int index) {
    final ElNavigationMenuItem item = widget.items[index];
    if (!item._isTrigger) {
      return _ElNavigationMenuTrigger(
        label: item.label,
        open: false,
        chevron: false,
        onTap: item.onTap,
    ...
    final Widget trigger = _ElNavigationMenuTrigger(
      label: item.label,
      open: _open == index,
      chevron: true,
    ...
    return ElPopover(
      open: _open == index,
      align: ElPopoverAlign.start,
      sideOffset: ElNavigationMenu.panelOffset,
      origin: ElPopoverOriginModel.selfCenter,
      slideSides: const <ElPopoverSide>{},
      barrier: ElPopoverBarrier.none,
      anchor: trigger,
...
/// shipped."*
///
/// **Where it sits is drift 1 on [ElNavigationMenu]**: Radix pins it to the
...
/// follows the open trigger. Mounted by [ElNavigationMenu] when its
...
class ElNavigationMenuIndicator extends StatelessWidget {
  const ElNavigationMenuIndicator({super.key, required this.width});
  ...
  @override
  Widget build(BuildContext context) {
    final double caret = ElNavigationMenu.caretSize;
    return SizedBox(
      width: width,
      height: ElNavigationMenu.indicatorHeight,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        // square is turned.
        borderRadius: BorderRadius.only(topLeft: Radius.circular(ElRadii.sm)),
        border: Border.all(
...
/// `ring-1 ring-foreground/10` — the same rim [ElPopoverSurface] wears.
const double _caretRingAlpha = 0.10;

/// The 40px pill: `navigationMenuTriggerStyle()`, with or without a chevron.
class _ElNavigationMenuTrigger extends StatefulWidget {
  const _ElNavigationMenuTrigger({
    required this.label,
    required this.open,
  ...
  @override
  State<_ElNavigationMenuTrigger> createState() =>
      _ElNavigationMenuTriggerState();
}

class _ElNavigationMenuTriggerState extends State<_ElNavigationMenuTrigger> {
  bool _hovered = false;
    ...
    return MouseRegion(
      ...
      child: Semantics(
        ...
        child: ElPress(
          onTap: widget.onTap,
          child: SizedBox(
            height: ElNavigationMenu.triggerHeight,
            ...
            child: Container(
              decoration: BoxDecoration(
                ...
                color: lit ? theme.secondary : elTransparent,
              ...
              child: Padding(
                padding: EdgeInsets.symmetric(
                  horizontal: ElNavigationMenu.triggerPaddingX,
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: <Widget>[
                    ...
                    if (widget.chevron) ...<Widget>[
                      SizedBox(width: ElNavigationMenu.triggerGap),
    ...
    return DefaultTextStyle.merge(
      ...
      child: TweenAnimationBuilder<double>(
        ...
        child: ElIcon(
          ...
          sizePx: ElNavigationMenu.chevronPx,
...
/// string is not `true`"*.
class ElNavigationMenuLink extends StatefulWidget {
  const ElNavigationMenuLink({
    super.key,
    required this.child,
    this.active = false,
    this.onTap,
  ...
  static double get iconPx => el(4);

  @override
  State<ElNavigationMenuLink> createState() => _ElNavigationMenuLinkState();
}

class _ElNavigationMenuLinkState extends State<ElNavigationMenuLink> {
  bool _hovered = false;
    ...
    return MouseRegion(
      ...
      child: Semantics(
        ...
        child: ElPress(
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              color: lit ? theme.accent : elTransparent,
              borderRadius: BorderRadius.circular(ElRadii.md),
            ),
            child: Padding(
              padding: EdgeInsets.symmetric(
                horizontal: ElNavigationMenuLink.paddingX,
                vertical: ElNavigationMenuLink.paddingY,
              ),
