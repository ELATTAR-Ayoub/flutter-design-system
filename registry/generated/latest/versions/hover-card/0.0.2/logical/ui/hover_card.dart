...
/// the placement comes from [popoverPlacement] and the paint from
/// [PopoverSurface] — the same two objects the combobox and the date picker
/// use. What it cannot borrow is [Popover] itself: that widget lays a
...
class HoverCard extends StatefulWidget {
  const HoverCard({
    ...
    this.width,
    this.openDelay = MotionDurations.hoverCardShowDelay,
    this.closeDelay = MotionDurations.hoverCardHideDelay,
  });

  /// `HoverCardTrigger asChild`.
  final Widget trigger;

  /// `HoverCardContent`'s children — laid out under [HoverCardContent]'s
  /// `p-2.5`.
  ...
  static double get defaultWidth => space(72);
...
class _HoverCardState extends State<HoverCard>
  ...
  /// Built in [initState], never lazily — see `Popover`'s note.
...
class _HoverCardLayout extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    final PopoverPlacement placement = popoverPlacement(
      ...
      content: childSize,
      viewport: size,
      sideOffset: HoverCard.sideOffset,
    );
    onPlaced(placement);
...
class HoverCardContent extends StatelessWidget {
  const HoverCardContent({super.key, required this.child});

  final Widget child;

  /// `p-2.5`.
  static double get padding => space(2.5);
