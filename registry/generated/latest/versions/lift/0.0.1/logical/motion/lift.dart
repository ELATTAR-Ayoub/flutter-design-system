...
/// would put those in the wrong place. [ElLiftCard] is the standard
/// appearance, built on this.
class ElLift extends StatefulWidget {
  const ElLift({
    super.key,
  ...
  @override
  State<ElLift> createState() => _ElLiftState();
}

class _ElLiftState extends State<ElLift> {
  bool _hovered = false;
...
class ElLiftCard extends StatefulWidget {
  const ElLiftCard({
    super.key,
    required this.builder,
    this.radius,
  ...
  @override
  State<ElLiftCard> createState() => _ElLiftCardState();
}

class _ElLiftCardState extends State<ElLiftCard>
  ...
  late final Animation<double> _tint = CurvedAnimation(
    parent: _controller,
    curve: ElCurves.standard,
    reverseCurve: ElCurves.standard.flipped,
  );
  ...
  void _hover(bool hovered) {
    _controller.duration = elAnimationDuration(context, ElDurations.base);
    if (hovered) {
      _controller.forward();
    } else {
      _controller.reverse();
  ...
  List<BoxShadow> _shadowAt(List<BoxShadow> lifted, double t) {
    ...
    final List<BoxShadow> none = <BoxShadow>[
      for (int i = 0; i < lifted.length; i++)
        const BoxShadow(color: elTransparent), // allow-hardcoded: CSS `none`
    ];
  ...
  Widget build(BuildContext context) {
    ...
    final BorderRadius radius =
        widget.radius ?? BorderRadius.circular(ElRadii.xl);
    final Color resting = widget.borderColor ?? theme.border;
    final Color hovering = widget.hoverBorderColor ?? resting;
    final List<BoxShadow> lifted = (widget.shadow ?? ElShadows.e3).outerShadows(
      theme,
    );
        ...
        return GestureDetector(
          ...
          child: AnimatedBuilder(
            ...
            builder: (BuildContext context, Widget? child) =>
                Transform.translate(
                  offset: Offset(0, ElTransforms.liftY * _rise.value),
                  child: DecoratedBox(
            ...
            child: Padding(
              ...
              padding: widget.padding.add(
                const EdgeInsets.all(ElWidths.hairline),
              ),
