...
class ToggleGroup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // A row of targets that does not fit scrolls rather than clipping its
    // last option. The group still hugs its items — the scroll view only
    // supplies the room when there is not enough of it.
    final Widget scrollable = SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      child: Align(alignment: AlignmentDirectional.centerStart, child: group),
    );

    if (label == null) return scrollable;
    return Semantics(container: true, label: label, child: scrollable);
  }
}
