...
/// `toggleVariants(default/default)` — i.e. a [ElToggle] — + the two-class
...
/// as its first child — and [ElSlidingPillGroup] already paints it first, so
...
/// [ElSlidingPillGroup] is the transcription of all of that, including the
...
/// item, and [ElSlidingPillGroup] documents an out-of-range `activeIndex` as
...
@immutable
class ElToggleGroupItem {
  const ElToggleGroupItem({
    required this.label,
    this.child,
    this.enabled = true,
  });
...
/// [ElSlidingPillGroup] is positional, it measures the child *at an index*, so
...
/// through [ElSlidingPillGroup] that nothing else in the port exercises.
///
...
class ElToggleGroup extends StatelessWidget {
  const ElToggleGroup({
    super.key,
    required this.items,
  ...
  final List<ElToggleGroupItem> items;
  ...
  Widget build(BuildContext context) {
    ...
    final Widget group = ElSlidingPillGroup(
      ...
      children: <Widget>[
        for (int i = 0; i < items.length; i++)
          ElToggle(
            ...
            // re-asserting it: `ElToggle` always asks for `!pressed`, so the
            ...
            // through [ElText] here would restate all three.
