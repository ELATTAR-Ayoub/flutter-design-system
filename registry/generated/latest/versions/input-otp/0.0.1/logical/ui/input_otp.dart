...
// [ElFieldVisibility] — the field family's one keyboard-avoidance hook, which
...
/// follows `ElInputGroup`'s theme-split, not the bare field's flat 20.
...
/// than as a tween through a step curve — the shape `ElSignOn.frameAt` already
...
class ElInputOtp extends StatefulWidget {
  ElInputOtp({
    super.key,
    this.maxLength = 6,
  ...
  static double get slotSize => el(8);

  /// The separator's width: a 16px glyph in a box with no padding of its own.
  static double get separatorWidth => ElIcon.pxFor(ElIconSize.md);
  ...
  @override
  State<ElInputOtp> createState() => _ElInputOtpState();
}

class _ElInputOtpState extends State<ElInputOtp> {
  ...
  @override
  void didUpdateWidget(ElInputOtp old) {
  ...
  Widget build(BuildContext context) {
    ...
    for (int g = 0; g < widget.groups.length; g++) {
      if (g > 0) strip.add(const ElInputOtpSeparator());
    ...
    final Widget hidden = EditableText(
      ...
      cursorColor: elTransparent,
      backgroundCursorColor: elTransparent,
      selectionColor: elTransparent,
      showCursor: false,
    ...
    field = SizedBox(
      width: ElInputOtp.widthFor(widget.groups),
      height: ElInputOtp.slotSize,
      child: field,
    );
    ...
    // [ElFieldVisibility]. A one-time code arrives while the keyboard is open by
  ...
  Widget _group({
    ...
    final double width = count * ElInputOtp.slotSize;
        ...
        children: <Widget>[
          for (final int i in order)
            PositionedDirectional(
              start: (i - first) * ElInputOtp.slotSize,
              top: 0,
              width: ElInputOtp.slotSize,
              height: ElInputOtp.slotSize,
              child: ElInputOtpSlot(
                char: _charAt(i),
                active: _isActive(i),
                invalid: invalid,
                first: i == first,
    ...
    if (invalid) {
      final double alpha = ElTheme.kindOf(context) == ElThemeKind.dark
...
/// nothing autofocuses.
class ElInputOtpSlot extends StatelessWidget {
  const ElInputOtpSlot({
    super.key,
    this.char,
    this.active = false,
    this.invalid = false,
  ...
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final bool dark = ElTheme.kindOf(context) == ElThemeKind.dark;
    ...
    final BorderRadius radius = BorderRadius.horizontal(
      ...
      left: Radius.circular(first ? ElRadii.lg : 0),
      right: Radius.circular(last ? ElRadii.lg : 0),
    );
  ...
  Widget _content(BuildContext context, ElThemeData theme) {
    return Stack(
      alignment: Alignment.center,
      children: <Widget>[
        if (char != null)
          // `text-sm` — Inter 13/400 at `--foreground`. Not mono; see drift 13.
          ElText(char!, ElComponentType.textSm, color: theme.foreground),
...
class _FakeCaret extends StatelessWidget {
  ...
  static double get height => el(4);

  @override
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    return ElKeyframePlayer(
      duration: ElDurations.caret,
      repeat: true,
      ...
      // visible** (inputs-map §6.3). `ElKeyframeFill.none` freezes the player at
...
class ElInputOtpSeparator extends StatelessWidget {
  const ElInputOtpSeparator({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return const ExcludeSemantics(
      ...
      // `--foreground` — and turns with a `ElField`'s invalid colouring, the
      // same as `currentColor` does.
      child: ElIcon(ElIconGlyph.minus),
    );
  }
}
