...
/// drives only the box. That is why [ElAvatar.sizePx] exists beside
/// [ElAvatar.size] — the same split `ElIcon.sizePx` carries.
///
...
enum ElAvatarSize {
  ...
  double get px => switch (this) {
    ElAvatarSize.sm => el(6),
    ElAvatarSize.md => el(8),
    ElAvatarSize.lg => el(10),
  };

  /// `group-data-[size=default]/avatar:size-2.5` and its two siblings — the
  /// diameter an [ElAvatarBadge] takes under this rung.
  double get badge => switch (this) {
    ElAvatarSize.sm => el(2),
    ElAvatarSize.md => el(2.5),
    ElAvatarSize.lg => el(3),
  };
}
...
typedef ElAvatarRing = ({Color color, double width});

/// `ring-2` — the width both ring sites use.
double get elAvatarRingWidth => 2;

/// A round identity mark: [image] if it loads, initials underneath if it does
/// not.
class ElAvatar extends StatelessWidget {
  const ElAvatar({
    super.key,
    required this.fallback,
    this.image,
    this.size = ElAvatarSize.md,
    this.fallbackSpec,
    this.sizePx,
    this.ring,
    this.badge,
  ...
  final ElAvatarSize size;

  /// The fallback's resolved type.
  ///
  /// Defaults to [ElComponentType.textSm] — a bare `<AvatarFallback>` is
  ...
  /// [ElComponentType.avatarFallback] for its own `.type-num-sm` collision.
  final ElTypeSpec? fallbackSpec;
  ...
  final ElAvatarRing? ring;

  /// `<AvatarBadge/>` — `absolute right-0 bottom-0 z-10`.
  final ElAvatarBadge? badge;
  ...
  Widget build(BuildContext context) {
    ...
    final Widget content = DecoratedBox(
      ...
      child: Center(
        child: ElText(
          fallback,
          fallbackSpec ?? ElComponentType.textSm,
    ...
    Widget circle = SizedBox(
      ...
      child: Stack(
        ...
        children: <Widget>[
          ...
          IgnorePointer(
            child: CustomPaint(
              painter: ElAvatarRimPainter(
                color: theme.border,
                lighten: theme.kind == ElThemeKind.dark,
              ),
            ),
          ),
          if (badge != null)
      ...
      circle = DecoratedBox(
        decoration: BoxDecoration(
          ...
          boxShadow: ElShadowSpec(<ElShadowLayer>[
            ElShadowLayer(0, 0, 0, ring!.width, (ElThemeData _) => ring!.color),
          ]).outerShadows(theme),
        ),
        child: circle,
      );
...
/// avatar's `data-size` through [ElAvatarSize.badge], exactly as the
...
class ElAvatarBadge {
  const ElAvatarBadge({required this.fill, this.child});
...
class ElAvatarGroup extends StatelessWidget {
  const ElAvatarGroup({super.key, required this.children});

  /// The avatars, then optionally a [ElAvatarGroupCount] last.
  final List<Widget> children;

  /// `-space-x-2` — 8px of overlap per neighbour.
  static double get overlap => el(2);

  /// The ring the group forces onto each child.
  static ElAvatarRing ringOf(BuildContext context) =>
      (color: ElTheme.of(context).background, width: elAvatarRingWidth);
...
class ElAvatarGroupCount extends StatelessWidget {
  const ElAvatarGroupCount(this.label, {super.key, this.spec});
  ...
  static double get diameter => el(8);
...
class ElAvatarRimPainter extends CustomPainter {
  const ElAvatarRimPainter({required this.color, required this.lighten});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final RRect hole = RRect.fromRectAndRadius(
      outer.deflate(ElWidths.hairline),
      Radius.circular(corner.x - ElWidths.hairline),
    );
    canvas.drawDRRect(
      ring,
      hole,
  ...
  @override
  bool shouldRepaint(ElAvatarRimPainter old) =>
