...
/// drives only the box. That is why [Avatar.sizePx] exists beside
/// [Avatar.size] — the same split `Icon.sizePx` carries.
///
...
enum AvatarSize {
  ...
  double get px => switch (this) {
    AvatarSize.sm => space(6),
    AvatarSize.md => space(8),
    AvatarSize.lg => space(10),
  };

  /// `group-data-[size=default]/avatar:size-2.5` and its two siblings — the
  /// diameter an [AvatarBadge] takes under this rung.
  double get badge => switch (this) {
    AvatarSize.sm => space(2),
    AvatarSize.md => space(2.5),
    AvatarSize.lg => space(3),
  };
}
...
typedef AvatarRing = ({Color color, double width});

/// `ring-2` — the width both ring sites use.
double get avatarRingWidth => 2;
    ...
    Widget circle = SizedBox(
      ...
      child: Stack(
        ...
        children: <Widget>[
          ...
          IgnorePointer(
            child: CustomPaint(
              painter: AvatarRimPainter(
                color: theme.border,
                lighten: theme.kind == ResolvedColorMode.dark,
              ),
            ),
          ),
          if (badge != null)
      ...
      circle = DecoratedBox(
        decoration: BoxDecoration(
          ...
          boxShadow: ShadowStyle(<ShadowLayer>[
            ShadowLayer(0, 0, 0, ring!.width, (ThemeTokens _) => ring!.color),
          ]).outerShadows(theme),
        ),
        child: circle,
      );
...
/// avatar's `data-size` through [AvatarSize.badge], exactly as the
...
class AvatarBadge {
  const AvatarBadge({required this.fill, this.child});
...
class AvatarGroup extends StatelessWidget {
  const AvatarGroup({super.key, required this.children});

  /// The avatars, then optionally a [AvatarGroupCount] last.
  final List<Widget> children;

  /// `-space-x-2` — 8px of overlap per neighbour.
  static double get overlap => space(2);

  /// The ring the group forces onto each child.
  static AvatarRing ringOf(BuildContext context) =>
      (color: ThemeScope.of(context).background, width: avatarRingWidth);
...
class AvatarGroupCount extends StatelessWidget {
  const AvatarGroupCount(this.label, {super.key, this.spec});
  ...
  static double get diameter => space(8);
...
class AvatarRimPainter extends CustomPainter {
  const AvatarRimPainter({required this.color, required this.lighten});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final RRect hole = RRect.fromRectAndRadius(
      outer.deflate(BorderWidths.hairline),
      Radius.circular(corner.x - BorderWidths.hairline),
    );
    canvas.drawDRRect(
      ring,
      hole,
  ...
  @override
  bool shouldRepaint(AvatarRimPainter old) =>
