...
/// — see [StarfieldCluster.tileOriginIn]. A port that rescaled would change
/// the population.
///
...
/// | toast | `.feedback-surface [data-content]::before / ::after` (L3403–3406) — a **descendant** combinator, not a child one, because sonner owns the live markup and may wrap the content slot; a `>` would work on the Elattar preview and silently do nothing on the real toast. |
///
...
/// entire toast.** That is why this widget is mounted by `FeedbackSurface`
...
/// [StarfieldCluster.matrixFor] is that one expression.
///
...
/// is sitting on — see [AmbientPattern.glowFor].
library;
...
// The two `--star-glow*` theme tokens live on `ThemeTokens`
...
/// conversion at all (`feedback_surface.dart`'s `_deepBlur`). Two lengths in one
/// `filter` list, two different meanings — and reading `drop-shadow` as a sigma
/// would double every glow on the page.
///
/// Unlike `ShadowLayer.blurRadius` there is nothing to invert here: that
...
/// ([IconPaths.sparkleElement]), so an instance is only ever three numbers
...
class Sparkle {
  ...
  Rect get box =>
      Rect.fromLTWH(x, y, IconPaths.viewBox * scale, IconPaths.viewBox * scale);
...
@immutable
class StarfieldCluster {
  const StarfieldCluster({
    required this.tile,
  ...
  /// `animation-duration` — [MotionDurations.sway] 44s here, [MotionDurations.swayAlt]
  ...
  Offset hoverTranslateAt(double u) {
    final double eased = MotionCurves.enter.transform(u.clamp(0.0, 1.0));
...
const StarfieldCluster starfieldDense = StarfieldCluster(
  ...
  sway: MotionDurations.sway,
  fromDegrees: -6,
  toDegrees: 6,
  hoverTranslate: Offset(-5, 3),
  hoverScale: 1.06,
  sparkles: <Sparkle>[
    Sparkle(opacity: 0.95, x: 214, y: 54, scale: 0.46),
    Sparkle(opacity: 0.78, x: 240, y: 24, scale: 0.26),
    Sparkle(opacity: 0.85, x: 180, y: 76, scale: 0.32),
    Sparkle(opacity: 0.66, x: 248, y: 72, scale: 0.20),
    Sparkle(opacity: 0.55, x: 196, y: 16, scale: 0.18),
    Sparkle(opacity: 0.58, x: 146, y: 40, scale: 0.22),
    Sparkle(opacity: 0.50, x: 232, y: 92, scale: 0.16),
    Sparkle(opacity: 0.42, x: 112, y: 66, scale: 0.15),
  ],
);
...
const StarfieldCluster starfieldThin = StarfieldCluster(
  ...
  sway: MotionDurations.swayAlt,
  fromDegrees: 5,
  toDegrees: -5,
  hoverTranslate: Offset(-3, -2),
  hoverScale: 1.04,
  sparkles: <Sparkle>[
    Sparkle(opacity: 0.78, x: 168, y: 18, scale: 0.24),
    Sparkle(opacity: 0.55, x: 132, y: 42, scale: 0.17),
    Sparkle(opacity: 0.60, x: 190, y: 46, scale: 0.18),
    Sparkle(opacity: 0.42, x: 96, y: 14, scale: 0.13),
    Sparkle(opacity: 0.38, x: 150, y: 60, scale: 0.12),
  ],
);
...
/// `position: absolute; inset: 0` means, and it is how `FeedbackSurface` mounts
...
/// sparkles'.
class AmbientPattern extends StatefulWidget {
  const AmbientPattern({super.key, required this.bloom2, this.hovered = false});
  ...
  static const List<StarfieldCluster> clusters = <StarfieldCluster>[
    starfieldDense,
    starfieldThin,
  ];
  ...
  /// literal on `FeedbackSurface.voidFor`'s precedent: CSS `white` *is*
  /// `hsl(0 0% 100%)`, and this file states no colour of its own.
  static final Color starInk = hslColor(0, 0, 100);
  ...
  static Color glowFor(ThemeTokens theme, Color bloom2) => switch (theme.kind) {
    ResolvedColorMode.dark => starInk.withValues(alpha: theme.starGlowMix),
    ResolvedColorMode.light => OklabColor.mix(
      bloom2,
      transparent,
      theme.starGlowMix,
    ),
  ...
  /// surface."*
  static ({double tight, double wide}) sigmasFor(ThemeTokens theme) => (
  ...
  /// assertion can reach: it falls out of [StarfieldCluster.tileOriginIn] and
  /// the host's height, and the map's whole *anchored, not tiled* note exists
  /// to stop a port from rescaling it away.
  @visibleForTesting
  static List<Sparkle> visibleIn(StarfieldCluster cluster, Size box) {
    final Offset origin = cluster.tileOriginIn(box);
    return <Sparkle>[
      for (final Sparkle sparkle in cluster.sparkles)
        if (sparkle.box.shift(origin).overlaps(Offset.zero & box)) sparkle,
    ];
  }

  @override
  State<AmbientPattern> createState() => _StarfieldState();
}

class _StarfieldState extends State<AmbientPattern>
  ...
  /// [build] re-reads both through [effectiveMotionDuration] on every pass, the way
  /// `Spinner` and `KeyframePlayer` do.
  late final List<AnimationController> _sways = <AnimationController>[
    for (final StarfieldCluster cluster in AmbientPattern.clusters)
  ...
  void didChangeDependencies() {
    ...
    final bool stilled =
        effectiveMotionDuration(context, MotionDurations.sway) == Duration.zero;
  ...
  @override
  void didUpdateWidget(AmbientPattern old) {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < _sways.length; i++) {
      _sways[i].duration = effectiveMotionDuration(
        context,
        AmbientPattern.clusters[i].sway,
      );
    }
        ...
        child: AnimatedBuilder(
          ...
          builder: (BuildContext context, Widget? child) => CustomPaint(
            painter: _StarfieldPainter(
              ...
              hoverT: _hover.value,
              stilled: stilled,
              glow: AmbientPattern.glowFor(theme, widget.bloom2),
              sigmas: AmbientPattern.sigmasFor(theme),
            ),
          ),
        ),
      ),
...
class _StarfieldPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < AmbientPattern.clusters.length; i++) {
      final StarfieldCluster cluster = AmbientPattern.clusters[i];
      canvas.save();
  ...
  void _paintCluster(Canvas canvas, Rect box, StarfieldCluster cluster) {
  ...
  void _paintSparkles(Canvas canvas, Rect box, StarfieldCluster cluster) {
    canvas.save();
    ...
    for (final Sparkle sparkle in cluster.sparkles) {
      ...
      canvas.drawPath(
        IconPaths.sparkle(),
        Paint()
          ..color = AmbientPattern.starInk.withValues(alpha: sparkle.opacity),
      );
      canvas.restore();
    }
    canvas.restore();
