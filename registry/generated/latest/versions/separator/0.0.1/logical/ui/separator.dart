...
/// A 1px `--border` rule.
class ElSeparator extends StatelessWidget {
  const ElSeparator({
    super.key,
    this.orientation = ElSeparatorOrientation.horizontal,
  });

  /// `<Separator orientation="vertical" />`.
  const ElSeparator.vertical({super.key})
    : orientation = ElSeparatorOrientation.vertical;

  final ElSeparatorOrientation orientation;

  /// `h-px` / `w-px` — the rule's thickness on its short axis.
  static double get thickness => ElWidths.hairline;

  @override
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final bool horizontal = orientation == ElSeparatorOrientation.horizontal;
