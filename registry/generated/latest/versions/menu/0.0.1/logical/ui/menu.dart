...
/// on [ElMenu] so a test can pin the sum rather than the total.
///
...
///     same answer `ElButton`'s geometry gives, reached the same way.
...
///     [ElPopover]'s own set — except that the slide runs on **all four**
...
/// [ElMenuIndicatorSide] is that drift, named.
library;
...
///
/// Modelled as one sealed family for the reason [ElSelectChild] is: the
...
@immutable
class ElMenuItem extends ElMenuChild {
  const ElMenuItem({
    required this.label,
    this.icon,
    this.lucideIcon,
    this.subtitle,
    this.shortcut,
    this.variant = ElMenuItemVariant.normal,
    this.enabled = true,
    this.inset = false,
    this.onSelect,
  });
  ...
  ///
  /// It is [ElCommandItem.subtitle]'s slot, one file over and on the same
  ...
  /// [ElIcon.lucide] beside `ElIcon`. Ignored when [icon] is given.
  final ElLucideGlyph? lucideIcon;

  /// `*MenuShortcut`'s content — *"a real number in the normal product face"*
  /// as often as a key hint.
  final String? shortcut;

  final ElMenuItemVariant variant;
...
@immutable
class ElMenuCheckboxItem extends ElMenuChild {
...
/// One `*MenuRadioItem`, inside a [ElMenuRadioGroup].
@immutable
class ElMenuRadioItem {
  const ElMenuRadioItem({
    required this.value,
    required this.label,
    this.enabled = true,
  });
...
/// Same "controlled with no handler" drift as [ElMenuCheckboxItem]: the page's
/// `<DropdownMenuRadioGroup value="value">` has no `onValueChange`.
@immutable
class ElMenuRadioGroup extends ElMenuChild {
  const ElMenuRadioGroup({
    required this.value,
  ...
  final String? value;
  final List<ElMenuRadioItem> children;
  final ValueChanged<String>? onChanged;
}

/// `*MenuLabel` — `px-3 py-2 text-xs font-medium text-muted-foreground`.
///
/// **Weight 500**, which is [ElComponentType.menuHeading] and not
/// [ElComponentType.menuLabel]: `SelectLabel` writes no `font-*` class and
/// inherits 400, this one writes `font-medium`. Same 12px rung, one weight
/// apart — selects-map drift 6, now with a fourth spelling.
@immutable
class ElMenuLabel extends ElMenuChild {
  const ElMenuLabel(this.text, {this.child, this.inset = false});
...
@immutable
class ElMenuSeparator extends ElMenuChild {
...
/// hang.
@immutable
class ElMenuGroup extends ElMenuChild {
  const ElMenuGroup({required this.children});
...
@immutable
class ElMenuSub extends ElMenuChild {
  const ElMenuSub({
    required this.label,
    required this.children,
    this.icon,
...
abstract final class ElMenu {
  ...
  ///
  /// Read off the type spec, not measured, exactly as [ElSelect.itemHeight] is
  ...
  /// A row carrying a [ElMenuItem.subtitle] — **52.7464**.
  ///
  ...
  ///
  /// `ElCommandItem`'s two-line row is **48.7** — the same two line boxes in the
  ...
  static double get twoLineItemHeight {
    final ElTypeSpec caption = ElType.caption;
    return itemHeight + el(1) + (caption.size ?? 0) * (caption.height ?? 1);
  }

  /// `*MenuLabel`'s `px-3 py-2 text-xs` — 12px in a 16px line box, so **32**.
  static double get labelHeight {
    final ElTypeSpec spec = ElComponentType.menuHeading;
    return (spec.size ?? 0) * (spec.height ?? 1) + el(2) * 2;
  }
  ...
  static double get minWidthSubDropdown => el(24);

  /// `data-inset:pl-9` — 36.
  static double get insetPadding => el(9);

  /// `[&_svg:not([class*='size-'])]:size-4` — the box every row icon is forced
  /// into, whatever `size` the `Icon` was asked for.
  static double get iconSize => el(4);
  ...
  static double get iconStroke => ElIcon.strokeFor(ElIcon.pxFor(ElIconSize.sm));
}
...
class ElMenuSurface extends StatelessWidget {
  const ElMenuSurface({
    super.key,
    required this.child,
    this.kind = ElMenuSurfaceKind.content,
  });

  final Widget child;
  final ElMenuSurfaceKind kind;

  @override
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    return switch (kind) {
      // `ElPopoverSurface` already *is* `shadow-md ring-1 ring-foreground/10`
      // over `--popover` at `rounded-lg`; the family's own recipe, reused
      // rather than restated.
      ElMenuSurfaceKind.content => ElPopoverSurface(child: child),
      ElMenuSurfaceKind.subRinged => ElPopoverSurface(
        shadow: ElShadows.tailwindLg,
        child: child,
      ),
      ElMenuSurfaceKind.subBordered => ElPopoverSurface(
        shadow: ElShadows.tailwindLg,
        ring: false,
...
class _Row {
  ...
  final _RowKind kind;
  final ElMenuItem? item;
  final ElMenuCheckboxItem? checkbox;
  final ElMenuRadioItem? radio;
  final ElMenuRadioGroup? radioGroup;
  final ElMenuLabel? label;
  final ElMenuSub? sub;
  ...
  /// is laid out. A [ElMenuLabel] with a custom [ElMenuLabel.child] is
  /// intrinsic and reports its plain-text height instead.
  double get height => switch (kind) {
    // The one row-like kind that can be two lines tall: only [ElMenuItem]
    // carries a subtitle, and a check row, a radio row and a sub-trigger
    // have no slot for one.
    _RowKind.item =>
      item!.subtitle == null ? ElMenu.itemHeight : ElMenu.twoLineItemHeight,
    _RowKind.checkbox || _RowKind.radio || _RowKind.sub => ElMenu.itemHeight,
    _RowKind.label => ElMenu.labelHeight,
    _RowKind.separator => ElMenu.separatorHeight,
  };
}

/// Every row in document order, with the focusable ones indexed.
class _MenuGeometry {
  _MenuGeometry(List<ElMenuChild> children) {
    void walk(List<ElMenuChild> list) {
      for (final ElMenuChild child in list) {
        switch (child) {
          case ElMenuItem():
            _add(_RowKind.item, enabled: child.enabled, item: child);
          case ElMenuCheckboxItem():
            _add(_RowKind.checkbox, enabled: child.enabled, checkbox: child);
          case ElMenuRadioGroup():
            for (final ElMenuRadioItem row in child.children) {
              _add(
                _RowKind.radio,
                enabled: row.enabled,
                radio: row,
  ...
  void _add(
    ...
    ElMenuItem? item,
    ElMenuCheckboxItem? checkbox,
    ElMenuRadioItem? radio,
    ElMenuRadioGroup? radioGroup,
    ElMenuLabel? label,
    ElMenuSub? sub,
  }) {
...
/// three hand it to [ElPopover].
class ElMenuContent extends StatefulWidget {
  const ElMenuContent({
    super.key,
    required this.children,
    ...
    this.minWidth,
    this.kind = ElMenuSurfaceKind.content,
    this.indicatorSide = ElMenuIndicatorSide.end,
    this.autofocus = true,
  ...
  /// `min-w-*`. Null takes [ElMenu.minWidthMenu].
  ///
  ...
  final ElMenuIndicatorSide indicatorSide;
  ...
  static double heightOf(List<ElMenuChild> children) =>
      _MenuGeometry(children).contentHeight;

  @override
  State<ElMenuContent> createState() => _ElMenuContentState();
}

class _ElMenuContentState extends State<ElMenuContent> {
  late _MenuGeometry _menu = _MenuGeometry(widget.children);

  final FocusNode _focus = FocusNode(debugLabel: 'ElMenuContent');
  ...
  @override
  void didUpdateWidget(ElMenuContent oldWidget) {
  ...
  /// opposite of what `ElSelect`'s own menu does.
  void _move(int step) {
  ...
  KeyEventResult _onKey(FocusNode node, KeyEvent event) {
    ...
    switch (key) {
      ...
      case LogicalKeyboardKey.arrowLeft:
        ...
        // content lets it past — [ElMenuContent.onEscape] is what tells the two
      ...
      child: _sized(
        ElMenuSurface(
          ...
          child: Padding(
            ...
            padding: EdgeInsets.symmetric(vertical: ElMenu.contentPadding),
            child: column,
          ),
        ),
      ),
  ...
  Widget _sized(Widget child) {
    ...
    return ConstrainedBox(
      constraints: BoxConstraints(
        minWidth: widget.minWidth ?? ElMenu.minWidthMenu,
      ),
  ...
  Widget _pad(Widget child) => Padding(
    padding: EdgeInsets.symmetric(horizontal: ElMenu.contentPadding),
    child: child,
  );

  Widget _buildRow(BuildContext context, ElThemeData theme, _Row row) {
    switch (row.kind) {
      case _RowKind.separator:
      ...
      case _RowKind.item:
        final ElMenuItem item = row.item!;
        return _pad(
          _MenuRow(
            theme: theme,
            label: item.label,
      ...
      case _RowKind.checkbox:
        final ElMenuCheckboxItem item = row.checkbox!;
        return _pad(
          _MenuRow(
            theme: theme,
            label: item.label,
      ...
      case _RowKind.radio:
        final ElMenuRadioItem item = row.radio!;
        return _pad(
          _MenuRow(
            theme: theme,
            label: item.label,
      ...
      case _RowKind.sub:
        ...
        final Widget trigger = _MenuRow(
          ...
          label: sub.label,
          leading: sub.icon,
          trailing: ElIconGlyph.chevronRight,
          enabled: sub.enabled,
          inset: sub.inset,
        ...
        return _pad(
          ElPopover(
            ...
            slideSides: ElPopoverSide.values.toSet(),
            // One dismissable branch with its parent — see [ElPopoverBarrier].
            barrier: ElPopoverBarrier.none,
            onDismiss: _closeSub,
            content: (BuildContext context, ElPopoverAnchorMetrics metrics) =>
                ElMenuContent(
                  children: sub.children,
                  kind: widget.kind == ElMenuSurfaceKind.content
                      ? ElMenuSurfaceKind.subBordered
                      : widget.kind,
                  minWidth: ElMenu.minWidthSub,
                  indicatorSide: widget.indicatorSide,
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.only(
        left: label.inset ? ElMenu.insetPadding : el(3),
        right: el(3),
        top: el(2),
        bottom: el(2),
      ),
...
class _MenuRow extends StatelessWidget {
  ...
  final ElIconGlyph? leading;

  /// The same slot over the generated registry — see [ElMenuItem.lucideIcon].
  final ElLucideGlyph? leadingLucide;

  /// `ChevronRightIcon className="ml-auto"` on a sub-trigger.
  final ElIconGlyph? trailing;

  /// [ElMenuItem.subtitle] — the row's second line.
  final String? subtitle;
  ...
  bool get _destructive => variant == ElMenuItemVariant.destructive;
  ...
  Widget build(BuildContext context) {
    ...
    final double leadingPad = inset
        ? ElMenu.insetPadding
        : check && indicatorSide == ElMenuIndicatorSide.start
        ? ElMenu.insetPadding
        : el(3);
    final double trailingPad = check && indicatorSide == ElMenuIndicatorSide.end
        ? el(9)
        : el(3);
    ...
    Widget row = Padding(
      ...
      child: Row(
        ...
        children: <Widget>[
            ...
            child: Row(
              ...
              children: <Widget>[
                if (leading != null || leadingLucide != null) ...<Widget>[
                  ...
                  DefaultTextStyle.merge(
                    ...
                    child: leading != null
                        ? ElIcon(
                            leading!,
                            sizePx: ElMenu.iconSize,
                            strokeOverride: ElMenu.iconStroke,
                          )
                        : ElIcon.lucide(
                            leadingLucide!,
                            sizePx: ElMenu.iconSize,
                ...
                Flexible(
                  ...
                  // it carries a [ElMenuItem.subtitle] — `flex-col items-start
                    ...
                    children: <Widget>[
                      ...
                      if (subtitle != null) ...<Widget>[
                        ...
                        ElText(
                          ...
                          // `ElCommandItem.meta`'s does one file over.
                          color: theme.mutedForeground,
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          softWrap: false,
          ...
          if (shortcut != null)
            Padding(
              padding: EdgeInsetsDirectional.only(start: el(2)),
              child: ElText(
                shortcut!,
                ElComponentType.menuShortcut,
                color: highlighted
                    ? theme.accentForeground
                    : theme.mutedForeground,
                maxLines: 1,
      ...
      row = Stack(
        ...
        children: <Widget>[
          ...
          if (checked!)
            Positioned.fill(
              child: Align(
                alignment: indicatorSide == ElMenuIndicatorSide.end
                ...
                child: Padding(
                  ...
                  padding: EdgeInsetsDirectional.only(
                    start: indicatorSide == ElMenuIndicatorSide.start
                        ? el(1.5)
                        : 0,
                    end: indicatorSide == ElMenuIndicatorSide.end ? el(3) : 0,
                  ),
    ...
    row = SizedBox(
      height: subtitle == null ? ElMenu.itemHeight : ElMenu.twoLineItemHeight,
      child: row,
    );
...
/// `onPressed`.
class ElMenuPointerDown extends StatelessWidget {
  const ElMenuPointerDown({
    super.key,
    required this.child,
...
abstract final class ElMenuMotion {
  /// `data-open:animate-in` and friends.
  static Duration get duration => ElDurations.overlay;

  /// All four `data-[side=*]:slide-in-from-*` utilities.
  static Set<ElPopoverSide> get slideSides => ElPopoverSide.values.toSet();
}
