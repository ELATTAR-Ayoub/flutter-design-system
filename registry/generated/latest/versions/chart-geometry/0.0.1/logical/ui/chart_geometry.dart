...
({double min, double max}) elChartNiceDomain(
  double min,
  double max, {
  int tickCount = 5,
  bool allowDecimals = true,
}) {
  final List<double> ticks = elChartNiceTicks(
    min,
    max,
    tickCount: tickCount,
...
class ElLinearScale {
  ...
  factory ElLinearScale.nice({
    ...
    if (lo == hi) hi = lo + 1;
    final ({double min, double max}) d = elChartNiceDomain(
      lo,
      hi,
      tickCount: tickCount,
    );
    return ElLinearScale(
      domainMin: d.min,
      domainMax: d.max,
  ...
  List<double> ticks({int tickCount = 5}) =>
      elChartNiceTicks(domainMin, domainMax, tickCount: tickCount);
}

/* ── Bar layout ──────────────────────────────────────────────────────────── */

/// One `Bar`'s offset inside its band, and its width.
typedef ElBarSlot = ({double offset, double size});
...
List<ElBarSlot> elBarSlots({
  required double bandSize,
  required int barCount,
  double barCategoryGapRatio = 0.1,
  double barGap = 4,
}) {
  if (barCount < 1) return const <ElBarSlot>[];
...
RRect elBarRRect(Rect rect, List<double> radii) {
...
enum ElCurveType { linear, natural, monotone, step }
...
Path elCurvePath(List<Offset> points, ElCurveType type) {
...
Path elAreaPath(List<Offset> top, List<Offset> base, ElCurveType type) {
...
void _emitCurve(
  ...
  switch (type) {
    case ElCurveType.linear:
      start(pts.first);
      for (int i = 1; i < pts.length; i++) {
        path.lineTo(pts[i].dx, pts[i].dy);
      }
    case ElCurveType.step:
      start(pts.first);
      for (int i = 1; i < pts.length; i++) {
    ...
    case ElCurveType.natural:
      _emitNatural(path, pts, start);
    case ElCurveType.monotone:
...
/// instead.
Offset elPolarToCartesian(double cx, double cy, double radius, double angle) {
...
double elPolarMaxRadius(double width, double height, {double inset = 10}) =>
...
Path elSectorPath({
  ...
  final Path path = Path();
  final Offset outerStart = elPolarToCartesian(cx, cy, outerRadius, startAngle);
  path.moveTo(outerStart.dx, outerStart.dy);
  _arc(path, cx, cy, outerRadius, startAngle, tempEnd);
  if (innerRadius > 0) {
    final Offset innerEnd = elPolarToCartesian(cx, cy, innerRadius, tempEnd);
...
_Tangent _tangentCircle(
  ...
  final Offset circleTangency = elPolarToCartesian(cx, cy, radius, centerAngle);
  final Offset lineTangency = elPolarToCartesian(
    cx,
    cy,
...
void _arc(
  ...
  // same statement `elPolarToCartesian` already makes.
...
Path elPolarPolygonPath(List<Offset> points) {
  final Path path = Path();
