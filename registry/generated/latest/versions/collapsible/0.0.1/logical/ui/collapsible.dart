...
class ElUnfold extends StatefulWidget {
  const ElUnfold({super.key, required this.open, required this.child});
  ...
  @override
  State<ElUnfold> createState() => _ElUnfoldState();
}

class _ElUnfoldState extends State<ElUnfold>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controller = AnimationController(
    vsync: this,
    duration: ElDurations.jelly,
  ...
  late final Animation<double> _curve = CurvedAnimation(
    ...
    reverseCurve: FlippedCurve(ElCurves.inOut),
  );

  @override
  void didUpdateWidget(ElUnfold old) {
    super.didUpdateWidget(old);
    if (widget.open == old.open) return;
    _controller.duration = elAnimationDuration(context, ElDurations.jelly);
...
/// [ElUnfold]; this widget owns the stacking and nothing else.
class ElCollapsible extends StatelessWidget {
  const ElCollapsible({
    super.key,
    required this.open,
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: <Widget>[
        trigger,
        ElUnfold(open: open, child: content),
      ],
    );
  }
}
