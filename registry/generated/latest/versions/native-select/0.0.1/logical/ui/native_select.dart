...
/// | the **open list** | **DIVERGENT.** The port mounts its own [ElSelectMenu] — `SelectContent`'s surface, rows, tick and keyboard — where the OS would have drawn its picker. `<option class="bg-[Canvas] text-[CanvasText]">` asks for the platform's system colours; the port paints `--popover` |
///
...
/// committing. `ElSelect` — a Radix menu — opens on the arrows instead, and the
...
/// | property | value |
/// |---|---|
/// | height | **32px** (`h-8`) — the `ElSelect` beside it is 40. selects-map drift 8 |
...
/// list is a [ElSelectChild] list and [ElSelectGroup] is one of its members.
library;
...
/// is the same menu.
double get _menuMinWidth => el(36);

/// The two rungs of `data-size` on the control.
enum ElNativeSelectSize {
  /// `data-[size=sm]` — `h-7`, `rounded-[min(var(--radius-md),10px)]`, `py-0.5`.
  sm,

  /// The default — `h-8`, `rounded-lg`, `py-1`. Named [md] because `default` is
  /// a Dart keyword, the same rename [ElSelectSize] carries.
  md;

  /// The attribute value the reference writes.
  String get label => this == ElNativeSelectSize.md ? 'default' : 'sm';

  /// `h-8` / `h-7`.
  double get height => this == ElNativeSelectSize.md ? el(8) : el(7);

  /// `rounded-lg`, or the `min()` the `sm` rung asks for — which resolves to
  /// `--radius-md` because 10 is the smaller of the two.
  double get radius => this == ElNativeSelectSize.md ? ElRadii.lg : ElRadii.md;

  /// `py-1` / `py-0.5`. Inert against a fixed height, transcribed anyway.
  double get insetY => this == ElNativeSelectSize.md ? el(1) : el(0.5);
}

/// The operating system's picker, as far as Flutter can carry it.
class ElNativeSelect<T> extends StatefulWidget {
  const ElNativeSelect({
    super.key,
    required this.options,
    required this.value,
    required this.onChanged,
    this.size = ElNativeSelectSize.md,
    this.enabled = true,
    this.invalid = false,
    this.expand = false,
    this.width,
  ...
  /// same switch, as [ElSelect.expand].
  final bool expand;

  /// An explicit measure, which beats both [expand] states — [ElSelect.width]'s
  ...
  static double get menuOffset => el(1);

  @override
  State<ElNativeSelect<T>> createState() => _ElNativeSelectState<T>();
}

class _ElNativeSelectState<T> extends State<ElNativeSelect<T>> {
  ...
  FocusNode get _focusNode =>
      widget.focusNode ??
      _scope?.focusNode ??
      (_ownedFocusNode ??= FocusNode(debugLabel: 'ElNativeSelect'));
  ...
  /// flattens them.
  List<ElSelectOption<T>> get _flat {
    final List<ElSelectOption<T>> flat = <ElSelectOption<T>>[];
    for (final ElSelectChild<T> child in widget.options) {
      switch (child) {
  ...
  int get _selectedIndex {
    final List<ElSelectOption<T>> flat = _flat;
  ...
  /// The open list's highlight, which does wrap — it is [ElSelectMenu], and the
  /// menu is Radix's.
  void _move(int step) {
    final List<ElSelectOption<T>> flat = _flat;
  ...
  Color _fill(ElThemeData theme) {
    if (theme.kind == ElThemeKind.light) return elTransparent;
    return theme.input.withValues(
      alpha: _hovered ? _darkHoverFillAlpha : _darkFillAlpha,
    );
  }

  Color _border(ElThemeData theme) {
    if (_invalid) {
  ...
  Widget build(BuildContext context) {
    ...
    // else it could show.
    final ElSelectOption<T>? shown = flat.isEmpty ? null : flat[_selectedIndex];
                ...
                TweenAnimationBuilder<Color?>(
                  ...
                  builder: (BuildContext context, Color? ring, Widget? child) {
                    ...
                    return ElMachineSurface(
                      ...
                      spec: ringColor.a == 0
                          ? ElShadows.none
                          : ElButton.withFocusRing(ElShadows.none, ringColor),
                      radius: BorderRadius.circular(widget.size.radius),
                      fill: fill ?? elTransparent,
                      border: Border.all(
        ...
        children: <Widget>[
          ...
          Positioned.fill(
            child: Align(
              ...
              child: Padding(
                padding: EdgeInsets.only(right: el(2.5)),
                child: const ElIcon(
    ...
    // picker appears. `ElPopover` supplies the positioner and the collision
    // flip; `ElSelectMenu` supplies everything inside it.
    control = ElPopover(
      open: _open,
      side: ElPopoverSide.bottom,
      align: ElPopoverAlign.start,
      sideOffset: ElNativeSelect.menuOffset,
