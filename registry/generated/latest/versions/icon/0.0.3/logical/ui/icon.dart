...
class Icon extends StatelessWidget {
  ...
  /// enum.
  ///
  /// [IconGlyph] is the icons page's own whitelist — the glyphs that page
  ...
  ///
  /// So this constructor takes a [LucideGlyph] from `icon_paths.g.dart` and
  ...
  /// `ICON_SIZES[size]`.
  static double pxFor(IconSize size) => switch (size) {
    IconSize.xs => 12,
    IconSize.sm => 14,
    IconSize.md => 16,
    IconSize.lg => 20,
  ...
  static double strokeFor(double px) {
    final double scaled = (_authoredStroke * IconPaths.viewBox) / px;
  ...
  ///
  /// [path] and [fill] are in lucide's 24-unit space ([IconPaths.viewBox]);
  ...
  /// registry rather than in [IconGlyph].
  @visibleForTesting
  static void paintGlyph(
    ...
    canvas.clipRect(
      const Rect.fromLTWH(0, 0, IconPaths.viewBox, IconPaths.viewBox),
    );
    canvas.drawPath(
      path,
      Paint()
    ...
    // skips this entirely because [IconPaths.fillPathFor] returns null.
    if (fill != null) {
  ...
  ///
  /// [IconTone.inherit] is `text-current`: it takes the colour of the
  /// surrounding text, which in Flutter is the nearest [DefaultTextStyle] —
  /// how a `Button` colours the icon inside it — falling back to
  /// `--foreground`, the colour `<body>` sets.
  static Color colorFor(BuildContext context, IconTone tone) {
    final ThemeTokens theme = ThemeScope.of(context);
    return switch (tone) {
      IconTone.normal => theme.foreground,
      IconTone.muted || IconTone.subtle => theme.mutedForeground,
      IconTone.action => theme.actionText,
      IconTone.value => theme.premiumText,
      IconTone.success => theme.successText,
      IconTone.warning => theme.warningText,
      IconTone.info => theme.infoText,
      IconTone.error => theme.destructiveText,
      IconTone.inherit =>
...
class _GlyphPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) => Icon.paintGlyph(
    canvas,
    size,
    path: glyph != null ? IconPaths.pathFor(glyph!) : lucide!.toPath(),
    fill: glyph != null ? IconPaths.fillPathFor(glyph!) : lucide!.toFillPath(),
    color: color,
    strokeWidth: strokeWidth,
  );
