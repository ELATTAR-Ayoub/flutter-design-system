...
/// `data-size="default"`.)* Reproduced as measured; see [ElItemGroup.gap].
///
/// ## The rest, measured
///
...
/// [ElItemVariant] and [ElItem.alignStart] are built rather than recorded.
library;
...
class ElItemGroup extends StatelessWidget {
  const ElItemGroup({super.key, required this.children, this.gapOverride});
...
/// One row.
class ElItem extends StatelessWidget {
  const ElItem({
    super.key,
    this.media,
    required this.content,
    this.actions,
    this.variant = ElItemVariant.normal,
  ...
  final ElItemVariant variant;

  /// `items-start`, written at the call site over the base list's
  /// `items-center`.
  ///
  /// It moves the **actions** and nothing else: [ElItemMedia] is already
  /// `self-start` whenever the row has a description, and [ElItemContent] fills
  ...
  static EdgeInsets get padding =>
      EdgeInsets.symmetric(horizontal: el(3), vertical: el(2.5));
  ...
  Widget build(BuildContext context) {
    ...
    final Color stroke = switch (variant) {
      ElItemVariant.normal || ElItemVariant.muted => elTransparent,
      ElItemVariant.outline => theme.border,
    };
    return Container(
      padding: padding,
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(radius),
        color: variant == ElItemVariant.muted
            ? theme.muted.withValues(alpha: mutedFillAlpha)
            : null,
        border: Border.all(color: stroke, width: ElWidths.hairline),
      ),
  ...
  /// `bg-muted/50` on [ElItemVariant.muted]. RECORDED, NOT BUILT beyond this
...
class ElItemMedia extends StatelessWidget {
  const ElItemMedia({super.key, required this.child, this.nudged = true});
  ...
  static double get size => el(4);

  /// `translate-y-0.5`.
  static double get nudge => el(0.5);

  @override
  Widget build(BuildContext context) {
    final Widget box = SizedBox.square(dimension: size, child: child);
    // Inside [ElItem] this sits in a stretched slot, so the alignment has a
...
class ElItemContent extends StatelessWidget {
  const ElItemContent({super.key, required this.children});
...
class ElItemTitle extends StatelessWidget {
  const ElItemTitle(this.text, {super.key});
...
class ElItemDescription extends StatelessWidget {
  const ElItemDescription(this.text, {super.key});
  ...
  Widget build(BuildContext context) => ElText(
    text,
    ElComponentType.itemDescription,
...
class ElItemActions extends StatelessWidget {
  const ElItemActions({super.key, required this.children});
