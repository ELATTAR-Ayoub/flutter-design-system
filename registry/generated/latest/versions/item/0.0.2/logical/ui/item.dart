...
/// `data-size="default"`.)* Reproduced as measured; see [ItemGroup.gap].
///
/// ## The rest, measured
///
...
/// [ItemVariant] and [Item.alignStart] are built rather than recorded.
library;
...
class ItemGroup extends StatelessWidget {
  const ItemGroup({super.key, required this.children, this.gapOverride});
...
class Item extends StatelessWidget {
  const Item({
    ...
    this.actions,
    this.variant = ItemVariant.normal,
  ...
  /// `items-center`.
  ///
  /// It moves the **actions** and nothing else: [ItemMedia] is already
  /// `self-start` whenever the row has a description, and [ItemContent] fills
  ...
  static EdgeInsets get padding =>
      EdgeInsets.symmetric(horizontal: space(3), vertical: space(2.5));
  ...
  Widget build(BuildContext context) {
    ...
    final Color stroke = switch (variant) {
      ItemVariant.normal || ItemVariant.muted => transparent,
      ItemVariant.outline => theme.border,
    };
    return Container(
      padding: padding,
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(radius),
        color: variant == ItemVariant.muted
            ? theme.muted.withValues(alpha: mutedFillAlpha)
            : null,
        border: Border.all(color: stroke, width: BorderWidths.hairline),
      ),
  ...
  /// `bg-muted/50` on [ItemVariant.muted]. RECORDED, NOT BUILT beyond this
...
class ItemTitle extends StatelessWidget {
  const ItemTitle(this.text, {super.key});
...
class ItemDescription extends StatelessWidget {
  const ItemDescription(this.text, {super.key});
