...
/// a file"*. All three are here: [ElCalendar] and [ElDatePicker].
///
...
///    `1rem`. See [ElCalendarType.dayNumber].
...
/// neither of the first two. [ElCalendar] reproduces that, resolving `today`
/// through [ElClock] so the capture rig can freeze both renderers on one
...
/// rounded chip. It is [ElCalendarBandPainter], and it issues at most two draw
...
///
/// Both ends are calendar dates, never instants: see [ElCalendarDay.of].
@immutable
class ElDateRange {
  const ElDateRange({this.from, this.to});
  ...
  bool includes(DateTime day) {
    ...
    final DateTime d = ElCalendarDay.of(day);
    final DateTime a = ElCalendarDay.of(start);
    final DateTime b = to == null ? a : ElCalendarDay.of(to!);
    return !d.isBefore(a) && !d.isAfter(b);
  }

  /// `modifiers.range_start`.
  bool isStart(DateTime day) =>
      from != null && ElCalendarDay.isSameDay(day, from!);

  /// `modifiers.range_end`.
  bool isEnd(DateTime day) => to != null && ElCalendarDay.isSameDay(day, to!);
  ...
  static ElDateRange? addToRange(DateTime date, ElDateRange? initial) {
    final DateTime d = ElCalendarDay.of(date);
    final DateTime? from = initial?.from;
    final DateTime? to = initial?.to;

    if (from == null && to == null) {
      // `range = { from: date, to: min > 0 ? undefined : date }`, and min is 0.
      return ElDateRange(from: d, to: d);
    }
    if (from != null && to == null) {
      if (ElCalendarDay.isSameDay(from, d)) {
        return ElDateRange(from: from, to: d);
      }
      if (d.isBefore(ElCalendarDay.of(from))) {
        return ElDateRange(from: d, to: ElCalendarDay.of(from));
      }
      return ElDateRange(from: from, to: d);
    }
    // from != null && to != null
    final DateTime a = ElCalendarDay.of(from!);
    final DateTime b = ElCalendarDay.of(to!);
    if (ElCalendarDay.isSameDay(a, d) && ElCalendarDay.isSameDay(b, d)) {
      // `required` is false on this page, so the whole selection clears.
      return null;
    }
    if (ElCalendarDay.isSameDay(a, d)) return ElDateRange(from: a, to: d);
    if (ElCalendarDay.isSameDay(b, d)) return ElDateRange(from: d, to: d);
    if (d.isBefore(a)) return ElDateRange(from: d, to: b);
    return ElDateRange(from: a, to: d);
  }

  @override
  bool operator ==(Object other) =>
      other is ElDateRange && other.from == from && other.to == to;

  @override
  int get hashCode => Object.hash(from, to);

  @override
  String toString() => 'ElDateRange($from → $to)';
}
...
@immutable
class ElCalendarMonth {
  ElCalendarMonth(DateTime month) : month = ElCalendarDay.startOfMonth(month);
  ...
  int get leadingDays => ElDateFormat.weekIndex(month);

  /// How many days the month itself holds.
  int get dayCount => ElCalendarDay.daysInMonth(month);
  ...
  List<DateTime> get days {
    final DateTime first = ElCalendarDay.addDays(month, -leadingDays);
    return <DateTime>[
      for (int i = 0; i < weekCount * 7; i++) ElCalendarDay.addDays(first, i),
    ];
  }
  ...
  /// paragraph knows — [ElComponentType.textSm] at 13px over 1.428571, or
  ...
  double outerHeight(double weekdayRowHeight, ElCalendarSurface surface) =>
      _cellSize +
      _monthGap +
  ...
  static double paddingFor(ElCalendarSurface surface) =>
      surface == ElCalendarSurface.card ? el(3) : el(2);

  /// Only the Panel calendar draws one.
  static double borderWidthFor(ElCalendarSurface surface) =>
      surface == ElCalendarSurface.card ? ElWidths.hairline : 0;

  @override
  bool operator ==(Object other) =>
      other is ElCalendarMonth && other.month == month;
...
/// A month grid.
///
/// Use [ElCalendar.single] or [ElCalendar.range]; the two differ only in what
/// they hold and what they report.
class ElCalendar extends StatefulWidget {
  /// `mode="single"` — §5's Panel specimen and §7's popover.
  const ElCalendar.single({
    super.key,
    DateTime? selected,
    ...
    this.defaultMonth,
    this.onMonthChanged,
    this.surface = ElCalendarSurface.card,
    this.autoFocus = false,
  }) : mode = ElCalendarMode.single,
  ...
  const ElCalendar.range({
    super.key,
    ElDateRange? selected,
    ValueChanged<ElDateRange?>? onSelected,
    this.month,
    this.defaultMonth,
    this.onMonthChanged,
    this.surface = ElCalendarSurface.card,
    this.autoFocus = false,
  }) : mode = ElCalendarMode.range,
       selectedRange = selected,
       onRangeSelected = onSelected,
       selectedDay = null,
       onDaySelected = null;

  final ElCalendarMode mode;
  ...
  final ElDateRange? selectedRange;

  /// `onSelect` in range mode, already run through [ElDateRange.addToRange].
  final ValueChanged<ElDateRange?>? onRangeSelected;

  /// `month` — a controlled displayed month. **The page passes none.**
  final DateTime? month;

  /// `defaultMonth` — the uncontrolled seed. **The page passes none**, which
  /// is what makes [ElClock] load-bearing (ruling L2).
  ...
  final ElCalendarSurface surface;
  ...
  @override
  State<ElCalendar> createState() => _ElCalendarState();
}

class _ElCalendarState extends State<ElCalendar> {
  /// The displayed month. Null until [didChangeDependencies] resolves
  /// `getInitialMonth`, which needs the [ElClock] and therefore an inherited
  ...
  late final FocusNode _focus = FocusNode(debugLabel: 'ElCalendar');
  ...
  void didChangeDependencies() {
    ...
    // not.
    _month ??= ElCalendarDay.startOfMonth(
      widget.month ?? widget.defaultMonth ?? ElClock.nowOf(context),
    );
  }

  @override
  void didUpdateWidget(ElCalendar old) {
    super.didUpdateWidget(old);
    final DateTime? controlled = widget.month;
    if (controlled != null && controlled != old.month) {
      _month = ElCalendarDay.startOfMonth(controlled);
    }
  }
  ...
  DateTime get _displayed =>
      _month ?? ElCalendarDay.startOfMonth(DateTime.now());

  void _goToMonth(DateTime month) {
    final DateTime next = ElCalendarDay.startOfMonth(month);
  ...
  void _select(DateTime day) {
    ...
    switch (widget.mode) {
      case ElCalendarMode.single:
        ...
        onSelected(
          current != null && ElCalendarDay.isSameDay(current, d) ? null : d,
        );
      case ElCalendarMode.range:
        final ValueChanged<ElDateRange?>? onSelected = widget.onRangeSelected;
        if (onSelected == null) return;
        onSelected(ElDateRange.addToRange(d, widget.selectedRange));
    }
  }
  ...
  KeyEventResult _onKey(FocusNode node, KeyEvent event) {
    ...
    switch (event.logicalKey) {
      case LogicalKeyboardKey.arrowLeft:
        next = ElCalendarDay.addDays(anchor, -1);
      case LogicalKeyboardKey.arrowRight:
        next = ElCalendarDay.addDays(anchor, 1);
      case LogicalKeyboardKey.arrowUp:
        next = ElCalendarDay.addDays(anchor, -7);
      case LogicalKeyboardKey.arrowDown:
        next = ElCalendarDay.addDays(anchor, 7);
      case LogicalKeyboardKey.home:
        next = ElCalendarDay.addDays(anchor, -ElDateFormat.weekIndex(anchor));
      case LogicalKeyboardKey.end:
        next = ElCalendarDay.addDays(
          anchor,
          6 - ElDateFormat.weekIndex(anchor),
        );
      case LogicalKeyboardKey.pageUp:
        next = ElCalendarDay.addMonths(anchor, -1);
    ...
    if (!ElCalendarDay.isSameMonth(next, _displayed)) _goToMonth(next);
    return KeyEventResult.handled;
  }
  ...
  DateTime _defaultFocus() {
    final DateTime? day = switch (widget.mode) {
      ElCalendarMode.single => widget.selectedDay,
      ElCalendarMode.range => widget.selectedRange?.from,
    };
    if (day != null && ElCalendarDay.isSameMonth(day, _displayed)) {
      return ElCalendarDay.of(day);
    }
    return _displayed;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final ElCalendarMonth grid = ElCalendarMonth(_displayed);
    final DateTime today = ElCalendarDay.of(ElClock.nowOf(context));
    final List<DateTime> days = grid.days;

    final TextStyle weekdayStyle = ElText.styleOf(
      context,
    ...
    final Widget month = Column(
      ...
      children: <Widget>[
        _Caption(
          month: _displayed,
          onPrevious: () => _goToMonth(ElCalendarDay.addMonths(_displayed, -1)),
          onNext: () => _goToMonth(ElCalendarDay.addMonths(_displayed, 1)),
        ),
  ...
  Widget _dayCell(DateTime day, int column, DateTime today) {
    final ElDateRange? range = widget.mode == ElCalendarMode.range
        ? widget.selectedRange
        : null;
    final bool selectedSingle =
        widget.mode == ElCalendarMode.single &&
        widget.selectedDay != null &&
        ElCalendarDay.isSameDay(day, widget.selectedDay!);

    return _DayCell(
      day: day,
      column: column,
      outside: !ElCalendarDay.isSameMonth(day, _displayed),
      today: ElCalendarDay.isSameDay(day, today),
      ...
      focused:
          _focusedDay != null &&
          ElCalendarDay.isSameDay(day, _focusedDay!) &&
          _focus.hasFocus,
      onPressed: () {
        _focus.requestFocus();
        _select(day);
...
class _CalendarSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color fill = switch (surface) {
      ElCalendarSurface.background => theme.background,
      ElCalendarSurface.card => theme.card,
      ElCalendarSurface.popover => elTransparent,
    };
    final bool bordered = surface == ElCalendarSurface.card;
    return DecoratedBox(
      decoration: BoxDecoration(
        color: fill,
        border: bordered
            ? Border.all(color: theme.border, width: ElWidths.hairline)
            : null,
      ...
      child: Padding(
        ...
        padding: EdgeInsets.all(
          ElCalendarMonth.paddingFor(surface) +
              ElCalendarMonth.borderWidthFor(surface),
        ),
        child: child,
      ),
    );
...
/// it goes through [ElText] rather than a [SizedBox]: `ElLineBox` is what
      ...
      children: <Widget>[
        for (int column = 0; column < 7; column++)
          SizedBox(
            ...
            child: Center(
              child: ElText(
                ElDateFormat.weekdayNarrow(column),
                ElComponentType.textSm,
                color: style.color,
              ),
            ),
...
/// `size-(--cell-size) p-0`.
///
/// Not a [ElButton]: the size ladder's four squares are 24 / 32 / 40 / 48 and
...
class _NavButtonState extends State<_NavButton> {
  ...
  Widget build(BuildContext context) {
    ...
    // `ElButtonSize` squares (24 / 32 / 40 / 48).
    return SizedBox(
      width: _cellSize,
      height: _cellSize,
...
/// implies — the same reading `ElButton._skin` documents.
Color _ghostFill(
  ElThemeData theme, {
  ...
  return elTransparent;
}

/// `ghost`'s ink: `text-muted-foreground`, `hover:text-foreground`.
Color _ghostInk(ElThemeData theme, {required bool hovered}) =>
...
class _DayCellState extends State<_DayCell> {
  ...
  BorderRadius _radius() {
    final Radius pill = Radius.circular(ElRadii.pill);
    final Radius cell = Radius.circular(ElRadii.md);
    Radius topLeft;
    Radius topRight;
    Radius bottomRight;
    Radius bottomLeft;
        ...
        children: <Widget>[
          ...
          Positioned.fill(
            child: _PressableCell(
              ...
              onPress: (bool value) => setState(() => _pressed = value),
              pressed: _pressed,
              label: ElDateFormat.dayLabel(widget.day),
              child: _CellSurface(
                fill: _fill(theme),
                ink: _ink(theme),
                radius: _radius(),
                ...
                child: Center(
                  child: ElText('${widget.day.day}', ElCalendarType.dayNumber),
                ),
              ),
            ),
          ),
...
class ElCalendarBandPainter extends CustomPainter {
  const ElCalendarBandPainter({
    required this.muted,
    required this.today,
  ...
  @override
  bool shouldRepaint(ElCalendarBandPainter old) =>
      old.muted != muted ||
      old.today != today ||
  ...
  Widget build(BuildContext context) {
    Widget content = MouseRegion(
      ...
      child: GestureDetector(
        ...
        // the flag is the frame, exactly as `ElButton` records under B1.
        child: Transform.scale(
          scale: pressed ? ElTransforms.buttonScale : 1,
          child: child,
        ),
      ),
    );
...
/// on [ElCurves.spring] reproduces that for free, which is why this mirrors
/// `ElButton`'s own `_SpringColors` rather than snapping.
class _CellSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Duration duration = elAnimationDuration(
      context,
      pressed ? ElDurations.tick : ElDurations.base,
    );
...
///
///  * an outline [ElButton] trigger, `justify-start`, with a 16px calendar
///    glyph and a label that **swaps typeface with state**;
///  * a [ElPopover] at `side: bottom`, `align: start`, `sideOffset: el(1)`
///    *(measured: the live popup lands 4px under the trigger with its left
///    edges flush)*;
///  * a [ElCalendar] on [ElCalendarSurface.popover], carrying `autoFocus`.
///
...
/// [ElButton.suppressPressScale] for it, and [ElButton.expanded] for the
/// `aria-expanded:bg-muted` beside it — see [_pressScaleSuppressed].
class ElDatePicker extends StatefulWidget {
  const ElDatePicker({
    super.key,
    required this.value,
    this.onChanged,
  ...
  /// passing [ElButton.suppressPressScale].
  static bool get pressScaleSuppressed => _pressScaleSuppressed;

  @override
  State<ElDatePicker> createState() => _ElDatePickerState();
}

class _ElDatePickerState extends State<ElDatePicker> {
  bool _open = false;
  ...
  Widget build(BuildContext context) {
    ...
    final Widget trigger = ElButton(
      ...
      // [ElDatePicker._pressScaleSuppressed].
      ...
      child: Row(
        ...
        children: <Widget>[
          ElIcon(
            ...
            sizePx: ElButton.iconPxFor(ElButtonSize.md),
            strokeOverride: ElIcon.strokeFor(ElIcon.pxFor(ElIconSize.sm)),
            tone: ElIconTone.subtle,
          ),
          SizedBox(width: ElButton.gapFor(ElButtonSize.md)),
          if (value == null)
            ElText(widget.placeholder, ElComponentType.buttonLabel)
          else
            ElText(ElDateFormat.dayMonthYear(value), ElType.numBase),
        ],
      ),
    );
