...
///   which [ElKeyframes.track] generalises.
...
///   resting style — stop 0. [ElKeyframeFill] is that distinction, and
///   [ElKeyframePlayer] is where it is enforced.
///
...
/// 360° on the wrap frame. Drive it through [ElRatchet.turnsAt] instead.
///
...
@immutable
class ElSteps extends Curve {
  const ElSteps(this.count, {this.jumpEnd = true}) : assert(count > 0);
  ...
  bool operator ==(Object other) =>
      other is ElSteps && other.count == count && other.jumpEnd == jumpEnd;
  ...
  @override
  String toString() =>
      'ElSteps($count, ${jumpEnd ? 'jump-end' : 'jump-start'})';
}
...
@immutable
class ElKeyframeStop<T> {
  const ElKeyframeStop(this.percent, this.value);
...
class ElKeyframes {
  ...
  static Animatable<double> doubles(
    List<ElKeyframeStop<double>> stops, {
    required Curve curve,
  ...
  static Animatable<Offset> offsets(
    List<ElKeyframeStop<Offset>> stops, {
    required Curve curve,
...
/// A [Tween] whose interpolation is supplied, so [ElKeyframes.track] can be
...
/// ([ElKeyframes.track]) and never here; a player that eased its own clock
/// would ease twice.
///
/// Reduced motion is the behaviour this widget exists for. `elAnimationDuration`
...
/// yet.
class ElKeyframePlayer extends StatefulWidget {
  const ElKeyframePlayer({
    super.key,
    required this.duration,
    required this.builder,
    this.fill = ElKeyframeFill.both,
    this.repeat = false,
    this.child,
  });
  ...
  @override
  State<ElKeyframePlayer> createState() => _ElKeyframePlayerState();
}

class _ElKeyframePlayerState extends State<ElKeyframePlayer>
    with SingleTickerProviderStateMixin {
  /// The duration named here is a placeholder for the first frame only:
  /// [build] re-reads it through [elAnimationDuration] on every pass, the way
  /// `ElPress` and `ElSlidingPillGroup` do.
  ...
  void didChangeDependencies() {
    ...
    final bool stilled =
        elAnimationDuration(context, widget.duration) == Duration.zero;
  ...
  @override
  void didUpdateWidget(ElKeyframePlayer old) {
  ...
  void _play() {
    if (_stilled ?? false) {
      _controller.stop();
      // §8.2, the whole point of [ElKeyframeFill]: `both` holds the final
      // stop, no fill reverts to the element's resting style.
      _controller.value = widget.fill == ElKeyframeFill.both
  ...
  Widget build(BuildContext context) {
    _controller.duration = elAnimationDuration(context, widget.duration);
...
class ElPopIn {
  ...
  static const Duration duration = ElDurations.popIn;
  static const Curve curve = ElCurves.out;
  static const ElKeyframeFill fill = ElKeyframeFill.both;

  static const List<ElKeyframeStop<double>> opacityStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 0), // allow-hardcoded: yuki-pop-in geometry
        ElKeyframeStop(55, 1), // allow-hardcoded: yuki-pop-in geometry
      ];

  static const List<ElKeyframeStop<Offset>> scaleStops =
      <ElKeyframeStop<Offset>>[
        ElKeyframeStop(0, Offset(0.25, 0.25)), // allow-hardcoded: yuki-pop-in
        ElKeyframeStop(55, Offset(0.92, 1.08)), // allow-hardcoded: yuki-pop-in
        ElKeyframeStop(80, Offset(1.04, 0.97)), // allow-hardcoded: yuki-pop-in
        ElKeyframeStop(92, Offset(0.99, 1.01)), // allow-hardcoded: yuki-pop-in
        ElKeyframeStop(100, Offset(1, 1)), // allow-hardcoded: yuki-pop-in
      ];

  static final Animatable<double> opacity = ElKeyframes.doubles(
    opacityStops,
    curve: curve,
  );

  /// `dx` = `scaleX`, `dy` = `scaleY`.
  static final Animatable<Offset> scale = ElKeyframes.offsets(
    scaleStops,
    curve: curve,
  );
}
...
/// `ElSlidingPillGroup` at [scale] and deleting `_jellyScale` — is a separate
/// change to a file this one does not own, and is deliberately not made here.
class ElJelly {
  const ElJelly._();

  static const Duration duration = ElDurations.animJelly;
  static const Curve curve = ElCurves.out;
  static const ElKeyframeFill fill = ElKeyframeFill.both;

  static const List<ElKeyframeStop<Offset>> scaleStops =
      <ElKeyframeStop<Offset>>[
        ElKeyframeStop(0, Offset(1, 1)), // allow-hardcoded: yuki-jelly geometry
        ElKeyframeStop(30, Offset(1.18, 0.82)), // allow-hardcoded: yuki-jelly
        ElKeyframeStop(45, Offset(0.88, 1.12)), // allow-hardcoded: yuki-jelly
        ElKeyframeStop(60, Offset(1.06, 0.94)), // allow-hardcoded: yuki-jelly
        ElKeyframeStop(78, Offset(0.98, 1.02)), // allow-hardcoded: yuki-jelly
        ElKeyframeStop(100, Offset(1, 1)), // allow-hardcoded: yuki-jelly
      ];
...
class ElSpringUp {
  ...
  static const Duration duration = ElDurations.springUp;
  static const Curve curve = ElCurves.settle;
  static const ElKeyframeFill fill = ElKeyframeFill.both;

  static const List<ElKeyframeStop<double>> opacityStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 0), // allow-hardcoded: yuki-spring-up geometry
        ElKeyframeStop(55, 1), // allow-hardcoded: yuki-spring-up geometry
      ];

  static const List<ElKeyframeStop<double>> translateYStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 32), // allow-hardcoded: yuki-spring-up geometry
        ElKeyframeStop(55, -4), // allow-hardcoded: yuki-spring-up geometry
        ElKeyframeStop(76, 1.5), // allow-hardcoded: yuki-spring-up geometry
        ElKeyframeStop(90, -0.5), // allow-hardcoded: yuki-spring-up geometry
        ElKeyframeStop(100, 0), // allow-hardcoded: yuki-spring-up geometry
      ];
  ...
  static final Animatable<double> translateY = ElKeyframes.doubles(
    translateYStops,
    curve: curve,
  );
}
...
class ElJellyIn {
  ...
  static const Duration duration = ElDurations.jelly;
  static const Curve curve = ElCurves.spring;
  static const ElKeyframeFill fill = ElKeyframeFill.both;

  static const List<ElKeyframeStop<double>> opacityStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 0), // allow-hardcoded: yuki-jelly-in geometry
        ElKeyframeStop(60, 1), // allow-hardcoded: yuki-jelly-in geometry
        ElKeyframeStop(100, 1), // allow-hardcoded: yuki-jelly-in geometry
      ];

  static const List<ElKeyframeStop<double>> scaleStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 0.92), // allow-hardcoded: yuki-jelly-in geometry
        ElKeyframeStop(60, 1.02), // allow-hardcoded: yuki-jelly-in geometry
        ElKeyframeStop(100, 1), // allow-hardcoded: yuki-jelly-in geometry
      ];

  static const List<ElKeyframeStop<double>> translateYStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 24), // allow-hardcoded: yuki-jelly-in geometry
        ElKeyframeStop(60, -4), // allow-hardcoded: yuki-jelly-in geometry
        ElKeyframeStop(100, 0), // allow-hardcoded: yuki-jelly-in geometry
      ];
  ...
  /// tables above.
  static final Animatable<double> scale = ElKeyframes.doubles(
    scaleStops,
    curve: curve,
  );
...
///
/// Eight held 45° positions of [ElDurations.ratchetStep] (175ms) each: 0°, 45°,
/// 90°, 135°, 180°, 225°, 270°, 315°. **360° is never displayed** — see
/// [ElSteps]. Infinite and unfilled, so it is one of the three demos the page
/// deliberately leaves unkeyed: a loop has nothing to replay.
class ElRatchet {
  const ElRatchet._();

  static const Duration duration = ElDurations.ratchet;
  ...
  static const Curve curve = ElSteps(positions);

  /// One held position — derived in the foundation layer from 1.4s ÷ 8.
  static Duration get step => ElDurations.ratchetStep;
...
class ElSignOnFrame {
  ...
  /// CSS `opacity`, applied **after** [brightnessFilter] — see [ElSignOn].
  ...
  List<Shadow> shadows(Color color) => <Shadow>[
    for (final double blur in glowBlurs)
      Shadow(color: color, blurRadius: ElSignOn.blurRadiusFor(blur)),
  ];
}
...
/// [ElSignOnFrame.brightnessFilter], then apply [ElSignOnFrame.opacity].
///
...
class ElSignOn {
  ...
  static const Duration duration = ElDurations.signOn;
  static const ElKeyframeFill fill = ElKeyframeFill.both;
  ...
  static const Curve curve = ElSteps(1);

  /// The 100% stop repeats the 70% stop verbatim, so the table has six rows,
  /// not seven.
  static const List<ElSignOnFrame> frames = <ElSignOnFrame>[
  ...
  /// holds for good.
  static ElSignOnFrame frameAt(double t) {
    final double percent = t.clamp(0.0, 1.0) * 100;
    ElSignOnFrame held = frames.first;
    for (final ElSignOnFrame frame in frames) {
  ...
  /// from [ElShadowLayer.blurRadius] rather than re-derived, so a correction
  ...
  static double blurRadiusFor(double cssBlur) =>
      ElShadowLayer(0, 0, cssBlur, 0, _unusedColor).blurRadius;

  /// [ElShadowLayer] resolves its colour against the theme; this one is only
  /// ever asked for its blur.
  static Color _unusedColor(ElThemeData theme) => elTransparent;
}
...
class ElReveal {
  ...
  static const Duration duration = ElDurations.reward;
  static const Curve curve = ElCurves.out;
  static const ElKeyframeFill fill = ElKeyframeFill.both;
  ...
  static const List<ElKeyframeStop<double>> opacityStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 0), // allow-hardcoded: pulls-reveal geometry
        ElKeyframeStop(100, 1), // allow-hardcoded: pulls-reveal geometry
      ];

  static const List<ElKeyframeStop<double>> rotationYStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, fromRadians),
        ElKeyframeStop(100, 0),
      ];

  static const List<ElKeyframeStop<double>> scaleStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, fromScale),
        ElKeyframeStop(100, 1), // allow-hardcoded: pulls-reveal geometry
      ];
  ...
  static final Animatable<double> rotationY = ElKeyframes.doubles(
    rotationYStops,
    curve: curve,
  );
...
class ElShimmer {
  ...
  static const Duration duration = ElDurations.shimmer;
  ...
  static LinearGradient gradient(ElThemeData theme) => LinearGradient(
...
class ElPulseLive {
  ...
  static const Duration duration = ElDurations.pulseLive;
  static const Curve curve = ElCurves.inOut;
  static const ElKeyframeFill fill = ElKeyframeFill.none;
  static const bool loops = true;

  /// The demo's dot: `size-2` (8px) `rounded-full bg-success`.
  static double get dotDiameter => el(2);
  static double get dotRadius => el(2) / 2;
  static Color get dotColor => ElPalette.success;
  ...
  static const List<ElKeyframeStop<double>> ringPhaseStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 0), // allow-hardcoded: pulls-pulse-live geometry
        ElKeyframeStop(50, 1), // allow-hardcoded: pulls-pulse-live geometry
        ElKeyframeStop(100, 0), // allow-hardcoded: pulls-pulse-live geometry
      ];

  static const List<ElKeyframeStop<double>> dotOpacityStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 1), // allow-hardcoded: pulls-pulse-live geometry
        ElKeyframeStop(50, 0.75), // allow-hardcoded: pulls-pulse-live geometry
        ElKeyframeStop(100, 1), // allow-hardcoded: pulls-pulse-live geometry
      ];

  static final Animatable<double> ringPhase = ElKeyframes.doubles(
    ringPhaseStops,
    curve: curve,
  );

  static final Animatable<double> dotOpacity = ElKeyframes.doubles(
    dotOpacityStops,
    curve: curve,
  );
...
/// [ElDurations] per row, and the page's source carries an
...
class ElSweep {
  ...
  static const List<ElKeyframeStop<double>> widthFactorStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 0), // allow-hardcoded: sweep geometry
        ElKeyframeStop(100, 1), // allow-hardcoded: sweep geometry
      ];

  /// `0 → 1`, for a `FractionallySizedBox.widthFactor` inside the clipped
  /// track.
  static final Animatable<double> widthFactor = ElKeyframes.doubles(
    widthFactorStops,
    curve: curve,
  );
}
...
/// animation runs its full [ElDurations.bloom] and translates by nothing; the
...
class ElTravel {
  ...
  static const Duration duration = ElDurations.bloom;
  static const ElKeyframeFill fill = ElKeyframeFill.both;

  /// `1.5rem`, the `size-6` chip's own width, subtracted by the `calc`.
  static double get inset => el(6);
...
class ElCheckDraw {
  ...
  static const Duration duration = ElDurations.checkDraw;
  static const Curve curve = ElCurves.out;
  static const ElKeyframeFill fill = ElKeyframeFill.both;
  ...
  static const List<ElKeyframeStop<double>> dashOffsetStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, dashArray),
        ElKeyframeStop(100, 0),
      ];

  static final Animatable<double> dashOffset = ElKeyframes.doubles(
    dashOffsetStops,
    curve: curve,
  );
...
class ElDashDraw {
  ...
  static const Duration duration = ElDurations.dashDraw;
  static const Curve curve = ElCurves.out;
  static const ElKeyframeFill fill = ElKeyframeFill.both;
  ...
  /// See [ElCheckDraw.drawnFractionAt].
  static double drawnFractionAt(double t) =>
      1 - dashOffset.transform(t.clamp(0.0, 1.0)) / dashArray;
}
...
class ElDotPop {
  ...
  static const Duration duration = ElDurations.dotPop;
  static const Curve curve = ElCurves.spring;
  static const ElKeyframeFill fill = ElKeyframeFill.both;

  static const List<ElKeyframeStop<double>> scaleStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 0), // allow-hardcoded: dot-pop geometry
        ElKeyframeStop(55, 1.35), // allow-hardcoded: dot-pop geometry
        ElKeyframeStop(100, 1), // allow-hardcoded: dot-pop geometry
      ];

  static const List<ElKeyframeStop<double>> opacityStops =
      <ElKeyframeStop<double>>[
        ElKeyframeStop(0, 0), // allow-hardcoded: dot-pop geometry
        ElKeyframeStop(55, 1), // allow-hardcoded: dot-pop geometry
        ElKeyframeStop(100, 1), // allow-hardcoded: dot-pop geometry
      ];
...
///   stylesheet.
///
/// The squash that lands on the arriving glyph is [ElJelly], delayed by
/// `--duration-fast` — see `icon_swap.dart`, which owns that composition.
class ElSwapRoll {
  const ElSwapRoll._();

  /// `--duration-slow`, on both properties.
  static const Duration duration = ElDurations.slow;

  /// `--ease-spring`, on both properties. Not `--ease-out`: the wheel is meant
  /// to overshoot.
  static const Curve curve = ElCurves.spring;

  /// `160%` of the translated box per step — [ElTransforms.swapRollTravel].
  ///
