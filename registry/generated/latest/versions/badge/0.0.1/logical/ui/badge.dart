...
/// icon call site (see [ElBadge.glyph]) and all five chips there measure the
...
final LinearGradient _rampChip = LinearGradient(
  ...
  colors: <Color>[
    elHsl(0, 0, 100).withValues(alpha: 0.18),
    elHsl(0, 0, 100).withValues(alpha: 0.05),
    elHsl(0, 0, 0).withValues(alpha: 0.14),
  ],
...
enum ElBadgeVariant {
  ...
  /// `ElButtonVariant.primary` carries.
  primary,
...
class ElBadge extends StatelessWidget {
  const ElBadge({
    super.key,
    required this.label,
    this.variant = ElBadgeVariant.primary,
    this.spec,
    this.paddingX,
    this.minWidth,
    this.glyph,
  ...
  /// Overrides [ElComponentType.badgeLabel].
  ///
  /// `SidebarMenuBadge` writes `.type-num-xs` onto the chip, which resolves to
  /// [ElComponentType.sidebarMenuBadge] rather than to the class it names —
  /// see that spec for the cascade. The [ElFieldLabel] `spec:` precedent
  ...
  static double get glyphGap => el(1);

  /// `[&>svg]:size-3!` — the 12px every chip glyph is forced to.
  static double get glyphSize => el(3);

  /// `h-5` — a hard 20px border box, label height notwithstanding.
  static double get height => el(5);

  /// `px-2`.
  static double get horizontalPadding => el(2);

  /// The `background-color` the variant fills with, or null for the three
  /// unfilled ones.
  Color? _fill(ElThemeData theme) => switch (variant) {
    ElBadgeVariant.primary => theme.primary,
    ElBadgeVariant.secondary => theme.secondary,
    ElBadgeVariant.destructive => theme.destructive.withValues(
      alpha: _tintAlpha,
    ),
    ElBadgeVariant.action => ElPalette.action.withValues(alpha: _tintAlpha),
    ElBadgeVariant.premium => ElPalette.value.withValues(alpha: _tintAlpha),
    ElBadgeVariant.success => ElPalette.success.withValues(alpha: _tintAlpha),
    ElBadgeVariant.warning => ElPalette.warning.withValues(alpha: _tintAlpha),
    ElBadgeVariant.info => ElPalette.info.withValues(alpha: _tintAlpha),
    ElBadgeVariant.outline ||
    ElBadgeVariant.ghost ||
    ElBadgeVariant.link => null,
  };

  Color _ink(ElThemeData theme) => switch (variant) {
    ElBadgeVariant.primary => theme.primaryForeground,
    ElBadgeVariant.secondary => theme.secondaryForeground,
    ElBadgeVariant.destructive => theme.destructiveInk,
    ElBadgeVariant.outline || ElBadgeVariant.ghost => theme.mutedForeground,
    ElBadgeVariant.link || ElBadgeVariant.action => theme.actionInk,
    ElBadgeVariant.premium => theme.valueInk,
    ElBadgeVariant.success => theme.successInk,
    ElBadgeVariant.warning => theme.warningInk,
    ElBadgeVariant.info => theme.infoInk,
  };

  /// `shadow-chip` everywhere but `premium`, which takes `shadow-btn-value`.
  ElShadowSpec _shadow() =>
      variant == ElBadgeVariant.premium ? ElShadows.btnValue : ElShadows.chip;

  @override
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final BorderRadius radius = BorderRadius.circular(ElRadii.pill);
    ...
    final BoxBorder border = Border.all(
      color: variant == ElBadgeVariant.outline ? theme.input : elTransparent,
      width: ElWidths.hairline,
    );

    final Widget text = ElText(
      label,
      spec ?? ElComponentType.badgeLabel,
      color: _ink(theme),
    );
