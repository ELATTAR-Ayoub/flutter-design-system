...
/// already declared once per theme. A [ElChartSeries] therefore carries a
/// resolved [Color], and the page resolves it from [ElTheme] on the way in.
library;
...
class ElChartText {
  ...
  static final ElTypeSpec xs = ElTypeSpec(
    family: ElFonts.sans,
    size: 12,
    height: 16 / 12,
  );
  ...
  static final ElTypeSpec xsMedium = ElTypeSpec(
    family: ElFonts.sans,
    size: 12,
    height: 16 / 12,
    wght: 500,
  ...
  static final ElTypeSpec xsTight = ElTypeSpec(
    family: ElFonts.sans,
    size: 12,
...
/// the builder receives a [BuildContext] and every call site uses [ElIcon] —
/// but the shape is kept so the four `icons` variants read the same way, and
/// every call site fills it with `ElIcon`.
class ElChartSeries {
  const ElChartSeries({this.label, this.color, this.icon});
...
class ElChartConfig {
  const ElChartConfig(this.entries);

  final Map<String, ElChartSeries> entries;

  ElChartSeries? operator [](String key) => entries[key];
  ...
  ElChartSeries? resolve(String key, Map<String, Object?>? payload) {
  ...
  ElChartConfig plus(Map<String, ElChartSeries> extra) =>
      ElChartConfig(<String, ElChartSeries>{...entries, ...extra});
}

/// Carries the config down to whatever renders inside a [ElChartContainer].
class ElChartScope extends InheritedWidget {
  const ElChartScope({super.key, required this.config, required super.child});

  final ElChartConfig config;
  ...
  static ElChartConfig of(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<ElChartScope>()?.config ??
      const ElChartConfig(<String, ElChartSeries>{});

  @override
  bool updateShouldNotify(ElChartScope old) => old.config != config;
}
...
/// hand, so the equivalent statement is to name it — [ElDurations.slow].
///
...
/// port on [ElCurves.out] would make the Flutter charts *better* than the
/// reference and therefore wrong, so [curve] is [ElCurves.cssEaseOut] and the
/// page documents the difference exactly as the reference's Animation section
/// does.
///
/// Reduced motion is resolved by the caller through `elAnimationDuration`
...
class ElChartMotion {
  const ElChartMotion._();

  /// `--duration-slow`, as `useChartMotion` reads it.
  static Duration get duration => ElDurations.slow;

  /// The CSS keyword `ease-out`, which is what recharts received.
  static Curve get curve => ElCurves.cssEaseOut;
}
...
class ElChartContainer extends StatelessWidget {
  const ElChartContainer({
    super.key,
  ...
  static double get plotHeight => el(64);
...
enum ElChartIndicator { dot, line, dashed }

/// One row of a tooltip.
class ElChartTooltipItem {
  const ElChartTooltipItem({
    required this.name,
    required this.value,
    this.color,
    this.payload,
...
/// holds at 12.
class ElChartTooltipContent extends StatelessWidget {
  const ElChartTooltipContent({
    super.key,
    this.config,
    this.label,
    required this.items,
  ...
  /// Normally read from [ElChartScope]; passed directly only by tests and by
  ...
  final List<ElChartTooltipItem> items;
  ...
  /// panel is sitting at.
  static double get minWidth => el(32);

  /// `border-border/50`.
  static const double _borderAlpha = 0.5;

  /// `h-2.5 w-2.5` — the dot indicator.
  static double get _dotSize => el(2.5);

  /// `w-1` — the line indicator.
  static double get _lineWidth => el(1);
  ...
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final ElChartConfig cfg = config ?? ElChartScope.of(context);
    ...
    return DefaultTextStyle(
      style: ElText.styleOf(context, ElChartText.xs, color: theme.foreground),
      child: Container(
        width: width,
        constraints: BoxConstraints(minWidth: width ?? minWidth),
        padding: EdgeInsets.symmetric(horizontal: el(2.5), vertical: el(1.5)),
        decoration: BoxDecoration(
          ...
          boxShadow: ElShadows.tailwindXl.outerShadows(theme),
        ),
  ...
  Widget? _label(BuildContext context, ElChartConfig cfg) {
    if (hideLabel || items.isEmpty) return null;
    final ElChartTooltipItem first = items.first;
    final String key = labelKey ?? first.dataKey ?? first.name;
    final ElChartSeries? entry = cfg.resolve(key, first.payload);
    ...
    if (labelFormatter != null) {
      return ElText(labelFormatter!(label ?? '', items), ElChartText.xsMedium);
    }
    if (value == null || value.isEmpty) return null;
    return ElText(value, ElChartText.xsMedium);
  }
  ...
  Widget _row(
    ...
    final ElChartSeries? entry = cfg.resolve(key, item.payload);
    final Color ink = item.color ?? entry?.color ?? theme.foreground;
    final bool nested = items.length == 1 && indicator != ElChartIndicator.dot;
    ...
    final bool stretch = indicator != ElChartIndicator.dot && !nested;
    final Widget row = Row(
      ...
      mainAxisSize: MainAxisSize.min,
      children: <Widget>[
        if (mark != null) ...<Widget>[mark, SizedBox(width: el(2))],
        Expanded(
          child: Row(
  ...
  Widget _indicator(ElThemeData theme, Color ink) {
    switch (indicator) {
      case ElChartIndicator.dot:
        return Container(
          width: _dotSize,
          height: _dotSize,
          decoration: BoxDecoration(
...
/// One legend key.
class ElChartLegendItem {
  const ElChartLegendItem({required this.name, this.color, this.label});
...
class ElChartLegendContent extends StatelessWidget {
  const ElChartLegendContent({
    super.key,
    this.config,
    required this.items,
    this.verticalAlign = ElChartLegendAlign.bottom,
    this.hideIcon = false,
    this.wrap = false,
    this.gap,
  });
  ...
  final ElChartConfig? config;
  final List<ElChartLegendItem> items;
  final ElChartLegendAlign verticalAlign;
  ...
  static double get swatchSize => el(2);
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> keys = <Widget>[
      for (final ElChartLegendItem item in items)
        ...
        : Row(
            ...
            children: <Widget>[
              for (int i = 0; i < keys.length; i++) ...<Widget>[
                if (i > 0) SizedBox(width: gap ?? el(4)),
                keys[i],
              ],
            ],
          );
    return Padding(
      padding: verticalAlign == ElChartLegendAlign.top
          ? EdgeInsets.only(bottom: el(3))
          : EdgeInsets.only(top: el(3)),
  ...
  Widget _key(
    ...
    ElThemeData theme,
    ElChartConfig cfg,
    ElChartLegendItem item,
  ) {
    final ElChartSeries? entry = cfg[item.name];
    final Widget swatch = entry?.icon != null && !hideIcon
        ? SizedBox(width: el(3), height: el(3), child: entry!.icon!(context))
        : Container(
            width: swatchSize,
            height: swatchSize,
            decoration: BoxDecoration(
...
enum ElChartLegendAlign { top, bottom }
...
String elChartNumber(num value) {
