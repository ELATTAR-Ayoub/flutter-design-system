...
class ChartText {
  ...
  static final TextStyleToken xs = TextStyles.small;

  /// `font-medium` on the tooltip's own label row.
  static final TextStyleToken xsMedium = TextStyles.small.derive(
    name: 'chart-label-medium',
    wght: 500,
  );
  ...
  static final TextStyleToken xsTight = TextStyles.small;
}
...
class ChartLegendContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      // A legend that wraps is already safe on a narrow column. One that was
      // asked for a single row is not: at 200% text on a 320px phone the keys
      // are wider than the chart, and a row that cannot fit has to scroll
      // rather than paint past the edge. Centred while it fits, exactly as
      // `Align` centred it before — the minimum width is what keeps that true.
      child: wrap
          ? Align(child: row)
          : LayoutBuilder(
