...
/// restating a class. The port keeps that split literally: [ElBubble] paints
/// nothing and owns the *box* (`w-fit max-w-[80%]`, the 4px column gap, the
/// self-alignment); [ElBubbleContent] is the painted surface and the only thing
...
/// | radius | **16px** — `rounded-xl` is [ElRadii.xl] here, not Tailwind's 12 |
/// | type | 13px / **21.125px** — `text-sm leading-relaxed` ([ElComponentType.bubbleContent]) |
...
/// the measured transition is **250ms on [ElCurves.out]** (the corpus default
...
/// [ElDurations.transitionDefault] on [ElCurves.out]. `showCount: always`
...
enum ElBubbleVariant {
  ...
  String get label => this == ElBubbleVariant.normal ? 'default' : name;
}
...
/// written shadow.
List<BoxShadow> _focusRing(ElThemeData theme) => ElShadowSpec(<ElShadowLayer>[
  ElShadowLayer(
    0,
    0,
    0,
    ElBubbleContent.focusRing,
    (ElThemeData t) => t.ring.withValues(alpha: ElBubbleContent.focusRingAlpha),
  ),
]).outerShadows(theme);

/// `<div data-slot="bubble-group">` — `flex min-w-0 flex-col gap-2`.
class ElBubbleGroup extends StatelessWidget {
  const ElBubbleGroup({super.key, required this.children});
...
class ElBubble extends StatelessWidget {
  const ElBubble({
    super.key,
    required this.child,
    this.variant = ElBubbleVariant.normal,
  ...
  final ElBubbleVariant variant;

  /// `data-align`. Inside a [ElMessage] the message's own align wins when this
  /// is left at its default — see [ElBubbleAlignScope].
  ...
  final ElBubbleReactions? reactions;

  @override
  Widget build(BuildContext context) {
    final ElBubbleAlign resolved = ElBubbleAlignScope.resolve(context, align);
    final bool ghost = variant == ElBubbleVariant.ghost;

    Widget bubble = _BubbleScope(
      variant: variant,
      align: resolved,
      child: Align(
        alignment: resolved == ElBubbleAlign.end
        ...
        // `ElGrid` asks one of every cell through [IntrinsicHeight].
        child: _MaxWidthFraction(
          factor: ghost ? 1 : maxWidthFraction,
          child: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: resolved == ElBubbleAlign.end
...
/// through: `ElGrid` measures its cells with [IntrinsicHeight], and a
...
/// The variant and alignment a [ElBubbleContent] reads off its bubble — the
...
/// `group-data-[align=end]/message:*` — a [ElMessage]'s align, published so a
/// [ElBubble] left at its default mirrors the row.
///
...
class ElBubbleAlignScope extends InheritedWidget {
  const ElBubbleAlignScope({
    super.key,
    required this.align,
    required super.child,
  });
  ...
  static ElBubbleAlign resolve(BuildContext context, ElBubbleAlign own) {
    if (own == ElBubbleAlign.end) return own;
    final ElBubbleAlignScope? scope = context
        .dependOnInheritedWidgetOfExactType<ElBubbleAlignScope>();
    return scope?.align ?? own;
  }

  @override
  bool updateShouldNotify(ElBubbleAlignScope old) => old.align != align;
}
...
class ElBubbleContent extends StatefulWidget {
  const ElBubbleContent({
    super.key,
    required this.child,
    this.onPressed,
    this.semanticsLabel,
  ...
  /// `px-3` — 12px.
  static double get paddingX => el(3);

  /// `py-2` — 8px.
  static double get paddingY => el(2);
  ...
  @override
  State<ElBubbleContent> createState() => _ElBubbleContentState();
}

class _ElBubbleContentState extends State<ElBubbleContent> {
  bool _hovered = false;
  bool _focused = false;
  ...
  Color _fill(ElThemeData theme, ElBubbleVariant variant, bool hovered) {
    final bool dark = theme.kind == ElThemeKind.dark;
    return switch (variant) {
      ElBubbleVariant.normal =>
        hovered
            ? theme.primary.withValues(alpha: _BubbleAlpha.primaryHover)
            : theme.primary,
      ElBubbleVariant.secondary =>
        hovered
            ? ElOklab.mix(
                theme.secondary,
                theme.foreground,
      ...
      ElBubbleVariant.ghost =>
        hovered
            ? (dark
                  ...
                  : theme.muted)
            : elTransparent,
      ElBubbleVariant.destructive => theme.destructive.withValues(
        alpha: hovered
            ? (dark
  ...
  Color _ink(ElThemeData theme, ElBubbleVariant variant, bool hovered) =>
      switch (variant) {
        ElBubbleVariant.normal => theme.primaryForeground,
        ElBubbleVariant.secondary => theme.secondaryForeground,
        ElBubbleVariant.muted || ElBubbleVariant.tinted => theme.foreground,
        ElBubbleVariant.outline || ElBubbleVariant.ghost => theme.foreground,
        ElBubbleVariant.destructive => theme.destructiveInk,
      };
  ...
  Widget build(BuildContext context) {
    ...
    final ElBubbleVariant variant = scope?.variant ?? ElBubbleVariant.normal;
    final ElBubbleAlign align = scope?.align ?? ElBubbleAlign.start;
    final bool ghost = variant == ElBubbleVariant.ghost;
    ...
    Widget surface = Padding(
      ...
      padding: ghost
          ...
          : EdgeInsets.symmetric(
              horizontal: ElBubbleContent.paddingX,
              vertical: ElBubbleContent.paddingY,
            ),
    ...
    // `group-data-[align=end]/bubble:self-end`.
    surface = Align(
      alignment: align == ElBubbleAlign.end
          ? AlignmentDirectional.centerEnd
          : AlignmentDirectional.centerStart,
      widthFactor: 1,
      heightFactor: 1,
...
class ElBubbleReaction {
  const ElBubbleReaction({
    required this.emoji,
    required this.count,
    required this.label,
    this.mine = false,
...
class ElBubbleReactions extends StatelessWidget {
  const ElBubbleReactions({
    super.key,
    this.side = ElBubbleSide.bottom,
    this.align = ElBubbleAlign.end,
    this.reactions,
    this.showCount = ElShowCount.hover,
    this.onReact,
    this.children,
  });
  ...
  static double get barePaddingX => el(1.5);
  static double get barePaddingY => el(0.5);
  ...
  final ElBubbleSide side;
  final ElBubbleAlign align;

  /// Draw these rather than [children].
  final List<ElBubbleReaction>? reactions;

  final ElShowCount showCount;

  /// Fired with the reaction's [ElBubbleReaction.label] when one is pressed.
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> items = data
        ? <Widget>[
            for (final ElBubbleReaction r in reactions!)
              _ReactionPill(
                reaction: r,
                showCount: showCount,
                onReact: onReact,
    ...
    Widget rail = Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: ElShadowSpec(<ElShadowLayer>[
          ElShadowLayer(0, 0, 0, ring, (ElThemeData t) => t.card),
        ]).outerShadows(theme),
      ),
    ...
    rail = FractionalTranslation(
      translation: Offset(0, side == ElBubbleSide.top ? -overhang : overhang),
      child: rail,
    );
    ...
    // `Stack` of its own: the parent data resolves against [ElBubble]'s own
    // stack, which is the one element that does not bounds-check a pointer.
    return PositionedDirectional(
      top: side == ElBubbleSide.top ? 0 : null,
      bottom: side == ElBubbleSide.bottom ? 0 : null,
      start: align == ElBubbleAlign.start ? inset : null,
      end: align == ElBubbleAlign.end ? inset : null,
      child: rail,
    );
  }
}
...
class _ReactionPill extends StatefulWidget {
  ...
  final ElBubbleReaction reaction;
  final ElShowCount showCount;
  final void Function(String label)? onReact;
...
class _ReactionPillState extends State<_ReactionPill> {
  ...
  /// `h-7` — 28px.
  static double get height => el(7);

  /// `px-2` — 8px.
  static double get paddingX => el(2);

  /// `gap-1` — 4px between the emoji and the count.
  static double get gap => el(1);

  /// `w-4` — the count's revealed width, measured at exactly 16px.
  static double get countWidth => el(4);

  @override
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final ElBubbleReaction r = widget.reaction;
    final bool open =
        widget.showCount == ElShowCount.always || _hovered || _focused;
    ...
    Widget pill = Container(
      ...
      decoration: BoxDecoration(
        color: r.mine
            ? ElPalette.action.withValues(alpha: _BubbleAlpha.mineFill)
        ...
        border: Border.all(
          color: _focused
              ? theme.ring
              : r.mine
              ? ElPalette.action.withValues(alpha: _BubbleAlpha.mineBorder)
              : theme.border,
          width: ElWidths.hairline,
        ),
        boxShadow: _focused ? _focusRing(theme) : null,
