...
/// drift 12, and the port renders [ElRadii.xl].
///
/// ## Measured geometry
///
...
///
/// ## Why this file does not build on `ElPopover`
///
...
/// `ElSelect` and `ElInputGroupAlign`. What this file does reuse from wave A2 is
/// the input-group *recipe*; it cannot reuse the widget, because [ElInputGroup]
...
///   label split ([ElComponentType.menuHeading]).
...
class ElCommandItem {
  ...
  ///
  /// The distinction is load-bearing twice. It rides [ElType.caption] rather
  /// than [ElComponentType.menuShortcut], and because it carries no
  ...
  /// [ElMenuItem] already makes. Ignored when [icon] is given.
...
/// One `CommandGroup`.
class ElCommandGroup {
  const ElCommandGroup({
    required this.items,
    this.heading,
    this.separatorBefore = false,
  });

  final List<ElCommandItem> items;

  /// `heading` — rendered `px-3 py-2` at [ElComponentType.menuHeading].
  ///
...
class ElCommand extends StatefulWidget {
  const ElCommand({
    super.key,
  ...
  /// see [ElCommand.sortsGroups].
  final List<ElCommandGroup> groups;
  ...
  /// cmdk's `filter` prop. Defaults to [elCommandScore]; return 0 to hide.
  ...
  /// cmdk's `onValueChange`, carrying the selected row's [ElCommandItem.searchValue].
  ...
  static double get listMaxHeight => el(72);

  /// `h-8!` — the input group, at 32px rather than the family's 40.
  static double get inputHeight => el(8);
  ...
  static double get emptyHeight {
    final ElTypeSpec spec = ElComponentType.sheetBody;
    return (spec.size ?? 0) * (spec.height ?? 1) + el(6) * 2;
  }
  ...
  @override
  State<ElCommand> createState() => _ElCommandState();
}

class _ElCommandState extends State<ElCommand> {
  ...
  FocusNode get _focusNode =>
      widget.focusNode ??
      (_ownedFocusNode ??= FocusNode(debugLabel: 'ElCommand'));
  ...
  @override
  void didUpdateWidget(ElCommand old) {
    ...
    if (old.groups != widget.groups && _visibleItems.isNotEmpty) {
      final List<ElCommandItem> visible = _visibleItems;
      if (!visible.any((ElCommandItem i) => i.searchValue == _value)) {
        _selectFirstItem();
      }
    }
  }
  ...
  double _score(ElCommandItem item) {
    final double Function(String, String, List<String>) filter =
        widget.filter ?? elCommandScore;
  ...
  List<_ResolvedGroup> get _resolved {
    if (_search.isEmpty || !widget.shouldFilter) {
      return widget.groups
          .map((ElCommandGroup g) => _ResolvedGroup(g, g.items))
          .toList();
    }

    final List<_ResolvedGroup> out = <_ResolvedGroup>[];
    for (final ElCommandGroup group in widget.groups) {
      final List<({ElCommandItem item, double score, int order})> scored =
          <({ElCommandItem item, double score, int order})>[];
      for (int i = 0; i < group.items.length; i++) {
        final ElCommandItem item = group.items[i];
      ...
      scored.sort((
        ({ElCommandItem item, double score, int order}) a,
        ({ElCommandItem item, double score, int order}) b,
      ) {
      ...
      out.add(
        _ResolvedGroup(
          ...
          scored
              .map(
                (({ElCommandItem item, double score, int order}) e) => e.item,
              )
              .toList(),
        ),
      );
  ...
  List<ElCommandItem> get _visibleItems => <ElCommandItem>[
    for (final _ResolvedGroup g in _resolved) ...g.items,
  ];

  /// `getValidItems()` — `[cmdk-item=""]:not([aria-disabled="true"])`.
  List<ElCommandItem> get _validItems =>
      _visibleItems.where((ElCommandItem i) => i.enabled).toList();
  ...
  void _selectFirstItemInner() {
    final List<ElCommandItem> valid = _validItems;
  ...
  void _move(int step) {
    ...
    final int at = valid.indexWhere(
      (ElCommandItem i) => i.searchValue == _value,
    );
    int next = at + step;
    if (widget.loop) {
      next = next < 0
  ...
  void _moveByGroup(int step) {
    ...
    final int at = groups.indexWhere(
      (_ResolvedGroup g) =>
          g.items.any((ElCommandItem i) => i.searchValue == _value),
    );
    if (at < 0) return _move(step);
    final int target = at + step;
    if (target < 0 || target >= groups.length) return _move(step);
    final Iterable<ElCommandItem> valid = groups[target].items.where(
      (ElCommandItem i) => i.enabled,
    );
  ...
  void _commit(ElCommandItem item) {
  ...
  KeyEventResult _onKey(FocusNode node, KeyEvent event) {
    ...
    if (key == LogicalKeyboardKey.enter ||
      ...
      final ElCommandItem? selected = _selectedItem;
  ...
  ElCommandItem? get _selectedItem {
    for (final ElCommandItem item in _visibleItems) {
  ...
  Widget build(BuildContext context) {
    ...
    _rowKeys.removeWhere(
      (String value, GlobalKey _) =>
          !_visibleItems.any((ElCommandItem i) => i.searchValue == value),
    );
      ...
      rows.add(
        _inset(
          _CommandGroupBlock(
            ...
            children: <Widget>[
              for (final ElCommandItem item in group.items)
                _CommandRow(
    ...
    list = ConstrainedBox(
      constraints: BoxConstraints(maxHeight: ElCommand.listMaxHeight),
      ...
      children: <Widget>[
        ...
        Padding(
          padding: EdgeInsets.only(
            left: ElCommand.padding,
            right: ElCommand.padding,
            top: ElCommand.padding,
          ),
          child: _CommandInput(
            theme: theme,
        ...
        SizedBox(height: ElCommand.padding),
      ],
    );
  ...
  Widget _inset(Widget child) => Padding(
    padding: EdgeInsets.symmetric(horizontal: ElCommand.padding),
    child: child,
  );
}
...
///
/// Not a [ElInputGroup]: that widget fixes the 40px pill, the `--card` fill and
...
class _CommandInput extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Color fill = theme.input.withValues(alpha: ElCommand.inputFillAlpha);
    ...
    Widget group = Row(
      ...
      children: <Widget>[
        ...
        Padding(
          padding: EdgeInsets.only(
            left: el(2),
            // `py-1.5`, the same inset [ElInputGroupAddon] carries.
            top: ElInputGroupAddon.insetY,
            bottom: ElInputGroupAddon.insetY,
          ),
          child: Opacity(
            opacity: ElCommand.searchGlyphOpacity,
            child: const ElIcon(
              ElIconGlyph.search,
    ...
    group = DecoratedBox(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: fill, width: ElWidths.hairline),
      ),
      child: group,
    );
...
class _CommandRow extends StatelessWidget {
  ...
  final ElThemeData theme;
  final ElCommandItem item;
  final bool selected;
  final VoidCallback onTap;
  final VoidCallback onHover;

  /// `in-data-[slot=dialog-content]:rounded-lg!` — see [ElCommand.inDialog].
  final bool inDialog;
  ...
  Widget build(BuildContext context) {
    ...
    Widget row = Row(
      ...
      children: <Widget>[
        if (item.icon != null || item.lucideIcon != null) ...<Widget>[
          if (item.icon != null)
            ElIcon(
              ...
              tone: selected
                  ? ElIconTone.normal
                  : (item.iconTone ?? ElIconTone.subtle),
            )
          else
            ElIcon.lucide(
              item.lucideIcon!,
        ...
        Expanded(
          ...
          // line, two stacked blocks when it carries a [ElCommandItem.subtitle].
          child: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: <Widget>[
        ...
        ...<Widget>[
          SizedBox(width: el(2)),
          Opacity(opacity: 0, child: ElIcon(ElIconGlyph.check, sizePx: el(4))),
        ],
      ],
    );
    ...
    row = DecoratedBox(
      decoration: BoxDecoration(
        ...
        color: selected ? theme.muted : elTransparent,
        borderRadius: BorderRadius.circular(inDialog ? ElRadii.lg : ElRadii.md),
      ),
      child: row,
    );
    ...
    row = Opacity(
      opacity: item.enabled ? 1 : ElCommand.disabledOpacity,
      child: row,
    );
