...
///
/// **That failure mode does not reproduce in Flutter.** [ElMachineSurface]
/// takes one [ElShadowSpec] and paints every layer of it, so a caller passing
...
/// nothing here clips [ElGlassPanel.child]. The two clips in this file are
...
const double _panelFillAlpha = ElSurfaceOpacity.glassPanel;
...
Color _panelFill(ElThemeData t) =>
    ElOklab.mix(t.card, elTransparent, _panelFillAlpha);

Color _clearPanelFill(ElThemeData t) =>
    ElOklab.mix(t.card, elTransparent, ElSurfaceOpacity.navigationGlass);

Color _controlFill(ElThemeData t) =>
    ElOklab.mix(t.foreground, elTransparent, _controlFillAlpha);

Color _rimStrong(ElThemeData t) => t.rimStrong;

Color _panelRing(ElThemeData t) =>
    ElOklab.mix(t.foreground, elTransparent, _panelRimAlpha);

Color _controlRing(ElThemeData t) =>
    ElOklab.mix(t.foreground, elTransparent, _controlRimAlpha);
...
ui.ColorFilter elSaturate(double s) => ui.ColorFilter.matrix(<double>[
...
/// twice sigma and which [ElShadowLayer.blurRadius] converts. So `--blur-xl`
...
final ui.ImageFilter _glassBackdrop = ui.ImageFilter.compose(
  outer: elSaturate(_panelSaturation),
  inner: ui.ImageFilter.blur(sigmaX: ElBlurs.xl, sigmaY: ElBlurs.xl),
);
...
const ElShadowLayer _topHighlight = ElShadowLayer(
  0,
  1,
  0,
  0,
...
const ElShadowLayer _panelRingLayer = ElShadowLayer(
  0,
  0,
  0,
  1,
...
const ElShadowLayer _controlRingLayer = ElShadowLayer(
  0,
  0,
  0,
  1,
...
ElShadowSpec _panelShadow(ElShadowSpec ambient) => ElShadowSpec(<ElShadowLayer>[
  _topHighlight,
  _panelRingLayer,
  ...ambient.layers,
]);

final ElShadowSpec _panelSpec = _panelShadow(ElShadows.e2);
final ElShadowSpec _panelDeepSpec = _panelShadow(ElShadows.e4);

/// Two inset layers only — `glass-control` declares no outer shadow.
const ElShadowSpec _controlSpec = ElShadowSpec(<ElShadowLayer>[
  _topHighlight,
  _controlRingLayer,
]);
...
double _shadowReach(ElShadowLayer layer) =>
...
Path _outsideShape(RRect shape, List<ElShadowLayer> ambient) {
  double reach = 0;
  for (final ElShadowLayer layer in ambient) {
...
///
/// So the ambient is lifted out of [ElMachineSurface] and painted here, under a
...
class _AmbientShadowPainter extends CustomPainter {
  ...
  final List<ElShadowLayer> ambient;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final ElShadowLayer layer in ambient.reversed) {
...
///
/// **Why that is a [Stack] and not one [ElMachineSurface].** Two of those
/// layers cannot go where `ElMachineSurface` puts them. The blurred backdrop
...
/// `ElMachineSurface` is exactly what only it can do: the inset layers, over
...
class _ElGlassSurface extends StatelessWidget {
  ...
  /// Resolved against the live theme, like every [ElShadowLayer] colour.
  final Color Function(ElThemeData) fill;
  ...
  Widget build(BuildContext context) {
    ...
    if (filter == null) {
      ...
      // also declares no outer shadow — so one `ElMachineSurface` is the whole
      ...
      assert(
        spec.layers.every((ElShadowLayer l) => l.inset),
    ...
    final List<ElShadowLayer> ambient = <ElShadowLayer>[
      for (final ElShadowLayer layer in spec.layers)
    ...
    return Stack(
      ...
      children: <Widget>[
        ...
        // `ElMachineSurface` can paint. It carries no fill and no outer layers
        // here; both are already on the canvas beneath it.
        ElMachineSurface(
          spec: ElShadowSpec(spec.insetLayers),
          radius: radius,
          child: content,
        ),
      ],
...
class ElGlassPanel extends StatelessWidget {
  const ElGlassPanel({
    super.key,
    required this.radius,
    this.padding,
  ...
  @visibleForTesting
  static ElShadowSpec get debugShadow => _panelSpec;

  /// The region the ambient layers are allowed to ink, given the painted
  /// [shape] — everything outside it and nothing within.
  ///
  /// Exposed for the same reason as [ElMachineSurface.debugInsetRing]: this is
  ...
  /// [ElGlassPanelDeep]; `glass-control` has no ambient at all.
  @visibleForTesting
  static Path debugAmbientClip(RRect shape, List<ElShadowLayer> ambient) =>
      _outsideShape(shape, ambient);

  /// The composed `blur(24px) saturate(1.5)`, shared with [ElGlassPanelDeep].
  @visibleForTesting
  static ui.ImageFilter? get debugBackdrop => _glassBackdrop;

  /// `--card` at 74%, resolved against [theme].
  @visibleForTesting
  static Color debugFill(ElThemeData theme) => _panelFill(theme);
...
/// identical to [ElGlassPanel]. Only the foundation-owned fill opacity is
/// lower, so content moving beneath remains visibly present through the blur.
class ElGlassPanelClear extends StatelessWidget {
  const ElGlassPanelClear({
    super.key,
    required this.radius,
    this.padding,
    required this.child,
  ...
  /// The same highlight, ring and `e2` ambient as [ElGlassPanel].
  @visibleForTesting
  static ElShadowSpec get debugShadow => _panelSpec;

  /// The same CSS-correct ambient exclusion as [ElGlassPanel].
  @visibleForTesting
  static Path debugAmbientClip(RRect shape, List<ElShadowLayer> ambient) =>
      _outsideShape(shape, ambient);

  /// The same composed blur and saturation as [ElGlassPanel].
  @visibleForTesting
  static ui.ImageFilter? get debugBackdrop => _glassBackdrop;

  /// The live theme's card colour at the navigation-glass opacity.
  @visibleForTesting
  static Color debugFill(ElThemeData theme) => _clearPanelFill(theme);
...
///
/// Byte-identical to [ElGlassPanel] except the ambient layer is
...
class ElGlassPanelDeep extends StatelessWidget {
  const ElGlassPanelDeep({
    super.key,
    required this.radius,
    this.padding,
    required this.child,
  ...
  /// [ElGlassPanel.debugShadow] with `--shadow-e4`'s layers in place of
  /// `--shadow-e2`'s. The first two layers are the same objects.
  @visibleForTesting
  static ElShadowSpec get debugShadow => _panelDeepSpec;

  /// See [ElGlassPanel.debugAmbientClip] — the same clip, a deeper ambient.
  @visibleForTesting
  static Path debugAmbientClip(RRect shape, List<ElShadowLayer> ambient) =>
      _outsideShape(shape, ambient);

  /// The composed `blur(24px) saturate(1.5)`, shared with [ElGlassPanel].
  @visibleForTesting
  static ui.ImageFilter? get debugBackdrop => _glassBackdrop;
...
/// [ElMachineSurface] and nothing else.
class ElGlassControl extends StatelessWidget {
  const ElGlassControl({
    super.key,
    required this.radius,
    this.padding,
    required this.child,
  ...
  @visibleForTesting
  static ElShadowSpec get debugShadow => _controlSpec;
  ...
  @visibleForTesting
  static Color debugFill(ElThemeData theme) => _controlFill(theme);
