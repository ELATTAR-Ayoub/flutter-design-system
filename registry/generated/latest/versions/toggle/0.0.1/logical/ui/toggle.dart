...
/// opposite of what [ElButton] does:
///
...
///   is the travelling pill in `ElToggleGroup`, not this.
///
/// The base class list, resolved (`toggle.tsx` L10):
///
/// | class | value |
/// |---|---|
/// | `group/toggle inline-flex items-center justify-center gap-1` | hugs its content; 4px between an icon and a label ([ElToggle.gap]) |
...
///   [ElToggleVariant.standard]: that variant is `bg-transparent` with no
///   border utility at all, so its box has zero border-width. It is live on
///   [ElToggleVariant.outline], which brings `border border-input` with it.
library;
...
enum ElToggleVariant {
  ...
  /// inner width and height on each side. [ElButton]'s base list carries
...
/// `disabled:opacity-50`.
///
/// Deliberately not [ElButton]'s 45%: the two components declare their own,
...
/// What the control paints in a given state — the same shape as `ElButton`'s
...
class _ToggleSkin {
  ...
  /// [ElToggleVariant.outline] — [ElToggleVariant.standard] has no border box
...
/// [inExclusiveGroup]) exist for `ElToggleGroup` alone. That is not a
...
/// do.
class ElToggle extends StatefulWidget {
  const ElToggle({
    super.key,
    required this.child,
    required this.pressed,
    this.onChanged,
    this.variant = ElToggleVariant.standard,
    this.size = ElToggleSize.md,
    this.label,
    this.focusNode,
    this.pressedFill,
    this.pressedInk,
  ...
  /// and re-resolving the class through [ElText] here would drop both.
  final Widget child;
  ...
  final ElToggleVariant variant;

  final ElToggleSize size;
  ...
  /// `h-7` / `h-8` / `h-9`.
  static double heightFor(ElToggleSize size) => switch (size) {
    ElToggleSize.sm => el(7),
    ElToggleSize.md => el(8),
    ElToggleSize.lg => el(9),
  };
  ...
  static double minWidthFor(ElToggleSize size) => switch (size) {
    ElToggleSize.sm => el(7),
    ElToggleSize.md => el(8),
    ElToggleSize.lg => el(9),
  };
  ...
  /// [ElToggleSize.sm]) need a child carrying `data-icon`, which nothing in
  ...
  ///
  /// Exposed rather than applied, exactly as [ElButton.gapFor] is: this
  ...
  /// `--radius-md` is [ElRadii.md], and the ceiling the CSS writes as a raw
  /// length is numerically [ElRadii.lg] — the radius every other size carries.
  ...
  static double radiusFor(ElToggleSize size) => switch (size) {
    ElToggleSize.sm => math.min(ElRadii.md, ElRadii.lg),
    ElToggleSize.md || ElToggleSize.lg => ElRadii.lg,
  };

  /// What `[&_svg:not([class*='size-'])]:size-*` would force an icon child to.
  ///
  /// A CSS descendant selector restyles a glyph it did not create; [ElIcon]
  /// takes its px up front, so this component cannot reach into its own child
  /// to resize it. The rule is exposed instead: pass
  /// `ElIcon(..., size: ElToggle.iconSizeFor(size))` and the child lands on
  /// the same 16px (14px on [ElToggleSize.sm]) the selector would have forced.
  ///
  ...
  static ElIconSize iconSizeFor(ElToggleSize size) => switch (size) {
    ElToggleSize.sm => ElIconSize.sm,
    ElToggleSize.md || ElToggleSize.lg => ElIconSize.md,
  };

  @override
  State<ElToggle> createState() => _ElToggleState();
}

class _ElToggleState extends State<ElToggle> {
  bool _hovered = false;
  bool _focused = false;
  ...
  /// pointer surface neither — the identical hand-wiring `ElButton` carries,
  ...
  _ToggleSkin _skin(ElThemeData theme) {
    final bool bordered = widget.variant == ElToggleVariant.outline;
    return _ToggleSkin(
      fill: widget.pressed
      ...
      border: bordered ? (_focused ? theme.ring : theme.input) : elTransparent,
  ...
  Widget build(BuildContext context) {
    ...
    final BorderRadius radius = BorderRadius.circular(
      ElToggle.radiusFor(widget.size),
    );
    ...
    Widget toggle = _TransitionAllColors(
      ...
      builder: (BuildContext context, Color fill, Color ink, Color border, Color ring) {
        return ElMachineSurface(
          spec: ElButton.withFocusRing(ElShadows.none, ring),
          radius: radius,
          fill: fill,
          ...
          // never spends.
          border: bordered
              ? Border.all(color: border, width: ElWidths.hairline)
              : null,
          child: Padding(
            // Just `px-*`: `ElMachineSurface` has already inset this child by
            // the border's width, the way `box-sizing: border-box` does.
            padding: EdgeInsets.symmetric(horizontal: ElToggle.paddingX),
            child: Center(
    ...
    toggle = SizedBox(height: ElToggle.heightFor(widget.size), child: toggle);
    ...
    toggle = ConstrainedBox(
      constraints: BoxConstraints(minWidth: ElToggle.minWidthFor(widget.size)),
      child: toggle,
    );

    // No `ElPress`, and no pointer-down tracking either: the class list has no
...
/// same trade `ElButton`'s `_SpringColors` makes, and for the same reason: one
