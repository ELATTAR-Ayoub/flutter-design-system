...
/// `rounded-sm` — so everything but the shape comes from [SelectionControl].
...
/// | `transition-[background-color,border-color,box-shadow] duration-fast ease-out` | 250ms — `duration-fast` emits nothing, see [MotionDurations.normal] |
///
...
/// `size-5`.
double get _boxSize => space(5);

/// `size-2` — the dot.
double get _dotSize => space(2);
...
class _RadioScope<T> extends InheritedWidget {
  ...
  /// The [FieldScope] the **group** consumed, if any.
  ///
...
class RadioGroup<T> extends StatefulWidget {
  ...
  /// The rows. Each holds a [RadioGroupItem] somewhere inside it — the
  ...
  /// `disabled` on the Root. ANDed with the enclosing [FieldScope]'s.
  final bool enabled;
  ...
  /// the wiring wraps. ORed with the enclosing [FieldScope]'s.
  final bool invalid;

  /// The node a failed submit lands on, adopted from the enclosing
  /// [FieldScope] when this is null.
  ///
  ...
  /// `Form.focusFirstError` land somewhere useful on the `payout` field,
  ...
  static double get defaultGap => space(2);
...
class _RadioGroupState<T> extends State<RadioGroup<T>> {
  ...
  final List<_RadioGroupItemState<T>> _items = <_RadioGroupItemState<T>>[];

  void _register(_RadioGroupItemState<T> item) => _items.add(item);
  void _unregister(_RadioGroupItemState<T> item) => _items.remove(item);
  ...
  bool isTabStop(T value) {
    ...
    final int first = _items.indexWhere(
      (_RadioGroupItemState<T> i) => i.enabled,
    );
  ...
  KeyEventResult moveFrom(T from, int step) {
    ...
    final List<_RadioGroupItemState<T>> enabled = _items
        .where((_RadioGroupItemState<T> i) => i.enabled)
        .toList(growable: false);
    if (enabled.length < 2) return KeyEventResult.ignored;

    final int here = enabled.indexWhere(
      (_RadioGroupItemState<T> i) => i.widget.value == from,
    );
    if (here < 0) return KeyEventResult.ignored;

    final _RadioGroupItemState<T> next =
  ...
  void _forwardToTabStop() {
    for (final _RadioGroupItemState<T> item in _items) {
  ...
  Widget build(BuildContext context) {
    final double gap = widget.gap ?? RadioGroup.defaultGap;
...
class RadioGroupItem<T> extends StatefulWidget {
  ...
  ///
  /// See [SelectionControl.forceFocusRing]. The radio matrix carries the
  ...
  ///
  /// Taken from the item's **own** nested [FieldScope] when it has one —
  ...
  @override
  State<RadioGroupItem<T>> createState() => _RadioGroupItemState<T>();
}

class _RadioGroupItemState<T> extends State<RadioGroupItem<T>> {
  final FocusNode _ownNode = FocusNode(debugLabel: 'RadioGroupItem');

  /// A nested field's node, when this item sits in a `Field` of its own that
  ...
  Widget build(BuildContext context) {
    ...
    assert(
      scope != null,
      'RadioGroupItem must be inside a RadioGroup of the same type — the '
    ...
    // `Field` — the arrangement the reference uses. Where they are the same
    ...
    // Only when the item has a field of its own — one labelled `Field` per
    ...
    return SelectionControl(
      ...
      onKey: _onKey,
      onTap: select,
      // Inside the hit-area expander, never around it — see [HitArea].
