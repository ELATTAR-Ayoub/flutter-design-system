...
///
/// **Why this does not compose `Popover`.** Three of the four things it would
/// borrow are wrong here: `Popover` slides on the bottom side only (ruling
...
///  * **It dismisses itself after [Tooltip.touchDwell]** if nothing else
...
/// therefore a constant — [MotionDurations.tooltipShowDelay] — rather than a scope. The
...
class Tooltip extends StatefulWidget {
  const Tooltip({
    ...
    required this.label,
    required this.child,
    this.delay = MotionDurations.tooltipShowDelay,
  ...
  static double get arrowSize => space(2.5);

  /// `rounded-xs` on the arrow.
  static double get arrowRadius => Radii.xs;

  /// `translate-y-[calc(-50% - 2px)]` — half the arrow, plus two.
  static double get arrowLift => arrowSize / 2 + Radii.xs;

  /// `px-3`.
  static double get horizontalPadding => space(3);
  ...
  /// the microsecond.
  ///
  /// Like [delay], it is **not** routed through [effectiveMotionDuration]: a dwell
  /// is not motion, and `prefers-reduced-motion` has nothing to say about how
  /// long a label a finger asked for stays legible.
  static Duration get touchDwell => MotionDurations.fast * 10;
...
class _TooltipState extends State<Tooltip> with SingleTickerProviderStateMixin {
  ...
  /// [Tooltip.touchDwell]'s timer, cancelled the moment anything else closes
  /// the label.
  ///
  ...
  /// pointer to leave, so nothing but another tap (or [Tooltip.touchDwell])
  /// may take it down.
  bool _byTouch = false;
  ...
  /// The hover delay is **not** routed through [effectiveMotionDuration]: it is a
  ...
  void _exit(PointerExitEvent event) {
    ...
    // the finger did to [Tooltip.touchDwell] and to the next tap.
  ...
  void _down(PointerDownEvent event) {
    ...
    _show();
    _dwell?.cancel();
    _dwell = Timer(Tooltip.touchDwell, () {
      if (mounted) _hide();
    });
  }
  ...
  @override
  Widget build(BuildContext context) => flutter.OverlayPortal(
    controller: _portal,
...
class TooltipContent extends StatelessWidget {
  ...
  /// Which edge the lane sits on — below the pill on [TooltipSide.top], left
  /// of it on [TooltipSide.right].
  final TooltipSide side;
  ...
  Widget build(BuildContext context) {
      ...
      child: DecoratedBox(
        ...
        child: Padding(
          padding: EdgeInsets.symmetric(
            horizontal: Tooltip.horizontalPadding,
            vertical: Tooltip.verticalPadding,
          ),
          child: StyledText(label, TextStyles.small, color: theme.background),
        ),
      ),
    );
    ...
    // exactly [Tooltip.arrowSize] of layout — which is where the measured
    ...
    return switch (side) {
      TooltipSide.top => Column(
        mainAxisSize: MainAxisSize.min,
        children: <Widget>[pill, lane],
      ),
      TooltipSide.right => Row(
        mainAxisSize: MainAxisSize.min,
...
class _ArrowPainter extends CustomPainter {
  const _ArrowPainter({required this.color, this.side = TooltipSide.top});
  ...
  void paint(Canvas canvas, Size size) {
    final double square = Tooltip.arrowSize;
    ...
    final Offset centre = switch (side) {
      ...
      TooltipSide.top => Offset(size.width / 2, -Radii.xs),
      TooltipSide.right => Offset(size.width + Radii.xs, size.height / 2),
    };

    final RRect diamond = RRect.fromRectAndRadius(
      Rect.fromCenter(center: centre, width: square, height: square),
      Radius.circular(Tooltip.arrowRadius),
    );
    canvas
      ..save()
