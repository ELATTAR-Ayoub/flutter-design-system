...
///   from [ElThemeKind] against [ElPalette] (ruling S3's precedent: raw-palette
///   access is this port's pattern where a token is not on [ElThemeData]).
///
...
///   [ElVoiceOrb.level], a plain listenable, for the reason in the divergence
///   note below.
///
...
///     [ElVoiceOrb.level] is that number. Everything downstream of it, from the
...
class ElOrbProgram {
  ElOrbProgram._(this.program, this.noise);
  ...
  static Future<ElOrbProgram?>? _pending;
  static ElOrbProgram? _loaded;
  ...
  static ElOrbProgram? get loaded => _loaded;

  /// Compiles and decodes on first call; every later call gets the same future.
  static Future<ElOrbProgram?> load() {
    if (_pending != null) return _pending!;
    return _pending = _load();
  }

  static Future<ElOrbProgram?> _load() async {
    try {
      ...
      _lastError = null;
      return _loaded = ElOrbProgram._(program, frame.image);
    } catch (error, stack) {
      _lastError = error;
...
class ElVoiceOrb extends StatefulWidget {
  const ElVoiceOrb({
    super.key,
    this.state = ElOrbState.idle,
    this.level,
  ...
  static List<double> offsetsForSeed(int seed) =>
      _ElVoiceOrbState._makeOffsets(seed).toList();

  @override
  State<ElVoiceOrb> createState() => _ElVoiceOrbState();
}

class _ElVoiceOrbState extends State<ElVoiceOrb>
  ...
  bool _stilled = false;
  ElOrbProgram? _resources = ElOrbProgram.loaded;
  ...
  void initState() {
    super.initState();
    if (_resources == null) {
      ElOrbProgram.load().then((ElOrbProgram? loaded) {
  ...
  @override
  void didUpdateWidget(ElVoiceOrb old) {
  ...
  void didChangeDependencies() {
    ...
    // The same question [elAnimationDuration] answers for every other looper in
  ...
  void _tick(Duration elapsed) {
    ...
    final bool listening = widget.state == ElOrbState.listening;
  ...
  Widget build(BuildContext context) {
    ...
    final ElThemeKind kind = ElTheme.kindOf(context);
    ...
    return RepaintBoundary(
      child: CustomPaint(
        ...
        painter: _OrbPainter(
          ...
          from: from,
          to: to,
          inverted: kind == ElThemeKind.dark,
        ),
      ),
    );
  }
  ...
  /// uses."* Both stops are ramp members, so they read off [ElPalette] rather
  /// than adding two fields to [ElThemeData] that nothing else would consume.
  static (Color, Color) _orbStops(ElThemeKind kind) => switch (kind) {
    ElThemeKind.light => (ElPalette.action, ElPalette.actionDark),
    ElThemeKind.dark => (ElPalette.actionBright, ElPalette.action),
  };
...
class _OrbPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final double side = size.shortestSide * ElVoiceOrb.discFraction;
    if (side <= 0) return;
