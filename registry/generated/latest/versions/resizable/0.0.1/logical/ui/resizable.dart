...
///     [ElResizablePanel.defaultSize] is a weight, [ElResizablePanel.minSize]
///     is pixels.
...
const double _handleWidth = ElWidths.hairline;
...
/// reasoning. 24 is `el(6)`, the same rung the grip is 24 *tall* at, so the
/// strip is a square around the visible grip rather than a number of its own.
double get handleGrab => el(6);
...
Size get gripSize => Size(el(1), el(6));
...
const String elResizableHandleFocusLabel = 'ElResizableHandle';

/// One panel's declaration.
class ElResizablePanel {
...
/// pair.
class ElResizablePanelGroup extends StatefulWidget {
  const ElResizablePanelGroup({
    super.key,
  ...
  final List<ElResizablePanel> panels;
  ...
  @override
  State<ElResizablePanelGroup> createState() => _ElResizablePanelGroupState();
}

class _ElResizablePanelGroupState extends State<ElResizablePanelGroup> {
  ...
  List<double> _normalised() {
    final double total = widget.panels.fold(
      0,
      (double a, ElResizablePanel p) => a + p.defaultSize,
    );
    if (total <= 0) {
    ...
    return <double>[
      for (final ElResizablePanel p in widget.panels)
        p.defaultSize / total * 100,
    ];
  }
...
class _GrabStripState extends State<_GrabStrip> {
  ...
  final FocusNode _focus = FocusNode(debugLabel: elResizableHandleFocusLabel);
