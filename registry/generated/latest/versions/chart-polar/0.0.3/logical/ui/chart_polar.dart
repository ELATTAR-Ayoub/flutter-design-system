...
/// chips. Here it is one function, [polarToCartesian].
///
...
/// overlapping series."*
enum PolarGridType { polygon, circle }
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints c) {
        ...
        final double maxRadius = polarMaxRadius(plot.width, plot.height);
  ...
  void _buildPie(
    ...
    for (int i = 0; i < pie.data.length; i++) {
      ...
      if (pie.outsideLabel || pie.labelBuilder != null) {
        ...
        final Offset anchor = polarToCartesian(
          centre.dx,
          centre.dy,
          outer + outer / 3,
          mid,
        );
        final Offset elbow = polarToCartesian(centre.dx, centre.dy, outer, mid);
        outside.add(
          _OutsideLabel(
            text: pie.labelBuilder != null
                ? pie.labelBuilder!(row)
                : chartNumber((row[pie.dataKey] as num?) ?? 0),
            anchor: anchor,
            elbow: elbow,
...
class RadarSpec {
  ...
  final Color? stroke;
  final double strokeWidth;
  final ChartDotSpec? dot;
}

/// `dot={{ r: 4, fillOpacity: 1 }}`.
@immutable
class ChartDotSpec {
  const ChartDotSpec({this.radius = 4, this.fillOpacity = 1});
  final double radius;
  final double fillOpacity;
}
...
class _RadarChartState extends State<RadarChart>
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints c) {
        ...
        final double radius =
            polarMaxRadius(box.width + 10, box.height + 10) * 0.8;
        final int n = widget.data.length;
        final double step = n == 0
            ? 0
            : (RadarChart.endAngle - RadarChart.startAngle) / n;
        final List<double> angles = <double>[
          for (int i = 0; i < n; i++) RadarChart.startAngle + step * i,
        ];

        double maxValue = 0;
        for (final Map<String, Object?> row in widget.data) {
          for (final RadarSpec s in widget.series) {
            maxValue = math.max(
              maxValue,
              ...
              if (widget.legend != null)
                Positioned(
                  ...
                  child: ChartLegendContent(
                    ...
                    items: <ChartLegendItem>[
                      for (final RadarSpec s in widget.series)
                        ChartLegendItem(name: s.dataKey, color: s.fill),
                    ],
                  ),
                ),
            ],
  ...
  List<Widget> _angleLabels(
    ...
    final PolarAngleAxis axis = widget.angleAxis!;
    return <Widget>[
      for (int i = 0; i < angles.length; i++)
        _PolarLabel(
          ...
          anchor: polarToCartesian(
            centre.dx,
            centre.dy,
            radius + PolarAngleAxis.tickSize,
            angles[i],
          ),
          angle: angles[i],
  ...
  List<Widget> _radiusLabels(
    ...
    final PolarRadiusAxis axis = widget.radiusAxis!;
    return <Widget>[
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (g != null) {
      ...
      for (int r = 0; r < ringRadii.length; r++) {
        ...
        final Path ring = g.gridType == PolarGridType.circle
            ? (Path()..addOval(Rect.fromCircle(center: centre, radius: rr)))
            : polarPolygonPath(<Offset>[
                for (final double a in angles)
                  polarToCartesian(centre.dx, centre.dy, rr, a),
              ]);
      ...
      if (g.radialLines) {
        for (final double a in angles) {
          canvas.drawLine(
            centre,
            polarToCartesian(centre.dx, centre.dy, radius, a),
            stroke,
          );
        }
      }
    ...
    for (final RadarSpec s in series) {
      ...
      final Path path = polarPolygonPath(points);
    ...
    final PolarAngleAxis? axis = angleAxis;
    if (axis != null) {
      final Paint stroke = Paint()
        ..color = theme.border
        ..strokeWidth = BorderWidths.hairline
    ...
    final PolarRadiusAxis? rAxis = radiusAxis;
    if (rAxis != null && rAxis.axisLine) {
      canvas.drawLine(
        centre,
        polarToCartesian(centre.dx, centre.dy, radius, rAxis.angle),
        Paint()
...
class RadialBarChart extends StatefulWidget {
  ...
  final List<Map<String, Object?>> data;
  final List<RadialBarSpec> series;
  final double innerRadius;
  final double outerRadius;
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints c) {
        ...
        final List<BarSlot> slots = barSlots(bandSize: bandSize, barCount: 1);
            ...
            children: <Widget>[
              ...
              for (final _ChipLabel chip in chips)
                Positioned(
                  ...
                  child: FractionalTranslation(
                    translation: const Offset(-0.5, -0.5),
                    child: _ArcChip(text: chip.text, isNumeric: chip.isNumeric),
                  ),
                ),
