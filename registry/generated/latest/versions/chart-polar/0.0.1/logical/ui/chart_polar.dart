...
/// chips. Here it is one function, [elPolarToCartesian].
///
...
/// overlapping series."*
enum ElPolarGridType { polygon, circle }

/// `PolarGrid`.
@immutable
class ElPolarGrid {
  const ElPolarGrid({
    this.gridType = ElPolarGridType.polygon,
  ...
  final ElPolarGridType gridType;
...
/// `PolarAngleAxis`.
@immutable
class ElPolarAngleAxis {
  const ElPolarAngleAxis({
    this.dataKey,
    this.tick = true,
    this.axisLine = true,
    this.tickLine = true,
...
/// `PolarRadiusAxis`.
@immutable
class ElPolarRadiusAxis {
  const ElPolarRadiusAxis({
    this.angle = 90,
    this.tick = true,
    this.axisLine = true,
    this.stroke,
...
/// `PieChart`.
class ElPieChart extends StatefulWidget {
  const ElPieChart({
    super.key,
    required this.pies,
    this.legend,
  ...
  @override
  State<ElPieChart> createState() => _ElPieChartState();
}

class _ElPieChartState extends State<ElPieChart>
  ...
  Widget _pieTooltip(
    ...
    final ElPieSpec pie = widget.pies.first;
  ...
  void _buildPie(
    ...
    for (int i = 0; i < pie.data.length; i++) {
      ...
      if (pie.outsideLabel || pie.labelBuilder != null) {
        ...
        final Offset anchor = elPolarToCartesian(
          centre.dx,
          centre.dy,
          outer + outer / 3,
          mid,
        );
        final Offset elbow = elPolarToCartesian(
          centre.dx,
          centre.dy,
          outer,
          mid,
        ...
        outside.add(
          _OutsideLabel(
            text: pie.labelBuilder != null
                ...
                : elChartNumber((row[pie.dataKey] as num?) ?? 0),
            anchor: anchor,
            elbow: elbow,
...
class ElRadarSpec {
  ...
  final Color? stroke;
  final double strokeWidth;
  final ElChartDotSpec? dot;
}

/// `dot={{ r: 4, fillOpacity: 1 }}`.
@immutable
class ElChartDotSpec {
  const ElChartDotSpec({this.radius = 4, this.fillOpacity = 1});
  final double radius;
  final double fillOpacity;
}

/// `RadarChart`.
class ElRadarChart extends StatefulWidget {
  const ElRadarChart({
    super.key,
    required this.data,
  ...
  final List<Map<String, Object?>> data;
  final List<ElRadarSpec> series;
  final ElPolarGrid? grid;
  final ElPolarAngleAxis? angleAxis;
  final ElPolarRadiusAxis? radiusAxis;
  final ElChartLegendSpec? legend;
  ...
  @override
  State<ElRadarChart> createState() => _ElRadarChartState();
}

class _ElRadarChartState extends State<ElRadarChart>
    with SingleTickerProviderStateMixin {
  late final AnimationController _entrance = AnimationController(
    vsync: this,
    duration: ElChartMotion.duration,
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints c) {
        ...
        final double radius =
            elPolarMaxRadius(box.width + 10, box.height + 10) * 0.8;
        final int n = widget.data.length;
        final double step = n == 0
            ? 0
            : (ElRadarChart.endAngle - ElRadarChart.startAngle) / n;
        final List<double> angles = <double>[
          for (int i = 0; i < n; i++) ElRadarChart.startAngle + step * i,
        ];

        double maxValue = 0;
        for (final Map<String, Object?> row in widget.data) {
          for (final ElRadarSpec s in widget.series) {
            maxValue = math.max(
              maxValue,
              ((row[s.dataKey] as num?) ?? 0).toDouble(),
            );
          }
        }
              ...
              if (widget.legend != null)
                Positioned(
                  ...
                  child: ElChartLegendContent(
                    ...
                    items: <ElChartLegendItem>[
                      for (final ElRadarSpec s in widget.series)
                        ElChartLegendItem(name: s.dataKey, color: s.fill),
                    ],
                  ),
                ),
            ],
  ...
  List<Widget> _angleLabels(
    ...
    final ElPolarAngleAxis axis = widget.angleAxis!;
    return <Widget>[
      for (int i = 0; i < angles.length; i++)
        _PolarLabel(
          ...
          anchor: elPolarToCartesian(
            centre.dx,
            centre.dy,
            radius + ElPolarAngleAxis.tickSize,
            angles[i],
          ),
          angle: angles[i],
  ...
  List<Widget> _radiusLabels(
    ...
    final ElPolarRadiusAxis axis = widget.radiusAxis!;
    return <Widget>[
...
class _PolarLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Positioned(
    left: anchor.dx,
    top: anchor.dy,
    child: FractionalTranslation(translation: _shift(angle), child: child),
  );
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (g != null) {
      ...
      for (int r = 0; r < ringRadii.length; r++) {
        ...
        final Path ring = g.gridType == ElPolarGridType.circle
            ? (Path()..addOval(Rect.fromCircle(center: centre, radius: rr)))
            : elPolarPolygonPath(<Offset>[
                for (final double a in angles)
                  elPolarToCartesian(centre.dx, centre.dy, rr, a),
              ]);
        ...
        if (fills != null && fills.isNotEmpty) {
          ...
          canvas.drawPath(
            ring,
            Paint()..color = fill.withValues(alpha: g.opacity),
          );
        }
      ...
      if (g.radialLines) {
        for (final double a in angles) {
          canvas.drawLine(
            centre,
            elPolarToCartesian(centre.dx, centre.dy, radius, a),
            stroke,
          );
        }
      }
    ...
    for (final ElRadarSpec s in series) {
      ...
      final Path path = elPolarPolygonPath(points);
      ...
      final ElChartDotSpec? dot = s.dot;
      if (dot != null) {
        for (final Offset p in points) {
          canvas.drawCircle(
            ...
            dot.radius,
            Paint()
              ..color = (s.fill ?? theme.foreground).withValues(
                alpha: dot.fillOpacity,
              ),
          );
        }
      }
    ...
    final ElPolarAngleAxis? axis = angleAxis;
    if (axis != null) {
      final Paint stroke = Paint()
        ..color = theme.border
        ..strokeWidth = ElWidths.hairline
    ...
    final ElPolarRadiusAxis? rAxis = radiusAxis;
    if (rAxis != null && rAxis.axisLine) {
      canvas.drawLine(
        centre,
        elPolarToCartesian(centre.dx, centre.dy, radius, rAxis.angle),
        Paint()
...
/// One `RadialBar`.
@immutable
class ElRadialBarSpec {
  const ElRadialBarSpec({
    required this.dataKey,
    this.fill,
...
/// `RadialBarChart`.
class ElRadialBarChart extends StatefulWidget {
  const ElRadialBarChart({
    super.key,
    required this.data,
    required this.series,
  ...
  final List<Map<String, Object?>> data;
  final List<ElRadialBarSpec> series;
  final double innerRadius;
  final double outerRadius;
  ...
  @override
  State<ElRadialBarChart> createState() => _ElRadialBarChartState();
}

class _ElRadialBarChartState extends State<ElRadialBarChart>
    with SingleTickerProviderStateMixin {
  late final AnimationController _entrance = AnimationController(
    vsync: this,
    duration: ElChartMotion.duration,
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints c) {
        ...
        final double bandSize = rows == 0
            ? 0
            : (widget.outerRadius - widget.innerRadius) / rows;
        ...
        if (widget.angleAxis?.domain == null) {
          for (final Map<String, Object?> row in widget.data) {
            for (final ElRadialBarSpec s in widget.series) {
              domainMax = math.max(
                domainMax,
                ((row[s.dataKey] as num?) ?? 0).toDouble(),
              );
        ...
        for (int i = 0; i < rows; i++) {
          ...
          final double inner = widget.innerRadius + bandSize * i + slotOffset;
...
class _RadialPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (g != null) {
      ...
      for (int i = 0; i < radii.length; i++) {
        ...
        if (fills != null && fills.isNotEmpty) {
          canvas.drawPath(
            ring,
            Paint()..color = i == 0 ? fills.first : fills.last,
          );
        }
        if (g.stroke != null) {
          canvas.drawPath(
    ...
    for (final _Arc arc in arcs) {
      final double end = arc.background
          ? arc.end
          : arc.start + (arc.end - arc.start) * t;
...
class _PolarText extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      ElText(text, ElChartText.xs, color: color, align: align);
}
