...
class ElAgentCommand {
  ...
  final String label;
  final String? hint;
  final ElAgentCommandGroup group;
  ...
  ///
  /// A [ElLucideGlyph] rather than the icons page's curated [ElIconGlyph]: the
...
/// Escape fail to close the palette — see [elSlashQuery] and the composer's
/// drift register.
...
String? elSlashQuery(String value, int caret) {
...
List<ElAgentCommand> elFilterCommands(
  List<ElAgentCommand> commands,
  String query,
) {
...
class _Group {
  ...
  final String title;
  final List<ElAgentCommand> items;
}
...
/// down to their bugs (see [ElAgentSlashPalette.scrollsGroupsNotRows]).
class ElAgentSlashPalette extends StatelessWidget {
  const ElAgentSlashPalette({
    super.key,
  ...
  /// Already filtered — the composer runs [elFilterCommands].
  final List<ElAgentCommand> commands;

  final int activeIndex;
  final ValueChanged<ElAgentCommand> onSelect;
  final ValueChanged<int> onHover;

  /// `max-h-64`.
  static double get maxHeight => el(64);

  /// `mb-2` — the gap between the palette's bottom edge and the composer's top.
  static double get bottomGap => el(2);
  ...
  static Duration get entrance => ElDurations.slow;
  ...
  static EdgeInsets get headingInsets =>
      EdgeInsets.fromLTRB(el(3), el(3), el(3), el(1));

  /// `px-3 py-2` on a row.
  static EdgeInsets get rowInsets =>
      EdgeInsets.symmetric(horizontal: el(3), vertical: el(2));

  /// `gap-3` between the glyph and the text column.
  static double get rowGap => el(3);

  /// `gap-1` between the two lines of the text column.
  static double get lineGap => el(1);

  /// `mt-1` on the glyph — it hangs one step below the first line's cap.
  static double get glyphTopInset => el(1);

  /// `size-4`.
  static double get glyphSize => el(4);
  ...
  static double get lucideStroke => ElIcon.strokeFor(ElIconPaths.viewBox);
  ...
  List<_Group> get _groups => <_Group>[
    _Group(
      'Skills',
      commands
          .where((ElAgentCommand c) => c.group == ElAgentCommandGroup.skill)
          .toList(),
    ),
    _Group(
      'Commands',
      commands
          .where((ElAgentCommand c) => c.group == ElAgentCommandGroup.command)
          .toList(),
    ),
...
class _PaletteBoxState extends State<_PaletteBox> {
  ...
  /// [ElAgentSlashPalette.scrollsGroupsNotRows].
  ...
  Widget build(BuildContext context) {
      ...
      rows.add(
        KeyedSubtree(
          ...
          child: Column(
            ...
            children: <Widget>[
              Padding(
                padding: ElAgentSlashPalette.headingInsets,
                child: ElText(
                  group.title,
                  ElType.caption,
              ...
              for (final ElAgentCommand command in group.items)
                Builder(
    ...
    final Widget box = DecoratedBox(
      decoration: BoxDecoration(
        ...
        boxShadow: ElShadows.e3.outerShadows(theme),
      ),
      child: ClipRRect(
        ...
        child: Padding(
          ...
          child: ConstrainedBox(
            constraints: BoxConstraints(
              maxHeight: ElAgentSlashPalette.maxHeight - ElWidths.hairline * 2,
            ),
    ...
    return TweenAnimationBuilder<double>(
      ...
      duration: elAnimationDuration(context, ElAgentSlashPalette.entrance),
      curve: ElCurves.out,
      builder: (BuildContext context, double t, Widget? child) => Opacity(
        opacity: t,
        child: Transform.translate(
          offset: Offset(0, ElAgentSlashPalette.rise * (1 - t)),
          child: child,
        ),
      ),
      child: box,
...
class _PaletteRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final ElLucideGlyph? glyph = command.icon;
      ...
      child: MouseRegion(
        ...
        child: Listener(
          ...
          child: ColoredBox(
            ...
            color: active ? theme.accent : elTransparent,
            child: Padding(
              padding: ElAgentSlashPalette.rowInsets,
              child: Row(
                // `items-start`.
                  ...
                  if (glyph != null) ...<Widget>[
                    Padding(
                      padding: EdgeInsets.only(
                        top: ElAgentSlashPalette.glyphTopInset,
                      ),
                      // `text-agent` — a colour utility on the glyph, which
                      // `ElIconTone.inherit` reads off the ambient text style
                      // exactly as `text-current` does.
                      child: DefaultTextStyle.merge(
                        style: TextStyle(color: theme.agent),
                        child: ElIcon.lucide(
                          glyph,
                          sizePx: ElAgentSlashPalette.glyphSize,
                          strokeOverride: ElAgentSlashPalette.lucideStroke,
                        ),
                      ),
                    ),
                    SizedBox(width: ElAgentSlashPalette.rowGap),
                  ],
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        if (command.hint != null) ...<Widget>[
                          SizedBox(height: ElAgentSlashPalette.lineGap),
                          ElText(
                            command.hint!,
                            ElType.caption,
                            color: theme.mutedForeground,
