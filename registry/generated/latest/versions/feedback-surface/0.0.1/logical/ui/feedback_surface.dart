/// `feedback-surface` — the iridescence behind an Alert and a toast
...
/// ```
///
/// so they are computed here through [OklabColor] from the two hues a variant
...
/// same reason `action-feedback` needs none: [fill] is opaque and went into this
...
/// about the layer's own `transform-origin`. See [BloomDrift.matrixFor].
///
...
/// `effectiveMotionDuration` — there is deliberately no out-of-view pause, because
...
// the resolver became the instance method [FeedbackSurface._ink], which is part
...
/// conversion (the same rule `Blurs` records).
final double _deepBlur = space(4);
...
final double _nearBlur = space(2.5);
...
@immutable
class BloomDriftStop {
  const BloomDriftStop(
    this.percent, {
...
/// [Keyframes.doubles]'s own rule: the clock stays linear so nothing is eased
/// twice.
@immutable
class BloomDrift {
  ...
  /// `static final` rather than `const` — the same shape `Shadows.controlPrimary`
  ...
  /// 2.2 on hover.
  static final BloomDrift deep = BloomDrift(
    duration: MotionDurations.cosmicDriftDeep,
    originX: 0.88,
    originY: 0.82,
    hoverScale: 2.2,
    stops: <BloomDriftStop>[
      BloomDriftStop(0, translate: Offset.zero, degrees: 0, scale: 1),
      BloomDriftStop(
        50,
  ...
  static final BloomDrift near = BloomDrift(
    duration: MotionDurations.cosmicDriftNear,
    originX: 0.90,
    originY: 0.86,
    hoverScale: 2.5,
    stops: <BloomDriftStop>[
      BloomDriftStop(0, translate: Offset.zero, degrees: 0, scale: 1.04),
      BloomDriftStop(
        35,
      ...
      BloomDriftStop(70, translate: Offset(0.03, 0.02), degrees: 6, scale: 1),
      BloomDriftStop(
        100,
  ...
  final List<BloomDriftStop> stops;

  /// `&:hover::before { scale: 2.2 }` — the standalone `scale` property, over
  /// [MotionDurations.bloom] on `--ease-out`.
  ...
  double _swellAt(double hoverT) =>
      1 +
      (hoverScale - 1) * MotionCurves.enter.transform(hoverT.clamp(0.0, 1.0));

  static double _readX(BloomDriftStop s) => s.translate.dx;
  static double _readY(BloomDriftStop s) => s.translate.dy;
  static double _readScale(BloomDriftStop s) => s.scale;
  static double _readRadians(BloomDriftStop s) => s.degrees * math.pi / 180;

  static Animatable<double> _track(
    List<BloomDriftStop> stops,
    double Function(BloomDriftStop) read,
  ) => Keyframes.doubles(<KeyframeStop<double>>[
    for (final BloomDriftStop stop in stops)
      KeyframeStop<double>(stop.percent, read(stop)),
...
// `ShadowLayer.color`: a variant's hue may be a raw ramp entry or a theme
// token, and only the theme knows which.

Color _actionBright(ThemeTokens t) => Palette.actionBright;
Color _action(ThemeTokens t) => Palette.action;
Color _value(ThemeTokens t) => Palette.value;
Color _success(ThemeTokens t) => Palette.success;
...
/// Paints `feedback-surface` behind [child], and hangs `.starfield` over it.
///
...
/// for a reason worth stating: the CSS `:hover` is on the `.feedback-surface`
...
/// worked around.
enum FeedbackVariant { neutral, info, success, warning, error, loading }

class FeedbackSurface extends StatefulWidget {
  const FeedbackSurface({
    super.key,
    this.variant = FeedbackVariant.neutral,
  ...
  Color Function(ThemeTokens) get bloom1 => switch (variant) {
    FeedbackVariant.neutral || FeedbackVariant.loading => _actionBright,
    FeedbackVariant.info => _info,
    FeedbackVariant.success => _success,
    FeedbackVariant.warning => _warning,
  ...
  /// caught by `.feedback-surface [data-content]::before`. Mounting it here rather
  ...
  ///
  /// A thin reader over [ThemeTokens.bloomVoid], kept because half the callers
  /// of this file hold a [ResolvedColorMode] and not a whole theme, and because the
  /// name says which custom property is being asked for.
  static Color voidFor(ResolvedColorMode kind) => switch (kind) {
    ResolvedColorMode.light => ThemeTokens.light.bloomVoid,
    ResolvedColorMode.dark => ThemeTokens.dark.bloomVoid,
  };
  ...
  BloomInk inkFor(ThemeTokens theme) {
    Color relit(Color source) {
      ...
      return OklabColor.fromOklch( // allow-hardcoded: converts semantic token inputs; no color literal is declared here
        from.l * theme.bloomL,
        from.c * theme.bloomC,
        from.h,
      );
    ...
    final ({double l, double c, double h}) coreLch = OklabColor.toOklch(core);

    return BloomInk(
      core: core,
      glow: relit(bloom1(theme)),
      // `calc(l + (1 - l) * var(--bloom-lift))` on the core's own lightness.
      hot: OklabColor.fromOklch( // allow-hardcoded: derives the effect-owned hot color from semantic token inputs
        coreLch.l + (1 - coreLch.l) * theme.bloomLift,
        coreLch.c * theme.bloomHotC,
        coreLch.h,
      ),
      voidInk: theme.bloomVoid,
      opacity: theme.kind == ResolvedColorMode.light
          ? _lightOpacity
          : _darkOpacity,
  ...
  @override
  State<FeedbackSurface> createState() => _BloomCosmicState();
}

class _BloomCosmicState extends State<FeedbackSurface>
    with TickerProviderStateMixin {
  /// The two drifts. The durations named here are placeholders for the first
  /// frame only — [build] re-reads both through [effectiveMotionDuration] on every
  /// pass, the way `Spinner` and `KeyframePlayer` do.
  late final AnimationController _deep = AnimationController(
    vsync: this,
    duration: BloomDrift.deep.duration,
  );
  late final AnimationController _near = AnimationController(
    vsync: this,
    duration: BloomDrift.near.duration,
  );
  ...
  void didChangeDependencies() {
    ...
    final bool stilled =
        effectiveMotionDuration(context, BloomDrift.deep.duration) ==
        Duration.zero;
  ...
  /// Supervisor ruling F2: both loopers build in full, and `effectiveMotionDuration`
  ...
  Widget build(BuildContext context) {
    ...
    _deep.duration = effectiveMotionDuration(context, BloomDrift.deep.duration);
    _near.duration = effectiveMotionDuration(context, BloomDrift.near.duration);
    _hover.duration = effectiveMotionDuration(context, MotionDurations.bloom);
...
class _BloomPainter extends CustomPainter {
  ...
  Matrix4 _matrix(BloomDrift drift, Rect layer, double t) => stilled
  ...
  void _paintDeep(Canvas canvas, Rect box) {
    ...
    canvas.save();
    canvas.transform(_matrix(BloomDrift.deep, layer, deepT).storage);
    ...
    _drawRadial(
      ...
      centreX: 0.94,
      centreY: 0.40,
      colors: <Color>[OklabColor.mix(ink.glow, ink.core, 0.60), ink.voidInk],
    ...
    _drawRadial(
      ...
      colors: <Color>[
        ink.core,
        OklabColor.mix(ink.core, ink.voidInk, 0.45),
        ink.voidInk,
      ],
  ...
  void _paintNear(Canvas canvas, Rect box) {
    ...
    canvas.save();
    canvas.transform(_matrix(BloomDrift.near, layer, nearT).storage);
    ...
    _drawRadial(
      ...
      centreY: 0.46,
      colors: <Color>[
        OklabColor.mix(ink.glow, ink.core, 0.50),
        OklabColor.mix(ink.core, ink.voidInk, 0.50),
        ink.voidInk,
      ],
    ...
    _drawRadial(
      ...
      centreY: 0.12,
      colors: <Color>[
        OklabColor.mix(ink.core, ink.hot, 0.60),
        ink.core,
        ink.voidInk,
      ],
    ...
    _drawRadial(
      ...
      centreY: 0.84,
      colors: <Color>[
        OklabColor.mix(ink.glow, ink.hot, 0.35),
        ink.glow,
        OklabColor.mix(ink.glow, ink.voidInk, 0.45),
        ink.voidInk,
      ],
  ...
  Paint _groupPaint(double sigma) => Paint()
    ..blendMode = ink.blend
    ..color = transparent.withValues(alpha: ink.opacity)
...
/// instead — the trick `background_effect.dart` documents.
