...
/// ([ElSafeArea]); a scroll view keeps scrolling under both bars and pays the
...
/// [ElSafeArea] is how a customer app inherits the fix without knowing it
...
/// [ElSafeArea] returns its child **unwrapped**, so no [Padding] and no
...
/// hands down, so nesting is safe: a [ElSafeArea] inside a sheet that already
/// paid the bottom bar reads zero and adds nothing.
///
/// ```dart
/// DecoratedBox(                       // paints to the screen edge
///   decoration: const BoxDecoration(/* … */),
///   child: ElSafeArea(bottom: false, child: Row(children: controls)),
/// )
/// ```
class ElSafeArea extends StatelessWidget {
  ...
  /// pairs this with a `ElSafeArea(bottom: false)` around the bar's contents,
