/// `action-feedback` — the default Button's surface (`app/globals.css`
...
/// `color-mix(in oklab, …)`, so each is computed here from [Palette] through
/// [OklabColor.mix] rather than pasted in as the hex it happens to resolve to
...
/// (colors.dart L110–115), so the texture's inks are [Palette.actionBright]
...
/// [Surface.fill] is a flat [Color] and cannot carry a gradient, so
...
/// [_SheenActionState._drive]). `:hover::before` and `:active::before` name
...
///
/// **Reduced motion.** Every duration is re-read through [effectiveMotionDuration]
...
/// The `action-feedback` surface: ramp, texture, beat, wrapped around [child].
///
...
class ActionFeedback extends StatefulWidget {
  ...
  static final List<Color> rampColors = <Color>[
    OklabColor.mix(Palette.actionBright, Palette.action, 0.20),
    Palette.action,
    OklabColor.mix(Palette.actionDark, Palette.action, 0.50),
    Palette.action,
    OklabColor.mix(Palette.actionBright, Palette.action, 0.10),
  ];
  ...
  static BlendMode beatBlendFor(ResolvedColorMode kind) => switch (kind) {
  ...
  @override
  State<ActionFeedback> createState() => _SheenActionState();
}
...
TweenSequenceItem<double> _beatStep(double from, double to, double weight) =>
    TweenSequenceItem<double>(
      ...
      ).chain(CurveTween(curve: MotionCurves.enter)),
      weight: weight,
    );

class _SheenActionState extends State<ActionFeedback>
  ...
  void _onTick(Duration elapsed) {
    _elapsed = elapsed;
    _beat.value = ActionFeedback.phaseAt(
      elapsed,
      _period,
  ...
  Widget build(BuildContext context) {
    ...
    final BlendMode blend = ActionFeedback.beatBlendFor(
      ThemeScope.kindOf(context),
    );
    ...
    _drive(rule, switch (rule) {
      _Beat.press => effectiveMotionDuration(
        context,
        MotionDurations.beatPress,
      ),
      _Beat.hover => effectiveMotionDuration(
        context,
    ...
    canvas.drawRect(
      ...
      Paint()
        ..shader = ui.Gradient.linear(
          from,
          to,
          ActionFeedback.rampColors,
          ActionFeedback.rampStops,
        ),
    );
  }
...
class _PseudoPainter extends CustomPainter {
  ...
  /// [ActionFeedback.beatBlendFor] for the live theme.
  ...
  void _paintBeat(Canvas canvas, Rect box) {
    final double opacity = ActionFeedback.beatOpacity.transform(beat);
    if (opacity <= 0) return;
    final double scale = ActionFeedback.beatScale.transform(beat);
    ...
    canvas.drawRect(
      ...
      Paint()
        ..shader = _radialShader(
          ...
          centreY: _beatCentre,
          colors: <Color>[
            Palette.actionBright.withValues(alpha: _beatCoreAlpha),
            Palette.actionBright.withValues(alpha: _beatMidAlpha),
            Palette.actionBright.withValues(alpha: 0),
          ],
    ...
    canvas.drawRect(
      ...
      Paint()
        ..shader = _radialShader(
          ...
          colors: <Color>[
            Palette.actionBright.withValues(alpha: _cornerAlpha),
            Palette.actionBright.withValues(alpha: 0),
          ],
      ...
      Paint()
        ..shader = ui.Gradient.linear(
          ...
          <Color>[
            ...
            Palette.actionBright.withValues(alpha: 0),
            Palette.actionBright.withValues(alpha: _diagonalAlpha),
            Palette.actionBright.withValues(alpha: 0),
          ],
    ...
    canvas.drawRect(
      ...
      Paint()
        ..shader = ui.Gradient.linear(
          ...
          from + tile,
          <Color>[
            Palette.actionBright.withValues(alpha: _striationAlpha),
            Palette.actionBright.withValues(alpha: _striationAlpha),
            Palette.actionBright.withValues(alpha: 0),
            Palette.actionBright.withValues(alpha: 0),
          ],
...
// them. `premium_surface.dart` carries the same four verbatim: the plan's ownership
