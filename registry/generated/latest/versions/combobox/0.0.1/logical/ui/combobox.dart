...
/// | addon button | **24 × 24**, radius **7px** ([ElRadii.addonButton]), `p-0`, ghost, and `data-pressed:bg-transparent` — the one button in the system whose press cancels its own fill | — |
...
///
/// That is why [ElPopover] wraps the input rather than the group: the anchor
/// box is the measured thing, not the visible pill.
///
/// | property | value | source |
/// |---|---|---|
/// | width | `--anchor-width` + `el(7)`, min-width beating width | *(measured 372)* |
/// | radius / fill | 12px, `--popover` | *(measured)* |
/// | elevation | `shadow-md` + 1px `--foreground`/10 ring — [ElPopoverSurface] | — |
...
/// | animation | **0.32s `cubic-bezier(0.22, 1, 0.36, 1)`** | *(measured — `--duration-overlay` on `--ease-out`, and [ElDurations.overlay]'s first consumer)* |
/// | list | `p-1`, cap `min(el(72) − el(9), available − el(9))` = 252, scrollbar hidden | *(measured: `p-1`, and the popup's own `max-h` tracking `--available-height`)* |
...
/// `String(inputValue).trim()` (`AriaCombobox.js:195`). [elCollatorContains] is
...
/// dependency may be added for it. [elCollatorContains] folds case and strips
...
typedef ElComboboxItem<T> = ElSelectOption<T>;
...
class ElCombobox<T> extends StatefulWidget {
  ...
  final List<ElComboboxItem<T>> items;
  ...
  @override
  State<ElCombobox<T>> createState() => _ElComboboxState<T>();
}

class _ElComboboxState<T> extends State<ElCombobox<T>> {
  ...
  FocusNode get _focusNode =>
      widget.focusNode ??
      _scope?.focusNode ??
      (_ownedFocusNode ??= FocusNode(debugLabel: 'ElCombobox'));
  ...
  @override
  void didUpdateWidget(ElCombobox<T> old) {
  ...
  String? _labelOf(T? value) {
    for (final ElComboboxItem<T> item in widget.items) {
  ...
  List<ElComboboxItem<T>> get _visible {
    ...
    final bool Function(String, String) match =
        widget.filter ?? elCollatorContains;
    return widget.items
        .where((ElComboboxItem<T> i) => match(i.label, query))
        .toList();
  }
  ...
  void _commit(int index) {
    final List<ElComboboxItem<T>> visible = _visible;
    if (index < 0 || index >= visible.length) return;
    final ElComboboxItem<T> item = visible[index];
  ...
  Widget build(BuildContext context) {
    ...
    Widget group = ElInputGroup(
      ...
      endAddon: ElInputGroupAddon(
        ...
        // not a `ElInputGroupButton`, so the inference cannot see it — this is
...
class _ComboboxPopupState<T> extends State<_ComboboxPopup<T>> {
  ...
  void _reveal() {
    ...
    final double top =
        el(1) + widget.highlighted * ElCombobox.itemHeight - el(1);
    final double bottom = top + ElCombobox.itemHeight + el(1) * 2;
        ...
        child: ClipRRect(
          ...
          child: ConstrainedBox(
            ...
            child: SingleChildScrollView(
              ...
              padding: EdgeInsets.all(empty ? 0 : el(1)),
              child: empty
