...
/// | track `transition-[background-color,box-shadow,border-color] duration-base ease-out` | 250ms — but from `--default-transition-duration`, not `duration-base`, which emits nothing ([MotionDurations.normal]) |
/// | thumb `transition-transform duration-base ease-spring` | the same 250ms, and it overshoots |
/// | `after:-inset-x-3 after:-inset-y-2` | a **66 × 38** target — measured from the 42 × 22 padding box, see [HitArea] |
///
...
double get _border => BorderWidths.hairline;

/// `p-0.5`.
double get _padding => space(0.5);
...
enum SwitchSize {
  ...
  String get label => this == SwitchSize.md ? 'default' : 'sm';

  /// `w-11` / `w-9`.
  double get trackWidth => this == SwitchSize.md ? space(11) : space(9);

  /// `h-6` / `h-5`.
  double get trackHeight => this == SwitchSize.md ? space(6) : space(5);

  /// `group-data-[size=default]/switch:size-5` / `:size-4`.
  double get thumbSize => this == SwitchSize.md ? space(5) : space(4);

  /// `translate-x-5` / `translate-x-4`.
  double get travel => this == SwitchSize.md ? space(5) : space(4);
}
...
class Switch extends StatelessWidget {
  ...
  /// A [FieldScope]'s node wins over none and loses to this, so
  /// `Form.focusFirstError` lands on the switch itself (ruling F4).
  final FocusNode? focusNode;

  /// The accessible name, for a switch whose visible label is a sibling —
  /// supplied by a `Field` through the scope when there is one.
  final String? label;
  ...
  Widget build(BuildContext context) {
    ...
    return SelectionControl(
      ...
      shadow: value ? Shadows.controlPrimary : Shadows.inset,
