...
///     is a [DragTarget] over `List<ElAgentAttachment>` — same three moments,
///     same `border-agent bg-agent/8`, same [elFormatBytes] refusal message
///     against [kElMaxFileBytes], with attachments where the reference has
///     `File`s.
...
///
/// The composer holds its own [EditableText] rather than a [ElInput], so it
/// wears the field family's one keyboard-avoidance hook — [ElFieldVisibility] —
...
/// software keyboard with a small margin. Inside [ElAgentConsole] the console's
...
/// The composer.
class ElAgentComposer extends StatefulWidget {
  const ElAgentComposer({
    super.key,
  ...
  final ValueChanged<List<ElAgentAttachment>>? onAttach;
  ...
  static double get maxRowsPx => el(50);

  /// `px-4 py-3` on the input.
  static EdgeInsets get inputInsets =>
      EdgeInsets.symmetric(horizontal: el(4), vertical: el(3));

  /// `px-2 pb-2` on the control row.
  static EdgeInsets get controlInsets =>
      EdgeInsets.only(left: el(2), right: el(2), bottom: el(2));

  /// `gap-1` between the controls.
  static double get controlGap => el(1);

  /// `p-3` on the file tray.
  static double get trayPadding => el(3);

  /// The inline-block descent under the input — drift 1.
  static double get inlineGap => el(1.5);

  /// `size-8` — the send, stop and plus controls are all one rung.
  static ElButtonSize get controlSize => ElButtonSize.iconSm;

  /// `size-4` on the send arrow — the base class list's, not the `size-8`
  /// rung's.
  static double get sendGlyphSize => ElButton.iconPxFor(ElButtonSize.icon);

  /// `size-3.5` on the stop square, written explicitly at the call site.
  static double get stopGlyphSize => ElButton.iconPxFor(ElButtonSize.iconSm);
  ...
  static double get messageTopGap => el(2);

  @override
  State<ElAgentComposer> createState() => _ElAgentComposerState();
}

class _ElAgentComposerState extends State<ElAgentComposer> {
  ...
  List<ElAgentAttachment> get _attachments =>
      widget.attachments ?? const <ElAgentAttachment>[];
  ...
  @override
  void didUpdateWidget(ElAgentComposer old) {
  ...
  String? get _query => (widget.commands?.isNotEmpty ?? false)
      ? elSlashQuery(_value, _caret)
      : null;

  List<ElAgentCommand> _matchesFor(String? query) => query == null
      ? const <ElAgentCommand>[]
      : elFilterCommands(widget.commands ?? const <ElAgentCommand>[], query);
  ...
  /// their own argument."*
  void _applyCommand(ElAgentCommand command) {
  ...
  void _takeFiles(List<ElAgentAttachment> files) {
    final ValueChanged<List<ElAgentAttachment>>? onAttach = widget.onAttach;
    if (onAttach == null) return;

    final List<ElAgentAttachment> accepted = <ElAgentAttachment>[];
    final List<String> refused = <String>[];
    for (final ElAgentAttachment file in files) {
      if (file.size > kElMaxFileBytes) {
    ...
    setState(
      () => _rejected = refused.isNotEmpty
          ? '${refused.join(", ")} — over the '
                '${elFormatBytes(kElMaxFileBytes)} limit'
          : null,
    );
  ...
  KeyEventResult _onKey(FocusNode node, KeyEvent event) {
    ...
    final List<ElAgentCommand> matches = _matchesFor(query);
  ...
  Widget build(BuildContext context) {
    ...
    final Widget shell = ElMachineSurface(
      ...
      fill: _dragging
          ...
          : theme.card,
      border: Border.all(
        color: _dragging ? theme.agent : theme.border,
      ...
      child: Column(
        ...
        children: <Widget>[
          if (_attachments.isNotEmpty)
            Container(
              padding: EdgeInsets.all(ElAgentComposer.trayPadding),
          ...
          _ComposerInput(
            ...
            placeholder: _dragging
                ...
                : (widget.placeholder ?? ElAgentComposer.defaultPlaceholder),
          ),
          // Drift 1 — the line box under an `inline-block` textarea.
          SizedBox(height: ElAgentComposer.inlineGap),
          Padding(
            padding: ElAgentComposer.controlInsets,
            child: Row(
              ...
              children: <Widget>[
                ...
                SizedBox(width: ElAgentComposer.controlGap),
                ...
                if (widget.busy && widget.onStop != null)
                  ElButton(
                    ...
                    size: ElAgentComposer.controlSize,
                    label: 'Stop',
                    onPressed: widget.onStop,
                    child: ElIcon.lucide(
                      ElLucide.square,
                      sizePx: ElAgentComposer.stopGlyphSize,
                    ),
                  )
                else
                  ElButton(
                    ...
                    child: ElIcon.lucide(
                      ElLucide.arrowUp,
                      sizePx: ElAgentComposer.sendGlyphSize,
                    ),
                  ),
              ],
            ),
    ...
    final Widget dropTarget = widget.onAttach == null
        ? body
        : DragTarget<List<ElAgentAttachment>>(
            onWillAcceptWithDetails:
                (DragTargetDetails<List<ElAgentAttachment>> details) {
                  setState(() => _dragging = true);
                  return true;
                },
            onLeave: (List<ElAgentAttachment>? _) =>
            ...
            builder:
                (
                  ...
                  List<List<ElAgentAttachment>?> candidate,
    ...
    return Focus(
      ...
      // avoidance hook, from [ElFieldVisibility]. The composer holds an
      // [EditableText] of its own rather than a [ElInput], so it wears the
      ...
      child: ElFieldVisibility(
        focusNode: _focusNode,
        child: _ComposerStack(
          gap: ElAgentSlashPalette.bottomGap,
          children: <Widget>[
            dropTarget,
            if (paletteOpen)
              ElAgentSlashPalette(
...
class _Message extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    padding: EdgeInsets.only(top: ElAgentComposer.messageTopGap),
    child: ElText(
      text,
      ElType.caption,
...
/// growing to [ElAgentComposer.maxRowsPx] and then scrolling.
...
class _ComposerInputState extends State<_ComposerInput> {
  ...
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final TextStyle style = ElText.styleOf(context, ElType.body);
    ...
    body = ConstrainedBox(
      constraints: BoxConstraints(maxHeight: ElAgentComposer.maxRowsPx),
      child: Padding(padding: ElAgentComposer.inputInsets, child: body),
    );
    ...
    // dims with it.
    return Opacity(
      opacity: widget.enabled ? 1 : ElAgentComposer.disabledInputOpacity,
      child: Semantics(
        textField: true,
        multiline: true,
        label: ElAgentComposer.inputLabel,
