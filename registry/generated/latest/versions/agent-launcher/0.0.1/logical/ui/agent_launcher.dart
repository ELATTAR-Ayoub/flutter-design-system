...
/// named beside it. [ElAgentLauncher] reproduces the snap.
///
/// The button's own `hover:border-agent/50` springs the way every `ElButton`
...
///
/// GAP CLOSED — `hover:border-agent/50` is painted. It was not: [ElButtonSurface]
...
/// site now passes [ElAgentLauncher.hoverRimAlpha] of `--agent` through it. The
...
/// `AgentLauncher`.
class ElAgentLauncher extends StatefulWidget {
  ...
  /// The face on the button. Null takes [ElAgentAvatarRegistry.renderer], the
  ...
  static double get dialogMaxWidth => el(320);

  /// `h-[min(88vh,52rem)]`.
  static const double dialogHeightFraction = 0.88;
  static double get dialogMaxHeight => el(208);
  ...
  /// [ElModalCompact.clampSize] takes the last word — 292.5 x 609 — and the
  ...
  static Size dialogSize(Size viewport) {
    ...
    return ElModalCompact.clampSize(
      Size(
  ...
  @override
  State<ElAgentLauncher> createState() => _ElAgentLauncherState();
}

class _ElAgentLauncherState extends State<ElAgentLauncher> {
  bool _hovered = false;
  ...
  Widget _button(BuildContext context, VoidCallback open) {
    ...
    return SizedBox.square(
      dimension: ElAgentLauncher.size,
      child: ElButton(
        ...
        label: widget.label,
        onPressed: open,
        child: ElAgentFace(state: ElAgentState.idle, avatar: widget.avatar),
      ),
    );
  }
}
...
class _FixedState extends State<_Fixed> {
  ...
  Widget build(BuildContext context) => OverlayPortal(
    ...
    overlayChildBuilder: (BuildContext context) => Positioned(
      right: ElAgentLauncher.inset,
      bottom: ElAgentLauncher.inset,
      child: widget.child,
    ),
...
class _Label extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return IgnorePointer(
      child: Padding(
        padding: EdgeInsets.only(right: ElAgentLauncher.labelGap),
        ...
        child: Transform.translate(
          offset: Offset(shown ? 0 : ElAgentLauncher.labelRest, 0),
          child: AnimatedOpacity(
            opacity: shown ? 1 : 0,
            duration: elAnimationDuration(context, ElDurations.base),
            curve: ElCurves.outFlex,
            child: Container(
              padding: ElAgentLauncher.labelPadding,
...
class _Dialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final Size box = ElAgentLauncher.dialogSize(MediaQuery.sizeOf(context));
    ...
    return Semantics(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: <BoxShadow>[
            ...ElDialogContent.ringSpec.outerShadows(theme),
            ...ElShadows.e4.outerShadows(theme),
          ],
        ),
        child: child,
      ),
