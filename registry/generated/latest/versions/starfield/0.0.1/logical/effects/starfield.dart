...
/// — see [ElStarfieldCluster.tileOriginIn]. A port that rescaled would change
/// the population.
///
...
/// | toast | `.bloom-cosmic [data-content]::before / ::after` (L3403–3406) — a **descendant** combinator, not a child one, because sonner owns the live markup and may wrap the content slot; a `>` would work on the Elattar preview and silently do nothing on the real toast. |
///
...
/// entire toast.** That is why this widget is mounted by `ElBloomCosmic`
...
/// [ElStarfieldCluster.matrixFor] is that one expression.
///
...
/// is sitting on — see [ElStarfield.glowFor].
library;
...
// The two `--star-glow*` theme tokens live on `ElThemeData`
...
///
/// Unlike `ElShadowLayer.blurRadius` there is nothing to invert here: that
...
/// ([ElIconPaths.sparkleElement]), so an instance is only ever three numbers
...
@immutable
class ElStarfieldCluster {
  const ElStarfieldCluster({
    required this.tile,
  ...
  /// `animation-duration` — [ElDurations.sway] 44s here, [ElDurations.swayAlt]
  ...
  Offset originIn(Size box) => Offset(box.width, corner.y > 0 ? box.height : 0);
  ...
  Offset hoverTranslateAt(double u) {
    final double eased = ElCurves.out.transform(u.clamp(0.0, 1.0));
  ...
  Matrix4 matrixFor(Size box, {required double swayT, required double hoverT}) {
...
const ElStarfieldCluster elStarfieldDense = ElStarfieldCluster(
  tile: Size(260, 96),
  corner: Alignment.bottomRight,
  sway: ElDurations.sway,
  fromDegrees: -6,
  toDegrees: 6,
  hoverTranslate: Offset(-5, 3),
  hoverScale: 1.06,
  sparkles: <ElSparkle>[
    ElSparkle(opacity: 0.95, x: 214, y: 54, scale: 0.46),
    ElSparkle(opacity: 0.78, x: 240, y: 24, scale: 0.26),
    ElSparkle(opacity: 0.85, x: 180, y: 76, scale: 0.32),
    ElSparkle(opacity: 0.66, x: 248, y: 72, scale: 0.20),
    ElSparkle(opacity: 0.55, x: 196, y: 16, scale: 0.18),
    ElSparkle(opacity: 0.58, x: 146, y: 40, scale: 0.22),
    ElSparkle(opacity: 0.50, x: 232, y: 92, scale: 0.16),
    ElSparkle(opacity: 0.42, x: 112, y: 66, scale: 0.15),
  ],
);

/// `::after` — the thin top-right cluster, `viewBox="0 0 200 64"`, five
/// sparkles (globals.css L3478).
const ElStarfieldCluster elStarfieldThin = ElStarfieldCluster(
  tile: Size(200, 64),
  corner: Alignment.topRight,
  sway: ElDurations.swayAlt,
  fromDegrees: 5,
  toDegrees: -5,
  hoverTranslate: Offset(-3, -2),
  hoverScale: 1.04,
  sparkles: <ElSparkle>[
    ElSparkle(opacity: 0.78, x: 168, y: 18, scale: 0.24),
    ElSparkle(opacity: 0.55, x: 132, y: 42, scale: 0.17),
    ElSparkle(opacity: 0.60, x: 190, y: 46, scale: 0.18),
    ElSparkle(opacity: 0.42, x: 96, y: 14, scale: 0.13),
    ElSparkle(opacity: 0.38, x: 150, y: 60, scale: 0.12),
  ],
);
...
/// `position: absolute; inset: 0` means, and it is how `ElBloomCosmic` mounts
...
/// sparkles'.
class ElStarfield extends StatefulWidget {
  const ElStarfield({super.key, required this.bloom2, this.hovered = false});
  ...
  static const List<ElStarfieldCluster> clusters = <ElStarfieldCluster>[
    elStarfieldDense,
    elStarfieldThin,
  ];
  ...
  /// literal on `ElBloomCosmic.voidFor`'s precedent: CSS `white` *is*
  /// `hsl(0 0% 100%)`, and this file states no colour of its own.
  static final Color starInk = elHsl(0, 0, 100);
  ...
  static Color glowFor(ElThemeData theme, Color bloom2) => switch (theme.kind) {
    ElThemeKind.dark => starInk.withValues(alpha: theme.starGlowMix),
    ElThemeKind.light => ElOklab.mix(bloom2, elTransparent, theme.starGlowMix),
  };
  ...
  /// surface."*
  static ({double tight, double wide}) sigmasFor(ElThemeData theme) => (
  ...
  /// assertion can reach: it falls out of [ElStarfieldCluster.tileOriginIn] and
  /// the host's height, and the map's whole *anchored, not tiled* note exists
  /// to stop a port from rescaling it away.
  @visibleForTesting
  static List<ElSparkle> visibleIn(ElStarfieldCluster cluster, Size box) {
    final Offset origin = cluster.tileOriginIn(box);
    return <ElSparkle>[
      for (final ElSparkle sparkle in cluster.sparkles)
        if (sparkle.box.shift(origin).overlaps(Offset.zero & box)) sparkle,
    ];
  }

  @override
  State<ElStarfield> createState() => _ElStarfieldState();
}

class _ElStarfieldState extends State<ElStarfield>
  ...
  /// [build] re-reads both through [elAnimationDuration] on every pass, the way
  /// `ElSpinner` and `ElKeyframePlayer` do.
  late final List<AnimationController> _sways = <AnimationController>[
    for (final ElStarfieldCluster cluster in ElStarfield.clusters)
  ...
  void didChangeDependencies() {
    ...
    final bool stilled =
        elAnimationDuration(context, ElDurations.sway) == Duration.zero;
  ...
  @override
  void didUpdateWidget(ElStarfield old) {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < _sways.length; i++) {
      _sways[i].duration = elAnimationDuration(
        context,
        ElStarfield.clusters[i].sway,
      );
    }
        ...
        child: AnimatedBuilder(
          ...
          builder: (BuildContext context, Widget? child) => CustomPaint(
            painter: _StarfieldPainter(
              ...
              hoverT: _hover.value,
              stilled: stilled,
              glow: ElStarfield.glowFor(theme, widget.bloom2),
              sigmas: ElStarfield.sigmasFor(theme),
            ),
          ),
        ),
      ),
...
class _StarfieldPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < ElStarfield.clusters.length; i++) {
      final ElStarfieldCluster cluster = ElStarfield.clusters[i];
      canvas.save();
  ...
  void _paintCluster(Canvas canvas, Rect box, ElStarfieldCluster cluster) {
  ...
  void _paintSparkles(Canvas canvas, Rect box, ElStarfieldCluster cluster) {
    canvas.save();
    ...
    for (final ElSparkle sparkle in cluster.sparkles) {
      ...
      canvas.drawPath(
        ElIconPaths.sparkle(),
        Paint()..color = ElStarfield.starInk.withValues(alpha: sparkle.opacity),
      );
      canvas.restore();
    }
    canvas.restore();
