...
/// |---|---|---|
/// | [ElEmpty] | `flex w-full min-w-0 flex-1 flex-col items-center justify-center gap-4 rounded-xl border-dashed p-6 text-center text-balance` | 24px padding, 16px gap, r**16**, centred |
/// | [ElEmptyHeader] | `flex max-w-sm flex-col items-center gap-2` | **384px**, 8px gap |
/// | [ElEmptyMedia] | `mb-2 flex shrink-0 items-center justify-center` + `size-8 rounded-lg bg-muted text-foreground` | **32 × 32**, r12, `--muted`, 8px below |
/// | [ElEmptyTitle] | `font-heading text-sm font-medium tracking-tight` | 13 / 500 / 18.5714 / −0.26px |
/// | [ElEmptyDescription] | `text-sm/relaxed text-muted-foreground` | 13 / 21.125 / `--muted-foreground` |
/// | [ElEmptyContent] | `flex w-full max-w-sm min-w-0 flex-col items-center gap-2.5 text-sm text-balance` | **384px**, 10px gap |
///
...
///
/// [ElEmpty] therefore paints **no border**, which is not an omission: a port
...
/// glyph on the page**.
///
/// Both halves are reproduced by [ElEmptyMedia.glyphSize] and
/// [ElEmptyMedia.glyphStroke], each derived from the rung it comes from rather
...
///
/// [ElEmpty] and [ElEmptyContent] both carry it. Flutter's line breaker has no
...
class ElEmpty extends StatelessWidget {
  const ElEmpty({super.key, required this.children});
...
/// measure.
class ElEmptyHeader extends StatelessWidget {
  const ElEmptyHeader({super.key, required this.children});
...
class ElEmptyMedia extends StatelessWidget {
  const ElEmptyMedia({
    super.key,
    required this.glyph,
    this.tone = ElIconTone.normal,
  });

  /// `size-8` — the tile.
  static double get box => el(8);

  /// `rounded-lg` — 12px.
  static double get radius => ElRadii.lg;

  /// `mb-2` — the gap to the title, on top of `EmptyHeader`'s own `gap-2`.
  static double get marginBottom => el(2);

  /// The box the glyph is **actually** drawn in: `size-4`, forced by
  /// `[&_svg:not([class*='size-'])]:size-4` over the `size="xl"` attribute.
  static double get glyphSize => ElIcon.pxFor(ElIconSize.md);
  ...
  static double get glyphStroke =>
      ElIcon.strokeFor(ElIcon.pxFor(ElIconSize.xl));
...
class ElEmptyTitle extends StatelessWidget {
  const ElEmptyTitle(this.text, {super.key});

  final String text;

  /// The resolved style — 13 / 500 / 18.5714 / **−0.26px**.
  ///
  /// Built from [ElComponentType.buttonLabel] plus one override rather than
  ...
  ///    `ElFonts.heading` and `ElFonts.sans` are both `"Inter Local"`
  ...
  ///    [ElType.h1] — the one spec in the type layer that transcribes exactly
  ///    that token — rather than typed here, so a retune of the token carries.
  ///
  /// FOLLOW-UP: fold this into a `ElComponentType.emptyTitle` when
  /// `typography.dart` is next open to this phase.
  static TextStyle styleOf(BuildContext context, {Color? color}) {
    final ElTypeSpec spec = ElComponentType.buttonLabel;
    return ElText.styleOf(
      context,
      spec,
      color: color,
    ).copyWith(letterSpacing: ElType.h1.tracking! * spec.size!);
  }
...
class ElEmptyDescription extends StatelessWidget {
  const ElEmptyDescription(this.text, {super.key});

  /// 13 / 400 / **1.625** — the same resolved rung
  /// [ElComponentType.textareaBody] carries.
  ///
  ...
  static ElTypeSpec get spec => ElComponentType.textareaBody;
...
class ElEmptyContent extends StatelessWidget {
  const ElEmptyContent({super.key, required this.children});
