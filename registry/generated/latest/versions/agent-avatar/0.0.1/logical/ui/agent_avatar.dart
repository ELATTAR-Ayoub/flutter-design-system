...
/// |---|---|
/// | `avatar/cube.tsx` | [ElAgentCube], [ElAgentCubeSpec], [ElCubeScene] |
/// | `avatar/scenes.ts` | [elAgentCubeScene], the nineteen recipes |
/// | `avatar/types.ts` | [ElAgentAvatarSize] and [ElCubeAvatar]'s four props |
/// | `avatar/cube-avatar.tsx` | [ElCubeAvatar], the crossfade, the idle cube |
/// | `globals.css` L3109–3177 | [ElAgentCubeKeyframes], the fourteen tables |
/// | `globals.css` L720–731 / L914–932 | [ElAgentCubeTokens] |
///
...
/// in the scene flashes at full opacity on the first frame."* [ElAgentCubeMotion
...
/// transform: none`, and [ElCubeScene.frozen] is exactly that: every cube fully
...
/// [ElDurations.shimmerText] because the page-local copy needs a token to read.
library;
...
class ElAgentCube {
  ...
  static Offset iso(double x, double y, [double z = 0]) =>
      Offset((x - y) * halfWidth, (x + y) * halfHeight - z * cubeHeight);
  ...
  /// recipe."*
  static Rect viewBoxOf(List<ElAgentCubeSpec> cubes) {
    double minX = double.infinity;
    double maxX = double.negativeInfinity;
    double minY = double.infinity;
    double maxY = double.negativeInfinity;

    for (final ElAgentCubeSpec cube in cubes) {
...
// `ElAgentCubeTokens` used to be declared here, under a standing FOLLOW-UP that
...
// This pass is the opening. The class is `ElThemeData`'s neighbour now and the
// tokens ride [ElThemeData.cube]; `ElAgentCubeTokens.light` / `.dark` are
// unchanged, so nothing that spends them had to move with them.
//
// `ElAgentCubeTokens.of(BuildContext)` did **not** travel, and its two call
// sites read `ElTheme.of(context).cube` instead — it was a second resolver for
// what the theme scope already does, which is the same tidy-up
// `bloom_cosmic.dart` made when its five knobs moved.
//
// What did NOT move, and why: [ElAgentCubeFaces] below mixes an accent cube's
...
class ElAgentCubeFaces {
  const ElAgentCubeFaces({
    required this.top,
    required this.left,
    required this.right,
  ...
  /// [ElAgentCube.dash] on the ghost, null everywhere else.
  ...
  static ElAgentCubeFaces neutral(ElAgentCubeTokens t) => ElAgentCubeFaces(
    top: t.top,
    left: t.left,
    right: t.right,
    stroke: t.stroke,
  ...
  static ElAgentCubeFaces error(ElAgentCubeTokens t) => ElAgentCubeFaces(
    top: t.errorTop,
    left: t.errorLeft,
    right: t.errorRight,
  ...
  static ElAgentCubeFaces accent(ElAgentCubeTokens t, Color accent) =>
      ElAgentCubeFaces(
        top: ElOklab.mix(accent, t.top, accentTopMix),
        left: accent,
        right: ElOklab.mix(accent, t.accentShade, accentRightMix),
        stroke: ElOklab.mix(accent, t.stroke, accentStrokeMix),
      );

  /// *"The 'missing box' — an accent cube at 45%, dashed. Reads as a slot to
  /// fill."*
  static ElAgentCubeFaces ghost(ElAgentCubeTokens t, Color accent) {
    final ElAgentCubeFaces lit = ElAgentCubeFaces.accent(t, accent);
    return ElAgentCubeFaces(
      ...
      stroke: ElOklab.mix(accent, t.ghostInk, ghostStrokeMix),
      dash: ElAgentCube.dash,
    );
  }

  /// `faces(cube)` — red beats dashed beats accent beats neutral.
  static ElAgentCubeFaces forCube(
    ElAgentCubeSpec cube,
    ElAgentCubeTokens tokens,
    Color accent,
  ) {
    if (cube.red) return ElAgentCubeFaces.error(tokens);
    if (cube.dashed) return ElAgentCubeFaces.ghost(tokens, accent);
    if (cube.accent) return ElAgentCubeFaces.accent(tokens, accent);
    return ElAgentCubeFaces.neutral(tokens);
  }
}
...
/// class.
class ElAgentCubeKeyframes {
  const ElAgentCubeKeyframes._();

  /// `var(--ease-in-out)`, every table except [ElAgentCubeKeyframe.spin3d].
  static const Curve curve = ElCurves.inOut;
  ...
  static const ElKeyframeFill fill = ElKeyframeFill.none;
  ...
  static final Map<ElAgentCubeKeyframe, Animatable<Offset>?>
  _translate = <ElAgentCubeKeyframe, Animatable<Offset>?>{
    // 0%,100% { translateY(0) } 50% { translateY(-5px) }
    ElAgentCubeKeyframe.bob: _y(<(double, double)>[(0, 0), (50, -5), (100, 0)]),
    // 0%,55%,100% { 0 } 20%,35% { -11px }
    ElAgentCubeKeyframe.rise: _y(<(double, double)>[
      (0, 0),
      (20, -11),
      (35, -11),
      (55, 0),
    ...
    ElAgentCubeKeyframe.appear: _y(<(double, double)>[(0, 8), (10, 0)]),
    // 0% { translateY(-24px) } 18% { translateY(0) }
    ElAgentCubeKeyframe.drop: _y(<(double, double)>[(0, -24), (18, 0)]),
    ElAgentCubeKeyframe.glide: ElKeyframes.offsets(<ElKeyframeStop<Offset>>[
      const ElKeyframeStop<Offset>(0, Offset.zero),
      const ElKeyframeStop<Offset>(100, glideTravel),
    ], curve: curve),
    ElAgentCubeKeyframe.blinkfade: null,
    ElAgentCubeKeyframe.blinkslow: null,
    // 0%,100% { 0 } 45%,60% { -6.5px }
    ElAgentCubeKeyframe.lift: _y(<(double, double)>[
      (0, 0),
      (45, -6.5),
      (60, -6.5),
      (100, 0),
    ]),
    ElAgentCubeKeyframe.lift2: _y(<(double, double)>[
      (0, 0),
      (45, -13),
      (60, -13),
      (100, 0),
    ]),
    ElAgentCubeKeyframe.settle: _y(<(double, double)>[
      (0, 0),
      (50, -3),
      (100, 0),
    ]),
    // 0%,60%,100% { 0 } 25%,40% { -20px }
    ElAgentCubeKeyframe.pull: _y(<(double, double)>[
      (0, 0),
      (25, -20),
      (40, -20),
      (60, 0),
    ...
    ElAgentCubeKeyframe.shake: _y(<(double, double)>[
      (0, 0),
      (20, -2.5),
      (40, 2.5),
      (60, -2),
    ...
    ElAgentCubeKeyframe.bounce: _y(<(double, double)>[
      (0, 0),
      (35, -16),
      (55, 0),
      (70, -6),
    ...
    ElAgentCubeKeyframe.spin3d: null,
  };

  static final Map<ElAgentCubeKeyframe, Animatable<double>?>
  _opacity = <ElAgentCubeKeyframe, Animatable<double>?>{
    // 0% 0 → 10% 1 → 93% 1 → 95% 0 → 100% 0. It ENDS at zero on purpose:
    // *"cubes arrive one by one and then hard-cut back, Game Boy logo style."*
    ElAgentCubeKeyframe.appear: _a(<(double, double)>[
      (0, 0),
      (10, 1),
      (93, 1),
      (95, 0),
    ...
    ElAgentCubeKeyframe.drop: _a(<(double, double)>[
      (0, 0),
      (18, 1),
      (93, 1),
      (95, 0),
    ...
    // 0%,100% { 0.15 } 50% { 0.95 }
    ElAgentCubeKeyframe.blinkfade: _a(<(double, double)>[
      (0, 0.15),
      (50, 0.95),
      (100, 0.15),
    ]),
    // 0%,100% { 1 } 50% { 0.25 }
    ElAgentCubeKeyframe.blinkslow: _a(<(double, double)>[
      (0, 1),
      (50, 0.25),
      (100, 1),
    ]),
...
class ElAgentCubeMotion {
  const ElAgentCubeMotion(
    this.name,
    this.seconds, {
    this.delay = 0,
  ...
  const ElAgentCubeMotion.appear(double seconds, double delay)
    : this(
        ElAgentCubeKeyframe.appear,
        seconds,
        delay: delay,
        startsHidden: true,
      );
  ...
  const ElAgentCubeMotion.drop(double seconds, double delay)
    : this(ElAgentCubeKeyframe.drop, seconds, delay: delay, startsHidden: true);

  final ElAgentCubeKeyframe name;
  ...
  ({double opacity, Offset translate}) sampleAt(double elapsed) {
    if (elapsed < delay || seconds <= 0) {
      return (opacity: startsHidden ? 0 : 1, translate: Offset.zero);
    }
    ...
    return (
      opacity: ElAgentCubeKeyframes.opacityAt(name, t),
      translate: ElAgentCubeKeyframes.translateAt(name, t),
    );
  }
}

/// `type Cube` — one entry in a scene recipe.
class ElAgentCubeSpec {
  const ElAgentCubeSpec({
    required this.x,
    required this.y,
    this.z = 0,
    this.accent = false,
  ...
  final ElAgentCubeMotion? motion;

  /// *"A second wrapper, for scenes that compose two motions (glide + bob)."*
  final ElAgentCubeMotion? outer;

  ElAgentCubeSpec dividedBy(double speed) => ElAgentCubeSpec(
    x: x,
    y: y,
    z: z,
    accent: accent,
...
class ElAgentCubeScene {
  const ElAgentCubeScene({required this.cubes, required this.width});

  final List<ElAgentCubeSpec> cubes;
...
///
/// [ElAgentState.idle] has no entry, and the reference says why: *"it is a real
...
ElAgentCubeScene elAgentCubeScene(ElAgentState state, {double speed = 1}) {
  assert(
    state != ElAgentState.idle,
    'idle is not an isometric scene — ElCubeAvatar draws it directly',
  );
  final ElAgentCubeScene scene = _scenes[state]!();
  if (speed == 1) return scene;
  return ElAgentCubeScene(
    cubes: <ElAgentCubeSpec>[
      for (final ElAgentCubeSpec cube in scene.cubes) cube.dividedBy(speed),
    ],
    width: scene.width,
  );
}

final Map<ElAgentState, ElAgentCubeScene Function()>
_scenes = <ElAgentState, ElAgentCubeScene Function()>{
  /// *"Five cubes waiting their turn, bobbing in sequence."*
  ElAgentState.queued: () => ElAgentCubeScene(
    width: 190,
    cubes: <ElAgentCubeSpec>[
      for (int x = 0; x < 5; x += 1)
        ElAgentCubeSpec(
  ...
  ElAgentState.planning: () {
    final List<ElAgentCubeSpec> cubes = <ElAgentCubeSpec>[];
    for (int x = 0; x < 4; x += 1) {
      for (int y = 0; y < 2; y += 1) {
        for (int z = 0; z <= x; z += 1) {
          cubes.add(
            ElAgentCubeSpec(
              ...
              z: z.toDouble(),
              accent: z == x,
              motion: ElAgentCubeMotion.appear(3.6, x * 0.35 + z * 0.12),
            ),
          );
        }
      }
    }
    return ElAgentCubeScene(width: 160, cubes: cubes);
  },

  /// *"One cube pulled up out of the middle of a stack, over and over."*
  ElAgentState.retrieving: () {
    final List<ElAgentCubeSpec> cubes = <ElAgentCubeSpec>[];
    for (int x = 0; x < 3; x += 1) {
      for (int y = 0; y < 3; y += 1) {
        cubes.add(
          x == 1 && y == 1
              ? ElAgentCubeSpec(
                  ...
                  boost: _boost,
                  accent: true,
                  motion: const ElAgentCubeMotion(
                    ElAgentCubeKeyframe.pull,
                    2.6,
                  ),
                )
              : ElAgentCubeSpec(x: x.toDouble(), y: y.toDouble()),
        );
      }
    }
    return ElAgentCubeScene(width: _w, cubes: cubes);
  },

  /// *"Cubes falling onto a platform, one after another."*
  ElAgentState.ingesting: () {
    final List<ElAgentCubeSpec> cubes = <ElAgentCubeSpec>[];
    for (int x = 0; x < 2; x += 1) {
      for (int y = 0; y < 2; y += 1) {
        cubes.add(ElAgentCubeSpec(x: x.toDouble(), y: y.toDouble()));
        cubes.add(
          ElAgentCubeSpec(
            x: x.toDouble(),
            y: y.toDouble(),
            z: 1,
            accent: true,
            motion: ElAgentCubeMotion.drop(2.8, (x * 2 + y) * 0.4),
          ),
        );
      }
    }
    return ElAgentCubeScene(width: 120, cubes: cubes);
  },

  /// *"A diagonal ripple across a 4×4 — deliberately no opacity change."*
  ElAgentState.running: () {
    final List<ElAgentCubeSpec> cubes = <ElAgentCubeSpec>[];
    for (int x = 0; x < 4; x += 1) {
      for (int y = 0; y < 4; y += 1) {
        cubes.add(
  ...
  ElAgentState.delegating: () {
    ...
    return ElAgentCubeScene(width: 190, cubes: cubes);
  },
  ...
  ElAgentState.validating: () {
    final List<ElAgentCubeSpec> cubes = <ElAgentCubeSpec>[
      const ElAgentCubeSpec(x: 1, y: 1),
    ];
  ...
  ElAgentState.retrying: () {
    ...
    cubes.add(
      const ElAgentCubeSpec(
        ...
        boost: _boost,
        accent: true,
        motion: ElAgentCubeMotion(ElAgentCubeKeyframe.bounce, 2),
      ),
    );
    return ElAgentCubeScene(width: 120, cubes: cubes);
  },
    ...
    for (int i = 0; i < _ring.length; i += 1) {
      ...
      cubes.add(
        ElAgentCubeSpec(
          x: x,
          y: y,
          motion: ElAgentCubeMotion.appear(4, i * 0.26),
        ),
      );
    }
    cubes.add(
      const ElAgentCubeSpec(
        ...
        y: 1,
        accent: true,
        motion: ElAgentCubeMotion.appear(4, 2.1),
      ),
    );
    return ElAgentCubeScene(width: _w, cubes: cubes);
  },
      ...
      for (int y = 0; y < 4; y += 1) {
        ...
        cubes.add(
          ElAgentCubeSpec(
            ...
            motion: delay == null
                ? null
                : ElAgentCubeMotion(ElAgentCubeKeyframe.rise, 3, delay: delay),
          ),
        );
      }
    }
        ...
        for (int y = 0; y < 2; y += 1) {
          cubes.add(
            ElAgentCubeSpec(
              x: x.toDouble(),
              y: y.toDouble(),
              motion: const ElAgentCubeMotion(ElAgentCubeKeyframe.settle, 2.4),
            ),
          );
        }
      }
    ...
    for (final (double x, double y, double delay) in <(double, double, double)>[
      ...
      cubes.add(
        ElAgentCubeSpec(
          ...
          y: y,
          accent: true,
          motion: ElAgentCubeMotion.appear(3, delay),
        ),
      );
    }
    return ElAgentCubeScene(width: 196, cubes: cubes);
  },
  ...
  ElAgentState.searching: () {
    ...
    cubes.add(
      const ElAgentCubeSpec(
        ...
        motion: ElAgentCubeMotion(ElAgentCubeKeyframe.bob, 1.3),
      ),
    );
    return ElAgentCubeScene(width: _w, cubes: cubes);
  },
  ...
  ElAgentState.reading: () {
          ...
          cubes.add(
            ElAgentCubeSpec(
              ...
              motion: z == 0
                  ...
                  : ElAgentCubeMotion(
                      z == 1
                          ? ElAgentCubeKeyframe.lift
                          : ElAgentCubeKeyframe.lift2,
                      2.2,
                    ),
            ),
          );
    ...
    return ElAgentCubeScene(width: 150, cubes: cubes);
  },
      ...
      for (int x = 0; x < 4; x += 1) {
        cubes.add(
          ElAgentCubeSpec(
            ...
            motion: row == 2
                ? null
                : ElAgentCubeMotion.appear(3.2, (1 - row) * 0.5 + x * 0.12),
          ),
        );
      }
    }
      ...
      for (int x = 0; x < 4; x += 1) {
        cubes.add(
          ElAgentCubeSpec(
            ...
            y: y.toDouble(),
            accent: y == 2 && x == 3,
            motion: ElAgentCubeMotion.appear(3.4, (y * 4 + x) * 0.13),
          ),
        );
      }
    }
        ...
        for (int z = 0; z < 2; z += 1) {
          cubes.add(
            ElAgentCubeSpec(
              ...
              z: z.toDouble(),
              accent: z == 1,
              motion: const ElAgentCubeMotion(ElAgentCubeKeyframe.settle, 3.2),
            ),
          );
        }
      }
    }
    return ElAgentCubeScene(width: 110, cubes: cubes);
  },
};
...
class ElCubeScene extends StatelessWidget {
  const ElCubeScene({
    super.key,
    required this.scene,
    required this.width,
  ...
  Widget build(BuildContext context) {
    final Rect viewBox = ElAgentCube.viewBoxOf(scene.cubes);
    ...
    return SizedBox(
      ...
      child: CustomPaint(
        painter: _CubeScenePainter(
          cubes: ElAgentCube.sorted(scene.cubes),
          viewBox: viewBox,
          tokens: ElTheme.of(context).cube,
          accent: accent,
          elapsed: elapsed,
          frozen: frozen,
        ),
...
class _CubeScenePainter extends CustomPainter {
  ...
  final List<ElAgentCubeSpec> cubes;
  final Rect viewBox;
  final ElAgentCubeTokens tokens;
  final Color accent;
  final double elapsed;
  final bool frozen;
  ...
  void _paintCube(Canvas canvas, ElAgentCubeSpec cube, double scale) {
    ...
    final Offset origin =
        ElAgentCube.iso(cube.x, cube.y, cube.z) +
        outer.translate +
        inner.translate;

    final ElAgentCubeFaces faces = ElAgentCubeFaces.forCube(
      cube,
      tokens,
      accent,
    );
    ...
    if (layered) {
      canvas.saveLayer(
        ...
        Rect.fromCenter(
          center: Offset.zero,
          width: ElAgentCube.halfWidth * 4,
          height: ElAgentCube.cubeHeight * 6,
        ),
        Paint()..color = elTransparent.withValues(alpha: opacity),
      );
    }
    ...
    _paintFace(canvas, ElAgentCube.leftFace, faces.left, faces, scale);
    _paintFace(canvas, ElAgentCube.rightFace, faces.right, faces, scale);
    _paintFace(canvas, ElAgentCube.topFace, faces.top, faces, scale);
  ...
  void _paintFace(
    ...
    final Paint stroke = Paint()
      ...
      ..strokeWidth = ElAgentCube.strokeWidth
...
class _IdleCube extends StatelessWidget {
  ...
  double get face => ElAgentCube.idleFace(scale);
    ...
    return SizedBox(
      ...
      child: CustomPaint(
        painter: _IdleCubePainter(
          face: size,
          perspective: ElAgentCube.idlePerspective(scale),
...
class _IdleCubePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final ElAgentCubeFaces lit = ElAgentCubeFaces.accent(tokens, accent);
    ...
    projected.sort(
      ((double, Color, List<Offset>) a, (double, Color, List<Offset>) b) =>
          a.$1.compareTo(b.$1),
    );
...
/// hero"*.
///
/// Named `ElAgentAvatarSize` rather than `ElAvatarSize` because the base
...
enum ElAgentAvatarSize {
  ...
  const ElAgentAvatarSize(this._box, this.scale);
...
class ElCubeAvatar extends StatefulWidget {
  const ElCubeAvatar({
    super.key,
    this.state = ElAgentState.idle,
  ...
  /// [ElAgentState.idle], as the reference's own signature does.
  ...
  static Duration get crossfade => ElDurations.fast;

  /// `agent-cube-spin3d 9s linear infinite` — idle's own clock, and the one
  /// duration in the family that is not a scene recipe's.
  static const ElAgentCubeMotion spin = ElAgentCubeMotion(
    ElAgentCubeKeyframe.spin3d,
    9,
  );

  @override
  State<ElCubeAvatar> createState() => _ElCubeAvatarState();
}

class _ElCubeAvatarState extends State<ElCubeAvatar>
    with TickerProviderStateMixin {
  /// A bare [Ticker] rather than an [AnimationController], for the reason
  /// `ElSheenAction` states: a scene runs up to three periods and twenty
  ...
  late final AnimationController _fade = AnimationController(
    vsync: this,
    duration: ElCubeAvatar.crossfade,
  );
  ...
  void didChangeDependencies() {
    ...
    final bool frozen =
        elAnimationDuration(context, ElCubeAvatar.crossfade) == Duration.zero;
  ...
  @override
  void didUpdateWidget(ElCubeAvatar old) {
  ...
  Widget _scene(ElAgentState state, Color accent, double elapsed) {
    if (state == ElAgentState.idle) {
      return _IdleCube(
        scale: widget.size.scale,
        accent: accent,
        motion: ElCubeAvatar.spin.dividedBy(widget.speed),
        elapsed: elapsed,
        frozen: _frozen,
      );
    }
    final ElAgentCubeScene scene = elAgentCubeScene(state, speed: widget.speed);
    return ElCubeScene(
      scene: scene,
  ...
  Widget build(BuildContext context) {
    ...
    final Color accent = widget.accent ?? ElTheme.of(context).agent;
    final ElAgentState? outgoing = _outgoing;
            ...
            _loose(
              AnimatedBuilder(
                ...
                builder: (BuildContext context, Widget? child) => Opacity(
                  opacity: ElCurves.out.transform(_fade.value),
                  child: child,
                ),
