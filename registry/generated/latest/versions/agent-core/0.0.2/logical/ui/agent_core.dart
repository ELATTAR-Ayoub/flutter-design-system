...
///
/// **Naming.** Types are prefixed `Agent…` where a bare name would collide
/// with the vendored chat primitives already in the corpus — `attachment.dart`
/// owns `Attachment`, and the agent's own attachment is a different object
...
/// surface.
///
/// The one place an alias could not be made: `AgentDelivery` is the delivery
/// *record* (a class), and the discriminant enum is `AgentDeliverySent`. A
/// build that used `AgentDelivery` as the enum must read
/// `AgentDeliverySent.content` instead — it is the only rename either side
...
enum AgentAttachmentKind { image, document, data, code, audio, other }

/// `AttachmentDelivery['sent']` — whether the agent can genuinely see the file.
enum AgentDeliverySent {
...
@immutable
class AgentDelivery {
  const AgentDelivery.content()
    : sent = AgentDeliverySent.content,
      reason = null;

  const AgentDelivery.reference(String this.reason)
    : sent = AgentDeliverySent.reference;

  const AgentDelivery.produced()
    : sent = AgentDeliverySent.produced,
      reason = null;

  final AgentDeliverySent sent;

  /// Only `reference` carries one; the TS union makes it non-optional there.
  final String? reason;
}

/// Alias — the console build's spelling of [AgentDelivery].
typedef AgentAttachmentDelivery = AgentDelivery;
...
class AgentAttachment {
  ...
  AgentAttachment copyWith({AgentDelivery? delivery, String? text}) =>
      AgentAttachment(
        id: id,
        name: name,
        mime: mime,
...
AgentAttachmentKind attachmentKind(String mime, String name) {
  if (mime.startsWith('image/')) return AgentAttachmentKind.image;
  if (mime.startsWith('audio/')) return AgentAttachmentKind.audio;
  ...
  if (RegExp(r'^(csv|tsv|json|xlsx?|ods|parquet)$').hasMatch(ext)) {
    return AgentAttachmentKind.data;
  }
  ...
  if (RegExp(
    ...
    return AgentAttachmentKind.code;
  }
...
/// Alias — the transcript build's spelling of [attachmentKind].
AgentAttachmentKind agentAttachmentKind(String mime, String name) =>
...
bool isTextual(AgentAttachment attachment) {
  ...
  return _textualKinds.contains(attachment.kind) &&
      attachment.kind != AgentAttachmentKind.audio;
}
...
/// The two halves [serialiseAttachments] returns.
@immutable
class SerialisedMessage {
  const SerialisedMessage({required this.text, required this.attachments});
...
SerialisedMessage serialiseAttachments(
  String text,
  List<AgentAttachment> attachments,
) {
  if (attachments.isEmpty) {
    return SerialisedMessage(text: text, attachments: attachments);
  }

  final List<AgentAttachment> stamped = <AgentAttachment>[];
  final List<String> blocks = <String>[];
  final List<String> referenced = <String>[];

  for (final AgentAttachment attachment in attachments) {
    ...
    // not the picture"*.
    if (attachment.kind == AgentAttachmentKind.image && body != null) {
      blocks.add(
        <String>[
      ...
      stamped.add(attachment.copyWith(delivery: const AgentDelivery.content()));
      continue;
    }

    if (body != null && isTextual(attachment)) {
      final bool truncated = body.length > maxInlineChars;
      final String inlined = truncated
          ? body.substring(0, maxInlineChars)
          : body;
...
String stripProtocol(String text) => text
...
/// `TurnStatus`.
enum AgentTurnStatus { running, ok, error }

/// `AgentTurn` — the sealed union the transcript switches over.
sealed class AgentTurn {
  const AgentTurn({required this.id});

  final String id;
}

/// `UserTurn`.
class UserTurn extends AgentTurn {
  const UserTurn({
    required super.id,
    required this.text,
...
/// Alias — the console build's spelling of [UserTurn].
typedef AgentUserTurn = UserTurn;

/// `TextTurn`.
class TextTurn extends AgentTurn {
  const TextTurn({
    required super.id,
    required this.text,
    this.attachments = const <AgentAttachment>[],
  ...
  TextTurn notStreaming() => copyWith(streaming: false);

  TextTurn copyWith({bool? streaming}) => TextTurn(
    id: id,
    text: text,
...
/// Alias — the console build's spelling of [TextTurn].
typedef AgentTextTurn = TextTurn;

/// `ToolTurn`.
class ToolTurn extends AgentTurn {
  const ToolTurn({
    required super.id,
    required this.name,
...
/// Alias — the console build's spelling of [ToolTurn].
typedef AgentToolTurn = ToolTurn;

/// `ActionTurn` — a step the browser performed, rather than the server.
class ActionTurn extends AgentTurn {
  const ActionTurn({
    required super.id,
  ...
  final ApprovalOutcome? approval;
}

/// Alias — the console build's spelling of [ActionTurn].
typedef AgentActionTurn = ActionTurn;

/// `ActionTurn['approval']`.
enum ApprovalOutcome { pending, approved, rejected }

/// Alias — the console build's spelling of [ApprovalOutcome].
typedef AgentApprovalState = ApprovalOutcome;

/// `ErrorTurn`.
class ErrorTurn extends AgentTurn {
  const ErrorTurn({
    required super.id,
...
/// Alias — the console build's spelling of [ErrorTurn].
typedef AgentErrorTurn = ErrorTurn;
...
class PendingApproval {
  ...
  /// The [ActionTurn] this belongs to.
  final String turnId;
  final String action;
  final Map<String, Object?> params;
...
/// Alias — the console build's spelling of [PendingApproval].
typedef AgentPendingApproval = PendingApproval;
...
enum AgentState {
  ...
  String get wire => switch (this) {
    ...
    AgentState.callingTools => 'calling_tools',
    _ => name,
  };
  ...
  String get label => switch (this) {
    ...
    AgentState.retrieving => 'Retrieving knowledge',
    AgentState.ingesting => 'Ingesting data',
    AgentState.running => 'Running code',
    AgentState.delegating => 'Delegating to agent',
    ...
    AgentState.callingTools => 'Calling tools',
    AgentState.searching => 'Searching',
    AgentState.reading => 'Reading files',
    AgentState.recalling => 'Recalling context',
  ...
  bool get isBusy => !const <AgentState>{
    AgentState.idle,
    AgentState.done,
    AgentState.error,
  }.contains(this);
  ...
  /// honest.
  bool get isNarrating => const <AgentState>{
    AgentState.planning,
  ...
  LucideGlyph get glyph => switch (this) {
    AgentState.idle => Lucide.circle,
    AgentState.queued => Lucide.clock,
    AgentState.planning => Lucide.listChecks,
    AgentState.retrieving => Lucide.bookOpen,
    AgentState.ingesting => Lucide.database,
    AgentState.running => Lucide.terminal,
    AgentState.delegating => Lucide.arrowRightLeft,
    AgentState.awaitingApproval => Lucide.shieldQuestionMark,
    AgentState.validating => Lucide.checkCheck,
    AgentState.retrying => Lucide.rotateCw,
    AgentState.error => Lucide.triangleAlert,
    AgentState.summarizing => Lucide.scrollText,
    AgentState.thinking => Lucide.brain,
    AgentState.processing => Lucide.chartColumn,
    AgentState.callingTools => Lucide.wrench,
    AgentState.searching => Lucide.search,
    AgentState.reading => Lucide.bookOpen,
    // `HistoryIcon` — lucide's alias for `rotate-ccw-clock`, which is the
    // name the generated registry keys the geometry under.
    AgentState.recalling => Lucide.rotateCcwClock,
    AgentState.writing => Lucide.pencil,
    AgentState.done => Lucide.check,
  };
}

/// Alias — the console build reads the labels as a top-level map.
Map<AgentState, String> get agentStateLabel => <AgentState, String>{
  for (final AgentState s in AgentState.values) s: s.label,
};

/// Alias — the console build reads the wire spellings as a top-level map.
Map<AgentState, String> get agentStateId => <AgentState, String>{
  for (final AgentState s in AgentState.values) s: s.wire,
};

/// Alias — the console build's spelling of [AgentState.isBusy].
bool agentIsBusy(AgentState state) => state.isBusy;

/// Alias — the console build's spelling of [AgentState.isNarrating].
bool agentIsNarrating(AgentState state) => state.isNarrating;
...
typedef ToolStateMap = Map<String, AgentState>;

/// Alias — the console build's spelling of [ToolStateMap].
typedef AgentToolStateMap = ToolStateMap;
...
AgentState? stateForTool(String name, ToolStateMap? map) {
  if (map == null) return null;
  final AgentState? exact = map[name];
  if (exact != null) return exact;

  String? bestKey;
  AgentState? best;
  for (final MapEntry<String, AgentState> entry in map.entries) {
...
/// Alias — the console build's spelling of [stateForTool].
AgentState? agentStateForTool(String name, ToolStateMap? map) =>
...
String humaniseToolName(String name) {
...
enum AgentAttachmentSupport { content, reference, none }
...
class AgentCapabilities {
  const AgentCapabilities({
    this.attachments = AgentAttachmentSupport.content,
    this.models = true,
    this.approvals = true,
  });
...
@immutable
class AgentSendOptions {
  const AgentSendOptions({
    this.attachments = const <AgentAttachment>[],
    this.model,
    this.wireText,
  });
...
abstract class AgentTransport implements Listenable {
  /// The transcript, in order. Owned by the transport so it can settle its own
  /// tool calls.
  List<AgentTurn> get turns;

  Future<void> send(String text, [AgentSendOptions options]);
...
AgentState resolveAgentState({
  required List<AgentTurn> turns,
  required AgentSignals signals,
  ToolStateMap? toolStates,
}) {
  // 0. An agent that reports its own state is believed.
  final AgentState? declared = signals.declared;
  if (declared != null) return declared;

  final AgentTurn? last = turns.isEmpty ? null : turns.last;

  // 1. A fatal error outranks everything. A non-fatal one does not.
  if (last is ErrorTurn && last.fatal) return AgentState.error;

  // 2. Something is waiting on a human.
  final bool heldForApproval = turns.any(
    (AgentTurn turn) =>
        turn is ActionTurn && turn.approval == ApprovalOutcome.pending,
  );
  if (heldForApproval) return AgentState.awaitingApproval;

  // 3. A tool in flight.
  final ToolTurn? runningTool = _findLast<ToolTurn>(
    turns,
    (ToolTurn t) => t.status == AgentTurnStatus.running,
  );
  if (runningTool != null) {
    // A retry first, and whatever it does second.
    if (runningTool.attempt > 1) return AgentState.retrying;
    return stateForTool(runningTool.name, toolStates) ??
        AgentState.callingTools;
  }

  // 4. The browser is doing something on the agent's behalf.
  final ActionTurn? runningAction = _findLast<ActionTurn>(
    turns,
    (ActionTurn t) => t.status == AgentTurnStatus.running,
  );
  if (runningAction != null) return AgentState.processing;

  // 5. Prose is arriving.
  if (last is TextTurn && last.streaming) {
    return _narrationState(turns, last);
  }

  // 6. Sent, nothing back yet.
  if (signals.awaitingFirstEvent) return AgentState.queued;

  // 7. Loading with nothing else to say about it.
  if (signals.isLoading) return AgentState.thinking;

  // 8. A turn that finished.
  if (last != null && _isSettled(last)) return AgentState.done;
...
AgentState _narrationState(List<AgentTurn> turns, TextTurn current) {
  for (int i = turns.length - 1; i >= 0; i -= 1) {
    final AgentTurn turn = turns[i];
    if (identical(turn, current)) continue;
    if (turn is UserTurn) break;
    if (turn is ToolTurn || turn is ActionTurn) {
      return AgentState.summarizing;
    }
  }
...
bool _isSettled(AgentTurn turn) => switch (turn) {
  TextTurn() => !turn.streaming,
  ToolTurn() => turn.status != AgentTurnStatus.running,
  ActionTurn() => turn.status != AgentTurnStatus.running,
  ErrorTurn() => true,
  UserTurn() => false,
};

T? _findLast<T extends AgentTurn>(
...
class ConversationSummary {
  ...
  ConversationSummary copyWith({String? title, bool? pinned}) =>
      ConversationSummary(
        id: id,
...
abstract class ConversationStore extends ChangeNotifier {
...
/// [relativeTime] against the nearest [Clock] — the `?clock=` seam, so a
/// rig can freeze both sides of a parity measurement on the same instant.
String relativeTimeOf(BuildContext context, DateTime then) =>
    relativeTime(then, now: Clock.nowOf(context));
...
enum SwitchPhase {
  ...
  String get className => switch (this) {
    SwitchPhase.out => 'anim-blur-out',
    SwitchPhase.blurIn => 'anim-blur-in',
    SwitchPhase.idle => '',
  };
}
...
class BlurSwitchController extends ChangeNotifier {
  ...
  void switchTo(String id) {
    ...
    _set(SwitchPhase.out);
    Future<void>.delayed(outDuration, () {
      if (_disposed || generation != _generation) return;
      open(id);
      _set(SwitchPhase.blurIn);
      Future<void>.delayed(inDuration, () {
        if (_disposed || generation != _generation) return;
        _set(SwitchPhase.idle);
      });
    });
  }

  void _set(SwitchPhase next) {
