...
enum ElStatDirection {
  ...
  ElLucideGlyph get glyph => switch (this) {
    ElStatDirection.up => ElLucide.arrowUp,
    ElStatDirection.down => ElLucide.arrowDown,
    ElStatDirection.flat => ElLucide.minus,
  };
  ...
  String get sign => switch (this) {
    ...
    ElStatDirection.flat => '',
  };

  /// The `sr-only` word, which the port hands to [Semantics].
  String get word => switch (this) {
    ElStatDirection.up => 'Up',
    ElStatDirection.down => 'Down',
    ElStatDirection.flat => 'No change',
  };
}

/// `StatDelta` — *"the magnitude, unsigned. The component writes the sign."*
typedef ElStatDelta = ({String value, ElStatDirection direction});
...
enum ElStatState {
  ...
  /// em dash.
  bool get blank => this == ElStatState.error || this == ElStatState.empty;
}
...
class ElStatDeltaMark extends StatelessWidget {
  ...
  Color ink(ElThemeData theme) {
    if (delta.direction == ElStatDirection.flat) return theme.mutedForeground;
...
/// The figure.
class ElStat extends StatelessWidget {
  const ElStat({
    super.key,
    required this.label,
    required this.value,
    this.delta,
    this.betterWhen = ElStatDirection.up,
    this.hint,
    this.state = ElStatState.ready,
    this.message,
    this.disabled = false,
  });
  ...
  static double get rowGap => el(2);

  /// `gap-2` on the delta row, both axes (`flex-wrap`).
  static double get deltaGap => el(2);

  /// `<Skeleton className="h-5 w-28"/>` — the figure's placeholder.
  static Size get figureSkeleton => Size(el(28), el(5));

  /// `<Skeleton className="h-2.5 w-14"/>` — the delta's.
  static Size get deltaSkeleton => Size(el(14), el(2.5));

  /// `gap-1.5` between the error glyph and its message.
  static double get messageGap => el(1.5);

  /// The `.type-num-lg` line box the figure slot always holds.
  static double get figureHeight => ElType.numLg.size! * ElType.numLg.height!;

  @override
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final bool loading = state == ElStatState.loading;
    ...
    if (loading) {
      ...
      figure = Semantics(
        label: 'No value',
        child: ElText('—', ElType.numLg, color: theme.mutedForeground),
      );
    } else {
      figure = ElSwapIn(
        // `<span key={state}>` — a `loading → ready` transition replays it.
        replayKey: state,
        child: ElText(value, ElType.numLg, color: theme.foreground),
      );
    }
            ...
            children: <Widget>[
              if (delta != null)
                if (loading)
                  ...
                  ElStatDeltaMark(delta: delta!, betterWhen: betterWhen),
              if (state == ElStatState.error && message != null)
                Row(
                  mainAxisSize: MainAxisSize.min,
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: <Widget>[
                    ...
                    ElText(message!, ElType.small, color: theme.destructiveInk),
                  ],
                )
              else if (state == ElStatState.empty && message != null)
                ElText(message!, ElType.small)
              else if (hint != null)
                ElText(hint!, ElType.small),
            ],
          ),
        ],
      ],
