...
/// fill is [ThemeTokens.actionText] and never [ThemeTokens.primary].
///
...
///    `use-replay-on-state-change`, so there is no [StateChangeFeedback] here.
...
double get _trackHeight => space(2.5);

/// `size-5` — one thumb.
double get _thumbSize => space(5);
...
/// square: `18 + 8 + 8 = 34`. [HitArea] states that rule once for the whole
/// family and the three shipped controls now follow it too.
double get _hitInset => space(2);
...
double get _thumbReach => _thumbSize / 2 - BorderWidths.hairline + _hitInset;
...
/// arrangement `Checkbox` and `RadioGroup` already ask for.
...
class _SliderState extends State<Slider> {
  ...
  /// one.
  ///
  /// **Not `event.localPosition`.** [HitArea] forwards a hit in the margin to
  ...
  @override
  void didUpdateWidget(Slider old) {
  ...
  void _syncNodes() {
    while (_nodes.length < widget.values.length) {
      _nodes.add(FocusNode(debugLabel: 'Slider thumb ${_nodes.length}'));
    }
    while (_nodes.length > widget.values.length) {
  ...
  /// Where [value] sits between [Slider.min] and [Slider.max], 0 to 1.
  ...
  Widget build(BuildContext context) {
    ...
    // `StateGrid` wraps every cell in an `IntrinsicHeight`. A
    ...
    // stands outside the channel — the same ordering [HitArea] documents.
    return HitArea(
...
class _Thumb extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget knob = TweenAnimationBuilder<Color?>(
      ...
      // browser interpolates it — the same reasoning [SelectionControl]
      // records.
      tween: ColorTween(
      ...
      builder: (BuildContext context, Color? ring, Widget? child) => Surface(
        ...
        // layers.
        spec: Button.withFocusRing(Shadows.control, ring ?? theme.ring),
