...
/// | thumb `transition-[transform,box-shadow] duration-fast ease-spring` | the ring springs; see [ElTransforms.sliderThumbHoverScale] for what does not |
...
/// fill is [ElThemeData.actionInk] and never [ElThemeData.primary].
///
...
///    `use-replay-on-state-change`, so there is no [ElJellyReplay] here.
...
double get _trackHeight => el(2.5);

/// `size-5` — one thumb.
double get _thumbSize => el(5);
...
/// square: `18 + 8 + 8 = 34`. [ElHitArea] states that rule once for the whole
/// family and the three shipped controls now follow it too.
double get _hitInset => el(2);
...
double get _thumbReach => _thumbSize / 2 - ElWidths.hairline + _hitInset;
...
/// arrangement `ElCheckbox` and `ElRadioGroup` already ask for.
class ElSlider extends StatefulWidget {
  const ElSlider({
    super.key,
...
class _ElSliderState extends State<ElSlider> {
  ...
  /// one.
  ///
  /// **Not `event.localPosition`.** [ElHitArea] forwards a hit in the margin to
  ...
  @override
  void didUpdateWidget(ElSlider old) {
  ...
  void _syncNodes() {
    while (_nodes.length < widget.values.length) {
      _nodes.add(FocusNode(debugLabel: 'ElSlider thumb ${_nodes.length}'));
    }
    while (_nodes.length > widget.values.length) {
  ...
  /// Where [value] sits between [ElSlider.min] and [ElSlider.max], 0 to 1.
  ...
  double _thumbLeft(double value) => _fraction(value) * (_width - _thumbSize);
  ...
  Widget build(BuildContext context) {
    ...
    // `ElStateGrid` wraps every cell in an `IntrinsicHeight`. A
      ...
      child: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          return Stack(
            ...
            children: <Widget>[
              Positioned.fill(
                child: _Track(theme: theme, range: _rangeRect()),
              ),
              for (int i = 0; i < widget.values.length; i++)
                Positioned(
                  ...
                  child: _Thumb(
                    ...
                    scale: _active == i
                        ? ElTransforms.sliderThumbActiveScale
                        : _hovered == i
                    ...
                    semantics: (Widget child) => Semantics(
                      ...
                      onIncrease: _operable
                          ? () => _nudge(i, widget.step)
                          : null,
                      onDecrease: _operable
                          ? () => _nudge(i, -widget.step)
                          : null,
                      child: child,
                    ),
    ...
    // stands outside the channel — the same ordering [ElHitArea] documents.
    return ElHitArea(
  ...
  (double, double) _rangeRect() {
    final double low = widget.values.length > 1
        ? _fraction(widget.values.first)
        : 0;
...
///    box-shadow]`, and it tweens on [ElCurves.spring] over
///    [ElDurations.transitionDefault];
///  * the **scale** is Tailwind v4's `scale` property, which that list does not
///    name, so it snaps. See [ElTransforms.sliderThumbHoverScale].
class _Thumb extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget knob = TweenAnimationBuilder<Color?>(
      ...
      // browser interpolates it — the same reasoning [ElSelectionControl]
      // records.
      tween: ColorTween(
      ...
      builder: (BuildContext context, Color? ring, Widget? child) =>
          ElMachineSurface(
            ...
            // layers.
            spec: ElButton.withFocusRing(ElShadows.btn, ring ?? theme.ring),
