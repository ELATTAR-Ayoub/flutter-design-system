...
/// | class | line | effect |
/// |---|---|---|
/// | `flex w-fit items-stretch` | 8 | a shrink-to-fit row whose children stretch to the tallest — this is the whole reason a height-less [ElButtonGroupText] comes out 40px |
...
/// the left end keeps the child's own radius — a `rounded-pill` [ElButton]
/// leaves a 20px stadium on a 40px control — while the right end is always
/// 12px.
///
/// ## Corners, and why a clip alone cannot make one
///
/// CSS restyles a child from its parent. Flutter cannot: [ElButton] paints
/// `BorderRadius.circular(ElRadii.pill)` in its own build (`button.dart:471`)
...
///
/// Members this file declares are not bled at all. [ElButtonGroupText] is told
/// its shape through `_ElSlot`, an [InheritedWidget] standing in for the
/// cascade, and paints it directly — corners, dropped left border and all;
/// [ElButtonGroupSeparator] is a 1px rule with no corners and no border, so
...
/// (or, for a [ElButtonGroupText], because it was told not to paint one). Two
...
/// group can hold — `--input` for an outline [ElButton], `--destructive/25` for
/// a destructive one, transparent for the other five variants and `--border`
/// for a [ElButtonGroupText] (which paints its own). Group B's first seam
...
/// [ElButtonVariant.destructive] (`button.tsx`, and `button.dart:115`).
///
...
/// it. A member that cannot (any [ElButton]) is wrapped in a `_BledSlot`
/// instead, which does the reading on its behalf.
class _ElSlot extends InheritedWidget {
  const _ElSlot({required this.shape, required super.child});

  final _SlotShape shape;

  static _SlotShape? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<_ElSlot>()?.shape;

  @override
  bool updateShouldNotify(_ElSlot oldWidget) => oldWidget.shape != shape;
}
...
class ElButtonGroup extends StatelessWidget {
  ...
  /// The members, in source order — buttons, [ElButtonGroupText] cells and
  /// [ElButtonGroupSeparator] rules.
  ///
  ...
  /// rules declare them — `ElRadii.pill` is 999 here, and clamps to half the
  ...
  static bool _hasDataSlot(Widget child) => child is! ElButtonGroupText;
  ...
  /// member. A trailing [ElButtonGroupText] is invisible to the selector, so
  ...
  static double _ownRadius(Widget child) {
    if (child is ElButton) return ElRadii.pill;
    if (child is ElButtonGroupText) return ElRadii.lg;
  ...
  static _SlotShape _shapeOf(List<Widget> children, int index) {
    ...
    return _SlotShape(
      ...
      // last one.
      right: forced ? ElRadii.lg : (last ? own : 0),
      leftBorder: first,
    );
  }

  /// The resting `border-color` of a [ElButton] variant — the colour of the
  ...
  static Color _frameOf(ElThemeData theme, ElButtonVariant variant) {
    switch (variant) {
  ...
  Widget _member(ElThemeData theme, int index) {
    final Widget child = children[index];
    final _SlotShape shape = _shapeOf(children, index);
    if (child is ElButtonGroupText) {
      return _ElSlot(shape: shape, child: child);
    }
    if (child is ElButton) {
      return _BledSlot(
        shape: shape,
...
/// [ElButton]. That frame is a stroke rather than a [BoxDecoration] border for
...
/// reaches straight past it to the last Button. [ElButtonGroup] reproduces
/// that, including the case where the reach is visibly wrong.
class ElButtonGroupText extends StatelessWidget {
  const ElButtonGroupText(this.text, {super.key, this.numeric = false});
  ...
  /// [ElComponentType.buttonGroupNum], transcribed once in the foundation layer
  /// rather than re-derived here.
  final bool numeric;

  /// `px-2.5` — 10px.
  static double get paddingX => el(2.5);
  ...
  Widget build(BuildContext context) {
    ...
    // with a full border.
    final _SlotShape shape =
        _ElSlot.maybeOf(context) ??
        const _SlotShape(left: ElRadii.lg, right: ElRadii.lg, leftBorder: true);
        ...
        child: Padding(
          ...
          padding: EdgeInsets.fromLTRB(
            ElButtonGroupText.paddingX +
                (shape.leftBorder ? ElWidths.hairline : 0),
            ElWidths.hairline,
            ElButtonGroupText.paddingX + ElWidths.hairline,
            ElWidths.hairline,
          ),
          ...
          child: Center(
            ...
            child: ElText(
              text,
              numeric
                  ? ElComponentType.buttonGroupNum
                  : ElComponentType.buttonGroupText,
              maxLines: 1,
              softWrap: false,
            ),
          ),
...
///
/// `w-px` (**1px**, [ElWidths.hairline]) + `self-stretch` + `my-px` (**1px of
...
class ElButtonGroupSeparator extends StatelessWidget {
  const ElButtonGroupSeparator({super.key});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      // (row height − 2) tall.
      padding: EdgeInsets.symmetric(vertical: ElWidths.hairline),
      child: SizedBox(
        width: ElWidths.hairline,
        child: ColoredBox(color: ElTheme.of(context).input),
      ),
    );
  }
}
...
void _elStrokeFrame(
  ...
  if (color.a == 0) return;
  final RRect r = shape.deflate(ElWidths.hairline / 2);
...
/// [ElButtonGroupText]'s border — every run of it, minus the left one when
...
class _BledSlot extends SingleChildRenderObjectWidget {
  ...
  /// The member's own resting `border-color` — see `ElButtonGroup._frameOf`.
  final Color frame;
