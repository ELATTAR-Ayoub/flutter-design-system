...
/// three, so they are stated once here as [ElSelectionControl] and each
/// component supplies only its own skin — the same split `ElButton` makes
...
double get _hitInsetX => el(3);

/// `after:-inset-y-2` — 8px above and below.
double get _hitInsetY => el(2);
...
class ElHitArea extends SingleChildRenderObjectWidget {
  const ElHitArea({
    super.key,
  ...
  ///
  /// Pass `tester.renderObject(find.byType(ElHitArea))`.
  @visibleForTesting
  ...
  RenderObject createRenderObject(BuildContext context) =>
      _RenderHitArea(insets, border ?? ElWidths.hairline);
  ...
  void updateRenderObject(
    ...
    (renderObject as _RenderHitArea)
      ..insets = insets
      ..border = border ?? ElWidths.hairline;
  }
}
...
///
/// The replay itself is a re-key, which is the mechanism `ElKeyframePlayer`
...
class ElJellyReplay extends StatefulWidget {
  const ElJellyReplay({super.key, required this.state, required this.child});
  ...
  @override
  State<ElJellyReplay> createState() => _ElJellyReplayState();
}

class _ElJellyReplayState extends State<ElJellyReplay> {
  ...
  @override
  void didUpdateWidget(ElJellyReplay old) {
...
class ElSelectionControl extends StatefulWidget {
  const ElSelectionControl({
    super.key,
    required this.width,
  ...
  /// 42 × 34. See [ElHitArea] for the measurement.
  final double width;
  final double height;
  ...
  /// [ElDurations.transitionDefault] for all three controls.
  ///
  ...
  /// Handed to [ElJellyReplay]; a change replays the squash.
  ...
  /// object above [ElHitArea] tests its own bounds before it asks a child, so a
  ...
  @override
  State<ElSelectionControl> createState() => _ElSelectionControlState();
}

class _ElSelectionControlState extends State<ElSelectionControl> {
  bool _focused = false;
  ...
  /// [ElSelectionControl.inert] fixes.
  bool get _focusable =>
  ...
  /// on Space. Wired by hand for the same reason `ElButton` wires it: a bare
  ...
  Color _borderTarget(ElThemeData theme) {
  ...
  Color _ringTarget(ElThemeData theme) {
    if (widget.invalid) {
  ...
  Widget build(BuildContext context) {
                ...
                TweenAnimationBuilder<Color?>(
                  ...
                  builder: (BuildContext context, Color? ring, Widget? child) =>
                      ElMachineSurface(
                        ...
                        // composition `ElInput` makes, for the same reason: the socket is
    ...
    control = ElJellyReplay(state: widget.jellyState, child: control);
