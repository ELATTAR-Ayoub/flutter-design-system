...
/// | variant | shape |
/// |---|---|
/// | [ElMarkerVariant.normal] | bare row |
/// | [ElMarkerVariant.separator] | rule — label — rule |
/// | [ElMarkerVariant.border] | label with a rule under it |
///
...
class ElMarker extends StatelessWidget {
  const ElMarker({
    super.key,
    this.variant = ElMarkerVariant.normal,
    this.icon,
    required this.label,
  });

  final ElMarkerVariant variant;
  ...
  static double get ruleGap => el(1);

  /// `pb-2` on the `border` variant.
  static double get borderPadding => el(2);
  ...
  Widget build(BuildContext context) {
    ...
    final Widget text = ElText(
      ...
      align: variant == ElMarkerVariant.separator ? TextAlign.center : null,
    );
    ...
    final List<Widget> children = <Widget>[
      if (variant == ElMarkerVariant.separator) ...<Widget>[
        rule,
        SizedBox(width: ruleGap),
      ],
      if (icon != null) ...<Widget>[
        SizedBox.square(dimension: el(4), child: icon),
        SizedBox(width: gap),
      ],
      // `flex-none` under `separator`; a plain flex item otherwise, and the
      // only child that carries width there.
      if (variant == ElMarkerVariant.separator)
        Flexible(child: text)
      else
        Expanded(child: text),
      if (variant == ElMarkerVariant.separator) ...<Widget>[
    ...
    if (variant == ElMarkerVariant.border) {
      row = Container(
