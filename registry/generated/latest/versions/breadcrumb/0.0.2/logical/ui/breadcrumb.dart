...
class _CrumbState extends State<_Crumb> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.entry.isPage) {
      return Semantics(
        ...
        child: StyledText(
          ...
          // inherited 400, which is exactly [TextStyles.small]. The
    ...
    // A crumb goes somewhere, so it is a link and not a button — and a link
    // that only a pointer can follow is not a link. `Press` carries the whole
    // contract: Tab reachability, Enter and Space, the click cursor, a focus
    // ring, a name, and a 44px target under a 20px-tall word.
    return Press(
      link: true,
      onTap: widget.entry.onTap,
      semanticLabel: widget.entry.label,
      ...
      child: TweenAnimationBuilder<Color?>(
        ...
        builder: (BuildContext context, Color? ink, Widget? _) => StyledText(
          widget.entry.label,
          TextStyles.small,
