...
/// [ElIcon.strokeFor] snaps 14 and 16 to the same 2.4.
///
...
@immutable
class ElBreadcrumbEntry {
  /// `BreadcrumbLink` — a step in the trail.
  const ElBreadcrumbEntry.link(this.label, {this.onTap}) : isPage = false;

  /// `BreadcrumbPage` — the current page. `role="link" aria-disabled="true"
  /// aria-current="page"`, and deliberately not clickable.
  const ElBreadcrumbEntry.page(this.label) : onTap = null, isPage = true;
...
/// never asks for.
class ElBreadcrumb extends StatelessWidget {
  const ElBreadcrumb({super.key, required this.items});

  final List<ElBreadcrumbEntry> items;
  ...
  static double get separatorPx => el(3.5);
        ...
        row.add(
          ...
          ExcludeSemantics(
            child: ElIcon(
              ...
              strokeOverride: ElIcon.strokeFor(ElIcon.pxFor(ElIconSize.md)),
            ),
          ),
        );
      }
      row.add(_ElCrumb(entry: items[i]));
    }
...
class _ElCrumb extends StatefulWidget {
  const _ElCrumb({required this.entry});

  final ElBreadcrumbEntry entry;

  @override
  State<_ElCrumb> createState() => _ElCrumbState();
}

class _ElCrumbState extends State<_ElCrumb> {
  bool _hovered = false;
    ...
    if (widget.entry.isPage) {
      return Semantics(
        ...
        child: ElText(
          ...
          // inherited 400, which is exactly [ElComponentType.textSm]. The
      ...
      child: GestureDetector(
        ...
        child: Semantics(
          ...
          child: TweenAnimationBuilder<Color?>(
            ...
            curve: ElCurves.out,
            builder: (BuildContext context, Color? ink, Widget? _) => ElText(
              widget.entry.label,
              ElComponentType.textSm,
