...
///
/// The contract it stands on — [ElAgentTransport], [elResolveAgentState],
/// [ElAgentCapabilities], [ElAgentSendOptions], [elSerialiseAttachments] — is
...
///    [elResolveAgentState]'s branch 2 — *"something is waiting on a human"* —
...
///  * **The status line's shimmer is not `ElShimmerText`.** See
///    [ElAgentShimmerText] — `anim-shimmer-text` is a different animation from
...
///     none. The consequence is exact and bounded: [ElAgentFeatures.speech] and
///     [ElAgentFeatures.microphone] are honoured as flags, [ElAgentVoice] is
...
///     over its hint; [ElMenuItem] had `label` and `shortcut` and no child slot,
...
///     [ElMenuItem.subtitle], on `ElCommandItem.subtitle`'s own terms. This file
...
///    reference filters inside the card; `ElWelcomeCard` takes a capability
...
/// field in the system is [ElFieldVisibility], which this console's own
...
// `ElBlurSwitch` only. The widget form of `blurClass` is owned by the
...
@immutable
class ElAgentFeatures {
  const ElAgentFeatures({
    this.avatar = true,
  ...
  static const ElAgentFeatures all = ElAgentFeatures();
...
/// `AgentPersona`.
@immutable
class ElAgentPersona {
  const ElAgentPersona({
    required this.name,
    this.blurb,
...
@immutable
class ElAgentModel {
  const ElAgentModel({required this.id, required this.label, this.hint});
...
/// `AgentConsole`.
class ElAgentConsole extends StatefulWidget {
  const ElAgentConsole({
    super.key,
    required this.transport,
    this.persona,
    this.toolStates,
    this.models = const <ElAgentModel>[],
    this.commands = const <ElAgentCommand>[],
    this.avatar,
    this.features = ElAgentFeatures.all,
    this.accent,
    this.speed,
    this.renderToolResult,
    this.describeApproval,
    this.headerSlot,
    this.switchPhase = ElSwitchPhase.idle,
    this.height,
  });

  final ElAgentTransport transport;
  final ElAgentPersona? persona;
  final ElToolStateMap? toolStates;
  final List<ElAgentModel> models;
  ...
  final ElAgentFeatures features;
  ...
  final Widget Function(ElToolTurn turn)? renderToolResult;
  ...
  /// has. [ElBlurSwitch] returns its child untouched on
  /// [ElSwitchPhase.idle], so a console with no history behind it pays nothing.
  final ElSwitchPhase switchPhase;
  ...
  static double get headerGap => el(3);
  static double get headerInset => el(6);

  /// `space-y-4` in the scroller, and `pr-1` beside its scrollbar.
  static double get turnGap => el(4);
  static double get scrollerInset => el(1);
  ...
  static double get pinTolerance => el(8);

  @override
  State<ElAgentConsole> createState() => _ElAgentConsoleState();
}

class _ElAgentConsoleState extends State<ElAgentConsole> {
  final TextEditingController _draft = TextEditingController();
  final FocusNode _composerFocus = FocusNode();
  final ScrollController _scroller = ScrollController();

  List<ElAgentAttachment> _attachments = <ElAgentAttachment>[];
  ...
  @override
  void didUpdateWidget(ElAgentConsole old) {
  ...
  bool _onScroll(ScrollNotification notification) {
    if (notification is ScrollUpdateNotification && _scroller.hasClients) {
      ...
      _pinned = p.maxScrollExtent - p.pixels < ElAgentConsole.pinTolerance;
    }
    return false;
  }
  ...
  ElAgentState get _state => elResolveAgentState(
    turns: widget.transport.turns,
    signals: ElAgentSignals(
      isLoading: widget.transport.isLoading,
      // `awaitingFirstEvent && !transport.turns.some(isAssistantTurn)`.
      awaitingFirstEvent:
          _awaitingFirstEvent &&
          !widget.transport.turns.any((ElAgentTurn t) => t is! ElUserTurn),
    ),
  ...
  Future<void> _send(String raw) async {
    ...
    final List<ElAgentAttachment> outgoing = _attachments;
    setState(() {
      _draft.clear();
      _attachments = <ElAgentAttachment>[];
    ...
    final ElSerialisedMessage payload = outgoing.isEmpty
        ? ElSerialisedMessage(
            text: text,
            attachments: const <ElAgentAttachment>[],
          )
        : elSerialiseAttachments(text, outgoing);
  ...
  void _stop() {
    final List<ElAgentTurn> turns = widget.transport.turns;
  ...
  void _useCapability(ElAgentCommand command) {
    final VoidCallback? run = command.run;
    if (run != null) {
      run();
      return;
    }
    final String text = command.directive != null
        ? '${command.directive} '
        : '/${command.id} ';
  ...
  List<ElAgentCommand> get _allCommands {
    if (!widget.features.commands) return const <ElAgentCommand>[];
    return <ElAgentCommand>[
      ...widget.commands,
      ElAgentCommand(
        id: 'clear',
        label: 'Clear',
  ...
  Widget build(BuildContext context) {
    final ElAgentTransport transport = widget.transport;
    final ElAgentState state = _state;

    return SizedBox(
      height: widget.height,
      child: Padding(
        padding: EdgeInsets.all(ElAgentConsole.padding),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: <Widget>[
            if (widget.features.avatar) ...<Widget>[
              _header(context, state),
              SizedBox(height: ElAgentConsole.gap),
            ],
            ...
            ElAgentComposer(
              ...
              onAttach: widget.features.attachments
                  ? (List<ElAgentAttachment> files) => setState(
                      () => _attachments = <ElAgentAttachment>[
                        ..._attachments,
                        ...files,
                      ],
  ...
  Widget _header(BuildContext context, ElAgentState state) {
    final ElThemeData theme = ElTheme.of(context);
    return Padding(
      padding: EdgeInsets.only(right: ElAgentConsole.headerInset),
      child: Row(
        ...
        children: <Widget>[
          ...
          SizedBox(width: ElAgentConsole.headerGap),
          Expanded(
            child: Column(
              ...
              children: <Widget>[
                ...
                Align(
                  ...
                  child: ElAgentStatusLine(state: state),
                ),
              ],
            ),
          ),
  ...
  Widget _transcript(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      if (empty)
        ElWelcomeCard(
          ...
          capabilities: <ElAgentCapability>[
            for (final ElAgentCommand c in widget.commands)
              if (c.group == ElAgentCommandGroup.skill)
                ElAgentCapability(
                  id: c.id,
                  label: c.label,
                  hint: c.hint,
          ...
          onUseCapability: (ElAgentCapability capability) => _useCapability(
            widget.commands.firstWhere(
              (ElAgentCommand c) => c.id == capability.id,
            ),
          ),
          disabled: !transport.isReady,
          avatar: _avatarSlot,
        ),
      for (final ElAgentTurn turn in transport.turns) ..._turn(context, turn),
      for (final ElPendingApproval approval in transport.pendingApprovals)
        ElApprovalCard(approval: approval, describe: widget.describeApproval),
    ...
    final Widget column = Column(
      ...
      children: <Widget>[
        for (int i = 0; i < rows.length; i += 1) ...<Widget>[
          if (i > 0) SizedBox(height: ElAgentConsole.turnGap),
          rows[i],
        ],
      ],
    );
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: Padding(
        padding: EdgeInsets.only(right: ElAgentConsole.scrollerInset),
        child: LayoutBuilder(
          builder: (BuildContext context, BoxConstraints constraints) =>
              SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  ...
                  child: ElBlurSwitch(phase: widget.switchPhase, child: column),
                ),
              ),
        ),
      ),
  ...
  /// `ElWelcomeCard`'s own seam — a box of the size it asks for, filled with
  /// whatever renderer this console was given.
  Widget _avatarSlot(BuildContext context, double size) => ElAgentFace(
  ...
  List<Widget> _turn(BuildContext context, ElAgentTurn turn) {
    switch (turn) {
      case ElUserTurn():
        return <Widget>[ElUserMessage(turn: turn)];

      case ElTextTurn():
        if (!_stopped.contains(turn.id)) {
          return <Widget>[ElAgentMessage(turn: turn)];
        }
        ...
        return <Widget>[
          ElAgentMessage(turn: turn.notStreaming()),
          SizedBox(height: ElAgentConsole.turnGap),
          const ElMarker(
  ...
  Widget _errorRow(BuildContext context, String message) {
    ...
    return Container(
      ...
      child: ElText(message, ElType.small, color: theme.destructiveInk),
    );
  }
}
...
class _ModelPicker extends StatelessWidget {
  ...
  static double get menuWidth => ElContainers.xs;

  /// `h-8 gap-1 px-2` on the trigger.
  static EdgeInsets get triggerPadding =>
      EdgeInsets.symmetric(horizontal: el(2));

  @override
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    ElAgentModel? current;
    for (final ElAgentModel model in models) {
      if (model.id == modelId) current = model;
    }

    return ElDropdownMenu(
      width: menuWidth,
      side: ElPopoverSide.top,
      enabled: !disabled,
      trigger: ElButton(
      ...
      children: <ElMenuChild>[
        ...
        for (final ElAgentModel model in models)
          ElMenuItem(
            ...
            // slot — and so sat beside the label — for as long as [ElMenuItem]
            // had no second line; see [ElMenuItem.subtitle].
            subtitle: model.hint,
