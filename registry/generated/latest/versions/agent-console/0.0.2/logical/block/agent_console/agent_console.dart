...
///
/// The contract it stands on — [AgentTransport], [resolveAgentState],
/// [AgentCapabilities], [AgentSendOptions], [serialiseAttachments] — is
...
///    [resolveAgentState]'s branch 2 — *"something is waiting on a human"* —
...
///  * **The status line's shimmer is not `AttachmentStatusText`.** See
///    [AgentStatusText] — `anim-shimmer-text` is a different animation from
...
///     [MenuItem.subtitle], on `CommandItem.subtitle`'s own terms. This file
...
///    reference filters inside the card; `WelcomeCard` takes a capability
...
/// field in the system is [FieldVisibility], which this console's own
...
import '../../components/ui/agent_avatar.dart';
import '../../components/ui/agent_composer.dart';
import '../../components/ui/agent_core.dart';
import '../../components/ui/agent_face.dart';
// `BlurSwitch` only. The widget form of `blurClass` is owned by the
...
import '../../components/ui/agent_history.dart';
import '../../components/ui/agent_slash_palette.dart';
import '../../components/ui/agent_transcript.dart';
import '../../components/ui/button.dart';
import '../../components/ui/dropdown_menu.dart';
import '../../components/ui/icon.dart';
import '../../components/ui/icon_paths.g.dart';
...
class AgentFeatures {
  ...
  static const AgentFeatures all = AgentFeatures();
...
@immutable
class AgentModel {
  const AgentModel({required this.id, required this.label, this.hint});
...
class AgentConsole extends StatefulWidget {
  const AgentConsole({
    ...
    this.persona,
    this.toolStates,
    this.models = const <AgentModel>[],
    this.commands = const <AgentCommand>[],
    this.avatar,
    this.features = AgentFeatures.all,
    this.accent,
    this.speed,
  ...
  final Widget Function(ToolTurn turn)? renderToolResult;
  ...
  /// has. [BlurSwitch] returns its child untouched on
  /// [SwitchPhase.idle], so a console with no history behind it pays nothing.
  final SwitchPhase switchPhase;
  ...
  static double get headerGap => space(3);
  static double get headerInset => space(6);

  /// `space-y-4` in the scroller, and `pr-1` beside its scrollbar.
  static double get turnGap => space(4);
  static double get scrollerInset => space(1);
  ...
  static double get pinTolerance => space(8);
...
class _AgentConsoleState extends State<AgentConsole> {
  ...
  List<AgentAttachment> _attachments = <AgentAttachment>[];
  ...
  @override
  void didUpdateWidget(AgentConsole old) {
  ...
  bool _onScroll(ScrollNotification notification) {
    if (notification is ScrollUpdateNotification && _scroller.hasClients) {
      final flutter.ScrollPosition p = _scroller.position;
      _pinned = p.maxScrollExtent - p.pixels < AgentConsole.pinTolerance;
    }
    return false;
  }
  ...
  AgentState get _state => resolveAgentState(
    ...
    signals: AgentSignals(
      ...
      awaitingFirstEvent:
          _awaitingFirstEvent &&
          !widget.transport.turns.any((AgentTurn t) => t is! UserTurn),
    ),
  ...
  Future<void> _send(String raw) async {
    ...
    final List<AgentAttachment> outgoing = _attachments;
    setState(() {
      _draft.clear();
    ...
    final SerialisedMessage payload = outgoing.isEmpty
        ? SerialisedMessage(text: text, attachments: const <AgentAttachment>[])
        : serialiseAttachments(text, outgoing);
  ...
  void _stop() {
    final List<AgentTurn> turns = widget.transport.turns;
  ...
  void _useCapability(AgentCommand command) {
  ...
  List<AgentCommand> get _allCommands {
    if (!widget.features.commands) return const <AgentCommand>[];
    return <AgentCommand>[
      ...widget.commands,
      AgentCommand(
        id: 'clear',
  ...
  Widget build(BuildContext context) {
    final AgentTransport transport = widget.transport;
    final AgentState state = _state;

    return SizedBox(
      height: widget.height,
      child: Padding(
        padding: EdgeInsets.all(AgentConsole.padding),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: <Widget>[
            if (widget.features.avatar) ...<Widget>[
              _header(context, state),
              SizedBox(height: AgentConsole.gap),
            ],
            ...
            AgentComposer(
              ...
              onAttach: widget.features.attachments
                  ? (List<AgentAttachment> files) => setState(
                      () => _attachments = <AgentAttachment>[
                        ..._attachments,
                        ...files,
                      ],
  ...
  Widget _header(BuildContext context, AgentState state) {
    final ThemeTokens theme = ThemeScope.of(context);
    return Padding(
      padding: EdgeInsets.only(right: AgentConsole.headerInset),
      child: Row(
        ...
        children: <Widget>[
          ...
          SizedBox(width: AgentConsole.headerGap),
          Expanded(
            child: Column(
              ...
              children: <Widget>[
                ...
                Align(
                  ...
                  child: AgentStatusLine(state: state),
                ),
              ],
            ),
          ),
  ...
  Widget _transcript(BuildContext context) {
    ...
    final List<Widget> rows = <Widget>[
      if (empty)
        WelcomeCard(
          ...
          capabilities: <AgentCapability>[
            for (final AgentCommand c in widget.commands)
              if (c.group == AgentCommandGroup.skill)
                AgentCapability(
                  id: c.id,
                  label: c.label,
                  hint: c.hint,
          ...
          onUseCapability: (AgentCapability capability) => _useCapability(
            widget.commands.firstWhere(
              (AgentCommand c) => c.id == capability.id,
            ),
          ),
          disabled: !transport.isReady,
          avatar: _avatarSlot,
      ...
      for (final PendingApproval approval in transport.pendingApprovals)
        ApprovalCard(approval: approval, describe: widget.describeApproval),
    ...
    final Widget column = Column(
      ...
      children: <Widget>[
        for (int i = 0; i < rows.length; i += 1) ...<Widget>[
          if (i > 0) SizedBox(height: AgentConsole.turnGap),
          rows[i],
        ],
      ],
    );
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: Padding(
        padding: EdgeInsets.only(right: AgentConsole.scrollerInset),
        child: LayoutBuilder(
          builder: (BuildContext context, BoxConstraints constraints) =>
              SingleChildScrollView(
                ...
                child: ConstrainedBox(
                  ...
                  child: BlurSwitch(phase: widget.switchPhase, child: column),
                ),
              ),
        ),
      ),
  ...
  /// `WelcomeCard`'s own seam — a box of the size it asks for, filled with
  /// whatever renderer this console was given.
  Widget _avatarSlot(BuildContext context, double size) => AgentFace(
...
class _ModelPicker extends StatelessWidget {
  ...
  static double get menuWidth => Containers.xs;
  ...
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    AgentModel? current;
    for (final AgentModel model in models) {
      ...
      children: <MenuChild>[
        ...
        for (final AgentModel model in models)
          MenuItem(
            ...
            // slot — and so sat beside the label — for as long as [MenuItem]
            // had no second line; see [MenuItem.subtitle].
            subtitle: model.hint,
