...
/// row ladder, the paddings and the press feel all come from [ElButton], and
...
/// | menu row hover | `--secondary` reached over 250ms on `--ease-spring`, overshooting to rgb(43,43,46) past a rgb(39,39,42) target — [ElButton]'s own colour spring, untouched |
///
...
///     figures and −0.01em survive. See [ElComponentType.sidebarMenuBadge].
...
///     opposite outcome. See [ElComponentType.avatarFallback].
///  3. **`SidebarGroupLabel`'s `px-3 pr-10` never renders on the page.** Every
///     label on it is inside a [ElSidebarCollapsibleGroup], which passes
...
///     job and can. Reproduced: [ElSidebarRail] takes no focus.
...
///     149.5px list item that contains it (measured). [ElSidebarMenu] takes
...
///     control has a keyboard-reachable twin ([ElSidebarTrigger]) doing the
///     same job.
...
enum ElSidebarSide {
  ...
  bool get isLeft => this == ElSidebarSide.left;
}

/// `Sidebar variant`.
enum ElSidebarVariant {
...
enum ElSidebarMenuButtonSize {
  ...
  /// Button's own ladder."*
  ElButtonSize get button => switch (this) {
    ElSidebarMenuButtonSize.sm => ElButtonSize.xs,
    ElSidebarMenuButtonSize.md => ElButtonSize.sm,
    ElSidebarMenuButtonSize.lg => ElButtonSize.lg,
  };
}

/// `SidebarMenuSubButton size`.
enum ElSidebarMenuSubButtonSize {
  /// → `Button` `xs`.
  sm,
  ...
  ElButtonSize get button => switch (this) {
    ElSidebarMenuSubButtonSize.sm => ElButtonSize.xs,
    ElSidebarMenuSubButtonSize.md => ElButtonSize.sm,
  };
}
...
class ElSidebarScope extends InheritedWidget {
  const ElSidebarScope({
    super.key,
    required this.open,
  ...
  /// [ElSidebarProvider.variant] for why a *provider* knows this at all.
  final ElSidebarVariant variant;

  /// `data-state` — `"expanded"` or `"collapsed"`.
  bool get collapsed => !open;

  static ElSidebarScope? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<ElSidebarScope>();

  static ElSidebarScope of(BuildContext context) {
    final ElSidebarScope? scope = maybeOf(context);
    assert(
      scope != null,
      'ElSidebarScope.of must be used within a '
      'ElSidebarProvider.',
    );
    return scope!;
  }
  ...
  @override
  bool updateShouldNotify(ElSidebarScope old) =>
      old.open != open ||
...
/// The three `data-*` attributes [ElSidebar] writes on itself, published to the
/// regions inside it.
///
...
class ElSidebarChrome extends InheritedWidget {
  const ElSidebarChrome({
    super.key,
    required this.side,
    required this.variant,
  ...
  bool get iconMode => collapsible == ElSidebarCollapsible.icon;

  static ElSidebarChrome? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<ElSidebarChrome>();

  /// False outside a [ElSidebar], which is what an element carrying no
  /// `data-collapsible` resolves to.
  static bool iconModeOf(BuildContext context) =>
      maybeOf(context)?.iconMode ?? false;

  @override
  bool updateShouldNotify(ElSidebarChrome old) =>
      old.side != side ||
      old.variant != variant ||
...
class ElSidebarProvider extends StatefulWidget {
  ...
  /// The wrapper's flex children — a [ElSidebar] and usually a
  /// [ElSidebarInset].
  ...
  /// site passes the same value here and to its [ElSidebar]; they must agree,
  ...
  @override
  State<ElSidebarProvider> createState() => _ElSidebarProviderState();
}

class _ElSidebarProviderState extends State<ElSidebarProvider> {
  ...
  bool _isMobile(BuildContext context) =>
      ElSidebarProvider.isMobileWidth(MediaQuery.sizeOf(context).width);
  ...
  bool _onKey(KeyEvent event) {
    ...
    if (event.logicalKey != ElSidebarProvider.shortcut) return false;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.variant == ElSidebarVariant.inset) {
      row = ColoredBox(color: theme.sidebar, child: row);
    }

    return ElSidebarScope(
      open: _isOpen,
...
/// [collapsible] is [ElSidebarCollapsible.none], a `Sheet` on mobile, and the
...
class ElSidebar extends StatelessWidget {
  ...
  /// Must match [ElSidebarProvider.variant] — see that field.
  final ElSidebarVariant variant;
  ...
  ///
  /// [ElSidebarCollapsible.none] only, and it is a page override rather than a
  ...
  /// on both edges. 64px.
  static double get insetIconGap => ElWidths.sidebarIcon + el(4);

  /// `w-[calc(var(--sidebar-width-icon)+(--spacing(4))+2px)]` — the same
  /// measure for the **container**, plus the two hairlines. 66px.
  static double get insetIconWidth => insetIconGap + ElWidths.hairline * 2;

  /// `p-2` on the container under `floating` / `inset`.
  static double get framePadding => el(2);

  /// `w-4` — the rail's own width.
  static double get railWidth => el(4);

  @override
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final ElSidebarScope scope = ElSidebarScope.of(context);

    if (collapsible == ElSidebarCollapsible.none) {
      // `flex h-full w-(--sidebar-width) flex-col bg-sidebar
      //  text-sidebar-foreground`.
      return ElSidebarChrome(
        side: side,
        variant: variant,
        collapsible: null,
        child: _panelInk(
          theme,
          SizedBox(
            width: expand ? null : ElWidths.sidebar,
            child: ColoredBox(
              color: theme.sidebar,
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
    ...
    // `:has()` on the panel's own children — a [ElSidebarRail] among them is
    ...
    // to its column.
    final bool hasRail = children.any((Widget w) => w is ElSidebarRail);

    final bool collapsed = scope.collapsed;
    final ElSidebarCollapsible? mode = collapsed ? collapsible : null;
    final bool iconMode = mode == ElSidebarCollapsible.icon;
    final bool offcanvas = mode == ElSidebarCollapsible.offcanvas;
    final bool framed = variant != ElSidebarVariant.sidebar;

    final double gapWidth = offcanvas
        ? 0
        : iconMode
        ? (framed ? insetIconGap : ElWidths.sidebarIcon)
        : ElWidths.sidebar;
    final double containerWidth = iconMode
        ? (framed ? insetIconWidth : ElWidths.sidebarIcon)
        : ElWidths.sidebar;
    // `data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]`
    // — the panel keeps its width and leaves.
    final double slide = offcanvas ? -ElWidths.sidebar : 0;
    // `group-data-[side=left]:border-r` / `border-l`, on the plain variant only.
    final double border = framed ? 0 : ElWidths.hairline;
                ...
                builder: (BuildContext context, double width, Widget? _) =>
                    TweenAnimationBuilder<double>(
                      ...
                      builder: (BuildContext context, double dx, Widget? _) {
                        ...
                        final double railCentre = side.isLeft
                            ? left + width - border
                            : left + border;
                        return SizedBox(
                          width: gap,
  ...
  Widget _panelInk(ElThemeData theme, Widget child) => DefaultTextStyle.merge(
    style: TextStyle(color: theme.sidebarForeground),
    child: child,
  );

  /// `sidebar-container` + `sidebar-inner`.
  Widget _container(ElThemeData theme, {required bool framed}) {
    final bool floating = variant == ElSidebarVariant.floating;
    ...
    if (floating) {
      ...
      inner = DecoratedBox(
        decoration: BoxDecoration(
          ...
          boxShadow: ElShadows.tailwindSm.outerShadows(theme),
        ),
        child: ClipRRect(
    ...
    return DecoratedBox(
      decoration: BoxDecoration(
        border: Border(
          right: side.isLeft
              ? BorderSide(color: theme.border, width: ElWidths.hairline)
              : BorderSide.none,
          left: side.isLeft
              ? BorderSide.none
              : BorderSide(color: theme.border, width: ElWidths.hairline),
      ...
      child: Padding(
        padding: EdgeInsets.only(
          left: side.isLeft ? 0 : ElWidths.hairline,
          right: side.isLeft ? ElWidths.hairline : 0,
        ),
        child: inner,
      ),
    );
    ...
    return ElSidebarChrome(
      ...
      child: Semantics(
        ...
        child: ElSheetContent(
          side: side.isLeft ? ElSheetSide.left : ElSheetSide.right,
          // `[&>button]:hidden`.
...
///
/// Placement is [ElSidebar]'s (the strip has to know where the container's
/// edge currently is); this is the control itself, and it is what a call site
/// composes into the panel. Rendering it outside a [ElSidebar] renders nothing,
/// which is what `sm:flex` plus a `hidden` base amounts to on a page that has
/// no panel.
class ElSidebarRail extends StatelessWidget {
  const ElSidebarRail({super.key});

  /// `after:w-0.5` — *"a 2px hover hairline is a rule, the same class of
  /// measure as a border."*
  static double get hairline => el(0.5);

  @override
  Widget build(BuildContext context) => const SizedBox.shrink();
}

/// The rail as [ElSidebar] actually paints it, in the slot it computed.
...
class _RailSlotState extends State<_RailSlot> {
  ...
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final ElSidebarScope scope = ElSidebarScope.of(context);
    final ElSidebarChrome? chrome = ElSidebarChrome.maybeOf(context);
      ...
      child: GestureDetector(
        ...
        child: Semantics(
          ...
          child: AnimatedContainer(
            ...
            curve: ElCurves.outFlex,
            color: _hovered && widget.offcanvas ? theme.sidebar : elTransparent,
            child: Align(
              ...
              alignment: widget.offcanvas
                  ...
                  : Alignment.center,
              child: SizedBox(
                width: ElSidebarRail.hairline,
                child: ColoredBox(
                  color: _hovered ? theme.sidebarBorder : elTransparent,
                ),
              ),
            ),
          ),
...
class ElSidebarTrigger extends StatelessWidget {
  const ElSidebarTrigger({super.key, this.onPressed});
...
class ElSidebarInset extends StatelessWidget {
  const ElSidebarInset({super.key, required this.child});
  ...
  Widget build(BuildContext context) {
    ...
    if (scope.variant == ElSidebarVariant.inset) {
    ...
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: <Widget>[Expanded(child: main)],
      ),
    );
  }
}
...
EdgeInsets _regionPadding(BuildContext context) =>
    EdgeInsets.all(ElSidebarChrome.iconModeOf(context) ? el(2) : el(3));

/// `SidebarHeader` — `flex flex-col gap-2 p-3`.
class ElSidebarHeader extends StatelessWidget {
  const ElSidebarHeader({super.key, required this.children});
...
/// column puts a [ElSidebarContent] between them, and that is what takes the
/// slack.
class ElSidebarFooter extends StatelessWidget {
  const ElSidebarFooter({super.key, required this.children});
  ...
  Widget build(BuildContext context) => Padding(
    padding: _regionPadding(context),
    child: _GappedColumn(gap: ElSidebarHeader.gap, children: children),
  );
}
...
/// sit on the floor.
class ElSidebarContent extends StatelessWidget {
  const ElSidebarContent({super.key, this.children = const <Widget>[]});
  ...
  Widget build(BuildContext context) {
    ...
    return Expanded(
      child: ElSidebarChrome.iconModeOf(context)
...
/// inset.
class ElSidebarSeparator extends StatelessWidget {
  const ElSidebarSeparator({super.key});

  @override
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    final bool icon = ElSidebarChrome.iconModeOf(context);
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: icon ? el(2) : el(3)),
      child: SizedBox(
...
class ElSidebarGroup extends StatelessWidget {
  const ElSidebarGroup({super.key, required this.children});
...
class ElSidebarGroupContent extends StatelessWidget {
  const ElSidebarGroupContent({super.key, required this.child});
...
class ElSidebarGroupLabel extends StatelessWidget {
  const ElSidebarGroupLabel(this.label, {super.key, this.padding});

  final String label;

  /// Overrides `px-3 pr-10`.
  ///
  /// [ElSidebarCollapsibleGroup] passes [EdgeInsets.zero] — its `px-0`, which
  ...
  Widget build(BuildContext context) {
    ...
    final Widget row = SizedBox(
      height: height,
      child: Padding(
        padding: padding ?? EdgeInsets.only(left: el(3), right: el(10)),
        child: Align(
          alignment: AlignmentDirectional.centerStart,
          child: ElText(label, ElType.navSm, color: theme.sidebarForeground),
        ),
      ),
    );
...
/// padding."* Positioned by [ElSidebarCollapsibleGroup], which is the only
/// thing that holds one.
class ElSidebarGroupAction extends StatelessWidget {
  const ElSidebarGroupAction({
    super.key,
    required this.child,
    required this.label,
    this.onPressed,
...
class ElSidebarCollapsibleGroup extends StatefulWidget {
  ...
  /// A [ElSidebarGroupAction], anchored `top-4 right-3` in the group.
  final Widget? action;

  final bool defaultOpen;

  /// `h-6` on the trigger.
  static double get triggerHeight => el(6);

  /// `h-px`, and `h-1` while closed.
  static double get lineOpen => ElWidths.hairline;
  static double get lineClosed => el(1);

  @override
  State<ElSidebarCollapsibleGroup> createState() =>
      _ElSidebarCollapsibleGroupState();
}

class _ElSidebarCollapsibleGroupState extends State<ElSidebarCollapsibleGroup> {
  late bool _open = widget.defaultOpen;
  ...
  Widget build(BuildContext context) {
    ...
    final Widget line = TweenAnimationBuilder<double>(
      tween: Tween<double>(
            ...
            ? ElSidebarCollapsibleGroup.lineOpen
            : ElSidebarCollapsibleGroup.lineClosed,
      ),
      duration: elAnimationDuration(context, ElDurations.transitionDefault),
      curve: ElCurves.out,
    ...
    final Widget trigger = ElButton(
      ...
      surface: const ElButtonSurface(
        fill: elTransparent,
        hoverFill: elTransparent,
      ),
      // `px-0`.
    ...
    final Widget header = Row(
      ...
      children: <Widget>[
        Flexible(
          child: ElSidebarGroupLabel(widget.label, padding: EdgeInsets.zero),
        ),
        ...
        Expanded(
          child: Padding(
            padding: EdgeInsets.only(
              left: el(2),
              right: hasAction ? el(8) : el(2),
            ),
            child: SizedBox(
              height: ElSidebarCollapsibleGroup.triggerHeight,
              child: trigger,
            ),
          ),
        ),
        ...
        Stack(
          children: <Widget>[
            ...
            if (hasAction)
              ...
              Positioned(top: el(1), right: 0, child: widget.action!),
          ],
        ),
        ElUnfold(
          open: _open,
          child: ElSidebarGroupContent(child: widget.child),
        ),
      ],
    );
  }
...
/// What a [ElSidebarMenuItem] tells the row inside it.
///
...
///
/// **Why this is not [ElSlidingPillGroup].** That widget lays its options out
...
/// and [ElJelly] for the 600ms squash — so there is still one transcription of
/// each.
class ElSidebarMenu extends StatefulWidget {
  const ElSidebarMenu({super.key, required this.children});

  /// [ElSidebarMenuItem]s, usually.
  final List<Widget> children;
  ...
  @override
  State<ElSidebarMenu> createState() => _ElSidebarMenuState();
}

class _ElSidebarMenuState extends State<ElSidebarMenu>
  ...
  late final AnimationController _jelly = AnimationController(
    vsync: this,
    duration: ElDurations.animJelly,
  );
  ...
  /// Called from [ElSidebarMenuButton]'s own lifecycle — a plain list append,
  ...
  Widget build(BuildContext context) {
    ...
    _jelly.duration = elAnimationDuration(context, ElDurations.animJelly);
    ...
    for (int i = 0; i < widget.children.length; i++) {
      if (i > 0) rows.add(SizedBox(height: ElSidebarMenu.gap));
      ...
      child: Stack(
        ...
        children: <Widget>[
          if (rect != null)
            AnimatedPositioned(
              duration: travels
                  ? elAnimationDuration(context, ElDurations.base)
                  : Duration.zero,
              ...
              child: IgnorePointer(
                child: AnimatedOpacity(
                  ...
                  duration: travels
                      ? elAnimationDuration(context, ElDurations.fast)
                      : Duration.zero,
                  curve: ElCurves.out,
                  child: AnimatedBuilder(
                    animation: _jelly,
                    builder: (BuildContext context, Widget? child) {
                      final Offset scale = ElJelly.scale.evaluate(_jelly);
...
class ElSidebarMenuItem extends StatelessWidget {
  const ElSidebarMenuItem({
    super.key,
  ...
  /// The row — a [ElSidebarMenuButton], or a trigger wrapping one.
  final Widget button;

  /// A [ElSidebarMenuAction], `absolute top-1/2 right-1`.
  final Widget? action;

  /// A [ElSidebarMenuBadge], `absolute top-1/2 right-2`.
  final Widget? badge;

  /// A [ElSidebarMenuSub], which is in flow **under** the row and makes the
...
class ElSidebarMenuButton extends StatefulWidget {
  const ElSidebarMenuButton({
    super.key,
    required this.child,
    this.isActive = false,
    this.variant = ElButtonVariant.ghost,
    this.size = ElSidebarMenuButtonSize.md,
    this.tooltip,
    this.label,
    this.onPressed,
    this.expanded = false,
  ...
  /// The row's content — normally a [ElSidebarMenuRow].
  final Widget child;
  ...
  final ElSidebarMenuButtonSize size;
  ...
  static double get padding => el(2);

  /// `pr-10` — the lane an action reserves.
  static double get actionLane => el(10);

  /// `pr-16` — the wider lane a badge reserves.
  static double get badgeLane => el(16);

  /// `group-data-[collapsible=icon]:size-8!` — the collapsed row is a hard
  /// 32px square, and it gets there in one frame (measured).
  static double get iconSize => el(8);

  @override
  State<ElSidebarMenuButton> createState() => _ElSidebarMenuButtonState();
}

class _ElSidebarMenuButtonState extends State<ElSidebarMenuButton> {
  final GlobalKey _anchor = GlobalKey();
  _ElSidebarMenuState? _menu;
  ...
  @override
  void didUpdateWidget(ElSidebarMenuButton old) {
  ...
  void _sync(_ElSidebarMenuState? menu) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool ghost = widget.variant == ElButtonVariant.ghost;
    final ElButtonSurface? surface = !ghost
        ? null
        : icon
        ? const ElButtonSurface(fill: elTransparent, hoverFill: elTransparent)
        : widget.isActive
        ? ElButtonSurface(
            fill: elTransparent,
            hoverFill: elTransparent,
    ...
    if (icon) {
      ...
      button = SizedBox(
        height: ElSidebarMenuButton.iconSize,
        child: ConstrainedBox(
          constraints: BoxConstraints(maxWidth: ElSidebarMenuButton.iconSize),
          child: button,
        ),
      );
    }
...
class ElSidebarMenuRow extends StatelessWidget {
  const ElSidebarMenuRow({
    super.key,
    required this.label,
    this.leading,
    this.trailing,
  ...
  /// The glyph, already sized by the caller from [ElButton.iconPxFor].
  ...
  Widget build(BuildContext context) {
    final bool icon = ElSidebarChrome.iconModeOf(context);
    final double gap = ElButton.gapFor(size.button);
...
/// per row is 7.5px down a page of them. [ElLineBox] gives the paragraph its
/// CSS height back.
///
/// The style is read from the ambient [DefaultTextStyle] rather than passed:
/// what a row's label is typed as is the *rung's* business, and [ElButton] has
/// already resolved it by the time this builds.
class ElSidebarMenuLabel extends StatelessWidget {
  const ElSidebarMenuLabel(this.text, {super.key});
...
class ElSidebarMenuAction extends StatelessWidget {
  const ElSidebarMenuAction({
    super.key,
    required this.child,
    required this.label,
    this.onPressed,
...
/// [ElComponentType.sidebarMenuBadge] (drift 1).
class ElSidebarMenuBadge extends StatelessWidget {
  const ElSidebarMenuBadge(
    this.count, {
    super.key,
    this.variant = ElBadgeVariant.secondary,
  });
...
class ElSidebarMenuSkeleton extends StatelessWidget {
  const ElSidebarMenuSkeleton({
    super.key,
    this.showIcon = false,
    this.seed = '',
  });
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: Padding(
      ...
      child: Row(
        children: <Widget>[
          if (showIcon) ...<Widget>[
            ElSkeleton(width: el(4), height: el(4), radius: ElRadii.md),
            SizedBox(width: el(2)),
          ],
...
class ElSidebarMenuSub extends StatelessWidget {
  const ElSidebarMenuSub({super.key, required this.children});
  ...
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    if (ElSidebarChrome.iconModeOf(context)) return const SizedBox.shrink();
    ...
    return Padding(
      ...
      padding: EdgeInsets.only(left: el(4) + ElWidths.hairline, right: el(4)),
      child: DecoratedBox(
        decoration: BoxDecoration(
          border: Border(
            left: BorderSide(
        ...
        child: Padding(
          padding: EdgeInsets.only(
            left: el(3) - ElWidths.hairline,
            right: el(3),
            top: el(1),
            bottom: el(1),
          ),
          child: _GappedColumn(gap: el(1), children: children),
        ),
      ),
    );
  }
...
class ElSidebarMenuSubItem extends StatelessWidget {
  const ElSidebarMenuSubItem({super.key, required this.child});
...
class ElSidebarMenuSubButton extends StatelessWidget {
  const ElSidebarMenuSubButton({
    super.key,
    required this.label,
    this.isActive = false,
    this.variant = ElButtonVariant.ghost,
    this.size = ElSidebarMenuSubButtonSize.md,
    this.onPressed,
  });
  ...
  final bool isActive;
  final ElButtonVariant variant;
  final ElSidebarMenuSubButtonSize size;
  final VoidCallback? onPressed;
  ...
  Widget build(BuildContext context) {
    ...
    final bool ghost = variant == ElButtonVariant.ghost;
    return ElButton(
      variant: variant,
      size: size.button,
      radius: BorderRadius.circular(ElRadii.lg),
      ...
      surface: isActive && ghost
          ...
          : null,
      onPressed: onPressed ?? () {},
      child: ElSidebarMenuLabel(label),
    );
  }
}

/// `SidebarInput` — `Input` at `h-8 w-full bg-background shadow-none`.
class ElSidebarInput extends StatelessWidget {
