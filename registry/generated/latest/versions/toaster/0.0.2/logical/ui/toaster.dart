...
class Toast extends StatelessWidget {
  ...
  /// [TextStyles.nav], which is the same 13/500 on `text-sm`'s
  ...
  static final TextStyleToken titleSpec = TextStyles.small.derive(
    name: 'toast-title',
    wght: 500,
  );
  ...
  static final TextStyleToken descriptionSpec = TextStyles.small;
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      ...
      liveRegion: true,
      label: message.title,
      // Dismissal is offered to assistive technology as an action on the card
      // rather than as a focusable control. A toast arrives unbidden and leaves
      // on a clock; moving focus into one would take the caret out of whatever
      // the reader was doing, which is worse than the problem it solves. The
      // action button below *is* focusable, because that one is a choice the
      // reader came for.
      onDismiss: onDismiss,
...
class _ActionPillState extends State<_ActionPill> {
  ...
  Widget build(BuildContext context) {
    final ThemeTokens theme = ThemeScope.of(context);
    // The one control on a toast a reader actually came for, so it is a
    // control: Tab-reachable, activated by Enter and Space, ringed when focused,
    // and answering a finger over 44px however short the pill is. `Press`
    // carries all of that. Hover stays a local flag because the pill tints on
    // it, and `Press` reports it rather than a second `MouseRegion` doing so.
    return Press(
      onTap: () {
      ...
      onHoverChange: (bool value) => setState(() => _hovered = value),
      focusRadius: Radii.full,
      child: AnimatedContainer(
        duration: effectiveMotionDuration(context, MotionDurations.normal),
        curve: MotionCurves.enter,
        // A minimum, not a height: the label is real text and grows with the
        // reader's text scale.
        constraints: BoxConstraints(minHeight: _actionHeight),
