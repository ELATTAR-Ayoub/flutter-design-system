...
class ElChartMargin {
  ...
  /// `ElChartMargin(left: 12, right: 12)` gets zeros on the other two axes,
  /// which is exactly what React's shallow `defaultProps` merge does and what
  /// the measured plot boxes above show.
  static const ElChartMargin standard = ElChartMargin(
    top: 5,
    right: 5,
    bottom: 5,
    left: 5,
...
/// point.
enum ElChartLayout { horizontal, vertical }

/// `XAxis` / `YAxis` — `type={"number" | "category"}`.
enum ElChartAxisType { category, number }
...
class ElChartAxis {
  const ElChartAxis({
    this.dataKey,
    this.type = ElChartAxisType.category,
    this.tickLine = true,
    this.axisLine = true,
    this.tickMargin = 0,
    this.minTickGap = 5,
...
@immutable
class ElChartGrid {
  const ElChartGrid({this.horizontal = true, this.vertical = true});
...
enum ElChartSeriesKind { area, bar, line }
...
@immutable
class ElChartLabelList {
  const ElChartLabelList({
    this.dataKey,
    this.position = ElChartLabelPosition.top,
    this.offset = 5,
    this.color,
    this.formatter,
  });
  ...
  final String? dataKey;
  final ElChartLabelPosition position;
  final double offset;
...
enum ElChartLabelPosition { top, right, insideLeft, insideEnd }

/// One drawn series.
@immutable
class ElChartSeriesSpec {
  const ElChartSeriesSpec({
    required this.kind,
    ...
    this.fillOpacity = 1,
    this.strokeWidth = 1,
    this.curve = ElCurveType.linear,
    this.stackId,
    this.radii,
    this.cellFills,
    this.dot,
    this.labels = const <ElChartLabelList>[],
    this.gradient,
  });

  final ElChartSeriesKind kind;
  final String dataKey;
  final Color? stroke;
  final Color? fill;
  ...
  final ElChartDot? dot;
  final List<ElChartLabelList> labels;
...
@immutable
class ElChartTooltipSpec {
  const ElChartTooltipSpec({
    this.indicator = ElChartIndicator.dot,
  ...
  final ElChartIndicator indicator;
  ...
  final String Function(String label, List<ElChartTooltipItem> items)?
  labelFormatter;
  final Widget Function(BuildContext, ElChartTooltipItem, int)? formatter;
  final double? width;
}

/// `ChartLegend content={<ChartLegendContent />}`.
@immutable
class ElChartLegendSpec {
  const ElChartLegendSpec({
    this.align = ElChartLegendAlign.bottom,
    this.nameKey,
    this.wrap = false,
    this.gap,
    this.offset = 0,
  ...
  final ElChartLegendAlign align;
  final String? nameKey;
  final bool wrap;
  final double? gap;
  ...
  static double get reservedHeight => el(3) + 16;
}
...
class ElCartesianChart extends StatefulWidget {
  const ElCartesianChart({
    super.key,
    required this.data,
    required this.series,
    this.margin = ElChartMargin.standard,
    this.layout = ElChartLayout.horizontal,
    this.grid,
    this.xAxis,
    this.yAxis,
    this.legend,
  ...
  final List<ElChartSeriesSpec> series;
  final ElChartMargin margin;
  final ElChartLayout layout;
  final ElChartGrid? grid;
  final ElChartAxis? xAxis;
  final ElChartAxis? yAxis;
  final ElChartLegendSpec? legend;
  final ElChartTooltipSpec? tooltip;
  ...
  @override
  State<ElCartesianChart> createState() => _ElCartesianChartState();
}

class _ElCartesianChartState extends State<ElCartesianChart>
    with SingleTickerProviderStateMixin {
  late final AnimationController _entrance = AnimationController(
    vsync: this,
    duration: ElChartMotion.duration,
  );
  ...
  void didChangeDependencies() {
    ...
    final Duration d = elAnimationDuration(context, ElChartMotion.duration);
  ...
  Widget build(BuildContext context) {
    ...
    final ElChartConfig config = ElChartScope.of(context);
                ...
                Positioned.fill(
                  child: CustomPaint(
                    painter: _CartesianPainter(
                      layout: layout,
                      theme: theme,
                      t: ElChartMotion.curve.transform(_entrance.value),
                      activeIndex: active,
  ...
  Widget _legend(
    ...
    final ElChartLegendSpec spec = widget.legend!;
    return Positioned(
      left: layout.frame.left,
      width: layout.frame.width,
      bottom: spec.align == ElChartLegendAlign.bottom ? spec.offset : null,
      top: spec.align == ElChartLegendAlign.top ? spec.offset : null,
      child: ElChartLegendContent(
        ...
        items: <ElChartLegendItem>[
          for (final ElChartSeriesSpec s in widget.series)
            ElChartLegendItem(
  ...
  Widget _tooltip(
    ...
    final ElChartTooltipSpec spec = widget.tooltip!;
    final Map<String, Object?> row = widget.data[index];
    final List<ElChartTooltipItem> items = <ElChartTooltipItem>[
      for (final ElChartSeriesSpec s in widget.series)
        ElChartTooltipItem(
          name: s.dataKey,
          dataKey: s.dataKey,
    ...
    return Positioned(
      left: math.min(
        math.max(x + el(3), 0),
        math.max(layout.size.width - ElChartTooltipContent.minWidth, 0),
      ),
      top: layout.frame.top + layout.frame.height / 2,
      child: ElChartTooltipContent(
        config: config,
        label: label,
        items: items,
...
class _CartesianLayout {
  ...
  static _CartesianLayout resolve({
    ...
    required ElCartesianChart widget,
    required Size size,
  }) {
    final bool horizontal = widget.layout == ElChartLayout.horizontal;
    final ElChartAxis? xAxis = widget.xAxis;
    final ElChartAxis? yAxis = widget.yAxis;

    final double legendReserve =
        widget.legend != null &&
            widget.legend!.align == ElChartLegendAlign.bottom
        ? ElChartLegendSpec.reservedHeight
        : 0;
    final double legendTopReserve =
        widget.legend != null && widget.legend!.align == ElChartLegendAlign.top
        ? ElChartLegendSpec.reservedHeight
        : 0;
    ...
    final double left =
        ...
        (yAxis != null && !yAxis.hide
            ? (yAxis.size ?? ElChartAxis.defaultYSize)
            : 0);
    ...
    final double bottom =
        ...
        (xAxis != null && !xAxis.hide
            ? (xAxis.size ?? ElChartAxis.defaultXSize)
            : 0) +
        legendReserve;
    ...
    final ElChartAxis? categoryAxis = horizontal ? xAxis : yAxis;
    final ElChartAxis? valueAxis = horizontal ? yAxis : xAxis;
    final String categoryKey =
    ...
    final bool hasBars = widget.series.any(
      (ElChartSeriesSpec s) => s.kind == ElChartSeriesKind.bar,
    );

    // ── the category scale ────────────────────────────────────────────────
    final double catStart = horizontal ? frame.left : frame.top;
    final double catExtent = horizontal ? frame.width : frame.height;
    final ElBandScale band = ElBandScale(
      count: n,
      start: catStart,
      extent: catExtent,
    );
    final ElPointScale point = ElPointScale(
      count: n,
      start: catStart,
      extent: catExtent,
    );
    ...
    final ElChartGrid? grid = widget.grid;
    if (grid != null) {
    ...
    if (xAxis != null && !xAxis.hide) {
      final double baseline =
          frame.bottom + ElChartAxis.tickSize + xAxis.tickMargin + _capOffset;
      if (horizontal) {
        xTicks.addAll(
          _preserveEndTicks(
            context: context,
    ...
    if (yAxis != null && !yAxis.hide) {
      final double x = frame.left - ElChartAxis.tickSize - yAxis.tickMargin;
      if (horizontal) {
        for (final double v in value.ticks()) {
          yTicks.add(
            _AxisTick(
    ...
    final List<ElBarRect> bars = <ElBarRect>[];
    final List<_DotShape> dots = <_DotShape>[];
    final List<_SeriesLabel> labels = <_SeriesLabel>[];

    final List<ElChartSeriesSpec> barSeries = widget.series
        .where((ElChartSeriesSpec s) => s.kind == ElChartSeriesKind.bar)
        .toList();
    final Map<String?, int> barStackColumn = <String?, int>{};
    int barColumns = 0;
    for (final ElChartSeriesSpec s in barSeries) {
    ...
    final List<ElBarSlot> slots = elBarSlots(
    ...
    for (final ElChartSeriesSpec s in widget.series) {
      ...
      switch (s.kind) {
        case ElChartSeriesKind.area:
          ...
          areas.add(
            _AreaShape(
              fill: elAreaPath(topPts, basePts, s.curve),
              stroke: elCurvePath(topPts, s.curve),
              spec: s,
            ),
          );
        case ElChartSeriesKind.line:
          final List<Offset> pts = <Offset>[
            for (int i = 0; i < n; i++)
              Offset(categoryCoords[i], value.scale(hi[i])),
          ];
          lines.add(_LineShape(path: elCurvePath(pts, s.curve), spec: s));
          if (s.dot != null) {
            for (int i = 0; i < n; i++) {
          ...
          _collectLabels(
            ...
            pts,
            hi,
            ElChartLayout.horizontal,
          );
        case ElChartSeriesKind.bar:
          final int column = barStackColumn[s.stackId ?? s.dataKey] ?? 0;
          final ElBarSlot slot = slots[math.min(column, slots.length - 1)];
          for (int i = 0; i < n; i++) {
  ...
  static String _format(String Function(Object?)? f, Object? v) =>
      f != null ? f(v) : (v is num ? elChartNumber(v) : '${v ?? ''}');
  ...
  static void _collectLabels(
    ...
    List<Offset> anchors,
    List<double> values,
    ElChartLayout layout, {
    List<double>? barStart,
  }) {
    for (final ElChartLabelList spec in s.labels) {
      for (int i = 0; i < anchors.length; i++) {
        ...
        final String text = spec.formatter != null
            ? spec.formatter!(raw)
            : (raw is num ? elChartNumber(raw) : '${raw ?? ''}');
        switch (spec.position) {
          case ElChartLabelPosition.top:
            out.add(
              _SeriesLabel(
                text: text,
                x: anchors[i].dx,
          ...
          case ElChartLabelPosition.right:
            out.add(
              _SeriesLabel(
                text: text,
          ...
          case ElChartLabelPosition.insideLeft:
            out.add(
              _SeriesLabel(
                text: text,
          ...
          case ElChartLabelPosition.insideEnd:
            out.add(
              _SeriesLabel(
                text: text,
                x: anchors[i].dx - spec.offset,
  ...
  static double _measure(BuildContext context, String text) =>
      _widths.putIfAbsent(text, () {
        final TextPainter painter = TextPainter(
          text: TextSpan(
            text: text,
            style: ElText.styleOf(context, ElChartText.xs),
          ),
...
class _Stacks {
  ...
  static _Stacks build(ElCartesianChart widget) {
    final int n = widget.data.length;
...
class _CartesianPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final Paint grid = Paint()
      ..color = theme.border.withValues(alpha: ElChartGrid.strokeAlpha)
    ...
    for (final ElBarRect bar in layout.bars) {
      final Rect rect = _grow(bar, t);
      if (rect.isEmpty) continue;
      canvas.drawRRect(elBarRRect(rect, bar.radii), Paint()..color = bar.color);
    }
  }

  Rect _grow(ElBarRect bar, double t) {
...
class _TickLabel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final Widget text = ElText(tick.text, ElChartText.xs, color: color);
...
/// The painter behind `ElBarRect`, exposed so the package test can pin a bar's
/// rendered pixels without standing a whole chart up.
class ElBarSeriesPainter extends CustomPainter {
  const ElBarSeriesPainter({required this.bars});

  final List<ElBarRect> bars;

  @override
  void paint(Canvas canvas, Size size) {
    for (final ElBarRect bar in bars) {
      canvas.drawRRect(
        elBarRRect(bar.rect, bar.radii),
  ...
  @override
  bool shouldRepaint(ElBarSeriesPainter old) => old.bars != bars;
}
