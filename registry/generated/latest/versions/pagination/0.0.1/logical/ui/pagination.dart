...
/// Next. [ElButton.typeFor] already answers null for the four square rungs and
...
class ElPagination extends StatelessWidget {
  const ElPagination({super.key, required this.children});
...
class ElPaginationLink extends StatelessWidget {
  const ElPaginationLink({
    super.key,
    required this.label,
    this.isActive = false,
    this.onTap,
  ...
  Widget build(BuildContext context) {
    return Semantics(
      ...
      selected: isActive,
      child: ElButton(
        variant: isActive ? ElButtonVariant.outline : ElButtonVariant.ghost,
...
/// same body mirrored.
class ElPaginationStep extends StatelessWidget {
  ...
  const ElPaginationStep.next({super.key, this.text = 'Next', this.onTap})
    : _next = true;
  ...
  static double get tightPadding => el(1.5);

  /// The untouched edge keeps the `default` rung's own `px-4`.
  static double get loosePadding => ElButton.paddingXFor(ElButtonSize.md);

  @override
  Widget build(BuildContext context) {
    final Widget chevron = ElIcon(
      _next ? ElIconGlyph.chevronRight : ElIconGlyph.chevronLeft,
...
class ElPaginationEllipsis extends StatelessWidget {
  const ElPaginationEllipsis({super.key});

  /// `size-8`.
  static double get boxSize => el(8);
