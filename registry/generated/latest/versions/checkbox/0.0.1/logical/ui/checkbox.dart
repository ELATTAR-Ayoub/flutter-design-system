...
///    `useReplayOnStateChange` — [ElJellyReplay].
///
...
/// | `after:absolute after:-inset-x-3 after:-inset-y-2` | a **42 × 34** target under a 20px box — the insets grow from the 18px padding box, see [ElHitArea] |
...
enum ElCheckboxState {
  ...
  bool get isOn => this != ElCheckboxState.unchecked;
}
...
class ElCheckbox extends StatefulWidget {
  const ElCheckbox({
    super.key,
    this.state = ElCheckboxState.unchecked,
    this.onChanged,
    this.enabled = true,
    this.inert = false,
    this.invalid = false,
  ...
  final ElCheckboxState state;

  /// `onCheckedChange`, handed the state a click produces: an unchecked or
  /// indeterminate box becomes [ElCheckboxState.checked], a checked one
  /// becomes [ElCheckboxState.unchecked]. `null` disables the control.
  final ValueChanged<ElCheckboxState>? onChanged;

  /// A `Field` may disable a control that still carries a handler —
  /// `group-has-disabled/field:opacity-50`. ANDed with the enclosing
  /// [ElFieldScope]'s: a disabled field disables its control and the control
  /// cannot opt back in.
  final bool enabled;
  ...
  /// [ElSelectionControl.inert] for the full state table and for what the flag
  /// actually changes.
  ///
  ...
  ///
  /// See [ElSelectionControl.forceFocusRing]: the cell is a static fake, two of
  /// them sit on one page, and `aria-invalid` still beats it.
  final bool? forceFocusRing;

  /// A [ElFieldScope]'s node wins over the one this widget would otherwise
  /// leave to `Focus`, and loses to this — so `ElForm.focusFirstError` lands on
  /// the checkbox itself (ruling F4).
  final FocusNode? focusNode;

  /// The accessible name. A visible `ElFieldLabel` feeds this through the
  ...
  static ElCheckboxState nextAfter(ElCheckboxState state) =>
...
class _ElCheckboxState extends State<ElCheckbox> {
  ...
  @override
  void didUpdateWidget(ElCheckbox old) {
  ...
  Widget build(BuildContext context) {
    ...
    // four lines `ElInput` and `ElTextarea` carry.
    ...
    // Registering the toggle is what makes a tap on the visible `ElFieldLabel`
    // tick the box, the way clicking "I accept the terms" does on the web.
    // Re-registered on every build because the closure reads [ElCheckbox.state],
    ...
    final VoidCallback? toggle =
        ...
        ? () => widget.onChanged!(ElCheckbox.nextAfter(widget.state))
        : null;
    scope?.activator?.callback = toggle;

    final bool on = widget.state.isOn;

    final bool bar = widget.state == ElCheckboxState.indeterminate;
        ...
        ? KeyedSubtree(
            ...
            child: SizedBox(
              ...
              child: _Mark(
                ...
                dashArray: bar ? ElDashDraw.dashArray : ElCheckDraw.dashArray,
                duration: bar ? ElDashDraw.duration : ElCheckDraw.duration,
                drawnAt: bar
                    ? ElDashDraw.drawnFractionAt
                    : ElCheckDraw.drawnFractionAt,
    ...
    return ElSelectionControl(
      ...
      semantics: (Widget child) => Semantics(
        container: true,
        checked: widget.state == ElCheckboxState.checked,
        mixed: widget.state == ElCheckboxState.indeterminate,
...
class _MarkPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.save();
    // The same `viewBox` fit `ElIcon` performs: 24 units into the rendered box,
  ...
  static double get _viewBox => ElIconPaths.viewBox;
