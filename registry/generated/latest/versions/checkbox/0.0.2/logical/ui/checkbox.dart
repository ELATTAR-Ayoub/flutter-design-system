...
///    `useReplayOnStateChange` — [StateChangeFeedback].
///
...
/// | `after:absolute after:-inset-x-3 after:-inset-y-2` | a **42 × 34** target under a 20px box — the insets grow from the 18px padding box, see [HitArea] |
...
enum CheckboxState {
  ...
  bool get isOn => this != CheckboxState.unchecked;
}
...
double get _markSize => space(3.5);
...
class Checkbox extends StatefulWidget {
  ...
  /// indeterminate box becomes [CheckboxState.checked], a checked one
  /// becomes [CheckboxState.unchecked]. `null` disables the control.
  ...
  /// [SelectionControl.inert] for the full state table and for what the flag
  /// actually changes.
  ///
  ...
  ///
  /// See [SelectionControl.forceFocusRing]: the cell is a static fake, two of
  /// them sit on one page, and `aria-invalid` still beats it.
  final bool? forceFocusRing;

  /// A [FieldScope]'s node wins over the one this widget would otherwise
  /// leave to `Focus`, and loses to this — so `Form.focusFirstError` lands on
  /// the checkbox itself (ruling F4).
  final FocusNode? focusNode;

  /// The accessible name. A visible `FieldLabel` feeds this through the
  ...
  static CheckboxState nextAfter(CheckboxState state) =>
...
class _CheckboxState extends State<Checkbox> {
  ...
  @override
  void didUpdateWidget(Checkbox old) {
  ...
  Widget build(BuildContext context) {
    ...
    // four lines `Input` and `Textarea` carry.
    ...
    // Registering the toggle is what makes a tap on the visible `FieldLabel`
    // tick the box, the way clicking "I accept the terms" does on the web.
    // Re-registered on every build because the closure reads [Checkbox.state],
    ...
    final VoidCallback? toggle =
        ...
        ? () => widget.onChanged!(Checkbox.nextAfter(widget.state))
        : null;
    scope?.activator?.callback = toggle;

    final bool on = widget.state.isOn;

    final bool bar = widget.state == CheckboxState.indeterminate;
    ...
    return SelectionControl(
      ...
      semantics: (Widget child) => Semantics(
        ...
        mixed: widget.state == CheckboxState.indeterminate,
...
class _MarkPainter extends CustomPainter {
  ...
  static double get _viewBox => IconPaths.viewBox;
