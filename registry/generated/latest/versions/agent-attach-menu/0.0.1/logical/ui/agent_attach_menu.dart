...
///
/// This file composes `menu.dart`'s [ElMenuSurface] and [ElMenuPointerDown]
/// over [ElPopover] rather than [ElDropdownMenu], and the reason is the row:
...
/// not a shape [ElMenuItem] has a slot for. The chrome, the placement, the
...
class ElAgentAttachMenu extends StatelessWidget {
  const ElAgentAttachMenu({
    super.key,
    this.onPickFiles,
    this.commands,
  ...
  final ValueChanged<ElAgentCommand> onRunCommand;

  final bool disabled;

  /// `size-8` on the trigger — `size="icon"` asks for 40 and twMerge keeps 32.
  static ElButtonSize get triggerSize => ElButtonSize.iconSm;

  /// `w-80` on the content.
  static double get width => el(80);

  /// `max-h-96`.
  static double get maxHeight => el(96);
  ...
  static double get rowRadius => ElRadii.md;
  ...
  List<ElAgentCommand> get _skills => (commands ?? const <ElAgentCommand>[])
      .where((ElAgentCommand c) => c.group == ElAgentCommandGroup.skill)
      .toList();

  @override
  Widget build(BuildContext context) {
    final List<ElAgentCommand> skills = _skills;
...
class _AttachMenu extends StatefulWidget {
  ...
  final VoidCallback? onPickFiles;
  final List<ElAgentCommand> skills;
  final ValueChanged<ElAgentCommand> onRunCommand;
  final bool disabled;
  ...
  Widget build(BuildContext context) {
    ...
    return ElPopover(
      ...
      anchor: ElMenuPointerDown(
        ...
        child: ElMenuTriggerScope(
          open: _isOpen,
          child: ElButton(
            variant: ElButtonVariant.ghost,
            size: ElAgentAttachMenu.triggerSize,
            ...
            child: ElIcon.lucide(
              ElLucide.plus,
              sizePx: ElAgentAttachMenu.glyphSize,
            ),
          ),
        ),
      ),
      content: (BuildContext context, ElPopoverAnchorMetrics metrics) {
        final double pad = ElMenu.contentPadding;
        return SizedBox(
          width: ElAgentAttachMenu.width,
          child: ElMenuSurface(
            child: ConstrainedBox(
              constraints: BoxConstraints(
                maxHeight: ElAgentAttachMenu.maxHeight,
              ),
              // `overflow-y-auto`.
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    if (hasFiles)
                      Padding(
                        ...
                        child: _MenuRow(
                          ...
                          stroke: ElIcon.strokeFor(ElAgentAttachMenu.glyphSize),
                          title: 'Photos & files',
                    ...
                    if (hasSkills) ...<Widget>[
                      ...
                      for (final ElAgentCommand command in widget.skills)
                        Padding(
                          padding: EdgeInsets.symmetric(horizontal: pad),
                          child: _MenuRow(
                            // `command.icon ?? SparklesIcon`.
                            glyph: command.icon ?? ElLucide.sparkles,
                            stroke: ElAgentSlashPalette.lucideStroke,
                            title: command.label,
                            hint: command.hint,
                            truncateTitle: true,
                            onSelect: () =>
        ...
        child: Semantics(
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              ...
              color: _highlighted ? theme.accent : elTransparent,
              borderRadius: BorderRadius.circular(ElAgentAttachMenu.rowRadius),
            ),
            child: Padding(
              padding: ElAgentAttachMenu.rowInsets,
              child: Row(
                ...
                children: <Widget>[
                  ...
                  SizedBox(width: ElAgentAttachMenu.rowGap),
                  Flexible(
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        // [ElAgentAttachMenu.rowLinesHaveNoGap].
                        if (widget.hint != null)
                          ElText(
                            widget.hint!,
                            ElType.caption,
