...
/// them — [ElCarousel] asks for its own horizontal padding and grows its stack
...
/// browser's behaviour exactly. Compare `ElHitArea`, which exists for the
...
class ElCarouselController extends ChangeNotifier {
  ElCarouselController({this.vsync});
...
class ElCarousel extends StatefulWidget {
  const ElCarousel({
    super.key,
    required this.basis,
    required this.items,
  ...
  @override
  State<ElCarousel> createState() => _ElCarouselState();
}

class _ElCarouselState extends State<ElCarousel>
    with SingleTickerProviderStateMixin {
  late final ElCarouselController _controller = ElCarouselController(
    vsync: this,
  );
  final FocusNode _focus = FocusNode(debugLabel: 'ElCarousel');
  ...
  Widget build(BuildContext context) {
    ...
    // The engine has no `Duration` to hand [elAnimationDuration]; the reduced
...
class _Track extends StatelessWidget {
  ...
  final ElCarouselController controller;
  final double basis;
  final List<Widget> items;
...
/// plus `rounded-full`, which a [ElButton] is already.
      ...
      builder: (BuildContext context, _) {
        ...
        return ElButton(
          ...
          onPressed: enabled
              ...
              : null,
          child: ElIcon.lucide(
            forward ? ElLucide.chevronRight : ElLucide.chevronLeft,
