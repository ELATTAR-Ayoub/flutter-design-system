...
///
/// The arrival squash itself is [ElJelly] — `keyframes.dart` holds the one
...
class ElSlidingPillGroup extends StatefulWidget {
  ...
  @override
  State<ElSlidingPillGroup> createState() => _ElSlidingPillGroupState();
}

class _ElSlidingPillGroupState extends State<ElSlidingPillGroup>
  ...
  /// first frame only — [build] re-reads it through [elAnimationDuration] on
  /// every pass, the way `ElPress` does, so reduced motion stills the squash
  ...
  @override
  void didUpdateWidget(ElSlidingPillGroup old) {
      ...
      children: <Widget>[
        AnimatedPositioned(
          ...
          duration: travels
              ? elAnimationDuration(
                  context,
                  widget.travelDuration ?? ElDurations.base,
                )
              : Duration.zero,
