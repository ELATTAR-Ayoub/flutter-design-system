...
/// ```
///
/// So this file states no motion of its own: [ElShimmer] already carries the
...
/// see [ElShimmer]'s own note, which also explains why the reduced-motion
...
/// only gate is `elAnimationDuration`, and there is deliberately no
/// out-of-view pause.
library;
...
class ElSkeleton extends StatelessWidget {
  const ElSkeleton({super.key, this.width, this.height, this.radius});
  ...
  static double get defaultRadius => ElRadii.md;
  ...
  static InlineSpan span({double? width, double? height, double? radius}) =>
      WidgetSpan(
        ...
        child: ElSkeleton(width: width, height: height, radius: radius),
      );
