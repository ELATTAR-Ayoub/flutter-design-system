...
/// ```
///
/// so they are computed here through [ElOklab] from the two hues a variant
...
/// about the layer's own `transform-origin`. See [ElBloomDrift.matrixFor].
///
...
/// `elAnimationDuration` — there is deliberately no out-of-view pause, because
...
// `ElThemeData.bloomVoid` / `.bloomL` / `.bloomC` / `.bloomLift` / `.bloomHotC`
// and this file reads them like any other token. The FOLLOW-UP also asked for
// `ElBloomInk.of` to be deleted; there is no such member and has not been since
// the resolver became the instance method [ElBloomCosmic._ink], which is part
...
/// and it moves with [ElBloomCosmic.blendFor] rather than with the theme's
/// token list.
...
/// conversion (the same rule `ElBlurs` records).
final double _deepBlur = el(4);
...
@immutable
class ElBloomDriftStop {
  const ElBloomDriftStop(
    this.percent, {
...
/// [ElKeyframes.doubles]'s own rule: the clock stays linear so nothing is eased
/// twice.
@immutable
class ElBloomDrift {
  ...
  /// `static final` rather than `const` — the same shape `ElShadows.btnPrimary`
  ...
  /// 2.2 on hover.
  static final ElBloomDrift deep = ElBloomDrift(
    duration: ElDurations.cosmicDriftDeep,
    originX: 0.88,
    originY: 0.82,
    hoverScale: 2.2,
    stops: <ElBloomDriftStop>[
      ElBloomDriftStop(0, translate: Offset.zero, degrees: 0, scale: 1),
      ElBloomDriftStop(
        50,
        translate: Offset(-0.11, 0.08),
        degrees: 7,
        scale: 1.2,
      ),
      ElBloomDriftStop(
        100,
        translate: Offset(0.06, -0.09),
        degrees: -5,
        scale: 1.08,
      ),
    ],
  ...
  static final ElBloomDrift near = ElBloomDrift(
    duration: ElDurations.cosmicDriftNear,
    originX: 0.90,
    originY: 0.86,
    hoverScale: 2.5,
    stops: <ElBloomDriftStop>[
      ElBloomDriftStop(0, translate: Offset.zero, degrees: 0, scale: 1.04),
      ElBloomDriftStop(
        35,
        translate: Offset(-0.03, -0.02),
        degrees: -9,
        scale: 1.18,
      ),
      ElBloomDriftStop(70, translate: Offset(0.03, 0.02), degrees: 6, scale: 1),
      ElBloomDriftStop(
        100,
        translate: Offset(-0.02, 0.01),
        degrees: -4,
        scale: 1.14,
      ),
    ],
  ...
  final List<ElBloomDriftStop> stops;

  /// `&:hover::before { scale: 2.2 }` — the standalone `scale` property, over
  /// [ElDurations.bloom] on `--ease-out`.
  ...
  double _swellAt(double hoverT) =>
      1 + (hoverScale - 1) * ElCurves.out.transform(hoverT.clamp(0.0, 1.0));

  static double _readX(ElBloomDriftStop s) => s.translate.dx;
  static double _readY(ElBloomDriftStop s) => s.translate.dy;
  static double _readScale(ElBloomDriftStop s) => s.scale;
  static double _readRadians(ElBloomDriftStop s) => s.degrees * math.pi / 180;

  static Animatable<double> _track(
    List<ElBloomDriftStop> stops,
    double Function(ElBloomDriftStop) read,
  ) => ElKeyframes.doubles(<ElKeyframeStop<double>>[
    for (final ElBloomDriftStop stop in stops)
      ElKeyframeStop<double>(stop.percent, read(stop)),
...
// `ElShadowLayer.color`: a variant's hue may be a raw ramp entry or a theme
// token, and only the theme knows which.

Color _actionBright(ElThemeData t) => ElPalette.actionBright;
Color _action(ElThemeData t) => ElPalette.action;
Color _value(ElThemeData t) => ElPalette.value;
Color _valueBright(ElThemeData t) => ElPalette.valueBright;
Color _valueDark(ElThemeData t) => ElPalette.valueDark;
Color _success(ElThemeData t) => ElPalette.success;
Color _warning(ElThemeData t) => ElPalette.warning;
Color _info(ElThemeData t) => ElPalette.info;
Color _destructive(ElThemeData t) => t.destructive;
...
/// worked around.
class ElBloomCosmic extends StatefulWidget {
  ...
  const ElBloomCosmic.destructive({
    super.key,
    required this.radius,
    required this.fill,
    required this.child,
  ...
  const ElBloomCosmic.toastWarning({
    super.key,
    required this.radius,
    required this.fill,
    required this.child,
  ...
  final Color Function(ElThemeData) bloom1;

  /// `--bloom-2` — the core.
  final Color Function(ElThemeData) bloom2;
  ...
  static BlendMode blendFor(ElThemeKind kind) => switch (kind) {
  ...
  ///
  /// A thin reader over [ElThemeData.bloomVoid], kept because half the callers
  /// of this file hold a [ElThemeKind] and not a whole theme, and because the
  /// name says which custom property is being asked for.
  static Color voidFor(ElThemeKind kind) => switch (kind) {
    ElThemeKind.light => ElThemeData.light.bloomVoid,
    ElThemeKind.dark => ElThemeData.dark.bloomVoid,
  };
  ...
  @visibleForTesting
  ElBloomInk inkFor(ElThemeData theme) {
    Color relit(Color source) {
      final ({double l, double c, double h}) from = ElOklab.toOklch(source);
      return ElOklab.fromOklch(
        from.l * theme.bloomL,
        from.c * theme.bloomC,
        from.h,
      );
    ...
    final ({double l, double c, double h}) coreLch = ElOklab.toOklch(core);

    return ElBloomInk(
      core: core,
      glow: relit(bloom1(theme)),
      // `calc(l + (1 - l) * var(--bloom-lift))` on the core's own lightness.
      hot: ElOklab.fromOklch(
        coreLch.l + (1 - coreLch.l) * theme.bloomLift,
        coreLch.c * theme.bloomHotC,
        coreLch.h,
      ),
      voidInk: theme.bloomVoid,
      opacity: theme.kind == ElThemeKind.light ? _lightOpacity : _darkOpacity,
      blend: blendFor(theme.kind),
    );
  }

  @override
  State<ElBloomCosmic> createState() => _ElBloomCosmicState();
}

class _ElBloomCosmicState extends State<ElBloomCosmic>
    with TickerProviderStateMixin {
  /// The two drifts. The durations named here are placeholders for the first
  /// frame only — [build] re-reads both through [elAnimationDuration] on every
  /// pass, the way `ElSpinner` and `ElKeyframePlayer` do.
  late final AnimationController _deep = AnimationController(
    vsync: this,
    duration: ElBloomDrift.deep.duration,
  );
  late final AnimationController _near = AnimationController(
    vsync: this,
    duration: ElBloomDrift.near.duration,
  );
  ...
  void didChangeDependencies() {
    ...
    final bool stilled =
        elAnimationDuration(context, ElBloomDrift.deep.duration) ==
        Duration.zero;
  ...
  /// Supervisor ruling F2: both loopers build in full, and `elAnimationDuration`
  ...
  Widget build(BuildContext context) {
    ...
    _deep.duration = elAnimationDuration(context, ElBloomDrift.deep.duration);
    _near.duration = elAnimationDuration(context, ElBloomDrift.near.duration);
    _hover.duration = elAnimationDuration(context, ElDurations.bloom);
    ...
    if (widget.starfield) {
      content = Stack(
        ...
        children: <Widget>[
          ...
          // `z-index: 1`.
          Positioned.fill(
            child: ElStarfield(bloom2: widget.bloom2(theme), hovered: _hovered),
          ),
          widget.child,
        ],
      );
...
class _BloomPainter extends CustomPainter {
  ...
  Matrix4 _matrix(ElBloomDrift drift, Rect layer, double t) => stilled
  ...
  void _paintDeep(Canvas canvas, Rect box) {
    ...
    canvas.save();
    canvas.transform(_matrix(ElBloomDrift.deep, layer, deepT).storage);
    ...
    _drawRadial(
      ...
      centreX: 0.94,
      centreY: 0.40,
      colors: <Color>[ElOklab.mix(ink.glow, ink.core, 0.60), ink.voidInk],
    ...
    _drawRadial(
      ...
      colors: <Color>[
        ink.core,
        ElOklab.mix(ink.core, ink.voidInk, 0.45),
        ink.voidInk,
      ],
  ...
  void _paintNear(Canvas canvas, Rect box) {
    ...
    canvas.save();
    canvas.transform(_matrix(ElBloomDrift.near, layer, nearT).storage);
    ...
    _drawRadial(
      ...
      centreY: 0.46,
      colors: <Color>[
        ElOklab.mix(ink.glow, ink.core, 0.50),
        ElOklab.mix(ink.core, ink.voidInk, 0.50),
        ink.voidInk,
      ],
    ...
    _drawRadial(
      ...
      centreY: 0.12,
      colors: <Color>[
        ElOklab.mix(ink.core, ink.hot, 0.60),
        ink.core,
        ink.voidInk,
      ],
    ...
    _drawRadial(
      ...
      centreY: 0.84,
      colors: <Color>[
        ElOklab.mix(ink.glow, ink.hot, 0.35),
        ink.glow,
        ElOklab.mix(ink.glow, ink.voidInk, 0.45),
        ink.voidInk,
      ],
  ...
  Paint _groupPaint(double sigma) => Paint()
    ..blendMode = ink.blend
    ..color = elTransparent.withValues(alpha: ink.opacity)
  ...
  static Rect _layerRect(
    ...
    final double width = (box.width * widthFraction)
        .clamp(0.0, widthCap)
        .toDouble();
