...
/// [ElPopover]. What is left here is the trigger contract and the four numbers
...
/// | `max-h-(--radix-…-available-height)` | | capped at the room the chosen side leaves, which is what [ElPopover] already constrains its child to |
///
...
/// over. [ElButton] carried no flag for it while `button.dart` belonged to
/// another wave; it carries [ElButton.suppressPressScale] now, every trigger
/// passes it, and [ElDropdownMenu.pressScaleSuppressed] pins that it does.
///
...
/// pointer moved away while the menu stays open. [ElMenuTriggerScope] publishes
/// the open flag and [ElButton.expanded] resolves it, so the fill is now held
...
/// call site passes [ElButton.suppressPressScale] itself; `aria-expanded`
/// changes with a menu the call site does not own, so it comes from here.
class ElMenuTriggerScope extends InheritedWidget {
  const ElMenuTriggerScope({
    super.key,
    required this.open,
  ...
  static bool openOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<ElMenuTriggerScope>()?.open ??
      false;

  @override
  bool updateShouldNotify(ElMenuTriggerScope oldWidget) =>
      oldWidget.open != open;
}

/// A `DropdownMenu` — [trigger], and [children] under it.
///
/// Open state is a prop, the way [ElPopover]'s is and the way Radix's `open`
/// is: the caller owns the boolean. [ElDropdownMenu] is the *uncontrolled*
/// spelling — it owns the boolean itself, because every instance on the menus
/// page is uncontrolled and a caller that wants control can drive [ElPopover]
/// and [ElMenuContent] directly, which is what this widget is.
class ElDropdownMenu extends StatefulWidget {
  const ElDropdownMenu({
    super.key,
    required this.trigger,
    required this.children,
    this.width,
    this.align = ElPopoverAlign.start,
  ...
  /// placement itself is [ElPopover]'s, collision flip included; all this does
  ...
  /// the port's. True, as it is on [ElDatePicker] — the same attribute, one
  /// component over.
  ...
  /// is a call-site fact: [ElDropdownMenu] cannot suppress anything itself, so
  /// this is only true for as long as every trigger keeps passing
  /// [ElButton.suppressPressScale].
  static bool get pressScaleSuppressed => _pressScaleSuppressed;

  @override
  State<ElDropdownMenu> createState() => _ElDropdownMenuState();
}

class _ElDropdownMenuState extends State<ElDropdownMenu> {
  bool _open = false;
  ...
  Widget build(BuildContext context) {
    return ElPopover(
      ...
      side: widget.side,
      align: widget.align,
      sideOffset: ElDropdownMenu.sideOffset,
      ...
      anchor: ElMenuPointerDown(
        ...
        // site authored.
        child: ElMenuTriggerScope(open: _isOpen, child: widget.trigger),
      ),
