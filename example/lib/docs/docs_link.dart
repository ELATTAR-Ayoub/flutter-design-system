...
/// [TweenAnimationBuilder] cross-fade from `mutedForeground` to `actionInk`
...
/// [ElType.small], the size every other navigational row in the
...
class DocsLink extends StatefulWidget {
  ...
  /// The type class the label renders in. Defaults to [ElType.small], the
  /// size every other navigational row in the documentation shell uses.
  /// Nullable rather than defaulted in the constructor because [ElType.small]
...
class _DocsLinkState extends State<DocsLink> {
  ...
  Widget build(BuildContext context) {
    ...
    final ElTypeSpec spec = widget.spec ?? ElType.small;
    if (!_enabled) {
      return ElText(widget.label, spec, color: theme.mutedForeground);
    }
    ...
    return Semantics(
      link: true,
      // Only ever an override. Left null, the label comes from the [ElText]
        ...
        child: GestureDetector(
          ...
          child: TweenAnimationBuilder<Color?>(
            ...
            builder: (BuildContext context, Color? ink, Widget? _) {
              ...
              final Widget text = ElText(widget.label, spec, color: resolved);
