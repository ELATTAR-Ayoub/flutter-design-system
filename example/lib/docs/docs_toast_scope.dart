...
/// Threads the documentation app's [ElToastController] down to any control
/// that wants to confirm an action with a toast.
///
/// `lib/src/components/toaster.dart` ships [ElToaster] (the overlay host) and
/// [ElToastController] (the imperative queue a caller fires into), but
/// neither exposes an inherited lookup of its own — there is no
/// `ElToaster.of(context)`. Reaching for a top-level global instead would
...
/// Publishes the app's [ElToastController] to descendants.
///
...
class DocsToastScope extends InheritedWidget {
  ...
  /// The one controller [ElToaster] is mounted with at the app root.
  ...
  static ElToastController? maybeOf(BuildContext context) {
