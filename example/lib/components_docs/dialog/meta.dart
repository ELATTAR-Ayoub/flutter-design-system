...
/// copied verbatim: `button`, `icon`, `surface`,
/// `source-foundation`. [exports] is the manifest's own `exports` list,
/// also verbatim: the modal portal machinery (`OverlayPortal`,
/// `DialogOverlay`, `OpenTransition`) that a sheet, a drawer, and the
/// alert dialog all ride too, plus the dialog-specific composition
/// (`Dialog` down through `DialogMedia`). `page.dart` renders the real
...
/// `Section` panels rather than declaring a `ComponentDocSpec`. It has
...
const ComponentDocEntry dialogDoc = ComponentDocEntry(
  name: 'dialog',
  title: 'Dialog',
  description: 'An accessible modal portal with normal and media compositions.',
