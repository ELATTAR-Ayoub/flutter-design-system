/// Public documentation page for the `swap-in` motion primitive.
///
/// **Why `EffectSection`, not `ShowcaseSection`.** `ElSwapIn` has no variant
...
final ComponentDocSpec swapInDocSpec = ComponentDocSpec(
  ...
  sections: <DocsPageSection>[
    ...
    InstallSection(
      ...
      title: 'Installation',
      description:
          'swap-in has a real registry manifest: `elattar add swap-in` '
          'installs lib/src/motion/swap_in.dart and resolves both '
          'registryDependencies automatically. The Manual tab is for a '
          'project not using the CLI.',
      command: swapInDoc.command,
      manualFiles: <DocsCodeFile>[
        DocsCodeFile(
          path: 'lib/motion/swap_in.dart',
          title: '1. Copy the source',
          description:
              "Copy lib/src/motion/swap_in.dart's generated "
              '@motion/swap_in.dart payload into your motion folder.',
          code:
              "import 'package:elattar_design_system/elattar_design_system.dart';\n\n"
              '// Copy the generated swap-in source here when using manual '
              'mode.',
        ),
        DocsCodeFile(
          ...
          description:
              'Add the export line so ElSwapIn is reachable the same way '
              'the CLI path already makes it.',
          code: "export 'swap_in.dart';",
        ),
      ],
    ),
    SnippetSection(
    ...
    EffectSection(
      ...
      description:
          ...
          'figure is wrapped ElSwapIn(replayKey: state, …), so a loading '
    ...
    DisclosureSection(
      ...
      title: 'API Reference',
      description:
          'Every constructor parameter ElSwapIn declares, plus its two '
      ...
      child: DocsInstallFacts(
        ...
        facts: <DocsInstallFact>[
          ...
          const DocsInstallFact(
            ...
            description:
                'ElSwapIn is covered inside the shared motion suite: there '
          ...
          const DocsInstallFact(
            ...
            value: 'example/lib/components_docs/swap_in/page.dart',
...
class SwapInDocPage extends StatelessWidget {
  const SwapInDocPage({super.key, this.onNavigate});

  final ValueChanged<String>? onNavigate;

  @override
  Widget build(BuildContext context) => DocsLayout(
    route: swapInDoc.route,
    intro: DocsPageIntro(
      eyebrow: 'COMPONENTS / MOTION',
      title: swapInDoc.title,
      description: swapInDoc.description,
    ),
    breadcrumbs: const <ElBreadcrumbEntry>[
      ElBreadcrumbEntry.link('Components'),
      ElBreadcrumbEntry.page('Swap In'),
    ],
    ...
    child: KeyedSubtree(
      key: const ValueKey<String>('swap-in-doc-article'),
    ...
    children: <Widget>[
      Column(
        ...
        children: <Widget>[
          ...
          const KeyedSubtree(
            key: ValueKey<String>('swap-in-preview:springs'),
      ...
      Column(
        ...
        children: <Widget>[
          ...
          SizedBox(height: el(3)),
          const KeyedSubtree(
            key: ValueKey<String>('swap-in-preview:instant'),
...
const String _previewCode =
    ...
    'ElSwapIn(\n'
    '  replayKey: value,\n'
    '  child: ElText(value.toString(), ElType.numLg),\n'
    ')\n\n'
    '// Replaces instantly — no ElSwapIn, the plain comparison\n'
    'ElText(value.toString(), ElType.numLg)';
...
class _SpringPreviewState extends State<_SpringPreview> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: ElSwapIn(
        replayKey: _value,
        child: ElText('$_value', ElType.numLg, color: theme.foreground),
      ),
    );
  }
}
...
class _StatFigureSpecimenState extends State<_StatFigureSpecimen> {
  ...
  Widget build(BuildContext context) {
    final ElThemeData theme = ElTheme.of(context);
    return KeyedSubtree(
      key: const ValueKey<String>('swap-in-example:stat-figure'),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        mainAxisSize: MainAxisSize.min,
        children: <Widget>[
          ElText('Active users', ElType.small, color: theme.mutedForeground),
...
const String _statFigureCode =
    'Widget figure;\n'
    'if (loading) {\n'
    '  figure = ElSkeleton(width: w, height: h);\n'
    '} else {\n'
    '  figure = ElSwapIn(\n'
    '    // <span key={state}> — a loading -> ready swap replays it.\n'
    '    replayKey: state,\n'
    '    child: ElText(value, ElType.numLg),\n'
    '  );\n'
    '}';
          ...
          children: <Widget>[
            Column(
              ...
              children: <Widget>[
                ...
                SizedBox(height: el(3)),
                KeyedSubtree(
                  key: const ValueKey<String>('swap-in-example:replay-keyed'),
                  child: ElSwapIn(
                    replayKey: _revision,
                    child: ElText(
                      'Row $_revision',
...
const String _replayKeyCode =
    ...
    'ElSwapIn(replayKey: value, child: Text(\'Row \$value\'))\n\n'
    '// Unkeyed: same node, same widget — content changes, spring does not\n'
    'ElSwapIn(child: Text(\'Row \$value\'))';
...
class _ApiReferenceContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: <Widget>[
      const DocsApiTable(title: 'ElSwapIn', facts: _swapInApiFacts),
      SizedBox(height: el(6)),
      const DocsApiTable(
        title: 'ElSwapIn static values',
...
class _AccessibilityContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      _bullets(ElTheme.of(context), <String>[
        'Decorative by itself: ElSwapIn sets no Semantics node and '
        ...
        'Content, not container: this widget never hides or excludes '
            'anything from the semantics tree — unlike ElIconSwap, there is '
...
class _KeyboardContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      _bullets(ElTheme.of(context), <String>[
        'ElSwapIn takes no focus and handles no key: it is an '
            'ElKeyframePlayer wrapping Opacity and Transform.scale, nothing '
            'that reads input.',
...
class _ResponsiveContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      _bullets(ElTheme.of(context), <String>[
        'No breakpoint branching anywhere in swap_in.dart: BuildContext '
            'width is never read.',
...
class _DependenciesContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: <Widget>[
      _bullets(ElTheme.of(context), <String>[
        'File: lib/src/motion/swap_in.dart: one file, no companions; the '
            'registry manifest lists exactly one entry under "files".',
        'Flutter imports: package:flutter/widgets.dart only.',
        'Foundation imports: foundation/motion.dart (ElDurations.base, '
            'ElCurves.spring), motion/keyframes.dart (ElKeyframePlayer, the '
            'player this file is built on).',
        'registryDependencies, resolved automatically by `elattar add '
            'swap-in`: keyframes, source-foundation — copied verbatim from '
            'registry/motion/swap-in.json.',
        'Not a dependency of swap_in.dart itself, but its one live '
...
const List<DocsApiFact> _swapInStaticFacts = <DocsApiFact>[
  DocsApiFact(
    ...
    description: 'ElDurations.base — 250ms. var(--duration-base).',
  ),
  DocsApiFact(
    name: 'curve',
    type: 'static Curve',
    description: 'ElCurves.spring. var(--ease-spring).',
  ),
];
...
const List<DocsStateFact> _stateFacts = <DocsStateFact>[
  DocsStateFact(
    ...
    treatment:
        'ElKeyframePlayer plays 0 to 1 over ElDurations.base (250ms) on '
        'ElCurves.spring; opacity and scale both read the same eased t, '
  ...
  DocsStateFact(
    ...
    treatment:
        'ElKeyframePlayer is not re-keyed, so the same State object keeps '
  ...
  DocsStateFact(
    ...
    treatment:
        'ElKeyframePlayer routes its duration through elAnimationDuration '
        'internally, with ElKeyframeFill.both holding the end frame.',
    userSignal:
