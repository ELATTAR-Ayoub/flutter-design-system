...
///
/// `agent_markdown.dart` declares three widgets — [ElAgentMarkdown] (the
/// public entry point), [ElAgentCodeBlock] and [ElPreformattedCode] — plus
/// two supporting classes, [ElPrismPalette] and [ElCodeToken], and the
/// safety function [elSafeHref]. API Reference gives each its own table.
///
/// **Scoped, deliberately.** The file also declares a whole markdown block
/// model ([ElMarkdownBlock] and its seven subclasses), the parser that
/// produces it ([elParseMarkdown]), the inline-span renderer
/// ([elRenderMarkdownInline]) and a small syntax tokeniser
/// ([elTokenise]). All four are public Dart symbols, because the language
...
/// is: *"a string in, a block tree out."* [ElAgentMarkdown.text] is the
/// only door a caller is meant to use; nobody constructs an
/// [ElMarkdownFence] by hand. Giving the parser's own internal vocabulary
...
/// `elRenderMarkdownInline`'s own `link()` helper: it returns a `TextSpan`
/// with an underline and `theme.agent` ink and nothing else — no
  ...
  sections: <DocsPageSection>[
    ...
    InstallSection(
      ...
      manualFiles: <DocsCodeFile>[
        ...
        DocsCodeFile(
          ...
          description:
              'Add the export line so ElAgentMarkdown and its two '
    ...
    ShowcaseSection(
      ...
      description:
          ...
          'ElMarkdownNumbers blocks — but each item carries the number '
          'its author actually wrote (ElMarkdownOrderedItem.n), not a '
    ...
    ShowcaseSection(
      ...
      description:
          ...
          'elParseMarkdown, not re-parsed per cell. Right-aligned columns '
    ...
    ShowcaseSection(
      ...
      title: 'Code block',
      description:
          'A recognized language (elLanguageAliases has an entry for it) '
          'gets a header strip naming the normalised language over a '
          'Prism-tokenised body on ElPrismPalette\'s vscDarkPlus colours. '
    ...
    ShowcaseSection(
      ...
      description:
          ...
          'to a javascript: URL. elSafeHref refuses every scheme but '
    ...
    DisclosureSection(
      ...
      description:
          ...
          'elSafeHref / elLanguageAliases. See this page\'s own library '
          'doc for why the markdown block model and its parser are not '
          'given tables of their own.',
      children: const <DocsTocEntry>[
        DocsTocEntry(title: 'ElAgentMarkdown', anchor: 'api-elagentmarkdown'),
        DocsTocEntry(
          title: 'ElAgentCodeBlock',
          anchor: 'api-elagentcodeblock',
        ),
        DocsTocEntry(
          title: 'ElPreformattedCode',
          anchor: 'api-elpreformattedcode',
        ),
        DocsTocEntry(title: 'ElPrismPalette', anchor: 'api-elprismpalette'),
        DocsTocEntry(title: 'ElCodeToken', anchor: 'api-elcodetoken'),
        DocsTocEntry(
    ...
    DisclosureSection(
      ...
      title: 'States',
      description:
          'Read off ElAgentMarkdown._block, ElAgentCodeBlock.build and '
          'elRenderMarkdownInline, not inferred.',
...
class AgentMarkdownDocPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => DocsLayout(
    ...
    breadcrumbs: const <ElBreadcrumbEntry>[
      ...
      ElBreadcrumbEntry.page('Agent Markdown'),
    ],
...
ElAgentMarkdown(text: 'Model output, **bold** and all.')''';
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: <Widget>[
      const DocsAnchor(
        ...
        child: DocsApiTable(
          title: 'ElAgentMarkdown',
          facts: _agentMarkdownFacts,
        ),
      ),
      ...
      const DocsAnchor(
        ...
        child: DocsApiTable(
          title: 'ElPrismPalette',
          facts: _prismPaletteFacts,
        ),
      ),
      SizedBox(height: el(5)),
      const DocsAnchor(
        id: 'api-elcodetoken',
        child: DocsApiTable(title: 'ElCodeToken', facts: _codeTokenFacts),
      ),
      SizedBox(height: el(5)),
      const DocsAnchor(
...
const List<DocsApiFact> _agentMarkdownFacts = <DocsApiFact>[
  DocsApiFact(
    name: 'text',
    type: 'String',
    description: 'Required. The markdown source. elParseMarkdown(text) '
        'runs on every build.',
  ),
  DocsApiFact(
    name: 'textAlign',
  ...
  DocsApiFact(
    name: 'blockGap',
    type: 'double',
    description: 'Static. el(2) — 8. Margin-bottom on every block but the '
        'last.',
  ),
  DocsApiFact(
    name: 'listInset',
    type: 'double',
    description: 'Static. el(6) — 24. Both list forms\' own left inset.',
  ),
  DocsApiFact(
    name: 'itemGap',
    type: 'double',
    description: 'Static. el(1) — 4. Between list items.',
  ),
  DocsApiFact(
    name: 'quoteInset',
    type: 'double',
    description: 'Static. el(4) — 16. Between a blockquote\'s rule and its '
        'text.',
  ),
  DocsApiFact(
    name: 'quoteRule',
    type: 'double',
    description: 'Static. el(0.5) — 2. The blockquote\'s own left rule '
        'width.',
  ),
  DocsApiFact(
    name: 'quoteRuleAlpha',
    type: 'double',
    description: 'Static. 0.40. theme.agent at this alpha paints the rule.',
  ),
  DocsApiFact(
    name: 'cellPadX',
    type: 'double',
    description: 'Static. el(3) — 12. Horizontal padding in every table '
        'cell.',
  ),
  DocsApiFact(
    name: 'cellPadY',
    type: 'double',
    description: 'Static. el(2) — 8. Vertical padding in every table '
        'cell, before the half-hairline border correction.',
  ),
  DocsApiFact(
    name: 'headingTop',
    type: 'double',
    description: 'Static. el(1) — 4. A heading\'s own top margin.',
  ),
];
...
const List<DocsApiFact> _codeBlockFacts = <DocsApiFact>[
  ...
  DocsApiFact(
    ...
    description:
        ...
        'through elLanguageAliases before it decides which of the two '
        'render paths runs.',
  ),
  DocsApiFact(
    name: 'plainPadding',
    type: 'double',
    description: 'Static. el(3) — 12. Padding on the un-highlighted '
        '(unrecognised-language) form.',
  ),
  DocsApiFact(
    name: 'stripPadX',
    type: 'double',
    description: 'Static. el(3) — 12. Horizontal padding on the language '
        'header strip.',
  ),
  DocsApiFact(
    name: 'stripPadY',
    type: 'double',
    description: 'Static. el(2) — 8. Vertical padding on the language '
        'header strip.',
  ),
  DocsApiFact(
    ...
    description:
        ...
        'present in elLanguageAliases — the exact condition that routes '
...
const List<DocsApiFact> _preformattedCodeFacts = <DocsApiFact>[
  DocsApiFact(
    name: 'code',
    type: 'String',
    description: 'Required. Rendered one Text per line, unwrapped '
        '(softWrap: false).',
  ),
  DocsApiFact(
    name: 'color',
...
const List<DocsApiFact> _prismPaletteFacts = <DocsApiFact>[
  ...
  DocsApiFact(
    name: 'plain',
    type: 'Color',
    description: 'allow-hardcoded: #d4d4d4, the theme\'s plain-text '
        'colour.',
  ),
  DocsApiFact(
    name: 'keyword',
  ...
  DocsApiFact(
    name: 'type',
    type: 'Color',
    description: 'allow-hardcoded: #4ec9b0 (vscDarkPlus\'s class-name '
        'colour).',
  ),
  DocsApiFact(
    name: 'padding',
  ...
  DocsApiFact(
    name: 'margin',
    type: 'double',
    description: 'allow-hardcoded: 6.5 (.5em) — real vertical space above '
        'and below the highlighted body.',
  ),
  DocsApiFact(
    name: 'lineHeight',
    type: 'double',
    description: 'allow-hardcoded: 19.5 — 13px over Preflight\'s 1.5, and '
...
const List<DocsApiFact> _codeTokenFacts = <DocsApiFact>[
  ...
  DocsApiFact(
    name: 'color',
    type: 'Color',
    description: 'Required. One of ElPrismPalette\'s own colours.',
  ),
];
  ...
  Widget build(BuildContext context) =>
      _bullets(ElTheme.of(context), <String>[
        ...
        'A heading (### Findings) is not announced as a heading: '
            'ElRichText renders it in ElType.h4\'s visual style only — '
...
class _KeyboardContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      _bullets(ElTheme.of(context), <String>[
        'ElAgentMarkdown takes no focus and binds no key: there is no '
        ...
        'A rendered link cannot be tabbed to, and cannot be activated by '
            ...
            'recognizer — read straight off elRenderMarkdownInline\'s own '
...
class _DependenciesContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      _bullets(ElTheme.of(context), <String>[
        ...
        'Foundation imports: spacing.dart (el()), theme.dart, '
            'typography.dart, text_layout.dart (ElLineBox, ElInlineBox, '
            'elEngineLineHeight — for the code block\'s own line-box '
            'math), theme_scope.dart (ElText, ElRichText, ElTheme).',
...
class _ThemingContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      _bullets(ElTheme.of(context), <String>[
        ...
        'No override hatch: ElAgentMarkdown takes no colour parameter of '
            'its own beyond textAlign\'s layout effect. Restyling prose '
            'colours means changing the active ElThemeData, not passing '
            'an argument here.',
      ]);
}
...
const List<DocsStateFact> _stateFacts = <DocsStateFact>[
  DocsStateFact(
    ...
    treatment: 'elParseMarkdown(text) produces no blocks: '
        'ElAgentMarkdown.build returns SizedBox.shrink().',
    userSignal: 'An empty string, or one that parses to nothing, renders '
        'nothing at all.',
  ),
  DocsStateFact(
    state: 'Recognised language fence',
    treatment: 'ElAgentCodeBlock.normalise(language) is non-null: a '
        'header strip plus a Prism-tokenised body on ElPrismPalette.',
    userSignal: 'The ts block in Code block above.',
  ),
  DocsStateFact(
    state: 'Unrecognised or missing language',
    treatment: 'normalise(language) is null: a plain --muted block, no '
        'strip, no tokenising — ElPreformattedCode on theme.foreground.',
    userSignal: 'The swift block in Code block above.',
  ),
  DocsStateFact(
    state: 'Safe link',
    treatment: 'elSafeHref resolves a scheme: link() renders an '
        'underlined, theme.agent-coloured TextSpan.',
    userSignal: 'Styled, but still not tappable — see the library doc '
        'and Keyboard above.',
  ),
  DocsStateFact(
    state: 'Refused link',
    treatment: 'elSafeHref returns null: the label renders as a plain, '
        'unstyled TextSpan.',
