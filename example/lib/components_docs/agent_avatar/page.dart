...
/// [AgentAvatar] (the widget) and [AgentAvatarSize] (the size it takes) —
/// and those get complete API tables. [CubeScene] is the one other
...
/// [AgentAvatar] can reach for it directly. The remaining six —
/// [AgentCube], [AgentCubeFaces], [AgentCubeKeyframe],
/// [AgentCubeKeyframes], [AgentCubeMotion], [AgentCubeScene], and the
/// top-level `agentCubeScene` — are the isometric projection, the
/// fourteen keyframe tables, and the nineteen scene recipes: the engine
/// [AgentAvatar] runs on, exported because the file draws no library-level
...
/// reader of [AgentAvatar]'s own constructor ever reaches.
///
/// **State is not a per-value section.** [AgentState] (`agent_core.dart`)
...
/// all twenty in one grid instead, the shape `components/space/agent-demo.tsx`'s
/// own `AvatarMatrix` uses. `AgentState` itself — its wire spelling, its
...
const List<AgentState> _previewStates = <AgentState>[
  AgentState.idle,
  AgentState.thinking,
  ...
  sections: <DocsPageSection>[
    ShowcaseSection(
      ...
      description:
          ...
          'error, done. Every one is a live AgentAvatar, cross-fading on '
    ...
    InstallSection(
      ...
      manualFiles: <DocsCodeFile>[
        ...
        DocsCodeFile(
          ...
          description:
              'Add the export line so AgentAvatar and its supporting '
    ...
    ShowcaseSection(
      ...
      title: 'State Set',
      description:
          'All twenty AgentState values, each drawn at its default md '
          'size. Nineteen are isometric scenes resolved through '
          'agentCubeScene(state); idle is the one exception, a real '
    ...
    ShowcaseSection(
      ...
      title: 'Sizes',
      description:
          'AgentAvatarSize has four rungs: sm (32px), md (48px, the '
    ...
    ShowcaseSection(
      ...
      description:
          ...
          'from. Left null it resolves to theme.agentAccent, so an avatar '
    ...
    DisclosureSection(
      ...
      title: 'API Reference',
      description:
          'AgentAvatar and AgentAvatarSize in full — the surface a '
          'caller constructs — plus CubeScene, the one other widget with '
          'a public constructor, and the scene engine the three of them '
          'run on, named rather than tabulated.',
      children: const <DocsTocEntry>[
        DocsTocEntry(title: 'AgentAvatar', anchor: 'api-elcubeavatar'),
        DocsTocEntry(title: 'AgentAvatarSize', anchor: 'api-elagentavatarsize'),
        DocsTocEntry(title: 'CubeScene', anchor: 'api-elcubescene'),
    ...
    DisclosureSection(
      ...
      title: 'States',
      description:
          'AgentAvatar is a display, not a control: it has no hover, '
      ...
      child: DocsInstallFacts(
        ...
        facts: <DocsInstallFact>[
          ...
          const DocsInstallFact(
            ...
            description:
                'The package\'s own coverage of AgentAvatar and the scene '
          ...
          const DocsInstallFact(
            ...
            description:
                ...
                'AgentAvatar constructor parameter this page claims to '
...
class AgentAvatarDocPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => DocsLayout(
    ...
    breadcrumbs: const <BreadcrumbEntry>[
      ...
      BreadcrumbEntry.page('Agent Avatar'),
    ],
  ...
  Widget build(BuildContext context) => SingleChildScrollView(
    ...
    child: Padding(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: <Widget>[
          for (final AgentState state in _previewStates) ...<Widget>[
            _AvatarTile(
...
AgentAvatar(state: AgentState.idle),
AgentAvatar(state: AgentState.thinking),
AgentAvatar(state: AgentState.callingTools),
AgentAvatar(state: AgentState.awaitingApproval),
AgentAvatar(state: AgentState.error),
AgentAvatar(state: AgentState.done),''';
...
class _SizesSpecimen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SingleChildScrollView(
    ...
    child: Row(
      ...
      children: <Widget>[
        for (final AgentAvatarSize size in AgentAvatarSize.values) ...<Widget>[
          _AvatarTile(
...
AgentAvatar(state: AgentState.thinking, size: AgentAvatarSize.sm),
AgentAvatar(state: AgentState.thinking, size: AgentAvatarSize.md),
AgentAvatar(state: AgentState.thinking, size: AgentAvatarSize.lg),
AgentAvatar(state: AgentState.thinking, size: AgentAvatarSize.xl)''';
...
AgentAvatar(state: AgentState.running, size: AgentAvatarSize.lg),
AgentAvatar(
  state: AgentState.running,
  size: AgentAvatarSize.lg,
  accent: Palette.action,
...
AgentAvatar(state: AgentState.searching, size: AgentAvatarSize.lg),
AgentAvatar(
  state: AgentState.searching,
  size: AgentAvatarSize.lg,
  speed: 2.5,
...
class _ApiReferenceContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: <Widget>[
      const DocsAnchor(
        id: 'api-elcubeavatar',
        child: DocsApiTable(title: 'AgentAvatar', facts: _cubeAvatarFacts),
      ),
      SizedBox(height: space(5)),
      const DocsAnchor(
        id: 'api-elagentavatarsize',
        child: DocsApiTable(title: 'AgentAvatarSize', facts: _avatarSizeFacts),
      ),
      SizedBox(height: space(5)),
      const DocsAnchor(
        id: 'api-elcubescene',
        child: DocsApiTable(title: 'CubeScene', facts: _cubeSceneFacts),
      ),
      SizedBox(height: space(5)),
      DocsAnchor(id: 'api-scene-engine', child: _SceneEngineContent()),
    ],
  );
}
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: <Widget>[
      ...
      _bullets(ThemeScope.of(context), <String>[
        'AgentCube — a private-constructor namespace (no public '
            ...
            'back-to-front. Called by CubeScene, never by a caller.',
        'AgentCubeFaces — the four colours one cube is drawn in '
        ...
        'AgentCubeKeyframe — the fourteen @keyframes agent-cube-* names '
            ...
            "cube's own rotation.",
        'AgentCubeKeyframes — those fourteen tables, each as an '
            'Animatable over 0..1.',
        'AgentCubeMotion — one animation shorthand: which keyframe '
            'plays, its duration and delay (already divided by speed), '
            'whether it alternates, and whether the element starts hidden '
            'during the delay.',
        'AgentCubeSpec — one cube\'s entry in a scene recipe: its grid '
            'position and which motions it plays.',
        'AgentCubeScene / agentCubeScene(state, speed:) — a scene\'s '
            "own cube list and measured width; the function resolves an "
            'AgentState to one of the nineteen recipes.',
      ]),
    ],
  );
}
  ...
  Widget build(BuildContext context) =>
      _bullets(ThemeScope.of(context), <String>[
        ...
        'Known gap: the Semantics node carries no liveRegion. A screen '
            ...
            'the label changes underneath it; AgentStatusLine '
            '(agent_face.dart), which sits beside the avatar in a real '
            'console, is the one that marks liveRegion: true.',
        'Reduced motion: effectiveMotionDuration(context, AgentAvatar.'
...
class _KeyboardContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      _bullets(ThemeScope.of(context), <String>[
        'AgentAvatar takes no focus and handles no key event: it wraps '
  ...
  Widget build(BuildContext context) =>
      _bullets(ThemeScope.of(context), <String>[
        ...
        'The rendered box is entirely a function of size '
            '(AgentAvatarSize.box, one of 32/48/80/128px) and the '
    ...
    children: <Widget>[
      _bullets(ThemeScope.of(context), <String>[
        ...
        'registryDependencies, resolved automatically by `elattar add '
            ...
            'agent-core supplies AgentState and its .label; keyframes '
            'supplies the motion-map machinery AgentCubeKeyframes '
            'follows the precedent of; source-foundation supplies space(), '
            'ThemeScope, and the durations and curves every clock reads.',
...
class _ThemingContent extends StatelessWidget {
  ...
  Widget build(
    ...
    'accent resolves to ThemeScope.of(context).agentAccent when left null — '
    ...
    'The twelve --agent-cube-* tokens (neutral/accent/error/ghost, '
        ...
        '(AgentCubeTokens.light / .dark), resolved from '
        'ThemeTokens the same way every other token is: flipping '
        'ThemeController re-resolves them on the next frame.',
    'AgentCubeFaces derives the accent cube\'s top and right faces '
...
const List<DocsApiFact> _cubeAvatarFacts = <DocsApiFact>[
  DocsApiFact(
    ...
    description:
        ...
        'AgentState.idle, the reference\'s own default.',
  ),
  DocsApiFact(
    name: 'size',
  ...
  DocsApiFact(
    ...
    description:
        ...
        'resolves to theme.agentAccent, so the avatar follows a retheme '
        'automatically.',
  ),
  DocsApiFact(
    name: 'speed',
  ...
  DocsApiFact(
    ...
    description:
        'MotionDurations.fast (150ms) — how long the outgoing scene fades '
        'under the incoming one when state changes.',
  ),
  DocsApiFact(
    name: 'spin',
    type: 'static const AgentCubeMotion',
    description:
        "The idle cube's own clock: AgentCubeKeyframe.spin3d over 9s, "
...
const List<DocsApiFact> _cubeSceneFacts = <DocsApiFact>[
  DocsApiFact(
    ...
    type: 'AgentCubeScene',
    description:
        'Required. The recipe to draw, from agentCubeScene(state, '
        'speed: …).',
  ),
  DocsApiFact(
    name: 'width',
  ...
  DocsApiFact(
    ...
    type: 'Color',
    description:
        "Required. The resolved --agent-cube-accent this scene paints with.",
  ),
  DocsApiFact(
    name: 'elapsed',
    type: 'double',
...
const List<DocsStateFact> _stateFacts = <DocsStateFact>[
  DocsStateFact(
    state: 'idle',
    treatment:
        '_IdleCube — a real 3D-projected cube, agent-cube-spin3d '
        '9s linear infinite',
    userSignal: 'The one state that is not an isometric scene.',
  ),
  DocsStateFact(
    state: 'any of the other 19',
    treatment: 'CubeScene, resolved through agentCubeScene(state)',
    userSignal: 'An isometric scene of up to twenty animated cubes.',
  ),
  DocsStateFact(
    state: 'state change',
    treatment: 'AgentAvatar.crossfade — 150ms on MotionCurves.enter',
    userSignal:
  ...
  DocsStateFact(
    ...
    treatment: 'N/A',
    userSignal:
        'AgentAvatar takes no pointer input and is not focusable: it '
