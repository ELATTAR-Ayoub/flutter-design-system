...
///
/// `agent-face` is a brand-new page. `ElAgentFace`'s own constructor
...
/// `ElAgentFace` API table.
///
/// **No `pumpAndSettle` anywhere in this file.** The renderer this page
/// defaults to (`ElCubeAvatar`) runs a bare `Ticker`; the "live" status
/// line specimens shimmer on an infinite `ElKeyframePlayer`. Every test
...
/// Every named constructor parameter `ElAgentFace`'s own class declares
...
void main() {
  group('agent-face docs page', () {
    testWidgets(
      'renders the article, the full ElAgentFace API table, and a live '
      ...
      (WidgetTester tester) async {
        ...
        for (final String param in <String>[
          ...
          expect(
            find.text(param),
            findsWidgets,
            reason: 'ElAgentVoice.$param missing from API table',
          );
        }
        // ElAgentStatusLine's own table.
        for (final String param in <String>['gap', 'waveformBox', 'barsBox']) {
          expect(
            find.text(param),
            findsWidgets,
            reason: 'ElAgentStatusLine.$param missing from API table',
          );
        }
        // ElAgentAvatarRegistry's own table.
        for (final String param in <String>[
          'renderer',
          'orb',
          'waveform',
          ...
          expect(
            find.text(param),
            findsWidgets,
            reason: 'ElAgentAvatarRegistry.$param missing from API table',
          );
        }

        // The Voice section shows ElAgentFace at rest, listening, and
        // speaking — three live specimens, not just prose.
        expect(
          find.byKey(
            const ValueKey<String>('agent-face-example:voice-rest'),
          ),
          findsOneWidget,
        );
        final ElAgentFace listening = tester.widget<ElAgentFace>(
          find.descendant(
            of: find.byKey(
              const ValueKey<String>('agent-face-example:voice-listening'),
            ),
            matching: find.byType(ElAgentFace),
          ),
        );
        expect(listening.voice.listening, isTrue);
        expect(listening.voice.isActive, isTrue);

        final ElAgentFace speaking = tester.widget<ElAgentFace>(
          find.descendant(
            of: find.byKey(
        ...
        // second tile's ElAgentFace carries a non-null avatar builder, the
        // first's does not.
        final ElAgentFace defaultRenderer = tester.widget<ElAgentFace>(
          find.descendant(
            of: find.byKey(
        ...
        final ElAgentFace customRenderer = tester.widget<ElAgentFace>(
          find.descendant(
            of: find.byKey(
        ...
        expect(find.byType(ElLiveWaveform), findsOneWidget);
        expect(find.byType(ElBarVisualizer), findsOneWidget);

        expect(agentFaceDoc.name, 'agent_face');
        expect(agentFaceDoc.exports, contains('ElAgentFace'));
